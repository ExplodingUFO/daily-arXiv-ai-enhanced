...
Motivation: 现有布局生成模型虽然功能强大，但经常无法与人类审美判断对齐。现有的偏好数据集和奖励模型主要针对文本到图像生成，不适用于布局评估，因为布局质量取决于相同元素的空间排列。

Method: 1) 提出五阶段数据整理流程：语义分组、布局预测、过滤、聚类和VLM精炼，生成10,235个人工标注的偏好对；2) 使用4类标注方案（左好、右好、都好、都差）捕捉主观模糊性；3) 基于此数据集训练DesignSense视觉语言模型分类器。

Result: DesignSense在综合评估指标上显著优于现有开源和专有模型（在Macro F1上比最强专有基线提升54.6%）。前沿VLM在完整四类任务上表现不可靠且存在灾难性失败。在布局生成中，使用DesignSense进行RL训练将生成器胜率提高约3%，推理时缩放（生成多个候选并选择最佳）提供3.6%的改进。

Conclusion: 研究表明需要专门针对布局的偏好感知模型，DesignSense-10k数据集和DesignSense模型为图形布局评估提供了有效解决方案，并在实际布局生成中带来实质性质量提升。
...
TL;DR: ANTShapes是一个基于Unity引擎的神经形态视觉数据集模拟框架，用于解决DVS传感器数据稀缺问题，通过模拟可配置的3D场景和随机物体行为生成定制化数据集。


<details>
  <summary>Details</summary>
Motivation: 动态视觉传感器（DVS）的可用性有限，现有数据集样本数量不足或场景有限，这阻碍了神经形态计算机视觉应用的研究。

Method: 基于Unity引擎构建，模拟抽象可配置的3D场景，包含显示随机生成行为（如运动和旋转）的物体。物体行为采样和异常行为标记遵循中心极限定理的统计过程，通过调整少量参数即可创建任意数量的数据集样本。

Result: 开发了ANTShapes框架，能够生成包含任意数量样本的数据集，并导出相应的标签和帧数据，支持对象识别、定位和异常检测等多种应用。

Conclusion: ANTShapes通过允许模拟定制化数据集，解决了事件型计算机视觉研究中数据可用性的限制问题，为神经形态视觉研究提供了灵活的数据生成工具。
...
TL;DR: 提出LIDMark统一主动取证框架，通过152维地标身份水印联合解决深度伪造检测、篡改定位和来源追踪三个任务，使用因子化头解码器实现鲁棒提取。


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术快速发展对个人隐私和社会安全构成严重威胁，现有主动取证方法通常将检测、定位和追踪作为独立任务处理，缺乏统一框架联合解决这些核心问题。

Method: 提出LIDMark统一主动取证框架，采用152维地标-身份水印结构交织面部地标与唯一来源标识符。设计因子化头解码器，将共享骨干特征分解为回归头和分类头，分别鲁棒重建嵌入的地标和标识符。

Result: 实验表明LIDMark框架为深度伪造内容检测、定位和追踪提供了统一、鲁棒且不可察觉的解决方案。回归头支持"内在-外在"一致性检查实现检测和定位，分类头鲁棒解码来源标识符实现追踪。

Conclusion: LIDMark框架通过统一的水印设计和因子化解码器架构，成功实现了深度伪造检测、篡改定位和来源追踪的三功能一体化解决方案，填补了现有主动取证方法的空白。
...
<details>
  <summary>Details</summary>
Motivation: 现有时空场景图数据集在规模和多样性上有限，限制了视频场景图生成模型的发展。需要更大规模、更多样化的数据集来推动视频理解任务的发展，特别是需要能够捕捉复杂时空关系的中间表示。

Method: 1. 创建SVG2数据集：采用全自动流水线，结合多尺度全景分割、在线-离线轨迹跟踪（含自动新对象发现）、每轨迹语义解析和基于GPT-5的时空关系推理。2. 训练TRaSER模型：在视觉语言模型基础上增加轨迹对齐的token排列机制，包含对象轨迹重采样器和时间窗口重采样器，将原始视频和全景轨迹转换为紧凑的时空场景图。

Result: 在PVSG、VIPSeg、VidOR和SVG2测试集上，TRaSER相比最强开源基线：关系检测提升15-20%，对象预测提升30-40%；相比GPT-5提升13%；属性预测提升15%。将TRaSER生成的场景图用于视频问答时，相比仅使用视频或使用Qwen2.5-VL生成的场景图，准确率绝对提升1.5-4.6%。

Conclusion: SVG2数据集为视频场景图生成提供了大规模资源，TRaSER模型通过创新的轨迹对齐机制显著提升了场景图生成性能，证明了显式时空场景图作为中间表示在视频理解任务中的实用价值。
...
TL;DR: LE-NeuS是一个延迟高效的神经符号框架，通过自适应采样和批量命题检测大幅降低长视频问答的推理延迟，将延迟差距从90倍减少到约10倍，同时保持准确性优势。


<details>
  <summary>Details</summary>
Motivation: 现有的神经符号方法在长视频问答中虽然准确性显著提升，但推理延迟过高（比基础VLM提示慢90倍），不适合延迟敏感的边缘部署场景。

Method: 提出两种优化：1) CLIP引导的两阶段自适应采样，利用视觉冗余跳过语义相似的帧同时保留时间边界；2) 批量命题检测，在时间窗口上并行化VLM推理。
...
Method: 提出匹配-稠密化-整合方法：1) RGB-X图像匹配后进行引导点稠密化；2) 使用置信度感知稠密化和自匹配过滤获得更好的视图合成；3) 在3D高斯泼溅(3DGS)中整合结果。方法不使用X传感器的3D先验，仅假设RGB端使用近乎零成本的COLMAP

Result: 该方法实现了跨传感器视图合成，能够处理不同模态的RGB-X数据，为大规模真实世界RGB-X数据收集提供了突破性解决方案

Conclusion: 提出的方法消除了RGB-X传感器的繁琐校准需求，通过可扩展的解决方案推动了跨传感器学习的普及，解决了大规模真实世界RGB-X数据收集的关键瓶颈问题
...
TL;DR: 提出Evidential Neural Radiance Fields，一种概率方法，能在单次前向传播中同时量化偶然性和认知不确定性，在保持渲染质量的同时实现高效不确定性估计


<details>
  <summary>Details</summary>
Motivation: 神经辐射场（NeRF）在场景重建和新视角合成方面取得显著进展，但缺乏不确定性估计限制了其在安全关键场景中的应用。现有NeRF不确定性量化方法无法同时捕捉偶然性和认知不确定性，且往往以牺牲渲染质量或增加计算开销为代价
...
Conclusion: 该方法解决了NeRF中不确定性估计的关键限制，为安全关键应用提供了可靠的三维场景建模工具
...
TL;DR: HDFLIM：一种无需微调预训练模型即可实现跨模态对齐的超维度计算框架，通过符号操作在共享高维空间中构建关联表示，性能媲美端到端训练方法。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态对齐方法通常需要计算密集的多模态微调，涉及大规模参数更新、资源消耗大，且可能扰动预训练表示。然而，独立训练的基座模型可能已存在潜在语义兼容性，这引发了一个根本问题：能否在不修改模型本身的情况下实现跨模态对齐？

Method: HDFLIM框架将单模态嵌入投影到共享超维度空间，利用轻量级符号操作（绑定、捆绑和基于相似性的检索）在单次数据遍历中构建关联跨模态表示。标题生成通过高维记忆检索而非迭代梯度优化实现，保持视觉和语言模型完全冻结。

Result: HDFLIM实现了与端到端视觉语言训练方法相当的性能，生成的标题比零样本基线更具语义基础。通过解耦对齐与参数调优，表明跨基座模型的语义映射可以通过对各自嵌入的超维度编码进行符号操作来实现。

Conclusion: 这项工作指向了一种替代性的基座模型对齐范式：通过结构化表示映射而非大规模重新训练来集成冻结模型。超维度计算为跨模态对齐提供了一种资源高效、不扰动预训练表示的解决方案。
...
Method: 提出新的训练范式，通过图表渲染器生成伪对比样本：使用随机选取的文本元素创建合成图表，这些样本突出图表图像的结构差异而不需要修改原始数据。将这些伪对比样本纳入训练目标，使模型学习捕捉更精确且语义一致的图表结构
...
Method: 提出HART闭环框架，包含自验证关键区域聚焦机制和AP-GRPO后训练范式。AP-GRPO通过优势偏好组相对策略优化来鼓励准确的关键区域定位，提供可解释的推理路径。

Result: HART在广泛的高分辨率视觉任务上提升性能，优于强基线。应用于Qwen2.5-VL-7B后，在高分辨率视觉中心基准测试中甚至超越了Qwen2.5-VL-72B和LLaVA-OneVision-72B等更大规模模型。

Conclusion: HART证明了无需额外标注就能有效增强大模型在高分辨率视觉输入中的定位和推理能力，为高分辨率视觉理解提供了高效且可解释的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有第一人称视角人体姿态估计方法面临关键点不可见性挑战，但现有数据集缺乏可见性标注，方法也忽视不可见性问题，导致可见关键点预测精度受限
...
TL;DR: 论文提出了首个专门评估指令式图像编辑模型在小尺度物体编辑能力的基准测试DLEBench，包含1889个样本和7种指令类型，目标物体仅占图像面积1%-10%，并设计了双模式评估框架来解决LMM评判与人类判断的不一致问题。


<details>
  <summary>Details</summary>
Motivation: 尽管指令式图像编辑模型在现有基准测试中表现出良好的指令遵循和推理能力，但它们在小物体编辑方面的能力尚未得到充分探索，而这一能力对于精确的局部编辑和细节优化在真实和生成图像中都至关重要。

Method: 构建了DeepLookEditBench基准测试，包含1889个样本，覆盖7种指令类型，目标物体仅占图像面积1%-10%，涉及部分遮挡和多物体编辑等复杂场景。提出了包含细化评分标准的评估协议，以及解决LMM评判与人类判断不一致的双模式评估框架（工具驱动模式和Oracle引导模式）。

Result: 在10个指令式图像编辑模型上的实证结果显示，在小尺度物体编辑方面存在显著的性能差距，突显了需要专门基准测试来推进这一能力的发展。

Conclusion: 论文提出了首个专门评估指令式图像编辑模型小尺度物体编辑能力的基准测试DLEBench，揭示了现有模型在这一重要任务上的局限性，为未来研究提供了评估工具和方向。
...
TL;DR: BuildAnyPoint：一种从点云进行结构化3D建筑重建的新生成框架，使用松散级联扩散变换器（Loca-DiT）通过自回归网格生成恢复建筑抽象


<details>
  <summary>Details</summary>
Motivation: 从机载LiDAR和运动恢复结构（SfM）等不同分布的点云中恢复艺术家创建的建筑抽象是一个高度欠约束的问题，需要利用显式3D生成先验
...
Conclusion: BuildAnyPoint框架通过结合扩散模型和自回归网格生成，有效解决了从不同分布点云进行结构化3D建筑重建的挑战
...
<details>
  <summary>Details</summary>
Motivation: 将视觉语言模型应用于多器官医学影像面临两个主要挑战：模态特定的视觉-语言对齐和跨模态特征融合。现有方法在处理3D MRI多器官异常检测时存在局限性。

Method: 提出MedMAP框架，包含模态感知的视觉-语言对齐预训练阶段和下游任务微调阶段。预训练阶段通过模态感知编码器隐式捕获联合模态分布，改善视觉和文本表示的对齐。微调阶段保持文本编码器冻结，仅微调视觉编码器。同时构建了MedMoM-MRI3D数据集，包含7,392个3D MRI体积-报告对，涵盖12种MRI模态和9种异常。
...
TL;DR: ProtoDCS：一种用于视觉语言模型的开集测试时自适应框架，通过原型双检查分离机制和证据驱动适应策略，有效处理协变量偏移的分布内和分布外数据。


<details>
  <summary>Details</summary>
Motivation: 现有VLM测试时自适应方法在开集场景下存在局限：依赖硬阈值分离和熵最小化，导致对模糊OOD样本误判、产生过度自信预测，且参数更新计算成本高，难以实际部署。

Method: 提出原型双检查分离机制：使用概率高斯混合模型验证替代脆弱的阈值方法；证据驱动适应策略：利用不确定性感知损失和高效原型级更新，减少计算开销并缓解过度自信问题。

Result: 在CIFAR-10/100-C和Tiny-ImageNet-C数据集上的实验表明，ProtoDCS在已知类准确率和OOD检测指标上均达到最先进性能，显著优于现有方法。

Conclusion: ProtoDCS为视觉语言模型提供了一种鲁棒、高效的开集测试时自适应解决方案，通过概率分离和原型更新机制，有效解决了现有方法在开集场景下的脆弱性和计算效率问题。
...
TL;DR: 提出SDLS框架，通过语义解耦的潜在空间控制解决放射学报告生成中的历史比较幻觉问题，在保持临床准确性的同时显著减少幻觉


<details>
  <summary>Details</summary>
Motivation: 基于视觉语言模型的自动化放射学报告生成存在历史比较幻觉风险，即模型会生成当前研究不支持的历史发现。现有方法在幻觉抑制和临床准确性之间存在权衡

Method: 提出训练免费、推理时控制的SDLS框架：1) 使用LLM驱动的语义分解；2) 基于QR的正交化构建语义无关的干预向量；3) 利用几何约束过滤临床语义，确保仅针对"历史比较"轴进行控制
...
Method: 提出视觉-语言杂草分割框架，采用双编码器设计：冻结的CLIP嵌入与任务特定空间特征通过FiLM层融合，利用自然语言描述进行通道级特征调制，在包含地面图像和无人机图像的统一语料库上训练

Result: 在四个基准数据集上平均Dice分数达91.64%，比CNN基线提升4.98%；最具挑战性的杂草类别Dice分数达80.45%，比最佳基线提升15.42%；在有限目标域监督下保持稳定的分割性能

Conclusion: 视觉-语言对齐能够实现可扩展、标签高效的跨域分割模型，为实际农业应用提供了有前景的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 遥感基础模型快速发展，但模型发布格式、平台接口和数据规范的异质性导致实际应用和公平比较困难，显著增加了获取、使用和基准测试嵌入的成本

Method: 开发rs-embed Python库，提供统一的感兴趣区域（ROI）中心化接口，支持单行代码检索任何支持模型的嵌入，提供高效的批量处理功能

Result: 实现了对任何位置、任何时间范围的嵌入检索，支持大规模嵌入生成和评估，代码已在GitHub开源

Conclusion: rs-embed解决了遥感基础模型生态系统中的互操作性问题，降低了嵌入获取和比较的技术门槛，促进了模型的实际应用和公平评估
...
TL;DR: SourceSwap是一个自监督的源感知框架，通过频率分离扰动合成高质量伪对，实现跨对象对齐，无需逐对象微调即可进行零样本推理
...
Method: 通过初始噪声空间中的频率分离扰动从任何图像合成高质量伪对，改变外观同时保持姿态、粗略形状和场景布局；训练具有完整源条件化和无噪声参考编码器的双U-Net，实现直接对象间对齐

Result: SourceSwap在保真度、场景保持和自然和谐度方面表现优异，能够良好地迁移到主题驱动细化和面部交换等编辑任务，无需逐对象微调即可进行零样本推理

Conclusion: SourceSwap通过自监督学习和源感知框架有效解决了对象交换中的跨对象对齐问题，同时引入了高质量基准SourceBench，为对象交换领域提供了新的解决方案
...
TL;DR: HiDrop：一种通过层次化视觉token剪枝和晚期注入来高效训练多模态大语言模型的框架，能在压缩约90%视觉token的同时保持性能，并加速训练1.72倍。
...
Method: 提出HiDrop框架，包含两个关键创新：1) 晚期注入：绕过被动的浅层，在主动融合开始处引入视觉token；2) 凹金字塔剪枝与早期退出机制：通过层间相似性度量和可微分top-k算子动态调整中间层和深层的剪枝率。还包含持久位置编码、FlashAttention兼容的token选择和并行解耦视觉计算以消除动态token减少的隐藏开销。

Result: 实验表明HiDrop能压缩约90%视觉token，同时匹配原始性能，并加速训练1.72倍，为高效MLLM训练和推理设立了新的最先进水平。

Conclusion: HiDrop不仅为高效MLLM训练和推理设立了新标准，还提供了关于多模态融合层次性质的有价值见解。该工作揭示了视觉token剪枝应与MLLM层的真实层次功能对齐。
...
TL;DR: EgoGraph：一种用于超长第一人称视频理解的免训练动态知识图谱构建框架，通过编码跨实体长期依赖关系实现更好的视频理解
...
Method: 提出EgoGraph框架，采用统一的第一人称模式提取和抽象核心实体（人物、物体、地点、事件），结构化推理其属性和交互。开发时序关系建模策略捕获跨实体时序依赖，在多天内积累稳定的长期记忆。

Result: 在EgoLifeQA和EgoR1-bench基准测试中，EgoGraph在长期视频问答任务上实现了最先进的性能，验证了其作为超长第一人称视频理解新范式的有效性。

Conclusion: EgoGraph通过免训练动态知识图谱构建，能够编码长期跨实体依赖关系，为超长第一人称视频理解提供了更丰富、更连贯的语义表示，优于传统的基于片段的视频模型。
...
TL;DR: U-MLLMs在文本推理上表现良好，但在需要生成视觉答案时无法保持语义等价性，主要问题在于跨模态语义对齐而非生成保真度不足


<details>
  <summary>Details</summary>
Motivation: 现有评估通常将理解和生成能力分开评估，忽略了语义等价性（即无论输出模态如何都能表现一致推理结果的能力）。本研究旨在调查当前U-MLLMs是否满足这一前提。

Method: 引入VGUBench框架，通过三个诊断任务解耦推理逻辑与生成保真度：1)文本生成理解（建立文本响应推理准确性的基线）；2)视觉生成理解（评估生成代表正确答案的视觉响应的能力）；3)视觉渲染控制任务（评估将显式视觉描述直接渲染为图像的能力，无需复杂推理）。

Result: 评估显示显著差异：尽管在文本理解和视觉渲染方面表现强劲，但U-MLLMs在需要生成问题的视觉答案时表现出明显的性能崩溃。视觉回答性能与基本渲染质量之间几乎没有相关性。

Conclusion: 失败原因并非生成保真度不足，而是跨模态语义对齐的崩溃。研究为未来统一生成和理解模型提供了诊断见解。
...
TL;DR: 提出基于二阶差分（重建误差之差）的方法，通过方差缩减提高AI生成图像检测精度，解决传统基于重建误差的一阶差分方法在AI图像逼真度提升时失效的问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的AI图像已几乎无法与真实图像区分，这带来了滥用风险并给检测带来巨大挑战。现有基于重建误差（一阶差分）的检测器在AI图像越来越逼真时效果下降，需要更有效的检测方法。

Method: 提出差分中的差分方法：不直接使用重建误差（一阶差分），而是计算重建误差的差异（二阶差分），通过方差缩减来提高检测准确率。

Result: 大量实验表明该方法具有强大的泛化性能，能够在生成式AI时代可靠地检测AI生成的图像。

Conclusion: 通过引入二阶差分方法，有效解决了传统基于重建误差的检测器在AI图像逼真度提升时失效的问题，为AI生成图像检测提供了更可靠的解决方案。
...
TL;DR: UTPTrack提出首个统一令牌剪枝框架，联合压缩搜索区域、动态模板和静态模板，在保持性能的同时显著减少计算开销


<details>
  <summary>Details</summary>
Motivation: 单流Transformer跟踪器性能先进但计算开销大，现有令牌剪枝方法孤立处理各组件，忽略了组件间依赖关系，导致次优剪枝和精度下降

Method: UTPTrack采用注意力引导、令牌类型感知策略，统一建模冗余性，支持多模态和语言引导跟踪任务

Result: 在10个基准测试中，RGB跟踪剪除65.4%视觉令牌保持99.7%基线性能，统一跟踪剪除67.5%令牌保持100.5%基线性能，达到剪枝跟踪器精度-效率权衡的新SOTA

Conclusion: UTPTrack为高效视觉跟踪提供了稳健基础，首次实现多组件联合压缩，在RGB和多模态场景均表现优异
...
TL;DR: Selfment：一种完全自监督的框架，无需人工标注、预训练分割模型或后处理，直接从原始图像分割前景对象，通过图割和迭代优化实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 无需任何人工标注就能准确分割对象是计算机视觉的核心挑战之一。现有方法通常依赖人工标注、预训练模型或复杂的后处理，限制了其可扩展性和泛化能力。

Method: 1. 从自监督特征构建patch级亲和图，应用NCut获得初始粗粒度前景-背景分离；2. 引入迭代patch优化(IPO)，通过迭代patch聚类在特征空间逐步增强空间一致性和语义一致性；3. 使用精炼的掩码作为监督信号，通过对比和区域一致性目标训练轻量分割头。

Result: 在多个基准测试中达到新的SOTA：ECSSD (+4.0% F_max)、HKUIS (+4.6%)、PASCAL-S (+5.7%)。零样本泛化到伪装目标检测任务：CHAMELEON (0.910 S_m)、CAMO (0.792 F_β^ω)，超越所有无监督方法，媲美全监督SOTA方法。

Conclusion: Selfment证明了完全自监督分割的可行性，通过简单的图割和迭代优化框架实现了优异的性能和泛化能力，为无标注视觉理解开辟了新途径。
...
TL;DR: 提出Fourier Angle Alignment方法解决遥感旋转目标检测中的方向不一致和任务冲突问题，通过傅里叶旋转等变性分析角度信息，使用FAAFusion和FAA Head两个即插即用模块提升性能


<details>
  <summary>Details</summary>
Motivation: 遥感旋转目标检测中主流方法存在两个瓶颈：检测器颈部（neck）的方向不一致性和检测头（head）的任务冲突问题，需要新的方法来有效处理旋转目标的检测

Method: 提出Fourier Angle Alignment方法，利用傅里叶旋转等变性分析角度信息，将主方向对齐到特定方向。包含两个模块：1) FAAFusion在检测器颈部工作，将高层特征的主方向与低层特征对齐后融合；2) FAA Head作为新的检测头，将RoI特征预对齐到规范角度后与原始特征相加，再进行分类和回归

Result: 在DOTA-v1.0、DOTA-v1.5和HRSC2016数据集上显著提升先前工作性能。单尺度训练和测试下，在DOTA-v1.0上达到78.72% mAP的新SOTA结果，在DOTA-v1.5上达到72.28% mAP，验证了方法的有效性

Conclusion: Fourier Angle Alignment方法通过傅里叶旋转等变性有效解决了遥感旋转目标检测中的方向对齐问题，提出的FAAFusion和FAA Head模块能够显著提升检测性能，在多个基准数据集上取得了新的最先进结果
...
TL;DR: 提出基于3D几何基础模型的双手操作框架，通过扩散模型联合预测未来动作块和3D潜在表示，仅需RGB观测即可实现空间理解和预测


<details>
  <summary>Details</summary>
Motivation: 现有双手操作方法依赖2D特征（空间感知有限）或需要显式点云（真实场景难以可靠获取），而3D几何基础模型能从RGB图像快速稳健重建3D结构，这为构建仅需RGB观测的双手操作策略提供了机会

Method: 在预训练的3D几何基础模型上构建双手操作框架，策略融合几何感知潜在表示、2D语义特征和本体感知，使用扩散模型联合预测未来动作块和未来3D潜在表示（可解码为密集点云图），通过显式预测3D场景演化增强空间理解

Result: 在RoboTwin基准模拟和真实机器人执行中评估，方法在操作成功率、双臂协调性和3D空间预测精度方面均优于2D基线和点云基线，达到最先进性能

Conclusion: 通过利用3D几何基础模型和联合预测框架，实现了仅需RGB观测的高性能双手操作，为真实世界机器人操作提供了有效解决方案
...
TL;DR: 提出一种无需示例的持续学习框架，用于从全切片图像生成病理报告，通过构建紧凑的域足迹支持生成式回放，避免存储原始数据并防止灾难性遗忘。
...
Method: 构建紧凑的域足迹：在冻结的补丁嵌入空间中创建代表性形态标记的小型码本，包含切片级共现摘要和轻量级补丁计数先验。这些足迹支持生成式回放，合成反映特定域形态混合的伪WSI表示。同时，通过教师快照提供伪报告来监督更新模型。为处理报告规范变化，将特定域语言特征蒸馏为紧凑的风格描述符，用于引导生成。

Result: 在多个公开持续学习基准测试中，该方法优于无示例和有限缓冲回放基线，展示了足迹基生成式回放在实际临床环境部署中的有效性。

Conclusion: 提出的足迹基生成式回放框架为在演化临床环境中部署WSI到报告生成模型提供了实用解决方案，无需存储原始数据即可防止灾难性遗忘，并能自适应识别域特定特征。
...
TL;DR: 提出多模态多任务模型检测AI生成图像并识别生成模型，使用BERT和CLIP编码器提取特征，采用跨模态特征融合和多任务损失函数，结合伪标签数据增强，在CT2竞赛中获得第五名。


<details>
  <summary>Details</summary>
Motivation: 为了解决AI生成图像检测和识别具体生成模型的问题，需要开发能够同时处理这两个任务的系统，以应对日益增长的AI生成内容检测需求。

Method: 使用预训练的BERT和CLIP编码器分别提取文本和图像特征，采用跨模态特征融合策略，设计专门的多任务损失函数，并利用伪标签数据增强技术扩展训练数据集。

Result: 在CT2竞赛的Task A（AI生成图像检测）和Task B（生成模型识别）中均获得第五名，F1分数分别为83.16%和48.88%，证明了模型架构的有效性。

Conclusion: 提出的多模态多任务架构在AI生成内容检测方面表现出色，具有在实际场景中应用的潜力，为相关领域研究提供了有价值的参考。
...
TL;DR: CPN-YOLO：基于YOLOv8改进的SAR图像船舶检测框架，通过可学习大核去噪模块、PPA注意力机制特征增强和基于NWD的高斯相似度损失，提升复杂场景下的检测精度。


<details>
  <summary>Details</summary>
Motivation: SAR图像船舶检测在复杂场景下面临挑战：杂波和斑点噪声导致虚警，小目标容易漏检。现有方法在复杂场景下的鲁棒性不足，需要改进特征表示、多尺度建模和边界框相似度度量。

Method: 1. 可学习大核去噪模块：对输入进行预处理，生成更干净的表示和更具判别性的特征；2. 基于PPA注意力机制的特征提取增强策略：加强多尺度建模，提高对小船舶的敏感性；3. 基于归一化Wasserstein距离的高斯相似度损失：在复杂边界框分布下更好地度量相似性，提高泛化能力。

Result: 在HRSID和SSDD数据集上的实验验证了方法的有效性。在SSDD数据集上，CPN-YOLO超越YOLOv8基线，达到97.0%精度、95.1%召回率和98.9% mAP，整体性能优于其他代表性深度学习检测器。

Conclusion: CPN-YOLO通过针对性的改进有效解决了SAR图像船舶检测中的关键挑战，在复杂场景下实现了高精度检测，为SAR图像分析提供了有效的解决方案。
...
TL;DR: Ref-Adv是一个新的指称表达理解基准，通过抑制捷径来测试多模态大语言模型的视觉推理和基础能力，相比现有基准要求更严格的推理。


<details>
  <summary>Details</summary>
Motivation: 现有REC基准（RefCOCO等）存在三个主要问题：1）表达式过短，推理需求低；2）图像中干扰物少，目标容易定位；3）冗余描述符允许捷径解决方案，绕过真正的文本理解和视觉推理。需要创建更严格的基准来评估模型的真实视觉推理能力。

Method: 创建Ref-Adv数据集，包含真实图像上的指称表达式，通过精心设计：1）使用语言上非平凡的表达式；2）只包含唯一识别目标所需的信息；3）包含困难干扰物；4）标注推理方面（包括否定）。进行消融实验（词序扰动和描述符删除充分性）验证数据集需要超越简单线索的推理。

Result: 尽管在RefCOCO、RefCOCO+和RefCOCOg上表现强劲，当代多模态大语言模型在Ref-Adv上表现显著下降，揭示了模型对捷径的依赖以及在视觉推理和基础能力上的差距。提供了深入的失败分析。

Conclusion: Ref-Adv基准能够更准确地评估多模态大语言模型的视觉推理和基础能力，揭示了现有模型的局限性，旨在指导未来在视觉推理和基础方面的研究。
...
TL;DR: 该论文发现复杂视频推理过度依赖细粒度感知而非专家级推理能力，提出APPO算法通过注意力引导的感知策略优化来低成本增强模型感知能力。


<details>
  <summary>Details</summary>
Motivation: 通过实证观察发现，在复杂视频推理任务中，感知能力对性能的影响远大于推理能力。当感知能力基本固定时，将推理模型从Qwen3-8B升级到OpenAI-o3仅带来0.7%的性能提升；而感知模型规模从7B增加到32B则带来1.4%的性能提升。这表明增强感知能力比增强推理能力更为关键。因此，需要探索如何在不依赖昂贵细粒度标注信息的情况下，通过推理来增强感知能力。

Method: 提出APPO（Attention-guided Perception Policy Optimization）算法，利用token级密集奖励来改进模型的细粒度感知。核心思想是优化那些来自不同响应但主要关注相同关键视频帧的token（称为组内感知token）。该方法通过注意力机制引导，专注于提升对关键视频内容的感知能力。

Result: 在不同视频基准测试和不同规模模型（3B/7B）上的实验结果表明，APPO始终优于GRPO和DAPO方法，性能提升幅度在0.5%到4%之间。这证明了APPO能够有效增强模型的感知能力。

Conclusion: APPO提供了一种有前景的低成本方法，通过推理有效增强模型的感知能力，适用于多样化的场景和需求。这项工作强调了在复杂视频推理任务中感知能力的重要性，并为如何在不依赖昂贵标注的情况下提升感知能力提供了解决方案。
...
TL;DR: AgenticOCR是一种动态OCR解析范式，将静态的全文本OCR转换为查询驱动的按需提取系统，通过自主分析文档布局并选择性识别感兴趣区域，解决多模态RAG中页面级分块带来的上下文过载问题。


<details>
  <summary>Details</summary>
Motivation: 多模态检索增强生成扩展到视觉文档领域时面临挑战：页面级分块和检索会导致向生成器传递过多无关上下文，不仅过载注意力机制，还稀释关键证据；同时将信息丰富的页面压缩到有限视觉token预算会增加幻觉风险。

Method: 提出AgenticOCR动态解析范式，将OCR从静态全文本处理转变为查询驱动的按需提取系统。通过"thinking with images"方式自主分析文档布局，识别并选择性识别感兴趣区域，实现视觉token的按需解压缩，解耦检索粒度与刚性页面级分块。

Result: 实验结果表明AgenticOCR提高了视觉RAG系统的效率和准确性，在长文档理解方面达到专家级性能。该方法可作为视觉文档RAG堆栈的"第三个构建模块"，与标准嵌入和重排序模块协同工作。

Conclusion: AgenticOCR通过动态、查询驱动的OCR解析解决了多模态RAG中的关键瓶颈，实现了更精确的视觉信息提取，为复杂视觉文档处理提供了有效解决方案，代码和模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 解决CLIP等视觉语言模型在语义分割中因自注意力层不适当交互导致的问题，特别是针对遥感数据的特点进行优化

Method: 1. 使用SAM生成的掩码构建层次化方案，在多尺度上约束自注意力交互；2. 提出模型组合方法，平均多个遥感专用CLIP变体的参数，采用基于不同文本提示评估表征质量的新加权方案

Result: 在三个遥感基准测试中实现了最先进的性能，且无需额外训练

Conclusion: ReSeg-CLIP通过结合SAM掩码约束和模型组合策略，有效解决了CLIP在遥感语义分割中的局限性，为开放词汇遥感分析提供了高效解决方案
...
TL;DR: 提出一种基于地面特征密度分析和图像分类的视觉地点识别方法，通过相对高度估计和自适应裁剪处理显著高度变化，实现无需额外硬件的空中平台精确定位。


<details>
  <summary>Details</summary>
Motivation: 解决空中视觉地点识别在显著高度变化下的挑战，传统方法依赖气压高度计或ToF传感器等额外硬件，而单目深度估计方法精度有限，需要一种无需额外硬件且能适应高度变化的纯视觉解决方案。

Method: 通过分析图像中地面特征的密度来估计空中平台的相对高度，然后基于相对高度进行自适应裁剪生成规范查询图像，最后采用基于分类的VPR策略进行定位。

Result: 在多种地形和高度条件下，该方法在高度估计和VPR方面均表现出高精度和鲁棒性。相比单独VPR检索，相对高度估计模块使R@1和R@5分别提升29.85%和60.20%；相比传统单目深度估计方法，平均误差减少202.1米，R@1和R@5分别额外提升31.4%和44%。

Conclusion: 该方法建立了一个鲁棒的纯视觉三维视觉地点识别框架，为在显著高度变化和传感器有限条件下运行的空中平台提供了一种实用且可扩展的精确定位解决方案。
...
TL;DR: 提出一种自监督的3D占用流估计方法，无需人工标注或外部流监督，通过分离静态和动态有符号距离场并利用时间聚合学习运动


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要估计车辆周围环境的3D占用和运动，但现有方法依赖昂贵的3D占用和流标注、速度标签或预训练光流模型

Method: 将场景解耦为静态和动态有符号距离场，通过时间聚合隐式学习运动，并引入基于特征余弦相似度的强自监督流线索

Result: 在SemanticKITTI、KITTI-MOT和nuScenes数据集上验证了方法的有效性

Conclusion: 提出了一种无需人工标注的自监督3D占用流估计方法，通过场景解耦和时间聚合有效学习几何和运动信息
...
<details>
  <summary>Details</summary>
Motivation: 减少乳腺超声筛查中的诊断升级和不必要的活检转诊，通过选择性决策过程优化临床工作流程。

Method: 采用两阶段级联多智能体框架：轻量级筛查诊所智能体使用分类模型筛选低风险良性/正常病例；高风险病例升级到诊断诊所智能体，集成更丰富的感知和放射学描述工具进行二次决策。通过记忆库存储病理确认结果、图像嵌入、模型预测和历史智能体行动作为结构化决策轨迹，基于相似性检索进行检索条件化的上下文适应。

Result: 在10个乳腺超声数据集上的评估显示，BUSD-Agent将诊断升级从84.95%降至58.72%，总体活检转诊从59.50%降至37.08%，同时筛查特异性平均提高68.48%，诊断特异性提高6.33%。

Conclusion: 经验引导的级联多智能体框架能有效减少不必要的诊断升级和活检转诊，通过检索条件化的上下文适应动态调整模型信任和升级阈值，无需参数更新即可实现性能提升。
...
TL;DR: CLIP模型存在"半真"漏洞：在正确描述后添加错误细节反而会提高图像-文本相似度得分。作者提出CS-CLIP方法，通过组件级监督训练来提升组合理解能力。


<details>
  <summary>Details</summary>
Motivation: CLIP等双编码器模型存在一个违反直觉的问题：在正确的文本描述后添加一个看似合理但错误的细节（如错误的对象或关系），反而会提高图像-文本相似度得分。这种"半真"情况暴露了模型对caption各部分监督不足的问题，需要改进模型对组合语义的理解能力。

Method: 提出CS-CLIP（Component-Supervised CLIP）方法：1）将caption分解为实体和关系单元；2）为每个单元构造最小编辑的虚假版本（foil）；3）微调模型，使其对正确单元的评分高于虚假版本，同时保持标准的双编码器推理架构。

Result: 在COCO数据集上，原始CLIP对较短正确描述的偏好率仅为40.6%，当添加细节是关系时降至32.9%。CS-CLIP将半真准确率提升至69.3%，在已建立的组合基准测试中平均性能提升5.7个百分点，表明减少半真错误与更广泛的组合理解能力提升相关。

Conclusion: CLIP等对比训练模型存在对caption各部分监督不足的问题，导致"半真"漏洞。通过组件级监督训练（CS-CLIP）可以有效提升模型对组合语义的理解能力，减少半真错误，同时改善整体组合理解性能。
...
TL;DR: PointCoT：一个通过显式思维链推理增强多模态大语言模型3D点云理解能力的新框架，采用"观察-思考-回答"范式，在预测答案前生成几何基础推理依据。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在2D场景表现良好，但在3D点云理解方面存在挑战。现有方法主要关注3D特征与预训练模型的对齐，将几何推理视为隐式映射过程，绕过了中间逻辑步骤，导致几何幻觉问题——模型会生成看似合理但缺乏精确结构细节基础的错误响应。

Method: 提出PointCoT框架，采用"观察-思考-回答"范式，监督模型在预测最终答案前生成几何基础的推理依据。构建了Point-Reason-Instruct大规模基准数据集，包含约86k个具有分层思维链标注的指令调优样本。采用双流多模态架构，协同整合语义外观与几何真值信息。

Result: 在复杂推理任务上，PointCoT实现了最先进的性能表现，有效解决了几何幻觉问题，显著提升了3D点云理解的准确性和可靠性。

Conclusion: 通过引入显式思维链推理机制，PointCoT成功将多模态大语言模型的感知智能扩展到3D点云理解领域，为几何推理提供了可解释的基础，是解决当前3D理解中几何幻觉问题的有效方案。
...
TL;DR: CC-VQA：一种无需训练、基于冲突与相关性感知的KB-VQA方法，通过视觉中心冲突推理和相关性引导编解码，在多个基准上实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: KB-VQA中静态参数知识与动态检索信息之间存在冲突，现有方法主要源自语言模型，忽视了视觉信息在冲突中的作用，且检索上下文冗余影响冲突识别与缓解

Method: 包含两个核心组件：1）视觉中心上下文冲突推理，在内部和外部知识上下文中进行视觉语义冲突分析；2）相关性引导编码和解码，对低相关性语句进行位置编码压缩，使用相关性加权冲突评分进行自适应解码

Result: 在E-VQA、InfoSeek和OK-VQA基准测试中达到最先进性能，相比现有方法绝对准确率提升3.3%至6.4%

Conclusion: CC-VQA通过视觉中心冲突分析和相关性感知机制有效缓解KB-VQA中的知识冲突问题，无需训练即可显著提升性能
...
<details>
  <summary>Details</summary>
Motivation: 遮挡是机器人水果采摘中的关键挑战，未检测或定位不准确的水果会导致大量作物损失。需要解决遮挡场景下的水果检测和定位问题。

Method: 提出GDA-YOLO11非模态分割模型，包含架构改进和更新的非对称掩码损失。模型在修改的公共柑橘数据集上训练，通过欧几里得距离变换估计采摘点，并将2D点投影到3D坐标用于机器人采摘执行。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频扩散模型主要针对单事件生成优化，处理多事件提示时，由于缺乏明确的时间定位，往往会产生混合或崩溃的场景，破坏了预期的叙事结构。
...
TL;DR: NV-CoT是一种让多模态大语言模型使用连续数值坐标进行视觉推理的框架，通过将动作空间从离散词汇扩展到连续欧几里得空间，显著提升了区域定位精度和答案准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在模态不匹配和语义碎片化问题：基于文本化坐标的方法导致模态不匹配，而固定粒度补丁方法限制了精确区域选择且需要大量架构修改。需要一种更精确、更自然的区域定位方法。

Method: 提出数值视觉思维链框架，将MLLM动作空间从离散词汇扩展到连续欧几里得空间，使模型能直接生成边界框坐标作为动作。采用高斯或拉普拉斯策略替代分类标记策略，通过重参数化采样引入随机性，支持监督微调和强化学习，兼容GRPO风格策略优化。

Result: 在三个基准测试中与八个代表性视觉推理基线对比，NV-CoT显著提高了定位精度和最终答案准确性，同时加速了训练收敛，验证了连续动作视觉推理在MLLM中的有效性。

Conclusion: NV-CoT通过连续数值坐标的视觉推理框架，解决了现有方法的局限性，为MLLM提供了更精确、更自然的区域定位能力，推动了视觉推理技术的发展。
...
TL;DR: SpikeTrack：首个尖峰驱动的RGB目标跟踪框架，通过非对称设计和记忆检索模块实现高效能跟踪，在保持精度的同时大幅降低能耗


<details>
  <summary>Details</summary>
Motivation: 现有SNN跟踪框架要么不完全符合尖峰驱动计算范式，要么未能充分利用神经元的时空动态特性，导致效率与精度之间的权衡。需要开发既能充分利用SNN时空动态特性，又能保持尖峰驱动计算特性的RGB目标跟踪框架。

Method: 提出SpikeTrack框架，采用非对称设计：1）非对称时间步扩展，2）单向信息流，在利用时空动态的同时减少计算量。设计受神经推理机制启发的记忆检索模块，通过循环查询由模板初始化的紧凑记忆来检索目标线索并随时间增强目标感知。

Result: 在LaSOT数据集上超越TransT，同时能耗仅为后者的1/26。在SNN跟踪器中达到最先进水平，与先进ANN跟踪器保持竞争力。首次实现RGB跟踪在精度和能效方面的双重优势。

Conclusion: SpikeTrack是首个尖峰驱动的RGB目标跟踪框架，通过非对称设计和记忆检索模块成功解决了SNN跟踪中效率与精度的权衡问题，实现了高精度和低能耗的平衡，为SNN在视觉跟踪领域的应用提供了有效解决方案。
...
TL;DR: 该论文针对智能手机摄影中普通用户与专业摄影师之间的美学指导差距，提出了首个大规模美学指导数据集AesGuide和两阶段框架Venus，通过渐进式美学问题增强多模态大语言模型的美学指导能力，并基于思维链实现美学裁剪，在美学指导和裁剪任务上均取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 智能手机摄影普及但普通用户与专业摄影师之间存在明显差距，专业摄影师能够在拍摄过程中识别美学问题并提供可操作的拍摄指导。现有多模态大语言模型主要提供过度积极的反馈，无法识别问题或提供可操作指导，缺乏美学指导能力，因此在美学裁剪等任务上也表现不佳。

Method: 1) 引入首个大规模美学指导数据集AesGuide，包含10,748张照片，标注了美学评分、分析和指导；2) 提出两阶段框架Venus：第一阶段通过渐进式复杂美学问题（从简单评分到详细分析再到具体指导）增强MLLMs的美学指导能力；第二阶段通过基于思维链的推理激活其美学裁剪能力，实现可解释的构图优化。

Result: 实验表明Venus显著提升了美学指导能力，并在美学裁剪任务上达到了最先进的性能。该框架能够在照片创作的两个阶段（拍摄指导和后期裁剪）实现可解释和交互式的美学优化。

Conclusion: 该研究填补了计算美学中美学指导能力的空白，通过AesGuide数据集和Venus框架有效解决了现有MLLMs在美学指导和裁剪方面的局限性，为普通用户提供了接近专业摄影师水平的美学指导工具。
...
<details>
  <summary>Details</summary>
Motivation: 掩码图像生成模型(MIGMs)存在计算冗余问题：采样离散token时会丢失连续特征中的丰富语义。现有缓存特征方法在激进加速率下近似误差较大，主要原因是表达能力有限且未考虑采样信息
...
Conclusion: MIGM-Shortcut通过有效利用历史特征和采样信息，显著提升了掩码图像生成模型的效率，在保持生成质量的同时实现了大幅加速
...
TL;DR: 提出了一种有序潜在扩散模型，用于生成彩色眼底图像，该模型将糖尿病视网膜病变严重程度的顺序结构明确纳入生成过程，相比传统分类条件扩散模型能更好地捕捉疾病的连续进展。


<details>
  <summary>Details</summary>
Motivation: 当前生成式图像模型（如扩散模型）虽然可以通过提供补充训练数据来改善临床相关任务的性能，但大多数条件扩散模型将疾病阶段视为独立类别，忽略了疾病进展的连续性。在医学成像中，这种不匹配是有问题的，因为连续的病理过程通常只能通过粗糙、离散但有序的标签来观察（如眼科中的糖尿病视网膜病变分级）。

Method: 提出了一种有序潜在扩散模型，用于生成彩色眼底图像。该方法使用标量疾病表示代替传统的分类条件，将糖尿病视网膜病变严重程度的顺序结构明确纳入生成过程，从而实现相邻阶段之间的平滑过渡。

Result: 在EyePACS数据集上评估，与标准条件扩散模型相比，该模型在五个DR阶段中的四个阶段降低了Fréchet inception距离，并将二次加权κ从0.79提高到0.87。插值实验表明，该模型能够从有序的粗糙类别标签中学习到疾病进展的连续谱。
...
TL;DR: DeBiasLens：基于稀疏自编码器的视觉语言模型社会偏见缓解框架，通过定位社会属性神经元实现可解释的去偏，无需社会属性标签


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的黑盒推理过程可能导致社会偏见，现有去偏方法仅关注表面偏见信号，缺乏对模型内部动态的探索

Method: 使用稀疏自编码器在多模态编码器上定位社会属性神经元，通过无标签的面部图像或字幕数据集训练，选择性去激活与偏见最相关的神经元

Result: 有效缓解视觉语言模型的社会偏见行为，同时不损害其语义知识，为未来审计工具奠定基础

Conclusion: DeBiasLens为可解释、模型无关的偏见缓解提供了新框架，有助于在现实AI系统中优先考虑社会公平性
...
TL;DR: SR3R：一种前馈框架，直接从稀疏低分辨率多视角图像预测高分辨率3D高斯溅射表示，实现3D超分辨率的跨场景泛化和实时重建
...
Method: 将3D超分辨率重新定义为从稀疏低分辨率视图到高分辨率3D高斯溅射表示的直接前馈映射；引入SR3R框架，通过高斯偏移学习和特征细化来稳定重建并锐化高频细节；采用即插即用设计，可与任何前馈3DGS重建主干配对

Result: 在三个3D基准测试中超越现有最先进的3D超分辨率方法，实现强大的零样本泛化能力，甚至在未见场景上优于最先进的逐场景优化方法

Conclusion: SR3R通过直接学习从稀疏低分辨率视图到高分辨率3DGS表示的前馈映射，从根本上改变了3D超分辨率获取高频知识的方式，实现了跨场景的鲁棒泛化和高质量实时重建
...
TL;DR: SteerVAD：一种新颖的干预框架，通过主动引导和修正多模态大语言模型的内部表征来提升视频异常检测性能，仅需1%的训练数据即可达到SOTA效果。
...
Method: 提出SteerVAD框架：1）使用无梯度的表征可分性分析识别最具有判别性的注意力头作为潜在异常专家；2）设计分层元控制器，结合全局上下文和LAE输出生成动态修正信号；3）对LAE表征流形执行有针对性的各向异性缩放，放大异常相关维度并抑制固有偏见。
...
TL;DR: GuardAlign：一种无需训练的大视觉语言模型安全防御框架，通过最优传输增强的安全检测和跨模态注意力校准，有效降低不安全响应率同时保持模型实用性。


<details>
  <summary>Details</summary>
Motivation: 现有基于输入端的防御方法（使用CLIP检测不安全图像并在提示前添加安全前缀）在复杂场景中存在检测不准确、解码过程中安全信号不稳定的问题，需要更有效的安全防御方案。

Method: 提出GuardAlign框架，包含两个策略：1) OT增强安全检测：利用最优传输测量图像块与不安全语义的分布距离，无需额外计算成本即可准确识别恶意区域；2) 跨模态注意力校准：通过自适应重新分配各层注意力，增强安全前缀的影响，确保安全信号在生成过程中持续激活。

Result: 在六个代表性MLLM上的广泛评估显示，GuardAlign在SPA-VL上将不安全响应率降低高达39%，同时保持实用性，在VQAv2上准确率从78.51%提升至79.21%。

Conclusion: GuardAlign通过结合最优传输增强的安全检测和跨模态注意力校准，提供了一种无需训练的有效防御框架，显著提升大视觉语言模型的安全性同时保持其实用性。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言推理方面取得显著进展，但仍存在幻觉问题，即生成内容偏离视觉证据。现有缓解策略要么需要昂贵的训练监督，要么在推理时引入额外延迟。最近的视觉增强方法试图通过在解码过程中强化视觉令牌来解决此问题，但通常不加区分地注入所有令牌，导致背景区域干扰并分散模型对关键线索的注意力。

Method: 提出自适应视觉强化（AIR）框架，包含两个组件：1）基于原型的令牌缩减：将大量视觉令牌压缩为紧凑子集以抑制冗余；2）OT引导的补丁增强：量化隐藏状态与补丁嵌入之间的对齐，选择性地将最一致的补丁集成到前馈层中。该框架无需训练。

Result: 在代表性MLLMs上的广泛实验表明，AIR显著减少了幻觉，同时保留了一般能力，确立了其作为构建可靠MLLMs的有效解决方案。

Conclusion: AIR是一个无需训练的有效框架，通过选择性强化关键视觉信息来增强模型对显著视觉信息的依赖，有效缓解多模态大语言模型的幻觉问题，为构建可靠MLLMs提供了有效解决方案。
...
TL;DR: 提出统一框架从单张图像和视频序列重建3D服装人体，结合隐式缝纫模式和扩散模型学习2D UV空间服装形状先验，实现高保真静态和动态服装重建。


<details>
  <summary>Details</summary>
Motivation: 从单目图像和视频重建3D服装人体是虚拟试衣、虚拟人创建和混合现实的基础问题。尽管人体恢复取得显著进展，但准确重建服装几何（特别是宽松服装）仍是一个开放挑战。

Method: 结合隐式缝纫模式（ISP）与生成扩散模型学习2D UV空间服装形状先验；引入映射模型建立图像像素、UV图案坐标和3D几何之间的对应关系；扩展到动态重建时引入时空扩散方案和测试时指导以确保长期时间一致性；开发基于解析投影的约束，在可见区域保持图像对齐几何，同时在遮挡区域随时间推移保持连贯补全。

Result: 尽管仅在合成模拟布料数据上训练，但方法能很好地泛化到真实世界图像，在紧身和宽松服装上都持续优于现有方法。重建的服装保留精细几何细节，同时展现逼真的动态运动，支持纹理编辑、服装重定向和动画等下游应用。

Conclusion: 提出了一种统一框架，能够从单张图像和视频序列实现高保真3D服装重建，通过结合隐式缝纫模式和扩散模型学习形状先验，在静态和动态场景中都能准确重建服装几何，包括宽松服装的细节。
...
TL;DR: QE提出了一种针对视觉语言模型的token感知自适应误差补偿量化方法，通过混合专家机制处理不同token中重要通道的分布差异，显著提升量化性能。
...
Method: 提出Quant Experts (QE)：1) 将重要通道分为token无关和token相关两组；2) 对token无关组，设计共享专家使用低秩适配器补偿全局量化误差；3) 对token相关组，设计路由专家包含多个路由低秩适配器补偿特定token相关的局部量化误差。
...
TL;DR: EvalMVX：首个同时评估多视角立体视觉（MVS）、多视角偏振形状恢复（MVSfP）和多视角光度立体视觉（MVPS）的真实世界数据集，包含25个物体在20个视角和17种光照条件下的8500张偏振图像，提供对齐的真值3D网格用于定量基准测试。


<details>
  <summary>Details</summary>
Motivation: 当前真实世界数据集主要关注基于RGB输入的多视角立体视觉（MVS）基准测试，而多视角光度立体视觉（MVPS）和多视角偏振形状恢复（MVSfP）在高保真表面重建和稀疏输入中不可或缺，但尚未与MVS一起进行定量评估。需要确定不同MVX技术（MVS、MVSfP、MVPS）的工作范围。

Method: 提出EvalMVX数据集，包含25个物体，每个物体使用偏振相机在20个不同视角和17种光照条件（包括OLAT和自然光照）下采集，共8500张图像。每个物体提供对齐的真值3D网格，支持同时定量评估多种MVX方法。

Result: 基于EvalMVX评估了近年来发表的13种MVX方法，记录了最佳性能方法，并识别了在不同几何细节和反射类型下的开放问题。数据集和基准测试结果可为多视角3D重建的未来研究提供参考。

Conclusion: EvalMVX填补了同时评估MVS、MVSfP和MVPS技术的空白，为确定不同MVX方法的工作范围提供了标准化基准，有望推动多视角3D重建领域的研究发展。
...
TL;DR: DiffusionHarmonizer：用于神经重建场景的在线生成增强框架，通过单步时序条件增强器提升渲染真实感和时间一致性


<details>
  <summary>Details</summary>
Motivation: 神经重建方法（如NeRF和3D高斯泼溅）在模拟自动驾驶等机器人场景时存在两个主要问题：1）渲染新视角时出现伪影；2）难以真实地集成来自不同场景的动态对象。现有方法无法在保持时间一致性的同时提升真实感。

Method: 提出DiffusionHarmonizer在线生成增强框架，核心是从预训练多步图像扩散模型转换而来的单步时序条件增强器，可在单GPU上实时运行。通过定制数据构建流程创建合成-真实对，重点强调外观协调、伪影校正和光照真实感。

Result: 构建了可扩展系统，显著提升了研究环境和生产环境中的模拟保真度，能够生成时间一致且更真实的渲染输出。

Conclusion: DiffusionHarmonizer通过在线生成增强有效解决了神经重建场景中的伪影和动态对象集成问题，为自动驾驶等机器人系统的模拟提供了更高保真度的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有两阶段运动中心方法存在两个根本限制：1) 由于在运动估计之前进行显式前景分割导致的解耦优化引起的误差累积；2) 顺序处理带来的计算瓶颈。需要一种更高效、更准确的单阶段跟踪框架。
...
Conclusion: FocusTrack通过统一的运动-语义协同建模，成功解决了现有两阶段方法的误差累积和计算瓶颈问题，实现了高效准确的单阶段3D点云目标跟踪。
...
TL;DR: 提出一种高效的三维高斯溅射优化方法，通过重建感知的剪枝策略和新型DoG基元，在减少90%高斯数量的同时保持或提升渲染质量。


<details>
  <summary>Details</summary>
Motivation: 三维高斯溅射(3DGS)需要大量基元实现高保真度，导致冗余表示和高资源消耗，限制了其在复杂或大规模场景中的可扩展性。需要有效的剪枝策略和更具表达力的基元来减少冗余同时保持视觉质量。

Method: 1) 提出高效、集成的重建感知剪枝策略，根据重建质量自适应确定剪枝时机和细化间隔；2) 引入三维差分高斯(DoG)基元，在单个基元中联合建模正负密度，提高紧凑配置下高斯的表达能力。

Result: 方法显著提高了模型紧凑性，实现了高达90%的高斯数量减少，同时提供的视觉质量与最先进方法相似或在某些情况下更好。

Conclusion: 提出的重建感知剪枝策略和DoG基元有效解决了3DGS中的冗余问题，在保持高质量渲染的同时大幅减少了模型规模，为实际部署提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前手术视频识别方法依赖大规模标记数据和计算密集型预训练，成本高昂。本研究质疑这种重型预训练的必要性，探索利用视频中已有信息进行手术理解的可能性。

Method: TASOT扩展了ASOT，将时序动作分割建模为多模态最优传输问题。匹配成本定义为视觉和文本成本的加权组合：视觉项捕捉帧级外观相似性，文本项提供补充语义线索，两者通过时间一致的不平衡Gromov-Wasserstein公式共同正则化。

Result: 在多个基准手术数据集上，TASOT相比现有零样本方法取得显著改进：StrasBypass70 (+23.7)、BernBypass70 (+4.5)、Cholec80 (+16.5)、AutoLaparo (+19.6)。

Conclusion: 通过利用标准视觉和文本表示中的已有信息，无需复杂预训练流程即可实现精细的手术理解，挑战了当前依赖大规模手术特定预训练的主流范式。
...
TL;DR: RETA提出检索与拓扑对齐框架解决解耦数据集蒸馏中的拟合-复杂度鸿沟和锚点拉扯效应，通过动态检索连接和持续拓扑对齐提升合成图像质量与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有解耦数据集蒸馏方法使用静态真实图像块进行残差匹配，导致拟合-复杂度鸿沟和锚点拉扯效应，降低了类内多样性和泛化性能。

Method: 提出RETA框架：1) 动态检索连接(DRC)通过最小化教师特征空间的拟合-复杂度分数从预建池中选择真实图像块注入；2) 持续拓扑对齐(PTA)使用持续同调正则化合成过程，构建互k-NN特征图并计算持续图像，惩罚真实与合成集之间的拓扑差异。

Result: 在CIFAR-100、Tiny-ImageNet、ImageNet-1K及多个ImageNet子集上，RETA在可比时间和内存下持续超越多种基线方法，特别是在ImageNet-1K上达到64.3% top-1准确率（ResNet-18，每类50张图像），比先前最佳方法提升3.1%。

Conclusion: RETA通过动态检索连接和持续拓扑对齐有效解决了现有解耦数据集蒸馏方法的局限性，显著提升了合成图像质量和模型泛化性能，为大规模数据集压缩提供了新思路。
...
TL;DR: 提出HumanOrbit方法，从单张输入图像生成人物360°环绕视频，利用视频扩散模型实现多视角一致合成并重建纹理网格


<details>
  <summary>Details</summary>
Motivation: 现有基于图像扩散模型的多视角合成方法存在视角不一致和身份保持问题，而视频扩散模型在生成真实感结果方面表现优异，因此探索利用视频扩散模型进行多视角人物图像生成

Method: 提出HumanOrbit视频扩散模型，能够合成围绕人物的连续相机旋转，生成几何一致的新视角同时保持人物外观和身份；进一步提出重建流程从生成的多视角帧恢复纹理网格

Result: 实验验证HumanOrbit在多视角图像生成方面的有效性，重建的3D模型在完整性和保真度方面优于现有最先进基线方法

Conclusion: HumanOrbit能够从单张图像生成高质量的多视角人物视频，并重建出具有优越完整性和保真度的3D模型，为单图像到3D重建提供了有效解决方案
...
TL;DR: 提出一种考虑高维属性流形和图像空间布局的超像素层次结构，用于大规模高维图像的可视化探索
...
<details>
  <summary>Details</summary>
Motivation: 从单张肖像图像重建逼真且可动画的4D头部化身是计算机视觉中的基础挑战。现有方法主要依赖2D先验，难以实现一致的3D几何结构，需要更强的几何先验来提升重建质量。

Method: 提出几何感知扩散框架，联合合成肖像图像和对应的表面法线；使用无姿态表情编码器捕获隐式表情表示；将合成图像和表情潜在编码整合到基于3D高斯的化身中，实现逼真渲染和准确几何。

Result: 实验表明，该方法在视觉质量、表情保真度和跨身份泛化能力上显著优于现有最先进方法，同时支持实时渲染。

Conclusion: 通过几何感知扩散学习强几何先验，能够从单张肖像图像重建高质量、可动画的4D头部化身，在多个关键指标上超越现有方法，为头像重建提供了有效解决方案。
...
TL;DR: 提出Omnivorous Vision Encoder框架，解决预训练视觉编码器（如DINOv2）在多模态特征对齐上的不足，通过学习模态无关的特征空间，实现跨模态一致性表示。


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉编码器（如DINOv2）在单模态任务上表现优异，但其特征表示在不同模态间对齐性差。例如，同一场景的RGB图像和深度图的特征嵌入余弦相似度与两个随机无关图像几乎相同，缺乏跨模态一致性。

Method: 提出Omnivorous Vision Encoder框架，采用双重训练目标：1）最大化同一场景不同模态间的特征对齐；2）蒸馏目标，将学习到的表示锚定到完全冻结的教师模型（如DINOv2）输出。通过这种方式，学生编码器学习产生与模态无关的一致性嵌入。

Result: 训练得到的"全模态"编码器能够为给定场景产生一致的强大嵌入，无论输入模态如何（RGB、深度、分割等）。该方法在保持原始基础模型判别语义的同时，实现了鲁棒的跨模态理解。

Conclusion: Omnivorous Vision Encoder通过模态无关特征空间学习，有效解决了预训练视觉编码器的跨模态对齐问题，在保留基础模型优势的同时增强了多模态理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管机器学习医学图像分类器取得进展，但其安全性和可靠性仍是实际应用中的主要问题。现有审计方法主要依赖单模态特征或基于元数据的子组分析，解释性有限且常无法发现隐藏的系统性失败

Method: 引入首个自动化审计框架，将切片发现方法扩展到多模态表示，专门针对医学应用。使用MIMIC-CXR-JPG数据集在常见失败场景下进行综合实验

Result: 框架在失败发现和解释生成方面表现出强大能力。多模态信息通常允许更全面有效的分类器审计，而超越仅图像输入的单模态变体在资源受限场景中展现出强大潜力
...
Method: 提出敏感性感知缓存(SenCache)框架：1) 通过分析模型输出对噪声潜变量和时间步扰动的敏感性，形式化缓存误差；2) 证明敏感性是缓存误差的关键预测因子；3) 基于此设计动态缓存策略，在每样本基础上自适应选择缓存时间步。

Result: 在Wan 2.1、CogVideoX和LTX-Video上的实验表明，在相似计算预算下，SenCache比现有缓存方法获得更好的视觉质量。

Conclusion: SenCache为自适应缓存提供了理论基础，解释了先前经验启发式方法部分有效的原因，并将其扩展为动态、样本特定的方法，实现了更高效的视频生成推理加速。
...
TL;DR: MuViT是一种用于多分辨率显微镜图像分析的Transformer架构，通过世界坐标系统整合不同尺度的图像信息，在多个生物医学图像任务上优于传统ViT和CNN基线。


<details>
  <summary>Details</summary>
Motivation: 现代显微镜产生包含多尺度结构（从细胞形态到组织组织）的千兆像素图像，但现有视觉模型通常在单一分辨率下操作或从单一视图提取多尺度特征，限制了利用显微镜数据固有多分辨率特性的能力。

Method: 提出MuViT Transformer架构，将来自同一底层图像的真实多分辨率观测融合。通过将所有图像块嵌入共享的世界坐标系统，并将旋转位置编码扩展到这些坐标，使注意力机制能够在单个编码器中整合广域上下文和高分辨率细节。

Result: 在合成基准测试、肾脏组织病理学和高分辨率小鼠脑显微镜数据上，MuViT相比强大的ViT和CNN基线模型取得了一致的改进。多分辨率MAE预训练进一步产生了尺度一致的表征，增强了下游任务性能。

Conclusion: 显式的世界坐标建模为大规模显微镜分析中利用多分辨率信息提供了一种简单而强大的机制，能够有效整合宽视野上下文与高分辨率细节。
...
Method: 首先构建包含超过80k偏好对的SpatialReward-Dataset，基于此构建SpatialScore奖励模型来评估文本到图像生成中的空间关系准确性，并利用该奖励模型实现复杂空间生成的在线强化学习

Result: SpatialScore奖励模型在空间评估方面表现优异，甚至超越了领先的专有模型，在多个基准测试中显示出对图像生成空间理解的显著且一致的提升

Conclusion: 专门设计的奖励模型能够有效增强图像生成模型的空间理解能力，为解决复杂空间关系编码问题提供了有效解决方案
...
TL;DR: GTASR是一种用于真实世界图像超分辨率的轻量级一致性训练方法，通过轨迹对齐和双参考结构校正解决一致性漂移和几何解耦问题，实现高效高质量的超分辨率。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的真实世界图像超分辨率方法虽然感知质量优秀但计算成本高；现有蒸馏方法参数庞大且受限于教师模型能力；一致性模型虽然推理高效但存在一致性漂移累积和几何解耦问题。

Method: 提出GTASR方法：1) 轨迹对齐策略通过全路径投影修正切向量场；2) 双参考结构校正机制施加严格的结构约束，确保生成轨迹保持结构一致性。

Result: 大量实验验证GTASR在保持极低延迟的同时，性能优于代表性基线方法。
...
TL;DR: 论文从几何角度形式化了组合泛化的三个必要条件，证明线性分解和正交性是组合泛化的必然要求，为观察到的线性表示假设提供了理论依据，并通过实验验证了这些预测。


<details>
  <summary>Details</summary>
Motivation: 尽管现代模型在大规模数据上训练，但只能覆盖组合空间中极小部分，需要研究支持未见组合泛化的表示结构。论文旨在形式化组合泛化的必要条件，为观察到的线性表示现象提供理论解释。

Method: 首先形式化组合泛化的三个要求（可分性、可迁移性、稳定性），证明这些要求施加了必要的几何约束：表示必须线性分解为每个概念的分量，且这些分量在概念间必须正交。推导了维度界限，将可组合概念数量与嵌入几何联系起来。在CLIP、SigLIP、DINO等现代视觉模型上实证评估这些预测。

Result: 理论分析表明线性分解和正交性是组合泛化的必要条件。实验发现表示确实表现出部分线性分解，具有低秩、近正交的每个概念因子，且这种结构的程度与在未见组合上的组合泛化能力相关。维度界限将可组合概念数量与表示几何联系起来。

Conclusion: 线性表示假设中观察到的线性结构是组合泛化的必然结果。论文为这一现象提供了理论依据，并预测随着模型规模扩大，它们可能收敛到满足这些几何条件的表示结构。这为理解和设计具有组合泛化能力的系统提供了指导。
...
TL;DR: HAL模型通过分层因果数据生成过程和确定性对齐过程，利用高低层潜在变量不同时间尺度特性，实现弱监督动作分割，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 人类通过关键转换在多个抽象层次上感知动作，而机器依赖视觉特征容易过分割。高低层潜在变量具有不同演化速率：低层视觉变量变化快，高层动作变量变化慢更易识别，这启发了分层动作学习。
...
Result: 在温和假设下证明潜在动作变量严格可识别。在多个基准测试上的实验结果表明，HAL模型在弱监督动作分割任务中显著优于现有方法，证实了其实际应用有效性。

Conclusion: 通过利用高低层潜在变量的不同时间尺度特性，HAL模型成功实现了弱监督动作分割，其分层因果建模和变量对齐方法为解决视频理解中的分层推理问题提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 视频生成从秒级扩展到分钟级面临关键瓶颈：短视频数据丰富且保真度高，而连贯的长视频数据稀缺且局限于狭窄领域。

Method: 提出MMM训练范式（Mode Seeking meets Mean Seeking），通过解耦扩散Transformer统一表示，分离局部保真度和长期连贯性。使用全局流匹配头在长视频上监督学习捕捉叙事结构，同时采用局部分布匹配头通过模式寻求的反向KL散度将滑动窗口与冻结的短视频教师对齐。

Result: 该方法能够合成分钟级视频，通过监督流匹配从有限长视频中学习长程连贯性和运动，同时通过对齐每个滑动窗口段到冻结短视频教师来继承局部真实感，实现少步快速长视频生成。

Conclusion: 评估表明，该方法通过联合改进局部清晰度、运动和长程一致性，有效缩小了保真度-时长差距，为长视频生成提供了有效解决方案。
...
TL;DR: UFO-4D：一种统一的端到端前馈框架，仅从一对无位姿图像重建密集的显式4D表示，直接估计动态3D高斯泼溅，实现几何、运动和相机位姿的联合一致估计。


<details>
  <summary>Details</summary>
Motivation: 当前基于无位姿图像的密集4D重建方法存在两个主要问题：1）依赖缓慢的测试时优化；2）使用碎片化的、任务特定的前馈模型。需要一种统一的端到端前馈框架来解决这些问题。

Method: 核心方法是通过单个动态3D高斯表示的可微分渲染多个信号（外观、深度、运动），实现自监督图像合成损失。所有模态共享相同的几何基元，监督一个模态会自然地正则化并改进其他模态，这种协同作用克服了数据稀缺问题。

Result: UFO-4D在联合几何、运动和相机位姿估计方面比先前工作提升了高达3倍性能。该表示还支持跨新视角和时间的高保真4D插值。

Conclusion: UFO-4D通过统一的动态3D高斯表示和可微分多信号渲染，实现了从无位姿图像对进行密集4D重建的突破，显著提升了重建质量和效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有长文档主题分割方法的不足：传统判别式模型受固定窗口限制无法建模文档级语义；生成式大语言模型推理成本高且难以支持长输入

Method: 基于Qwen3-0.6B构建判别式分割模型，添加跨窗口上下文融合层和边界分类头，结合重叠滑动窗口策略；提出带标量校正的向量融合方法压缩超长片段表示

Result: 在WIKI-727K数据集上，相比Jina发布的三个基于Qwen2-0.5B的生成式模型，本方法获得更好的宏观平均F1分数，推理速度快两个数量级

Conclusion: 提出的判别式分割模型显著提升了长文档处理的实用性和可扩展性，支持超长文档段落边界检测，同时通过向量融合方法增强下游检索效率
...
TL;DR: Task-Lens是一个跨任务调查框架，评估了50个印度语音数据集在9个下游语音任务中的适用性，揭示了未充分利用的元数据潜力，并识别了资源不足的任务和语言。


<details>
  <summary>Details</summary>
Motivation: 包容性语音技术需求增长，但低资源语言（特别是印度多语言环境）的任务特定资源认知有限，现有调查通常只针对单一任务，缺乏全面的跨任务分析。

Method: 提出Task-Lens跨任务调查框架：1) 分析50个印度语音数据集（涵盖26种语言）的元数据和特性对9个下游任务的适用性；2) 提出任务对齐的增强方案以释放数据集潜力；3) 识别资源严重不足的任务和语言。

Result: 发现许多印度语音数据集包含未充分利用的元数据，可支持多个下游任务；揭示了跨任务关联和资源缺口，使研究人员能够探索现有数据集的更广泛应用，并优先为服务不足的任务和语言创建数据集。

Conclusion: Task-Lens通过跨任务分析缓解了印度语音数据集稀缺问题，展示了现有数据集的未开发潜力，为研究人员提供了探索数据集更广泛适用性的框架，并指导了未来数据集创建的重点方向。
...
TL;DR: SLATE提出了一种基于截断步级采样和密集LLM评判奖励的强化学习框架，解决语言模型搜索推理中的信用分配问题，显著降低梯度方差并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在信用分配问题：稀疏奖励方法（如Search-R1）无法将成功或失败归因于单个推理和检索决策；过程奖励方法（如StepSearch）依赖启发式奖励且仍采样完整轨迹，导致高梯度方差。

Method: SLATE框架包含两个核心思想：(1) 截断步级采样：生成k条共享共同前缀、仅在下一步不同的轨迹；(2) 密集LLM评判奖励：用能力强的LLM评估器替代启发式评分，评估每个推理步骤、搜索查询和答案的质量。

Result: 理论证明在相同密集奖励结构下，截断采样将优势估计方差降低最多T倍（T步轨迹）；在七个QA基准测试中，SLATE始终优于稀疏奖励和过程奖励基线，在更困难的多跳任务和较小模型上增益最大。

Conclusion: SLATE通过截断采样和密集LLM奖励有效解决了搜索增强语言模型训练中的信用分配问题，降低了梯度方差，提升了模型性能，特别是在复杂推理任务上表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型生成看似合理但实际不存在的幻觉引用，已在主要机器学习会议提交和接收论文中出现，暴露同行评审漏洞。现有工具对噪声和异构引用格式脆弱，缺乏标准化评估，而手动验证因参考文献列表快速增长变得不切实际。

Method: 提出多智能体验证管道，将引用检查分解为：主张提取、证据检索、段落匹配、推理和校准判断，评估引用来源是否真正支持其主张。构建大规模人工验证跨领域数据集，定义统一的引用忠实性和证据对齐指标。

Result: 实验显示最先进的大型语言模型存在大量引用错误，提出的框架在准确性和可解释性方面显著优于现有方法。提供了首个可扩展的引用审计基础设施。

Conclusion: 该工作为大型语言模型时代提供了首个可扩展的引用审计基础设施和实用工具，可提高科学引用的可信度，应对幻觉引用对科学完整性的威胁。
...
TL;DR: FHIRPath-QA：首个基于真实临床数据的患者特异性问答开源数据集和基准，采用文本转FHIRPath查询范式，显著减少LLM使用，提升临床问答的安全性和效率。


<details>
  <summary>Details</summary>
Motivation: 当前电子健康记录（EHR）接口难以提供精确可信的患者特异性答案，而基于检索的大语言模型方法存在计算效率低、易产生幻觉、难以在真实EHR中部署等问题。

Method: 提出文本转FHIRPath问答范式，将推理从自由文本生成转向FHIRPath查询合成；基于MIMIC-IV on FHIR Demo构建数据集，包含超过14k自然语言问题（患者和临床医生表述）与已验证的FHIRPath查询和答案。
...
TL;DR: 论文提出ProbCOPA数据集，包含210个人工标注的概率推理问题，发现人类推理具有概率性和多样性，而当前LLM无法产生类似人类的概率分布，揭示了LLM在非确定性推理方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 人类推理经常基于有限信息进行概率性推断，而当前LLM虽然在逻辑和数学任务上表现良好，但在这种开放式的非确定性推理方面的行为尚未得到充分探索。研究者希望了解LLM在概率推理任务上是否能够产生类似人类的判断。

Method: 1. 创建ProbCOPA数据集：包含210个手工制作的概率推理问题（英文）；2. 每个问题由25-30名人类参与者标注推理可能性；3. 比较8个最先进的推理LLM与人类判断的差异；4. 分析LLM的推理链以识别其推理模式。

Result: 1. 人类对推理可能性的判断呈现分级和多样化的概率分布；2. LLM无法产生类似人类的概率分布，存在系统性差异；3. 分析LLM推理链发现它们使用共同的推理模式来评估此类推理；4. 揭示了人类与LLM在概率推理方面的持续差异。

Conclusion: 研究揭示了LLM在概率推理方面与人类存在显著差异，强调了需要在非确定性环境中评估推理能力的重要性。当前LLM在开放式概率推理任务上尚未达到人类水平的表现。
...
TL;DR: 提出多智能体因果推理框架MACR，通过推理智能体扩展用户交互，偏置感知决策智能体缓解隐藏偏见，用于社交媒体自杀风险检测
...
Method: 提出多智能体因果推理框架MACR，包含两个协作智能体：推理智能体整合认知评价理论生成用户对帖子的反事实反应，通过认知、情感和行为三个维度分析用户交互；偏置感知决策智能体采用前门调整策略，利用推理智能体生成的反事实用户反应来缓解隐藏偏见

Result: 在真实世界对话数据集上的广泛实验证明了MACR在识别自杀风险方面的有效性和鲁棒性

Conclusion: MACR框架不仅缓解了隐藏偏见，还通过反事实知识丰富了用户交互的上下文信息，为社交媒体自杀风险检测提供了更全面的解决方案
...
TL;DR: BRIDGE框架通过合成高质量少数群体样本，减少教育自动评分系统中的偏见放大问题，特别针对英语学习者群体


<details>
  <summary>Details</summary>
Motivation: 教育自动评分系统依赖深度学习和大语言模型，但面临偏见放大风险，特别是对英语学习者等少数群体。模型可能继承并放大数据中已有的差异，导致对少数群体学生评分偏低，即使他们展示了相当的领域知识但使用不同的语言模式。

Method: 提出BRIDGE（Bias-Reducing Inter-group Data GEneration）框架，用于低资源评估设置。该方法不依赖有限的少数群体样本，而是通过将丰富的非英语学习者高分数样本中的结构相关（即评分标准对齐的知识和证据）内容"粘贴"到真实的英语学习者语言模式中，合成高质量的英语学习者样本。还引入鉴别器模型确保合成样本质量。

Result: 在加州科学测试数据集上的实验表明，BRIDGE有效减少了高分数英语学习者学生的预测偏见，同时保持了整体评分性能。该方法实现的公平性提升与使用额外真实人类数据相当，为大规模评估提供了成本效益高的公平评分解决方案。

Conclusion: BRIDGE框架通过创新的数据生成方法，有效缓解了教育自动评分系统中的偏见放大问题，特别是针对英语学习者群体，为大规模评估提供了公平且成本效益高的解决方案。
...
TL;DR: 提出了LFQA-HP-1M数据集（130万个人类成对偏好标注）和九个评估标准，基于这些特征的简单线性模型性能与最先进的LLM评估器相当，同时揭示了LLM评估器的各种偏差和脆弱性。
...
Method: 1) 构建LFQA-HP-1M数据集，包含130万个人类成对偏好标注；2) 提出九个答案质量评估标准；3) 基于这些特征训练简单线性模型；4) 分析LLM评估器的传递一致性、位置偏差、冗长偏差和对抗扰动的脆弱性。

Result: 1) 基于九个特征的简单线性模型性能与最先进的LLM评估器相当；2) 揭示了LLM评估器存在传递一致性、位置偏差和冗长偏差问题；3) 证明了LLM评估器对对抗扰动具有脆弱性；4) 提供了最大的公开LFQA偏好数据集。

Conclusion: 这项工作提供了大规模LFQA偏好数据集和基于标准的评估框架，实现了透明可靠的LFQA评估，同时揭示了当前LLM评估器的局限性。
...
TL;DR: 提出基于LLM驱动的框架，通过三层优化合成多轮任务导向对话，构建真实推理场景的数据集，用于评估和提升大语言模型的现实逻辑推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有推理基准无法充分反映真实世界场景的复杂性，数据集过于简化抽象，与现实任务流程、领域约束和操作规则脱节，且存在数据污染问题，传统众包方法构建数据集成本高、难以扩展。

Method: 提出LLM驱动的框架，通过三层优化合成多轮任务导向对话：1）基于真实任务场景生成对话；2）融入现实世界信息增强真实性；3）确保强上下文连贯性。围绕对话精心设计推理任务，并通过迭代优化持续提升任务质量和挑战性。

Result: 实验结果表明，基于合成数据的推理任务引入了非平凡的推理挑战，为提升LLMs的推理能力提供了有意义的支持。生成的数据集可作为评估和推进LLMs现实逻辑推理能力的有价值基准。

Conclusion: 该框架成功解决了现有推理评估的局限性，通过合成真实场景的多轮任务导向对话，构建了高质量、可扩展的推理数据集，有效支持LLMs现实逻辑推理能力的评估和提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有TRIZ矛盾挖掘方法主要依赖基于规则的系统或传统机器学习模型，在处理复杂专利语言时面临语义模糊、领域依赖和泛化能力有限的问题。大语言模型虽有强大语义理解能力，但直接应用于TRIZ参数提取存在幻觉问题和结构化TRIZ知识基础不足的挑战

Method: 提出TRIZ-RAGNER框架，将矛盾挖掘重新定义为语义级命名实体识别任务。框架集成三个核心组件：1）基于TRIZ知识库的密集检索；2）用于上下文精化的交叉编码器重排序；3）结构化LLM提示，从专利句子中提取改进和恶化参数。通过向LLM推理过程注入领域特定的TRIZ知识，减少语义噪声并提高提取一致性
...
Conclusion: TRIZ-RAGNER通过检索增强和结构化提示有效解决了传统方法和纯LLM在TRIZ矛盾挖掘中的局限性，为专利分析和系统创新提供了更准确、一致的矛盾参数提取框架
...
TL;DR: 提出基于智能体的动态基准测试范式，通过教师、协调者和学生智能体的协作迭代生成、验证和解决问题，实现难度自适应扩展，超越传统静态数据集限制。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型评估依赖静态数据集，存在可扩展性有限、无法捕捉模型演进推理能力的问题。需要一种能够适应模型发展、动态评估推理能力的新方法。

Method: 采用智能体中心基准测试范式：教师智能体生成候选问题，协调者智能体验证问题有效性并防御对抗攻击，学生智能体尝试解决验证后的问题。无效问题由教师修订，正确解答后教师生成更具挑战的变体，实现难度自适应扩展。

Result: 该协议系统性地暴露了传统基准测试无法揭示的边界案例推理错误，特别是在文本异常检测任务中，需要跨句子逻辑推理且抵抗模式匹配捷径。通过交叉模型配对性能和初始与最终问题间的进展等多维度评估，展示了方法的有效性。

Conclusion: 从固定数据集转向动态协议的方法为大语言模型评估提供了可持续方向，引入了以智能体中心基准测试共同进化为核心的研究议程，能够适应不断演进的语言模型能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决少样本文本分类中的三个核心问题：1) 语义纠缠导致特征边界模糊；2) 标签结构不清晰影响分类决策；3) 低资源条件下特征表示不足。这些问题限制了模型在有限标注数据下的性能。

Method: 1) 使用预训练语言模型编码文本获取基础语义表示；2) 引入由多维度语义因素组成的结构化提示，通过可学习的组合机制与文本特征融合；3) 构建结构化标签嵌入矩阵，采用跨空间对齐机制确保文本特征与标签属性的稳定匹配；4) 应用提示正交性约束和联合优化目标，保持不同语义因素间的独立性。

Result: 实验结果表明：1) 有效缓解了少样本文本分类中的语义冲突和标签模糊问题；2) 在准确率、精确率、召回率和AUC等指标上显著提升；3) 表现出强大的跨任务适用性；4) 通过敏感性实验验证了框架在不同条件下的稳定性和鲁棒性。

Conclusion: 提出的结构化提示优化框架能够有效提升少样本文本分类性能，通过多维度语义因素和跨空间对齐机制增强了语义理解和任务适应能力，为低资源条件下的文本分类提供了透明可控的解决方案。
...
Method: DiCo采用三阶段分治范式：1) 划分阶段：探索输入掩码序列，识别种子标记并扩展构建局部簇；2) 征服阶段：在不同局部簇间执行并行解码；3) 最终化阶段：使用细粒度复合解码方案解码剩余少量掩码标记。分治过程在划分和征服阶段间交替直至收敛。
...
TL;DR: GLUScope是一个用于分析Transformer语言模型中神经元的开源工具，专注于处理具有门控激活函数（如SwiGLU）的现代模型，通过分析四种符号组合来提供更全面的神经元功能理解。


<details>
  <summary>Details</summary>
Motivation: 现有工具主要针对较旧的Transformer模型，而现代模型普遍采用门控激活函数（如SwiGLU），这带来了新的分析挑战：仅理解正激活已不足够，需要同时考虑门激活和输入激活的正负符号组合。

Method: 开发GLUScope工具，针对每个神经元展示四种符号组合（门激活和输入激活的正负组合）的文本示例，并统计每种组合的出现频率，帮助研究者全面理解神经元功能。

Result: GLUScope能够处理现代门控激活函数模型，通过分析四种符号组合揭示神经元的不同功能模式，提供了比传统方法更全面的分析视角，并已通过实际案例展示了其能带来新的研究洞见。

Conclusion: GLUScope填补了针对现代门控激活函数Transformer模型的分析工具空白，通过同时考虑门激活和输入激活的符号组合，为可解释性研究提供了更全面、深入的神经元分析能力。
...
TL;DR: 提出中文语言与事实错误联合纠正任务CLFEC，构建多领域专业写作数据集，系统研究LLM纠正范式，发现统一处理优于分离流程，智能体工作流在合适模型下有效


<details>
  <summary>Details</summary>
Motivation: 传统中文纠错主要关注拼写和语法，事实错误纠正通常单独处理。但在段落级中文专业写作中，语言错误（词汇/语法/标点）和事实错误经常共存并相互影响，使得统一纠正既必要又具挑战性

Method: 提出CLFEC任务，构建涵盖时事、金融、法律、医学的多领域中文专业写作混合数据集，系统研究基于LLM的纠正范式，包括提示工程、检索增强生成和智能体工作流

Result: 分析揭示了实际挑战：专业纠正模型泛化能力有限、事实修复需要证据支撑、混合错误段落处理困难、对干净输入的过度纠正。结果显示在同一上下文中处理语言和事实错误优于分离流程，智能体工作流在合适骨干模型下有效

Conclusion: 数据集和实证发现为在工业环境中构建可靠的全自动校对系统提供了指导，统一处理语言和事实错误具有实际价值，智能体工作流是有效的解决方案方向
...
TL;DR: LLMs能够从严重退化的英语文本中恢复语义，即使内容词被随机替换为无意义字符串，也能翻译成接近原文的常规英语。
...
Method: 通过将英语文本中的内容词随机替换为无意义字符串（称为"Jabberwockified"英语），然后使用LLMs将这些退化文本翻译回常规英语，评估其恢复语义的能力。

Result: LLMs表现出惊人的能力，能够从严重退化的文本中恢复意义，在许多情况下生成的翻译接近原始文本。这表明结构线索对词汇意义的约束程度远超想象。

Conclusion: LLMs理解"Jabberwockified"英语的能力虽然是超人类的，但对理解语言结构有重要意义，表明高效的语言处理（无论是生物还是人工系统）很可能受益于句法、词汇语义和一般世界知识的紧密整合。
...
<details>
  <summary>Details</summary>
Motivation: 尽管基于Transformer的模型（如BERT）显著推进了多语言自然语言处理，但使用天城体文字书写的低资源语言尼泊尔语仍相对缺乏研究。需要评估现有多语言、印度语言、印地语和尼泊尔语BERT变体在尼泊尔语主题分类任务上的有效性。
...
Result: 印度语言模型表现最佳，特别是MuRIL-large模型获得了最高的F1分数90.60%，超过了多语言和单语言模型。NepBERTa也表现出竞争力，F1分数为88.26%。

Conclusion: 该研究为尼泊尔语文档级分类和更广泛的尼泊尔语NLP应用建立了稳健的基准，表明针对特定语言家族（如印度语言）训练的模型在低资源语言任务上可能优于通用多语言模型。
...
TL;DR: 从18世纪《百科全书》中提取地理坐标数据集，开发两阶段模型自动识别和标准化坐标，验证跨语言跨领域泛化能力


<details>
  <summary>Details</summary>
Motivation: 从历史文本中自动提取地理坐标具有挑战性，因为坐标表达方式多样且精度不一。为改进早期现代数字化文本的坐标检索，需要创建高质量数据集和有效方法

Method: 从《百科全书》74,000篇文章中筛选15,278个地理条目，人工标注4,798个含坐标条目。开发两阶段管道：分类器识别含坐标条目，第二个模型提取和标准化坐标。使用基于transformer的模型，测试编码器-解码器和解码器架构

Result: 交叉验证获得86% EM分数。在域外18世纪法语Trevoux词典上，微调模型获得61% EM分数；在19世纪英文《大英百科全书》第七版上获得77% EM分数。证明数据集作为训练数据的有效性，以及方法的跨语言跨领域泛化能力

Conclusion: 创建的地理坐标黄金标准数据集对训练模型有效，两阶段方法在跨语言和跨领域任务中表现出良好的泛化能力，为历史文本地理信息提取提供了实用解决方案
...
TL;DR: 论文提出了HLME情感增强记忆评估基准，用于评估主流记忆系统处理情感信息的能力，发现现有系统在情感记忆处理方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 当前LLM记忆系统在处理长时间交互时存在上下文丢失问题，但与人类认知相比，这些系统处理情感相关信息的有效性仍不明确，需要建立专门的评估基准来填补这一研究空白。

Method: 开发了HLME（类人记忆情感）数据集，从三个维度评估记忆系统：情感信息提取、情感记忆更新和情感记忆问答，对主流和最先进的记忆系统进行系统性评估。

Result: 实验结果表明，所有被评估的系统在三个任务中均未达到稳健的性能，揭示了当前记忆系统在处理情感记忆方面的普遍缺陷。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种系统化的方法来从语言模型的梯度中提取和学习特征方向，以便更好地理解和操作模型中的特征表示，同时提供完整的工具链来支持特征分析、模型重写和比较研究。

Method: GRADIEND方法基于事实-反事实的掩码语言建模（MLM）和因果语言建模（CLM）梯度分析，通过对比事实和反事实情境下的梯度差异来学习特征方向。gradiend包实现了该方法，提供数据创建、训练、评估、可视化、模型重写和多特征比较的统一工作流。

Result: 成功开发了gradiend开源包，并在英语代词范式和大规模特征比较两个用例中进行了演示，能够重现先前的研究用例，验证了方法的有效性和工具的实用性。

Conclusion: gradiend包为语言模型特征方向学习提供了一个全面、可操作的工具集，支持从梯度分析到模型重写的完整工作流程，有助于深入理解语言模型中的特征表示和进行可控的模型编辑。
...
<details>
  <summary>Details</summary>
Motivation: 西班牙语在21个国家使用，拥有超过4.41亿使用者，存在多种方言变体。YouTube作为大型媒体平台，仅提供单一的西班牙语自动字幕选项，这引发了该系统是否对某些方言存在偏见的疑问。

Method: 通过分析YouTube自动字幕系统在不同西班牙方言中的表现，比较来自不同地区的女性和男性说话者的字幕质量，识别系统性差异。

Result: 研究发现YouTube自动字幕系统存在系统性差异，这些差异可归因于特定方言，表明系统对某些西班牙方言存在偏见。

Conclusion: 数字平台上部署的算法技术需要根据用户群体的多样化需求和体验进行校准，本研究为此提供了进一步证据。
...
TL;DR: 论文通过504种配置的系统评估，发现LLM推理能力的效果高度依赖于任务复杂度：简单任务（如二元分类）性能下降，复杂任务（如27类情感识别）性能提升，挑战了"推理普遍提升性能"的普遍假设。


<details>
  <summary>Details</summary>
Motivation: 当前普遍认为大型语言模型的推理能力能够普遍提升各种语言任务的性能，但这一假设缺乏系统验证。本研究旨在通过全面评估不同推理架构在不同复杂度任务上的表现，检验推理能力是否真的普遍有效。

Method: 研究设计了504种配置，涵盖7个模型家族，包括自适应、条件式和基于强化学习的推理架构。评估在情感分析数据集上进行，任务复杂度从二元分类、五类分类到27类情感识别。采用零样本和少样本提示策略，并进行帕累托前沿分析评估效率-性能权衡。

Result: 1. 推理效果具有任务复杂度依赖性：二元分类性能下降达19.9 F1百分点，而27类情感识别提升达16.0百分点；2. 蒸馏推理变体在简单任务上比基础模型差3-18百分点，少样本提示可部分恢复性能；3. 少样本学习在大多数情况下优于零样本；4. 基础模型在效率-性能权衡中占主导，推理仅在复杂情感识别中合理，尽管计算开销达2.1-54倍。

Conclusion: 推理能力并非普遍有效，其效果高度依赖于任务复杂度。简单任务中推理会导致系统性过度思考而性能下降，复杂任务中推理才真正有益。这挑战了"推理普遍提升性能"的假设，为模型选择和部署提供了重要指导。
...
Conclusion: preference packing是一种有效的资源优化方法，特别适用于奖励模型、DPO等需要同一提示不同响应的训练场景，能显著减少训练时间和资源消耗。
...
TL;DR: ARGUS框架研究叙事在在线论证中的说服作用，通过标注故事存在和六个关键叙事特征，结合编码器分类器和零样本LLM分析叙事维度如何影响说服成功


<details>
  <summary>Details</summary>
Motivation: 虽然故事常被视为强大的说服工具，但其在在线非结构化论证中的具体作用仍未充分探索，需要系统研究叙事特征对说服效果的影响

Method: 提出ARGUS框架，创建新的ChangeMyView语料库，标注故事存在和六个关键叙事特征，整合两个理论框架，使用编码器分类器和零样本大语言模型识别故事和叙事特征，大规模分析叙事维度对说服成功的影响

Result: 开发了ARGUS框架和标注语料库，能够识别故事和六个叙事特征，并应用于大规模分析叙事维度在在线论证中的说服效果

Conclusion: ARGUS为研究叙事在论证中的说服作用提供了系统框架，通过整合理论框架和先进NLP技术，能够深入探索不同叙事特征如何影响在线论证的说服成功
...
TL;DR: 首次对古希腊技术散文进行系统、无参考的LLM机器翻译人工评估，评估三个商业LLM对盖伦医学文本的翻译质量，发现已翻译文本质量接近专家水平，未翻译文本质量较低但受术语密度影响，术语罕见度是翻译失败的重要预测因子。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在填补对大型语言模型在古希腊技术散文翻译中性能评估的空白，特别是针对缺乏参考翻译的低资源古代语言。通过系统评估LLM在已翻译和未翻译医学文本上的表现，为古典学术研究和古代语言自动评估流程设计提供依据。

Method: 研究评估三个商业LLM（Claude、Gemini、ChatGPT）对盖伦两部著作中20个段落长度文本的翻译。使用标准自动评估指标（BLEU、chrF++等）和专家人工评估，后者采用改进的多维质量指标框架，由领域专家团队对所有60个翻译进行评估。术语罕见度通过Diorisis古希腊语料库中的语料频率进行量化。

Result: 在已有英文翻译的说明性文本上，LLM达到高质量翻译（平均MQM得分95.2/100），接近专家水平。在未翻译的药理学文本上，总体质量较低（79.9/100），但方差较大，主要由两个术语密度极高的段落导致；排除这两个段落后，得分与已翻译文本相差仅4分。术语罕见度与翻译失败呈强负相关（r=-0.97）。自动指标在质量差异大的文本上与人判断呈中等相关，但无法区分高质量翻译。

Conclusion: LLM在古希腊技术散文翻译中表现出色，特别是在已有参考翻译的文本上。术语罕见度是翻译失败的关键预测因子。自动评估指标在低质量翻译上有效，但无法区分高质量翻译。研究为古典学术中LLM的应用和低资源古代语言的自动评估流程设计提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有Argumentative LLMs方法结合大语言模型和计算论证进行决策，旨在使决策结果对人类可解释且可争议。需要开发一个支持可视化解释和用户交互的实用系统。

Method: 开发基于Web的系统，实现ArgLLM赋能的智能体处理二元任务。系统支持可视化展示生成的解释，允许用户交互并识别和质疑系统推理中的错误。系统高度模块化，可从可信外部源获取信息。

Result: 成功开发了ArgLLM-App系统，公开可用（https://argllm.app），提供视频演示（https://youtu.be/vzwlGOr0sPM）。系统实现了ArgLLM赋能的智能体，支持可视化解释和用户交互功能。

Conclusion: ArgLLM-App是一个实用的基于Web的系统，实现了Argumentative LLMs方法，通过可视化解释和用户交互功能，使AI决策过程更加透明、可解释且可争议。
...
TL;DR: TASC框架包含TASC-ft和TASC-spec两种方法，前者在微调时扩展词汇表以包含高频输出n-gram，后者在推理时使用轻量级训练自由的推测解码，通过构建任务输出语料的n-gram草稿模型来加速小语言模型。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在需要高效率、低延迟的高吞吐量场景中应用广泛，但现有方法在效率和性能平衡方面存在不足，需要开发既能保持任务性能又能显著提升推理效率的加速方法。

Method: 提出TASC框架包含两个用例：1) TASC-ft：在微调时迭代地将高频输出n-gram添加到分词器词汇表中，然后微调模型以利用扩展的词汇表；2) TASC-spec：推理时的轻量级训练自由推测解码方法，从任务输出语料构建n-gram草稿模型，混合任务和上下文n-gram信息，避免额外训练并绕过草稿-目标词汇对齐约束。

Result: 在多个低输出变异性生成任务上证明了两种方法的有效性，显示了一致的推理效率改进，同时保持了任务性能。

Conclusion: TASC框架为小语言模型加速提供了有效的解决方案，TASC-ft通过词汇扩展优化微调过程，TASC-spec通过训练自由的推测解码提升推理效率，两者结合在不同任务场景下都能实现效率与性能的良好平衡。
...
TL;DR: 研究发现，在多轮对话中省略大语言模型自身的历史回复不仅不会降低响应质量，反而能减少上下文长度并避免错误传播


<details>
  <summary>Details</summary>
Motivation: 重新审视多轮对话中保留模型自身历史回复的设计选择，探究大语言模型是否真正需要依赖自己的先前响应

Method: 使用真实多轮对话数据，比较标准全上下文提示与仅用户轮次提示方法；分析对话结构，设计选择性过滤助手侧上下文的策略

Result: 移除助手历史回复对大部分轮次响应质量无影响，可将累计上下文长度减少10倍；发现36.4%的提示是自包含的，且存在上下文污染现象

Conclusion: 选择性省略助手侧历史可以提升响应质量并减少内存消耗，为更高效的对话系统设计提供了新思路
