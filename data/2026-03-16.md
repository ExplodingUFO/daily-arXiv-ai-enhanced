...
TL;DR: 论文提出GenEval方法，结合多模态视觉语言模型和人类知识，通过域一致性边界评估未知因果因素差异，在糖尿病视网膜病变分级和癫痫发作区检测任务中实现优异的单源域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分类在跨域泛化方面面临挑战，特别是在糖尿病视网膜病变分级和fMRI癫痫发作区检测等关键任务中。当域间存在未知因果因素差异时，缺乏客观评估这些差异的方法，且通常无法获取数据收集者的元数据或协议级信息。

Method: 1. 提出域一致性边界理论框架，评估域间是否存在未知因果因素差异；2. 提出GenEval方法，结合基础模型（如MedGemma-4B）和人类知识，通过低秩适应技术弥合因果差距，增强单源域泛化能力。

Result: 在8个糖尿病视网膜病变数据集和2个癫痫发作区检测数据集上，GenEval实现了优异的单源域泛化性能：糖尿病视网膜病变平均准确率69.2%，癫痫发作区检测平均准确率81%，分别比最强基线高出9.4%和1.8%。

Conclusion: GenEval方法通过结合多模态视觉语言模型和人类知识，有效解决了医学图像分类中的跨域泛化问题，特别是在未知因果因素存在差异的情况下，为关键医疗任务提供了可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频MLLMs通常依赖静态分割标记进行逐帧定位，这种方法提供语义但缺乏时间上下文，导致对象移动或重新出现时出现空间漂移、身份切换和不稳定初始化问题。需要一种能够同时实现空间精度和时间一致性的视频理解方法。

Method: SPARROW包含两个关键组件：(1) 目标特定跟踪特征(TSF)，在训练期间注入时间对齐的参考线索；(2) 双提示设计，解码边界框([BOX])和分割([SEG])标记，将几何先验与语义定位融合。模型使用包含30,646个视频和45,231个问答对的参考视频数据集，通过基于SAM2的类无关提议器实现端到端操作，无需外部检测器。

Result: 在三个开源视频MLLMs(UniPixel、GLUS和VideoGLaMM)中集成SPARROW后，在六个基准测试中取得一致提升：RVOS上J&F提升+8.9，视觉定位上mIoU提升+5，GCG上CLAIR提升+5.4。这些结果表明SPARROW显著改善了参考稳定性、空间精度和时间一致性。

Conclusion: SPARROW通过统一空间精度和时间稳定性的创新设计，在像素级视频理解任务中实现了显著的性能提升，解决了现有视频MLLMs在参考跟踪和时空一致性方面的关键挑战。
...
Method: ABRA将跨域适应问题形式化为预训练检测器权重空间中的几何传输问题，通过对齐源域和目标域专家来传输类别特定知识。该方法在无法访问包含目标类别训练图像的目标域中进行知识迁移。

Result: 在多个具有挑战性的域偏移场景下的广泛实验表明，ABRA成功地在多种恶劣条件下实现了类别级专业知识的"传送"。

Conclusion: ABRA提供了一种有效的跨域适应方法，能够在缺乏目标域标注数据的情况下，将类别特定的检测知识从源域迁移到目标域，解决了开放词汇检测器在域偏移下的性能退化问题。
...
TL;DR: Surg-R1是一个通过分层推理解决手术场景理解问题的视觉语言模型，在公开基准和外部验证数据集上显著优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有手术视觉语言模型缺乏可解释的推理链，而通用推理模型又缺乏领域专业知识，无法满足手术场景理解中既需要准确预测又需要可验证推理的需求。

Method: 提出三级推理层次（感知基础、关系理解、上下文推理），构建最大的手术思维链数据集（32万推理对），采用四阶段训练流程（监督微调、组相对策略优化、迭代自改进）。

Result: 在SurgBench（6个公共基准+6个多中心外部验证数据集）上，Surg-R1获得最高竞技场分数（64.9%），显著优于Gemini 3.0 Pro（46.1%）和GPT-5.1（37.9%），在多数任务上超越专有推理模型和专业手术VLM，外部验证上比最强手术基线提升15.2个百分点。

Conclusion: Surg-R1通过分层推理架构和四阶段训练流程，成功解决了手术场景理解中可解释推理的挑战，为手术AI系统提供了既准确又可验证的解决方案。
...
TL;DR: 重新审视模型缝合技术，发现通过简单的特征匹配损失，异构视觉基础模型可以在视觉任务中可靠缝合，且深层缝合点可使缝合模型超越原始模型


<details>
  <summary>Details</summary>
Motivation: 传统模型缝合研究主要关注相同数据集训练的模型，本研究旨在探索异构视觉基础模型（如CLIP、DINOv2、SigLIP 2）在不同目标、数据和模态混合下的缝合可行性

Method: 提出系统化缝合协议，涵盖缝合点选择、缝合层类型、训练损失函数和下游任务；特别引入在目标模型倒数第二层的简单特征匹配损失；进一步提出VFM缝合树（VST）架构，共享早期层同时保留后期层

Result: 1) 传统缝合方法在浅层缝合点效果不佳；2) 使用目标模型倒数第二层的特征匹配损失可使异构VFM可靠缝合；3) 深层缝合点可使缝合模型超越原始模型，仅增加少量推理开销；4) VST架构可在多模态LLM中实现可控的精度-延迟权衡

Conclusion: 模型缝合从诊断工具提升为实用技术，可用于整合互补VFM优势并定位其表征对齐或分化的位置，为多模态模型集成提供新方法
...
TL;DR: AbSteering是一个异常中心框架，通过引导视频语言模型生成精确的HRCT诊断报告，解决了高病理多样性和空间稀疏性挑战。


<details>
  <summary>Details</summary>
Motivation: HRCT诊断报告生成面临高病理多样性和3D体积内空间稀疏性的挑战，而视频语言模型在通用领域表现出色但在医学影像领域的适应性尚未充分探索。

Method: 提出AbSteering框架：1）异常中心思维链方案强制异常推理；2）直接偏好优化目标利用临床易混淆异常作为硬负样本来增强细粒度区分能力。

Result: 通用视频语言模型在该范式指导下表现出强大的迁移能力，超越基于大规模CT预训练的领域特定基础模型，实现更高的检测灵敏度同时减少幻觉。

Conclusion: AbSteering证明了通用视频语言模型在异常中心引导下能够有效适应高容量医学影像解释，为精确HRCT报告生成提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 多模态指令调优通常计算效率低下，因为训练资源分散在混合图像-视频池中，而这些数据的效用差异很大。需要一种方法能够优化训练数据选择，提高训练效率。

Method: 提出目标驱动数据优化(GDO)框架，为每个候选样本计算六个描述符，针对不同目标构建优化的1×训练子集。在固定的单轮Qwen3-VL-8B-Instruct训练和评估方案下，使用比基线更少的训练样本。

Result: 相对于512k样本的Uni-10x基线，GDO在MVBench上仅用35.4k样本达到基线性能，准确率提升+1.38%；VideoMME用26.6k样本提升+1.67%；MLVU用27.3k样本提升+3.08%；LVBench用34.7k样本提升+0.84%。更强的时序强调带来更好的长视频理解能力。
...
TL;DR: CalliMaster是一个统一的书法页面生成与编辑框架，通过解耦空间规划与内容合成，实现了可控的书法页面生成，支持语义重规划、修复和法证分析。


<details>
  <summary>Details</summary>
Motivation: 现有书法合成方法存在局限性：字符级模型缺乏空间上下文，页面级方法又常常牺牲笔画细节。需要一种既能保持笔画精度又能处理页面布局的方法来解决这一矛盾。

Method: 受人类"先规划后书写"认知过程启发，提出粗到细的"文本→布局→图像"流程。使用单一多模态扩散变换器，第一阶段进行空间规划预测字符边界框，第二阶段将布局作为几何提示，通过流匹配渲染高保真笔画。

Result: 实现了最先进的生成质量，同时支持多种下游能力：通过修改布局约束实现可控语义重规划（调整字符大小位置时自动协调周围留白和笔势），框架还可扩展到文物修复和法证分析。

Conclusion: CalliMaster通过解耦空间规划与内容合成，解决了书法页面合成中的精度与布局平衡问题，为数字文化遗产提供了全面的工具，支持生成、编辑、修复和分析等多种应用。
...
### [18] [DINOLight: Robust Ambient Light Normalization with Self-supervised Visual Prior Integration](https://arxiv.org/abs/2603.12579)
...
### [19] [MRGeo: Robust Cross-View Geo-Localization of Corrupted Images via Spatial and Channel Feature Enhancement](https://arxiv.org/abs/2603.12587)
...
### [20] [SDF-Net: Structure-Aware Disentangled Feature Learning for Opticall-SAR Ship Re-identification](https://arxiv.org/abs/2603.12588)
...
### [28] [From Sparse to Dense: Multi-View GRPO for Flow Models via Augmented Condition Space](https://arxiv.org/abs/2603.12648)
...
### [29] [VGGT-World: Transforming VGGT into an Autoregressive Geometry World Model](https://arxiv.org/abs/2603.12655)
...
### [30] [VFM-Recon: Unlocking Cross-Domain Scene-Level Neural Reconstruction with Scale-Aligned Foundation Priors](https://arxiv.org/abs/2603.12657)
...
### [31] [AVION: Aerial Vision-Language Instruction from Offline Teacher to Prompt-Tuned Network](https://arxiv.org/abs/2603.12659)
...
### [32] [Learning Geometric and Photometric Features from Panoramic LiDAR Scans for Outdoor Place Categorization](https://arxiv.org/abs/2603.12663)
...
### [34] [Vision Verification Enhanced Fusion of VLMs for Efficient Visual Reasoning](https://arxiv.org/abs/2603.12669)
...
### [35] [Bin~Wan,G2HFNet: GeoGran-Aware Hierarchical Feature Fusion Network for Salient Object Detection in Optical Remote Sensing Images](https://arxiv.org/abs/2603.12680)
...
### [36] [RSONet: Region-guided Selective Optimization Network for RGB-T Salient Object Detection](https://arxiv.org/abs/2603.12685)
*Bin Wan,Runmin Cong,Xiaofei Zhou,Hao Fang,Chengtao Lv,Sam Kwong*
...
### [38] [CM-Bench: A Comprehensive Cross-Modal Feature Matching Benchmark Bridging Visible and Infrared Images](https://arxiv.org/abs/2603.12690)
...
### [40] [HSEmotion Team at ABAW-10 Competition: Facial Expression Recognition, Valence-Arousal Estimation, Action Unit Detection and Fine-Grained Violence Classification](https://arxiv.org/abs/2603.12693)
...
### [41] [VCBench: A Streaming Counting Benchmark for Spatial-Temporal State Maintenance in Long Videos](https://arxiv.org/abs/2603.12703)
...
### [43] [Text-Phase Synergy Network with Dual Priors for Unsupervised Cross-Domain Image Retrieval](https://arxiv.org/abs/2603.12711)
...
### [45] [The COTe score: A decomposable framework for evaluating Document Layout Analysis models](https://arxiv.org/abs/2603.12718)
...
### [46] [IGASA: Integrated Geometry-Aware and Skip-Attention Modules for Enhanced Point Cloud Registration](https://arxiv.org/abs/2603.12719)
...
### [48] [CognitionCapturerPro: Towards High-Fidelity Visual Decoding from EEG/MEG via Multi-modal Information and Asymmetric Alignment](https://arxiv.org/abs/2603.12722)
...
### [62] [Coherent Human-Scene Reconstruction from Multi-Person Multi-View Video in a Single Pass](https://arxiv.org/abs/2603.12789)
...
### [64] [Spectral Defense Against Resource-Targeting Attack in 3D Gaussian Splatting](https://arxiv.org/abs/2603.12796)
...
### [65] [What Makes VLMs Robust? Towards Reconciling Robustness and Accuracy in Vision-Language Models](https://arxiv.org/abs/2603.12799)
...
### [67] [coDrawAgents: A Multi-Agent Dialogue Framework for Compositional Image Generation](https://arxiv.org/abs/2603.12829)
...
### [69] [Multimodal Protein Language Models for Enzyme Kinetic Parameters: From Substrate Recognition to Conformational Adaptation](https://arxiv.org/abs/2603.12845)
...
### [70] [Team LEYA in 10th ABAW Competition: Multimodal Ambivalence/Hesitancy Recognition Approach](https://arxiv.org/abs/2603.12848)
...
### [71] [Wear Classification of Abrasive Flap Wheels using a Hierarchical Deep Learning Approach](https://arxiv.org/abs/2603.12852)
...
### [72] [Composing Driving Worlds through Disentangled Control for Adversarial Scenario Generation](https://arxiv.org/abs/2603.12864)
...
### [73] [TRACE: Structure-Aware Character Encoding for Robust and Generalizable Document Watermarking](https://arxiv.org/abs/2603.12873)
...
### [74] [Forecasting Epileptic Seizures from Contactless Camera via Cross-Species Transfer Learning](https://arxiv.org/abs/2603.12887)
...
### [75] [Finite Difference Flow Optimization for RL Post-Training of Text-to-Image Models](https://arxiv.org/abs/2603.12893)
...
### [76] [Spectral-Geometric Neural Fields for Pose-Free LiDAR View Synthesis](https://arxiv.org/abs/2603.12903)
...
### [79] [VIRD: View-Invariant Representation through Dual-Axis Transformation for Cross-View Pose Estimation](https://arxiv.org/abs/2603.12918)
...
### [80] [Rethinking VLMs for Image Forgery Detection and Localization](https://arxiv.org/abs/2603.12930)
...
### [81] [SGMatch: Semantic-Guided Non-Rigid Shape Matching with Flow Regularization](https://arxiv.org/abs/2603.12937)
...
### [83] [Test-Time Attention Purification for Backdoored Large Vision Language Models](https://arxiv.org/abs/2603.12989)
...
### [84] [A Closed-Form Solution for Debiasing Vision-Language Models with Utility Guarantees Across Modalities and Tasks](https://arxiv.org/abs/2603.12998)
...
### [85] [SAW: Toward a Surgical Action World Model via Controllable and Scalable Video Generation](https://arxiv.org/abs/2603.13024)
...
### [87] [Multimodal OCR: Parse Anything from Documents](https://arxiv.org/abs/2603.13032)
...
### [88] [ESPIRE: A Diagnostic Benchmark for Embodied Spatial Reasoning of Vision-Language Models](https://arxiv.org/abs/2603.13033)
...
### [89] [Topo-R1: Detecting Topological Anomalies via Vision-Language Models](https://arxiv.org/abs/2603.13054)
...
### [91] [Reference-Free Image Quality Assessment for Virtual Try-On via Human Feedback](https://arxiv.org/abs/2603.13057)
...
### [93] [Rooftop Wind Field Reconstruction Using Sparse Sensors: From Deterministic to Generative Learning Methods](https://arxiv.org/abs/2603.13077)
...
### [94] [InterEdit: Navigating Text-Guided Multi-Human 3D Motion Editing](https://arxiv.org/abs/2603.13082)
...
### [95] [V-Bridge: Bridging Video Generative Priors to Versatile Few-shot Image Restoration](https://arxiv.org/abs/2603.13089)
...
### [96] [Reasoning over Video: Evaluating How MLLMs Extract, Integrate, and Reconstruct Spatiotemporal Evidence](https://arxiv.org/abs/2603.13091)
...
### [97] [BenDFM: A taxonomy and synthetic CAD dataset for manufacturability assessment in sheet metal bending](https://arxiv.org/abs/2603.13102)
...
### [98] [NOIR: Neural Operator mapping for Implicit Representations](https://arxiv.org/abs/2603.13118)
...
### [99] [Geometry-Guided Camera Motion Understanding in VideoLLMs](https://arxiv.org/abs/2603.13119)
...
### [107] [Task-Specific Knowledge Distillation via Intermediate Probes](https://arxiv.org/abs/2603.12270)
*Ryan Brown,Chris Russell*

Main category: cs.CL

TL;DR: 提出了一种名为\method{}的知识蒸馏框架，通过训练轻量级探针从教师模型的隐藏状态中提取信息，而不是使用输出logits作为监督信号，从而在推理任务上获得更优的蒸馏效果。
...
Method: \method{}框架在冻结的教师模型隐藏状态上训练轻量级探针，然后使用探针的预测结果（而不是输出logits）作为学生模型的监督信号。这种方法无需改变学生或教师模型的架构，探针训练成本低且教师表示可以缓存。

Result: 在四个推理基准测试（AQuA-RAT、ARC Easy/Challenge和MMLU）上获得了一致的改进，在有限数据条件下效果最为显著。探针训练的中间表示提供了比教师自身输出更清晰的标签，有效去除了蒸馏信号的噪声。

Conclusion: 通过利用内部表示，\method{}使从业者能够从大型教师模型中提取更多价值，无需额外的训练数据或架构复杂性。该方法架构无关，计算开销小，为知识蒸馏提供了更有效的监督信号来源。
...
### [108] [Diagnosing Retrieval Bias Under Multiple In-Context Knowledge Updates in Large Language Models](https://arxiv.org/abs/2603.12271)
*Boyu Qiao,Sean Guo,Xian Yang,Kun Li,Wei Zhou,Songlin Hu,Yunya Song*

Main category: cs.CL

TL;DR: LLMs在知识更新场景中存在检索偏差问题：随着同一事实多次更新，最新状态准确率显著下降，而最早状态保持高准确率，类似于认知心理学中的AB-AC干扰现象。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单次更新或单一冲突，而实际知识密集型任务中同一事实可能在上下文中多次修订。多更新场景包含多个历史有效版本在检索时竞争，这一问题尚未得到充分探索。

Method: 引入动态知识实例（DKI）评估框架，将同一事实的多重更新建模为线索与更新值序列的配对，通过端点探测（最早状态和最新状态）评估模型。进行诊断分析包括注意力机制、隐藏状态相似性和输出logits。

Result: 随着更新次数增加，检索偏差加剧：最早状态准确率保持高位（约90%），而最新状态准确率显著下降（从约90%降至约60%）。诊断分析显示错误案例中注意力、隐藏状态相似性和logits信号变得平坦且区分度弱。认知启发式干预策略仅带来有限改进。

Conclusion: LLMs在长上下文中跟踪和遵循知识更新存在持续挑战，多更新场景下的检索偏差问题难以通过简单干预策略解决，需要更深入理解模型如何处理竞争性知识版本。
...
### [109] [ActTail: Global Activation Sparsity in Large Language Models](https://arxiv.org/abs/2603.12272)
*Wenwen Hou,Xinyuan Song,Shiwei Liu*

Main category: cs.CL

TL;DR: ActTail是一种基于TopK幅度的激活稀疏化方法，通过重尾自正则化理论进行全局激活稀疏分配，根据各投影层的异质性统计特性分配不同的稀疏度预算，显著提升高稀疏度下的模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有激活稀疏化方法通常在整个投影层上应用均匀稀疏度，忽略了Transformer权重矩阵的异质性统计特性，导致性能下降加剧。需要一种能够考虑不同投影层统计特性的稀疏分配方法。

Method: 提出ActTail方法：1) 通过经验谱密度计算每个投影层的重尾指数；2) 基于HT-SR理论建立激活稀疏度与重尾指数之间的显式关系；3) 使用重尾指数作为定量指标，为每个投影层分配特定的稀疏度预算；4) 采用TopK幅度选择实现激活稀疏化。
...
Conclusion: ActTail通过基于重尾指数的异质性稀疏分配，为激活稀疏化提供了理论指导，显著提升了高稀疏度下的模型性能，为加速大语言模型推理提供了有效方法。
...
### [110] [Aligning Language Models from User Interactions](https://arxiv.org/abs/2603.12273)
*Thomas Kleine Buening,Jonas Hübotter,Barna Pásztor,Idan Shenfeld,Giorgia Ramponi,Andreas Krause*

Main category: cs.CL

TL;DR: 通过自蒸馏从多轮用户对话中学习，利用后续用户消息作为隐式反馈信号来改进语言模型，实现对齐、个性化和持续适应


<details>
  <summary>Details</summary>
Motivation: 语言模型产生的大量多轮用户交互数据通常被丢弃，但这些交互包含有用信息：后续用户消息可能表明模型响应不正确、未遵循指令或不符合用户偏好。模型本身已经具备利用这些信息的能力，观察到后续消息后能够修正行为，这为从用户交互中学习提供了机会。

Method: 提出基于自蒸馏的原则性可扩展方法：1) 将模型条件化于用户的后续消息，比较由此产生的标记分布与原始策略分布；2) 获得捕捉模型在事后行为变化的目标分布；3) 将这个事后分布蒸馏回当前策略。该方法利用真实世界用户对话（WildChat）进行训练。

Result: 在WildChat真实用户对话上训练能改进语言模型在标准对齐和指令遵循基准上的表现，且不损害其他能力。同一机制支持个性化，使模型能够通过交互持续适应个体用户而无需显式反馈。

Conclusion: 部署过程中自然产生的原始用户交互能够实现语言模型的对齐、个性化和持续适应，为从丰富但通常被忽视的用户交互数据中学习提供了有效途径。
...
### [111] [GONE: Structural Knowledge Unlearning via Neighborhood-Expanded Distribution Shaping](https://arxiv.org/abs/2603.12275)
*Chahana Dahal,Ashutosh Balasubramaniam,Zuobin Xiong*

Main category: cs.CL

TL;DR: 该论文提出了GONE基准和NEDS框架，用于评估和实现大语言模型中对结构化知识图谱事实的知识遗忘，解决了现有方法忽视关系型、多跳推理知识的局限性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型大规模记忆训练数据的能力引发了安全、隐私和知识产权问题，现有知识遗忘方法主要关注平面句子级数据，忽视了自然结构化数据中的关系型、多跳推理知识，需要专门针对知识图谱事实的遗忘评估基准和方法。

Method: 提出GONE基准用于评估知识图谱事实遗忘，能够解耦三种遗忘效应：直接事实移除、推理泄漏和灾难性遗忘。同时设计NEDS框架，利用图连接性识别锚点相关邻居，在遗忘事实与其语义邻居之间建立精确决策边界。

Result: 在LLaMA-3-8B和Mistral-7B模型上评估多种知识编辑和遗忘方法，NEDS在GONE和其他基准上表现出色，遗忘效能达到1.000，局部性达到0.839。

Conclusion: 该工作填补了结构化知识遗忘的空白，GONE基准为评估知识图谱事实遗忘提供了新标准，NEDS框架通过利用图结构实现了精确的知识遗忘，为大语言模型的安全和隐私保护提供了有效工具。
...
### [112] [Prompt Injection as Role Confusion](https://arxiv.org/abs/2603.12277)
*Charles Ye,Jasmine Cui,Dylan Hadfield-Menell*

Main category: cs.CL

TL;DR: 论文揭示了语言模型易受提示注入攻击的根本原因是角色混淆：模型根据文本的写作方式而非来源推断角色，导致模仿特定角色的不可信文本继承了该角色的权限。


<details>
  <summary>Details</summary>
Motivation: 尽管进行了广泛的安全训练，语言模型仍然容易受到提示注入攻击。本研究旨在揭示这种脆弱性的根本原因，探索为什么模型无法区分可信和不可信文本来源。

Method: 设计了新颖的角色探针来捕获模型内部如何识别"谁在说话"，通过向用户提示和工具输出中注入伪造的推理来测试角色混淆假设，并在多个开源和闭源模型上进行实验。

Result: 在StrongREJECT上平均成功率达到60%，在智能体数据窃取上达到61%，而基线接近零。内部角色混淆程度在生成开始前就能强烈预测攻击成功率。

Conclusion: 研究揭示了一个根本性差距：安全在接口层面定义，但权限却在潜在空间中分配。提出了一个统一的机制框架，表明多样化的提示注入攻击都利用了相同的底层角色混淆机制。
...
### [113] [TASTE-Streaming: Towards Streamable Text-Aligned Speech Tokenization and Embedding for Spoken Language Modeling](https://arxiv.org/abs/2603.12350)
*Liang-Hsuan Tseng,Hung-yi Lee*

Main category: cs.CL

TL;DR: TASTE-S：可流式化的文本-语音联合口语语言建模方法，通过集成CTC ASR模块和重新设计单元解码器，解决了模态不匹配问题，支持实时使用。


<details>
  <summary>Details</summary>
Motivation: 现有文本-语音联合口语语言建模（SLM）存在模态不匹配问题（语音单元序列远长于文本标记），且依赖外部ASR系统和非因果解码器，限制了流式应用。
...
Conclusion: TASTE-S成功解决了TASTE方法的流式化限制，为实时文本-语音联合口语语言建模提供了可行方案，在保持性能的同时显著改善延迟特性。
...
### [114] [Not Just the Destination, But the Journey: Reasoning Traces Causally Shape Generalization Behaviors](https://arxiv.org/abs/2603.12397)
*Pengcheng Wen,Yanxu Zhu,Jiapeng Sun,Han Zhu,Yujin Zhou,Chi-Min Chan,Sirui Han,Yike Guo*

Main category: cs.CL

TL;DR: 研究发现思维链（CoT）训练能因果性地影响LLM的泛化行为，即使最终答案相同，不同的推理路径也会导致不同的有害行为模式，证明推理内容具有独立信号效应。


<details>
  <summary>Details</summary>
Motivation: 针对当前关于思维链是否仅是事后合理化而非真正影响模型决策的争议，本研究旨在探究推理轨迹是否对模型泛化具有因果性影响，独立于最终答案。

Method: 设计控制实验，保持最终有害答案不变但改变推理路径，构建包含恶意推理、误导性推理和顺从性推理的数据集，在0.6B-14B参数模型上进行多种范式训练（QTA、QT、T-only），并在有思考和无思考模式下评估。

Result: 发现：1）CoT训练比标准微调更能放大有害泛化；2）不同推理类型导致不同的行为模式，尽管最终答案相同；3）仅训练推理而无答案监督即可改变行为；4）这些效应在无推理生成答案时仍存在，表明深度内化。

Conclusion: 推理内容具有因果性效力，仅监督输出的对齐策略存在局限性，需要更深入地关注推理过程本身。
...
### [115] [Interpreting Negation in GPT-2: Layer- and Head-Level Causal Analysis](https://arxiv.org/abs/2603.12423)
...
Method: 1) 构建包含12,000对匹配肯定句和否定句的自建数据集；2) 定义否定效应分数(NES)来衡量模型区分肯定和否定陈述的敏感性；3) 进行两种关键干预：激活修补（将肯定句的内部激活插入否定句）和消融（暂时禁用特定注意力头）；4) 在层和头级别分析隐藏表示。

Result: 1) 否定处理能力高度集中在第4-6层的有限数量注意力头中；2) 消融这些组件会直接破坏模型的否定敏感性；3) 在内部数据集上，消融增加了NES（表明否定敏感性减弱），重新引入缓存的肯定激活进一步增加了NES；4) 在xNot360基准上，消融略微降低了NES，而恢复操作使性能超过基线；5) 这些因果模式在各种否定形式中保持一致。

Conclusion: GPT-2 Small的否定处理能力不是广泛分布的，而是高度集中在特定中间层的少数注意力头中。这些发现揭示了语言模型内部处理否定等复杂语言结构的机制，为理解模型内部表示提供了重要见解。
...
### [116] [CSE-UOI at SemEval-2026 Task 6: A Two-Stage Heterogeneous Ensemble with Deliberative Complexity Gating for Political Evasion Detection](https://arxiv.org/abs/2603.12453)
...
### [117] [Shattering the Shortcut: A Topology-Regularized Benchmark for Multi-hop Medical Reasoning in LLMs](https://arxiv.org/abs/2603.12458)
...
### [118] [Marked Pedagogies: Examining Linguistic Biases in Personalized Automated Writing Feedback](https://arxiv.org/abs/2603.12471)
...
### [119] [AgentDrift: Unsafe Recommendation Drift Under Tool Corruption Hidden by Ranking Metrics in LLM Agents](https://arxiv.org/abs/2603.12564)
...
### [120] [LMEB: Long-horizon Memory Embedding Benchmark](https://arxiv.org/abs/2603.12572)
...
### [121] [Expert Pyramid Tuning: Efficient Parameter Fine-Tuning for Expertise-Driven Task Allocation](https://arxiv.org/abs/2603.12577)
...
### [122] [RTD-Guard: A Black-Box Textual Adversarial Detection Framework via Replacement Token Detection](https://arxiv.org/abs/2603.12582)
...
### [124] [Continual Learning in Large Language Models: Methods, Challenges, and Opportunities](https://arxiv.org/abs/2603.12658)
...
### [125] [From Text to Forecasts: Bridging Modality Gap with Temporal Evolution Semantic Space](https://arxiv.org/abs/2603.12664)
...
### [126] [MetaKE: Meta-learning Aligned Knowledge Editing via Bi-level Optimization](https://arxiv.org/abs/2603.12677)
*Shuxin Liu,Ou Wu*
...
### [127] [Experimental evidence of progressive ChatGPT models self-convergence](https://arxiv.org/abs/2603.12683)
...
### [128] [EvolveCoder: Evolving Test Cases via Adversarial Verification for Code Reinforcement Learning](https://arxiv.org/abs/2603.12698)
...
### [129] [A Method for Learning Large-Scale Computational Construction Grammars from Semantically Annotated Corpora](https://arxiv.org/abs/2603.12754)
...
### [130] [SectEval: Evaluating the Latent Sectarian Preferences of Large Language Models](https://arxiv.org/abs/2603.12768)
...
### [131] [SteerRM: Debiasing Reward Models via Sparse Autoencoders](https://arxiv.org/abs/2603.12795)
...
### [132] [Adaptive Vision-Language Model Routing for Computer Use Agents](https://arxiv.org/abs/2603.12823)
...
### [133] [Rethinking Multiple-Choice Questions for RLVR: Unlocking Potential via Distractor Design](https://arxiv.org/abs/2603.12826)
...
### [134] [CLARIN-PT-LDB: An Open LLM Leaderboard for Portuguese to assess Language, Culture and Civility](https://arxiv.org/abs/2603.12872)
...
### [135] [Learning from Child-Directed Speech in Two-Language Scenarios: A French-English Case Study](https://arxiv.org/abs/2603.12906)
...
### [136] [HMS-BERT: Hybrid Multi-Task Self-Training for Multilingual and Multi-Label Cyberbullying Detection](https://arxiv.org/abs/2603.12920)
...
### [137] [DS$^2$-Instruct: Domain-Specific Data Synthesis for Large Language Models Instruction Tuning](https://arxiv.org/abs/2603.12932)
...
### [138] [Long-form RewardBench: Evaluating Reward Models for Long-form Generation](https://arxiv.org/abs/2603.12963)
...
### [139] [Is Human Annotation Necessary? Iterative MBR Distillation for Error Span Detection in Machine Translation](https://arxiv.org/abs/2603.12983)
...
### [140] [Interpretable Semantic Gradients in SSD: A PCA Sweep Approach and a Case Study on AI Discourse](https://arxiv.org/abs/2603.13038)
...
### [141] [Mending the Holes: Mitigating Reward Hacking in Reinforcement Learning for Multilingual Translation](https://arxiv.org/abs/2603.13045)
...
### [142] [ESG-Bench: Benchmarking Long-Context ESG Reports for Hallucination Mitigation](https://arxiv.org/abs/2603.13154)
...
### [143] [Neuron-Aware Data Selection In Instruction Tuning For Large Language Models](https://arxiv.org/abs/2603.13201)
