...
TL;DR: 提出一种通过引入新颖鲁棒性损失来减少计算病理学基础模型对技术变异敏感性的方法，无需重新训练基础模型本身


<details>
  <summary>Details</summary>
Motivation: 当前计算病理学基础模型不仅捕获生物学相关特征，还捕获分析前变异和扫描仪特异性变异，这些变异会使基于基础模型特征训练的任务特定模型产生偏差预测

Method: 在训练下游任务特定模型时引入新颖的鲁棒性损失，使用包含27,042张WSI和6,155名患者的综合实验设置，从八个流行计算病理学基础模型的特征训练数千个模型

Result: 除了显著提高鲁棒性外，通过关注生物学相关特征还提高了预测准确性，成功缓解了计算病理学基础模型的鲁棒性问题
...
TL;DR: AeroDGS：基于物理引导的4D高斯泼溅框架，用于单目无人机视频的动态场景重建，解决空中条件下单视角、大空间范围、小空间足迹和大运动差异带来的深度模糊和运动估计不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 现有4D场景重建方法在无人机单视角拍摄条件下存在局限：大空间范围、动态物体空间足迹小且运动差异大，导致严重的深度模糊和不稳定运动估计，使单目空中重建成为病态问题。

Method: 提出AeroDGS框架，包含两个核心模块：1) 单目几何提升模块，从单目空中序列重建可靠的静态和动态几何；2) 物理引导优化模块，引入可微分的地面支撑、直立稳定性和轨迹平滑性先验，将模糊的图像线索转化为物理一致的运动。

Result: 在合成和真实无人机场景上的实验表明，AeroDGS优于现有最先进方法，在动态空中环境中实现了卓越的重建保真度。作者还构建了涵盖不同高度和运动条件的真实世界无人机数据集用于评估。
...
TL;DR: 提出了一种无需分割的肾肿瘤恶性预测深度学习框架，使用器官聚焦注意力损失函数，在公开和私有数据集上均优于传统分割依赖方法。


<details>
  <summary>Details</summary>
Motivation: 现有肾肿瘤恶性预测方法依赖手动分割3D CT图像，这过程耗时、昂贵且需要专家知识，限制了临床应用的效率和可扩展性。

Method: 开发了基于器官聚焦注意力损失函数的深度学习框架，通过修改图像补丁的注意力机制，使器官补丁仅关注其他器官补丁，从而在部署时无需进行3D肾CT图像分割。

Result: 在UF IDR私有数据集上获得AUC 0.685和F1分数0.872，在KiTS21公开数据集上获得AUC 0.760和F1分数0.852，性能优于依赖分割裁剪的传统模型。

Conclusion: 该方法提供了一种更高效可靠的肾肿瘤恶性预测方案，无需显式分割输入即可提升预测准确性，有望改善肾癌诊断的临床决策。
...
TL;DR: 本文系统分析了Vision Transformers中的伪影问题，发现其源于"懒惰聚合行为"——ViT使用语义无关的背景补丁作为捷径来表示全局语义，并提出选择性集成补丁特征到CLS令牌的解决方案。


<details>
  <summary>Details</summary>
Motivation: 尽管Vision Transformers在大规模数据预训练后能为各种下游任务提供通用表示，但在不同监督范式和下游任务中广泛观察到伪影问题。这些伪影的根本机制尚未得到充分阐明，需要系统分析其成因并提出解决方案。

Method: 通过系统分析ViT中的伪影，发现其源于懒惰聚合行为：ViT使用语义无关的背景补丁作为捷径来表示全局语义，这是由全局注意力和粗粒度语义监督驱动的。提出的解决方案是选择性集成补丁特征到CLS令牌，减少背景主导的捷径影响。

Result: 该方法在标签监督、文本监督和自监督三种范式下的12个基准测试中一致提升了性能，证明了解决方案的有效性和通用性。

Conclusion: 本文揭示了ViT中伪影的根本原因——懒惰聚合行为，并提出了一种有效的解决方案。这项工作为理解ViT行为提供了新视角，有助于改进ViT在各种监督范式下的表示学习能力。
...
TL;DR: DeBias-CLIP通过移除训练中的摘要句、应用句子子采样和文本标记填充来解决CLIP模型在长文本对齐中的偏差问题，实现了最先进的长文本检索性能。


<details>
  <summary>Details</summary>
Motivation: CLIP模型通过互联网规模的图像-文本对比学习获得可迁移的多模态特征，但其预训练主要基于短标题配对，导致模型偏向于编码显著物体的简单描述，在复杂场景和密集描述上的对齐效果较差。虽然近期工作通过在小型长标题数据集上微调来缓解此问题，但发现人类和LLM生成的长标题通常以一句话摘要开头，这形成了训练中的捷径，使注意力集中在开头句子和早期标记上，削弱了对标题其余部分的对齐。

Method: 提出DeBias-CLIP方法：1) 在训练时移除摘要句；2) 应用句子子采样，从长标题中随机选择句子子集进行训练；3) 使用文本标记填充来均匀分布监督信号到所有标记位置。该方法作为Long-CLIP的直接替代方案，不增加可训练参数。

Result: DeBias-CLIP在长文本检索任务上达到最先进性能，同时改善了短文本检索效果，对句子顺序排列的敏感性降低。模型能够更好地将监督信号分布到所有文本标记位置，而不是集中在开头部分。
...
TL;DR: 本文提出SimpleOCR训练策略，通过可视化问题设置诊断MLLMs的模态惰性问题，强制模型激活视觉文本提取能力，在少量数据下实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 研究多模态大语言模型(MLLMs)的视觉基础机制：模型是否真正"读取"图像中的文本，还是仅依赖文本提示中的参数捷径。通过可视化问题设置诊断出模型存在"模态惰性"问题，即使具备强大OCR能力，在需要视觉参与的场景中性能显著下降。

Method: 提出SimpleOCR即插即用训练策略：1) 引入可视化问题(VQ)设置，将文本查询直接渲染到图像上，强制模型进行视觉参与；2) 使用随机化样式将训练样本转换为VQ格式，使文本捷径失效；3) 强制模型激活和优化其视觉文本提取路径，无需架构修改。

Result: 1) 在Qwen2.5-VL上诊断出高达12.7%的性能下降，揭示模态惰性问题；2) SimpleOCR在四个代表性OOD基准上超越基础模型5.4%，比基于原始图像的GRPO提升2.7%；3) 具有极高数据效率，仅需8.5K样本(比最近RL方法少30倍)即可达到优异性能；4) 可与NoisyRollout等先进RL策略无缝集成获得互补改进。

Conclusion: MLLMs存在模态惰性问题，倾向于依赖文本捷径而非真正视觉理解。SimpleOCR通过结构化约束有效解决此问题，强制模型激活视觉文本提取能力，在少量数据下实现显著性能提升，为MLLMs的视觉基础机制提供了有效诊断和优化方案。
...
<details>
  <summary>Details</summary>
Motivation: 云端卸载方案存在隐私和延迟问题，特别是在可穿戴助手中，因此需要研究在边缘设备上实现实时在线情景记忆问答的可行性，以保护用户隐私并降低延迟。

Method: 采用双线程异步架构：描述符线程持续将视频转换为轻量级文本记忆，问答线程基于文本记忆进行推理回答查询。将流式约束集成到问答管道中，并在严格资源边界内评估MLLMs性能。

Result: 在QAEgo4D-Closed基准测试中，消费级8GB GPU配置达到51.76%准确率，TTFT为0.41秒；企业级本地服务器达到54.40%准确率，TTFT为0.88秒；云端方案为56.00%准确率。边缘方案在保持竞争力的同时实现了隐私保护。

Conclusion: 边缘解决方案在资源受限环境下能够实现具有竞争力的情景记忆检索性能，同时保护用户隐私，为可穿戴助手等应用提供了可行的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺筛查工作量大、时间敏感且文档繁重，需要将视觉发现转化为一致的BI-RADS评估、乳腺密度分类和结构化报告。现有视觉语言模型多依赖封闭云系统或紧密耦合架构，限制了隐私性、可重复性和适应性。

Method: 开发MammoWise本地多模型管道，支持任何Ollama托管的视觉语言模型和乳腺X光数据集。采用零样本、少样本、思维链提示，可选多模态检索增强生成（RAG）提供病例特定上下文。评估MedGemma、LLaVA-Med和Qwen2.5-VL在VinDr-Mammo和DMID数据集上的表现，使用参数高效微调（QLoRA）提升模型可靠性。

Result: 报告生成质量稳定，少样本提示和RAG能进一步提升性能。分类任务可行但对模型和数据集选择敏感。MedGemma经QLoRA微调后，BI-RADS准确率达0.7545，密度准确率达0.8840，钙化准确率达0.9341，同时保持报告质量。

Conclusion: MammoWise提供了一个实用且可扩展的框架，可在统一且可重复的工作流中部署本地视觉语言模型用于乳腺X光报告生成，解决了隐私、可重复性和适应性问题。
...
TL;DR: 提出Spatial Credit Redistribution (SCR)方法，通过重新分配高注意力源补丁的隐藏状态激活到其上下文，减少视觉语言模型的对象幻觉问题，在推理时无需训练，显著降低幻觉率且计算开销小。
...
Method: 提出空间信用再分配(SCR)，一种无需训练、在推理时进行的干预方法。该方法将隐藏状态激活从高注意力的源补丁重新分配到其上下文，由低熵输入引导。通过注意力引导的源选择机制，在六个模型家族上进行评估。

Result: 在POPE和CHAIR基准测试中，SCR将POPE-Adversarial的幻觉减少约4.7-6.0个百分点，CHAIR-s降低3.7-5.2个百分点（相对减少42-51%），CHAIR-i降低2.7-4.4个百分点（相对减少44-58%），CIDEr保持在0.8个百分点内。计算开销仅为43-56毫秒，比现有方法低3-6倍。

Conclusion: SCR通过解决空间信用崩溃问题，有效减少视觉语言模型的对象幻觉，且计算效率高，适用于实时应用。注意力引导的源选择是关键机制，随机选择会显著降低效果，证实了信用崩溃是主要驱动因素。
...
TL;DR: Pix2Key：一种基于开放词汇视觉词典的CIR方法，通过自监督预训练V-Dict-AE增强细粒度属性理解，在DFMM-Compose基准上显著提升检索性能


<details>
  <summary>Details</summary>
Motivation: 传统CIR方法存在局限性：监督三元组方法可能丢失细粒度线索，零样本方法通过图像描述+文本编辑可能忽略用户隐含意图并产生重复结果。需要一种能更好理解用户意图并保持结果多样性的方法。
...
Conclusion: Pix2Key通过开放词汇视觉词典表示和自监督预训练，有效解决了CIR中的意图理解和多样性问题，在性能和实用性方面均优于现有方法。
...
TL;DR: DisQ-HNet (DQH) 是一个从T1加权和FLAIR MRI合成tau-PET图像的框架，通过部分信息分解和半UNet解码器揭示各模态贡献，在阿尔茨海默病相关任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: tau-PET作为阿尔茨海默病病理的体内标记物成本高且可用性有限，需要开发基于MRI的替代方法，同时需要理解不同MRI模态对预测的贡献。

Method: 结合(1)基于部分信息分解(PID)的向量量化编码器，将潜在信息分解为冗余、独特和互补成分；(2)半UNet解码器，使用基于结构边缘线索的伪跳跃连接而非直接重用编码器特征来保持解剖细节。

Result: 在多个基线方法(VAE、VQ-VAE、UNet)比较中，DisQ-HNet保持了重建保真度，并在下游AD任务(包括Braak分期、tau定位和分类)中更好地保留了疾病相关信号。基于PID的Shapley分析提供了合成摄取模式的模态特异性归因。
...
TL;DR: DrivePTS：一种用于自动驾驶场景合成的扩散模型，通过渐进学习策略、多视图层次描述和频率引导结构损失，解决了现有方法在条件依赖、语义细节和结构保真度方面的不足。
...
Conclusion: DrivePTS通过解决条件依赖、语义细节和结构保真度问题，显著提升了自动驾驶场景生成的性能，为自动驾驶系统的鲁棒性和泛化性验证提供了更有效的数据增强技术。
...
TL;DR: SwiftNDC：基于神经深度校正场的快速3D重建框架，通过生成跨视角一致的深度图，创建干净均匀的点云几何初始化，显著加速3D高斯溅射的网格重建和新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有深度引导的3D重建方法存在尺度漂移、多视角不一致性等问题，需要大量细化才能获得高保真几何。需要一种快速且通用的框架来解决这些问题，提供可靠的几何初始化以加速下游重建任务。

Method: 提出SwiftNDC框架，核心是神经深度校正场，用于生成跨视角一致的深度图。通过反向投影和鲁棒的重投影误差滤波从精炼深度生成密集点云，为下游重建提供干净均匀的几何初始化。该几何初始化显著加速3D高斯溅射的网格重建，也可提升新视角合成的渲染质量。

Result: 在五个数据集上的综合研究表明：SwiftNDC持续减少准确网格重建的运行时间，提升视角合成的渲染保真度。在网格重建的两个数据集和视角合成的三个数据集上均表现优异，证明了神经深度细化与鲁棒几何初始化结合的有效性。

Conclusion: SwiftNDC通过神经深度校正场和鲁棒几何初始化的结合，实现了高保真且高效的3D重建。该方法不仅加速了网格重建过程，还提升了新视角合成的质量，为快速3D重建提供了有效的解决方案。
...
TL;DR: 提出QARMVC框架，通过信息瓶颈提取语义特征，利用重构差异量化细粒度噪声强度，设计质量加权对比学习和融合策略，在异构噪声场景下显著优于现有方法。
...
Method: QARMVC框架：1) 使用信息瓶颈机制提取内在语义进行视图重构；2) 利用重构差异量化细粒度污染强度，生成实例级质量分数；3) 分层学习策略：特征级设计质量加权对比目标自适应抑制噪声传播，融合级通过质量加权聚合构建高质量全局共识，并通过互信息最大化对齐和校正局部视图。

Result: 在五个基准数据集上的广泛实验表明，QARMVC始终优于最先进的基线方法，特别是在异构噪声强度场景下表现突出。

Conclusion: QARMVC通过细粒度质量评估和分层学习策略，有效解决了异构噪声下的多视图聚类问题，为现实世界复杂噪声环境提供了更鲁棒的解决方案。
...
TL;DR: 论文揭示扩散模型指导方法评估中的严重偏差问题，提出新的评估框架GA-Eval，并设计了一个无效但能提高评估分数的TDG方法，证明现有指导方法相比简单增加CFG尺度并无实质性优势。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于质疑新兴扩散指导方法是否真正实现了实质性的改进。作者发现现有评估存在严重偏差，人类偏好模型对大的指导尺度有强烈偏向，导致评估结果失真。
...
Result: 实验表明：1) 简单增加CFG尺度就能与大多数研究的扩散指导方法竞争；2) 所有方法相比标准CFG都遭受严重的胜率下降；3) 传统评估框架存在严重偏差，导致评估结果不可靠。

Conclusion: 该研究强烈促使社区重新思考扩散指导领域的评估范式和未来方向。现有指导方法并未带来实质性改进，评估偏差问题亟待解决。
...
TL;DR: GIFSplat是一个纯前馈的迭代精炼框架，用于从稀疏无位姿视图进行3D高斯泼溅重建，通过前馈残差更新和蒸馏扩散先验实现高效高质量重建。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D重建方法在性能提升（特别是域外数据）和保持秒级推理时间方面仍有局限，这源于一次性预测范式：模型受限于容量、缺乏推理时精炼、不适合持续注入生成先验。

Method: 提出GIFSplat框架：1）通过少量前馈残差更新逐步精炼当前3D场景；2）从增强的新视角渲染中蒸馏冻结扩散先验到高斯级线索，无需梯度反向传播或不断增加视图集，实现带生成先验的逐场景适应。

Result: 在DL3DV、RealEstate10K和DTU数据集上，GIFSplat持续优于最先进的前馈基线，PSNR提升高达+2.1 dB，同时保持秒级推理时间，无需相机位姿或任何测试时梯度优化。

Conclusion: GIFSplat通过前馈迭代精炼框架和生成先验蒸馏，在效率和质量的平衡上取得突破，为稀疏视图3D重建提供了高效且高质量的解决方案。
...
TL;DR: CMDM提出了一种因果运动扩散模型框架，通过因果扩散变换器在语义对齐的潜空间中进行自回归运动生成，解决了现有双向扩散模型缺乏时间因果性和自回归模型累积误差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法存在局限性：全序列双向扩散模型缺乏时间因果性，难以实时应用；自回归模型则存在不稳定性和累积误差问题。需要一种既能保持时间因果性又能保证生成质量的运动生成框架。

Method: 1. 提出MAC-VAE（运动-语言对齐因果VAE），将运动序列编码为时间因果的潜表示；2. 在潜表示上训练自回归扩散变换器，使用因果扩散强制进行时间有序的去噪；3. 引入具有因果不确定性的帧级采样计划，从部分去噪的前帧预测后续帧。

Result: 在HumanML3D和SnapMoGen数据集上的实验表明，CMDM在语义保真度和时间平滑性方面优于现有扩散和自回归模型，同时显著降低了推理延迟，支持高质量文本到运动生成、流式合成和长时程运动生成。

Conclusion: CMDM提供了一个统一的因果运动生成框架，通过因果扩散变换器和语义对齐潜空间，实现了高质量、实时可用的运动生成，在保持时间因果性的同时解决了自回归模型的稳定性问题。
...
TL;DR: SATtxt是一个面向地球观测的频谱感知视觉语言基础模型，仅需RGB输入即可推理，同时保留训练时学到的光谱线索，显著提升零样本分类、检索和线性探测性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言基础模型在地球观测应用中面临两个主要障碍：1）多光谱输入信息丰富但存在波段冗余和不对齐问题，难以一致利用；2）CLIP式文本编码器语义表达能力有限，削弱了细粒度对齐。同时，实际卫星系统常缺乏完整多光谱覆盖，使得仅用RGB进行推理成为可扩展部署的迫切需求。

Method: SATtxt采用两阶段框架：1）光谱表示蒸馏：通过轻量级投影器将冻结多光谱教师模型的光谱先验知识转移到RGB学生模型；2）频谱基础对齐：使用指令增强的大语言模型将蒸馏后的视觉空间与富有表现力的LLM嵌入空间进行对齐。

Result: 在EuroSAT、BigEarthNet和ForestNet数据集上，SATtxt相比基线方法平均提升零样本分类4.2%、检索5.9%、线性探测2.7%，展示了面向地球观测的频谱感知视觉语言学习的高效路径。

Conclusion: SATtxt通过光谱表示蒸馏和频谱基础对齐，实现了仅用RGB输入即可进行频谱感知推理的视觉语言基础模型，为地球观测任务提供了可扩展且高效的解决方案，显著提升了零样本理解和检索能力。
...
TL;DR: 提出Coded-E2LF方法，仅使用编码孔径和事件相机重建4D光场，无需强度图像
...
Conclusion: Coded-E2LF方法实现了纯事件驱动的4D光场重建，简化了硬件实现，为计算成像提供了新途径
...
TL;DR: CGSA是首个将对象中心学习(OCL)引入无源域自适应目标检测(SF-DAOD)的框架，通过将槽感知适应集成到DETR检测器中，在隐私敏感场景中实现更好的跨域适应性能。


<details>
  <summary>Details</summary>
Motivation: 现有SF-DAOD方法主要关注调整伪标签阈值或改进师生框架，但忽略了跨域数据中的对象级结构线索。作者认为对象中心学习(OCL)可以为SF-DAOD提供有价值的视觉先验，从而改善域不变适应。

Method: 提出CGSA框架：1) 分层槽感知(HSA)模块：逐步将图像解耦为作为视觉先验的槽表示；2) 类引导槽对比(CGSC)模块：引导槽朝向类语义，保持语义一致性并促进域不变适应。该框架基于DETR检测器构建。

Result: 在多个跨域数据集上的广泛实验表明，CGSA优于先前的SF-DAOD方法。理论推导和实验分析进一步证明了所提出组件和框架的有效性。

Conclusion: CGSA成功将对象中心学习引入SF-DAOD，展示了对象中心设计在隐私敏感适应场景中的潜力。该方法通过槽表示提供视觉先验，并通过类引导对比保持语义一致性，实现了更好的域适应性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D装配方法主要关注姿态估计，无法处理缺失几何；人类装配过程结合了结构推理和整体形状推断，因此需要将装配与生成相结合

Method: 提出CRAG框架，将3D装配重新定义为装配与生成的联合问题，通过相互增强的机制：装配提供零件级结构先验，生成注入整体形状上下文以消除装配歧义

Result: 在具有不同几何形状、可变零件数量和缺失部件的真实物体上实现了最先进的性能

Conclusion: 装配与生成是相互增强的过程，联合方法能够处理传统方法无法解决的缺失几何问题，CRAG展示了在复杂3D装配任务中的优越性能
...
TL;DR: 提出基于四焦点张量的多视图相机同步新框架，通过Tucker分解恢复相机矩阵，开发首个四焦点张量同步算法，并在现代数据集上验证有效性


<details>
  <summary>Details</summary>
Motivation: 在运动恢复结构中，四焦点张量比成对对应（本质矩阵）捕获更多信息，但通常被认为不实用且仅有理论意义。本文挑战这一观点，探索利用高阶信息进行相机同步的潜力
...
Result: 数值实验证明该方法在现代数据集上的有效性，表明使用高阶信息在同步中的潜力和重要性。四焦点张量同步算法能够从对应的四焦点张量集合中恢复n个相机

Conclusion: 四焦点张量在实际应用中具有可行性，不应仅被视为理论工具。提出的同步框架为利用高阶几何约束进行多视图重建开辟了新途径，展示了高阶信息在相机同步中的实际价值
...
Result: MWAM可无缝集成到CNN和ViT等不同架构骨干中，在各种任务和模态组合上带来一致的性能提升，不仅优化基础模型性能，还能进一步提升解决缺失模态问题的最先进方法的性能
...
<details>
  <summary>Details</summary>
Motivation: 医学影像算法开发需要体素级标注，但3D扫描的手动标注耗时昂贵，现有工具缺乏统一的队列导向工作流，无法在单一本地流程中集成导航、传播、交互校正和定量导出。

Method: 基于Napari多维度查看器，将3D体积视为切片序列，集成框/点提示与SAM2风格传播。采用本地优先工作流，支持标准DICOM/NIfTI格式，提供顺序标注、框优先提示、点提示细化、提示优先校正等功能。

Result: 开发了开源Python应用，支持多研究高效3D标注，保持图像几何完整性，提供每对象体积测量和3D体积渲染，可选N4偏置场校正，专为研究标注工作流设计。

Conclusion: Medical-SAM2 GUI提供了一个统一的本地工作流，解决了医学图像3D标注的实践限制，通过半自动方法显著提高了标注效率，支持研究社区使用。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像和视频生成方面表现出色，但多步迭代采样带来的巨大计算开销阻碍了实际部署。现有的基于缓存的加速方法依赖于固定或局部自适应调度，未考虑去噪轨迹的全局结构，容易导致误差累积和视觉伪影。

Method: DPCache将扩散采样加速问题形式化为全局路径规划问题。首先从小型校准集构建路径感知成本张量，量化在给定前一个关键时间步条件下跳过时间步的路径相关误差。然后利用该张量，通过动态规划选择最小化总路径成本同时保持轨迹保真度的最优关键时间步序列。在推理时，模型仅在关键时间步进行完整计算，中间输出使用缓存特征高效预测。

Result: 在DiT、FLUX和HunyuanVideo上的广泛实验表明，DPCache在保持最小质量损失的情况下实现了显著加速：在FLUX上，4.87倍加速时ImageReward提升+0.031，3.54倍加速时甚至超过全步基线+0.028 ImageReward，优于现有加速方法。

Conclusion: DPCache通过将扩散采样加速问题转化为全局路径规划，并利用路径感知成本张量和动态规划进行优化调度，提供了一种训练免费、有效的加速框架，在多个模型上实现了高质量加速，验证了路径感知全局调度框架的有效性。
...
TL;DR: 提出了一种实用的音视频质量评估数据集构建方法，并创建了当前最大最全面的AVQA数据集YT-NTU-AVQ
...
Method: 设计了众包主观实验框架打破实验室限制，采用系统化数据准备策略确保质量水平和语义场景的广泛覆盖，并扩展了额外标注

Result: 创建了YT-NTU-AVQ数据集，包含1,620个用户生成的音视频序列，是目前最大最全面的AVQA数据集

Conclusion: 提出的实用AVQA数据集构建方法有效解决了现有数据集的局限性，为模型发展和多模态感知机制研究提供了更好的支持
...
TL;DR: ArtPro：一种自监督框架，通过自适应整合运动提议来解决基于3D高斯泼溅的可微分渲染方法对初始部件分割高度敏感的问题，实现复杂多部件物体的鲁棒重建。
...
Method: 提出ArtPro框架，采用基于几何特征和运动先验的过分割初始化生成具有合理运动假设的部件提议；在优化过程中，通过分析空间邻居间的运动一致性动态合并提议，同时使用碰撞感知的运动剪枝机制防止错误的运动学估计。

Result: 在合成和真实世界物体上的大量实验表明，ArtPro能够鲁棒地重建复杂多部件物体，在准确性和稳定性方面显著优于现有方法。

Conclusion: ArtPro通过自适应整合运动提议，解决了现有自监督方法对初始分割敏感的问题，为复杂多部件物体的高保真数字孪生重建提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D占据方法主要针对室外驾驶场景，在室内场景中表现不佳。室内场景具有几何更密集、布局更复杂、语义更细粒度的特点，需要专门的方法来处理这些挑战。

Method: 采用仅使用二元占据标签（占据vs空闲）的几何监督范式。基于3D语言嵌入高斯作为统一中间表示，耦合细粒度3D几何和语言对齐语义嵌入。几何方面提出不透明度感知的泊松基体积聚合方法；语义方面提出渐进温度衰减调度，在渲染过程中逐渐锐化不透明度以增强高斯-语言对齐。

Result: 在Occ-ScanNet数据集上，开放词汇设置下达到59.50 IoU和21.05 mIoU，在所有现有占据方法中IoU最高，并在mIoU上大幅超越先前开放词汇方法。

Conclusion: 该方法成功解决了室内开放词汇3D占据预测的挑战，通过几何监督和语言嵌入高斯的结合，实现了对复杂室内环境的有效理解，为具身智能体提供了强大的环境感知能力。
...
TL;DR: ViCLIP-OT：专为越南语图像-文本检索设计的视觉语言基础模型，结合CLIP对比学习和SIGROT损失，在低资源语言场景下显著提升跨模态检索性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要针对高资源语言优化，在越南语等低资源语言场景下表现不佳。需要专门为越南语设计高效的跨模态检索模型，解决模态鸿沟问题。

Method: 提出ViCLIP-OT框架，集成CLIP风格的对比学习与相似图正则化最优传输损失（SIGROT），增强全局跨模态一致性并缓解模态鸿沟问题。

Result: 在三个越南语基准测试（UITOpenViIC、KTVIC、Crossmodal-3600）上，ViCLIP-OT在域内和零样本设置中均优于CLIP和SigLIP基线。在UIT-OpenViIC上平均Recall@K达到67.34%，比CLIP提升5.75个百分点；在Crossmodal-3600零样本评估中超越CLIP 11.72个百分点。嵌入空间分析证实了更好的对齐和更小的模态鸿沟。

Conclusion: SIGROT集成提供了一种有效且可扩展的低资源语言跨模态检索策略，对越南语及其他代表性不足语言环境中的智能多媒体检索系统具有实际应用价值。
...
TL;DR: 该论文提出了一种名为SoPE的球坐标系位置嵌入方法，用于改进3D大视觉语言模型中的位置建模，解决了传统RoPE在3D空间表示中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有3D大视觉语言模型继承的旋转位置嵌入(RoPE)在3D多模态理解中存在不足：1) 传统RoPE公式在编码3D token时无法保持三维空间结构；2) 其相对距离计算忽略了角度依赖性，限制了模型捕捉视觉表示中方向变化的能力。
...
TL;DR: 提出基于扩散模型的超声图像去斑方法，使用模拟数据训练，在保留解剖结构的同时抑制斑点噪声，性能优于传统方法和近期学习基线，并提供不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 超声成像虽然实时无创，但斑点噪声和相关伪影会降低图像质量，影响临床解读。现有方法在去斑效果和解剖结构保留之间存在权衡，需要更有效的解决方案。

Method: 基于Image Restoration Stochastic Differential Equations框架构建扩散模型。通过Matlab UltraSound Toolbox从无斑点的磁共振图像模拟超声图像，创建大规模配对数据集进行监督训练。模型在去斑的同时保留解剖边缘和对比度。

Result: 在模拟测试集上，该方法在去斑性能上一致优于经典滤波器和近期学习基线。通过跨模型方差量化预测不确定性，发现高不确定性区域与高重建误差相关。对模拟探头设置的敏感性分析显示存在域偏移问题。

Conclusion: 扩散模型在超声去斑任务中表现优异，不确定性量化提供临床实用指标。模拟参数敏感性导致的域偏移问题提示需要多样化训练和领域适应策略以实现稳健的临床部署。
...
Method: 提出正交子空间编辑：将模型隐藏状态分解为三个正交子空间（视觉证据、冲突先验和残差不确定性），从而能够选择性抑制幻觉模式而不干扰视觉基础。该方法数学上保证对先验子空间的编辑完全不影响视觉组件。

Result: 在POPE和CHAIR等基准测试中实现了最先进的幻觉减少效果，在MME上保持通用能力，同时保持高效推理。在多种架构上一致优于对比解码和静态子空间编辑基线方法。

Conclusion: HulluEdit为构建更可信赖的大视觉语言模型提供了新途径，通过正交子空间编辑在保持视觉基础能力的同时有效减少幻觉，平衡了效率和准确性。
...
Method: 提出Alignment-Aware Masked Learning (AML)：1) 显式估计像素级视觉-语言对齐分数；2) 在优化过程中过滤对齐不佳的区域；3) 聚焦于可信的视觉-语言线索进行训练
...
Method: 提出ProjFlow训练免费采样器，核心创新是骨骼感知度量编码骨骼拓扑结构。该度量使采样器能够通过在整个骨骼上协调分布修正来强制执行硬约束，避免朴素投影的不自然伪影。对于稀疏输入（如填充关键帧间的长间隙），引入使用伪观测的时间变化公式，在采样过程中逐渐衰减。

Result: 在代表性应用（运动修复和2D到3D提升）上的广泛实验表明，ProjFlow实现了精确约束满足，在零样本基线上匹配或提高了真实感，同时与基于训练的控制器保持竞争力。

Conclusion: ProjFlow通过骨骼感知度量和时间变化伪观测，实现了零样本、精确满足线性空间约束的人类运动生成，同时保持运动自然性，为动画任务提供了高效且有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成器过于关注美学质量，而忽视了生成视频中的空间约束，特别是动态空间关系的准确描绘

Method: 提出SPATIALALIGN自改进框架，采用零阶正则化直接偏好优化微调T2V模型；设计基于几何的DSR-SCORE度量标准定量评估视频与提示中动态空间关系的对齐程度；构建包含多样化动态空间关系的文本-视频对数据集

Result: 微调后的模型在空间关系描绘方面显著优于基线模型；DSR-SCORE提供了比依赖视觉语言模型更可靠的评估方法
...
TL;DR: 提出统一隐码恢复框架，实现篡改图像的检索与恢复，兼容多种水印方案，并在ImageNet-S基准上验证性能


<details>
  <summary>Details</summary>
Motivation: 当前图像真实性研究主要集中于深度伪造检测与定位，而篡改内容的恢复与事实检索相对不足，需要建立超越检测定位的通用图像恢复基础

Method: 采用统一隐码恢复框架，通过多尺度向量量化将语义和感知信息编码为紧凑隐码表示，利用条件Transformer模块增强上下文推理，兼容事后和生成中水印范式

Result: 在ImageNet-S基准测试中展现出有前景的检索和重建性能，同时保持与多种水印管道的完全兼容性

Conclusion: 该框架为超越检测和定位的通用图像恢复奠定了基础，为解决篡改内容的事实检索问题提供了新方向
...
TL;DR: TrajTok是一个端到端的视频分词器模块，通过统一的片段化器在时空维度上对像素进行隐式聚类，直接生成物体轨迹，实现与视频时长无关的语义自适应分词，显著提升视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型中的分词方法（如分块化）会产生过多冗余的token，严重限制了视频处理的效率和可扩展性。虽然基于轨迹的分词器通过解耦视频时长与token数量提供了有前景的解决方案，但它们依赖于复杂的外部分割和跟踪流程，这些流程速度慢且与任务无关。

Method: 提出TrajTok，一个端到端的视频分词器模块，完全集成并与视频模型共同训练以优化下游目标。它包含一个统一的片段化器，在空间和时间维度上对像素进行隐式聚类，直接生成物体轨迹。该方法强调下游适应性而非像素级分割精度，因此轻量高效。
...
Conclusion: TrajTok通过端到端的轨迹生成实现了语义自适应的视频分词，解决了传统方法中token冗余和外部依赖问题，显著提升了视频理解性能，同时保持了高效率，并展示了在多种视频任务中的通用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法生成部件级3D对象，但无法在开放世界场景中将部件组织成独立的实例。通过去偏聚类探针发现，这种失败源于模型内部分配机制缺乏结构约束。

Method: 将结构化3D场景生成重新定义为全局相关分配问题，在组合DiT模型的去噪循环中制定并求解熵最优传输目标。该公式施加两个结构约束：1）传输计划门控交叉注意力，强制图像块到部件级3D潜在表示的一对一独占路由；2）传输的竞争性鼓励相似块分组，通过基于边缘的成本进一步正则化，形成连贯对象。

Result: 在开放世界场景生成任务上，SceneTransporter优于现有方法，显著提高了实例级连贯性和几何保真度。

Conclusion: SceneTransporter通过最优传输框架有效解决了结构化3D场景生成中的实例组织问题，为单图像到3D场景的生成提供了新的解决方案。
...
Conclusion: 自监督骨架表示学习能有效提升轨迹预测模型对遮挡导致关节缺失的鲁棒性，为实际应用提供了更可靠的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 大气湍流导致长距离成像中的像素位移（倾斜）和模糊，严重影响图像质量，现有方法在同时处理这两种退化效应方面存在不足。
...
Conclusion: 光流引导的倾斜校正与高斯泼溅相结合能有效提升大气湍流条件下的图像恢复质量，在合成和真实场景中均表现出优越性能。
...
TL;DR: PhotoAgent是一个通过显式美学规划实现自主图像编辑的系统，将编辑过程建模为长时程决策问题，通过树搜索规划多步编辑动作，利用记忆和视觉反馈进行闭环执行，无需用户逐步提示。


<details>
  <summary>Details</summary>
Motivation: 当前基于指令的图像编辑方法高度依赖精心设计的指令，将任务分解和排序的负担完全放在用户身上。为了实现自主图像编辑，需要开发能够理解用户美学意图并自主规划编辑步骤的系统。

Method: PhotoAgent将自主图像编辑建模为长时程决策问题，系统通过推理用户美学意图，利用树搜索规划多步编辑动作，并通过记忆和视觉反馈进行闭环迭代执行。同时构建了UGC-Edit美学评估基准和测试集来支持可靠评估。

Result: 大量实验表明，PhotoAgent在指令遵循和视觉质量方面均优于基线方法。系统能够有效提升编辑效果，无需用户逐步指导。

Conclusion: PhotoAgent通过显式美学规划和闭环执行机制，实现了高质量的自主图像编辑，减轻了用户负担，为生成式模型的实用化应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有面部年龄变换方法存在三个主要问题：1）依赖数值年龄表示而忽略了生物和上下文线索的相互作用；2）在宽年龄变换中难以保持身份一致性；3）静态注意力和优化密集型反演限制了适应性、细粒度控制和背景一致性。
...
Conclusion: FaceTT通过创新的提示细化、高效反演和自适应注意力控制机制，解决了现有面部年龄变换方法在身份保持、背景一致性和老化真实性方面的局限性，实现了高质量的面部年龄变换。
...
<details>
  <summary>Details</summary>
Motivation: 视频息肉分割（VPS）在计算机辅助结肠镜检查中具有重要意义，但面临两大挑战：1）息肉与周围黏膜外观相似，语义区分度弱；2）视频帧间息肉位置和尺度变化大，导致分割不稳定。现有方法难以同时保证分割精度和实时临床适用性。

Method: 提出CMSA-Net框架，包含两个核心组件：1）因果多尺度聚合（CMA）模块：使用因果注意力机制从多个历史帧的多尺度特征中聚合语义信息，确保时间特征传播遵循严格时间顺序，减少噪声并提高特征可靠性；2）动态多源参考（DMR）策略：基于语义可分性和预测置信度自适应选择信息丰富且可靠的参考帧，提供强大多帧指导同时保持模型实时推理效率。

Result: 在SUN-SEG数据集上的大量实验表明，CMSA-Net实现了最先进的性能，在分割精度和实时临床适用性之间取得了良好平衡。

Conclusion: CMSA-Net通过因果多尺度特征聚合和动态参考帧选择，有效解决了视频息肉分割中的语义模糊性和时空变化问题，为临床实时应用提供了准确可靠的分割解决方案。
...
TL;DR: 提出了一种基于多光谱成像和机器学习的经济高效、可现场部署的土壤质地检测系统，用于预测土壤组成和USDA质地分类。
...
Method: 使用自制的低成本多光谱成像设备（365-940 nm，13个光谱波段）捕获土壤光谱特性。采用回归模型预测粘土、粉土和砂土百分比，同时使用直接分类器预测12个USDA质地类别。间接分类通过将回归组成映射到USDA土壤质地三角图实现。

Result: 组成预测的决定系数R²高达0.99，质地分类准确率超过99%。实验基于混合数据（按不同比例混合粘土、粉土和砂土）进行评估，使用USDA分类三角图作为基础。

Conclusion: 多光谱成像与数据驱动建模相结合，能够提供准确、非破坏性且可现场部署的土壤质地表征，适用于岩土工程筛选和精准农业应用。
...
TL;DR: CheXficient是一种通过主动数据筛选而非盲目扩大数据集训练的胸部X光基础模型，仅使用22.7%的数据和27.3%的计算资源就达到了与全数据训练相当或更好的性能。


<details>
  <summary>Details</summary>
Motivation: 医学影像基础模型通常遵循"不计成本扩大规模"的范式，但面临两个关键挑战：大规模医学数据集存在大量冗余和严重的类别不平衡，导致表示学习偏向过代表模式；不考虑数据质量异质性的盲目训练导致计算效率低下。

Method: 提出CheXficient胸部X光基础模型，采用主动、有原则的数据筛选策略，在预训练过程中选择性优先处理信息丰富的训练样本。模型仅使用1,235,004对CXR图像和报告中的22.7%进行预训练。

Result: CheXficient在仅消耗27.3%总计算预算的情况下，实现了与全数据对应模型和其他大规模预训练模型相当或更优的性能。在5种任务类型、20个个体基准测试中表现优异，包括零样本发现分类、跨模态检索、疾病预测、语义分割和放射学报告生成。

Conclusion: 主动数据筛选是医学视觉语言基础模型高效预训练和下游适应的可行、经济有效的替代方案。CheXficient系统性地优先处理欠代表训练样本，提高了对长尾或罕见病症的泛化能力，为医学基础模型的数据和计算需求提供了实用见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有全景语义分割模型严重依赖重力对齐假设，但在实际应用中（如手持设备抖动、无人机姿态变化）常出现3D旋转偏差，导致标准球面Transformer过度拟合全局纬度线索，在旋转场景下性能崩溃。
...
Result: 在Pose35数据集（±35°随机旋转）上达到72.03 mIoU，在任意完整SO(3)旋转下保持70.67 mIoU。相比之下，基线SphereUFormer从67.53 mIoU暴跌至25.26 mIoU，显示SO3UFormer在旋转鲁棒性上的显著优势。

Conclusion: SO3UFormer通过几何驱动的设计实现了对3D旋转的鲁棒性，解决了全景语义分割在实际应用中因相机姿态变化导致的性能退化问题，为无约束环境下的视觉感知提供了可靠解决方案。
...
TL;DR: 提出Chain of Flow（COF）框架，从单次心动周期的12导联ECG生成完整的4D心脏结构和运动，实现从预测模型到患者特异性虚拟心脏的转变


<details>
  <summary>Details</summary>
Motivation: 现有心脏数字孪生（CDT）框架局限于特定任务预测，而非构建患者特异性、可操作的虚拟心脏。需要能够从多模态信号重建个性化心脏解剖和生理，并支持广泛下游模拟的临床可行CDT

Method: 提出Chain of Flow（COF）基础性ECG驱动生成框架，整合cine-CMR和12导联ECG训练，学习心脏几何、电生理和运动动力学的统一表示，从单次心动周期重建完整4D心脏结构和运动

Result: 在多样化队列中评估显示，COF能准确恢复心脏解剖结构、腔室功能和动态运动模式。重建的4D心脏支持下游CDT任务，如容积测量、区域功能分析和虚拟cine合成

Conclusion: COF通过直接从ECG实现完整4D器官重建，将心脏数字孪生从狭窄的预测模型转变为完全生成性、患者特异性的虚拟心脏，为临床可行的CDT提供了基础框架
...
TL;DR: 该论文提出了一个多模态增强现实框架，通过将逼真的虚拟物体集成到真实世界铁路序列中，解决铁路障碍物检测中高质量标注数据稀缺的问题，并创建了OSDaR-AR公开数据集。


<details>
  <summary>Details</summary>
Motivation: 铁路智能交通系统中安全关键任务（如障碍物检测）面临高质量标注数据稀缺的问题。现有解决方案存在局限性：逼真模拟器存在"模拟到现实"差距，而简单的图像掩码技术缺乏时空一致性，无法生成具有正确外观和尺寸的增强单帧和多帧场景。

Method: 提出多模态增强现实框架，利用Unreal Engine 5功能，通过LiDAR点云和INS/GNSS数据将逼真虚拟物体集成到OSDaR23数据集的真实世界铁路序列中。采用基于分割的INS/GNSS数据细化策略，确保物体放置的准确性和跨RGB帧的时间稳定性。

Result: 创建了OSDaR-AR公开数据集，包含精心设计的增强序列，支持下一代铁路感知系统的开发。通过比较研究证实，提出的分割细化策略显著提高了增强序列的真实感。

Conclusion: 该研究成功开发了一个能够生成逼真增强铁路序列的框架，有效解决了铁路应用中高质量标注数据稀缺的问题，为铁路感知系统的开发提供了有价值的公开数据集资源。
...
TL;DR: 提出了首个面向全水域环境的大规模视频问答基准WaterVideoQA，并开发了多智能体神经符号系统NaviMind，用于提升自主水面舰艇的认知推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前自主导航系统在被动感知方面已取得显著进展，但在知识驱动的交互式环境认知方面存在根本性限制。特别是在高风险的海上导航领域，需要填补原始视觉感知与复杂认知推理之间的鸿沟，这是自主水面舰艇执行安全精确操作的关键前提。

Method: 1. 构建WaterVideoQA基准：包含6个不同水道类别的3,029个视频片段，整合多变光照和动态天气等多方面变量，通过五层分层认知框架严格测试ASV能力。
2. 开发NaviMind系统：首创的多智能体神经符号系统，通过自适应语义路由、情境感知分层推理和自主自反验证的协同作用，实现从表面模式匹配到符合法规的可解释决策的转变。

Result: 实验结果表明，该框架显著超越了现有基线，为动态海事环境中的智能可信交互建立了新范式。

Conclusion: 该研究填补了自主导航中知识驱动交互认知的空白，通过WaterVideoQA基准和NaviMind系统，为自主水面舰艇提供了从原始感知到复杂认知推理的桥梁，实现了更安全、精确且符合法规的决策能力。
...
TL;DR: MSJoE框架通过联合进化MLLM和轻量级关键帧采样器，实现高效的长视频理解，仅选择少量信息丰富的关键帧进行问答


<details>
  <summary>Details</summary>
Motivation: 高效理解长视频是多模态大语言模型面临的基本挑战，现有方法难以有效处理长视频内容，需要更高效的帧采样和理解机制

Method: 提出MSJoE框架：1) 推理出多个描述问题相关视觉视角的查询；2) 查询与冻结CLIP模型交互生成查询-帧相似度矩阵；3) 轻量级采样器预测关键帧采样权重，选择紧凑的信息帧集；4) 通过强化学习联合优化MLLM和采样器，实现查询推理、帧采样和关键帧理解的协同适应

Result: 在VideoMME、LongVideoBench、LVBench和MLVU等基准测试中，MSJoE相比基础MLLM获得8.0%的准确率提升，比最强基线方法高1.1%；收集了包含2.8K视频和7K问答对的新长视频QA数据集

Conclusion: MSJoE通过联合进化MLLM和采样器，实现了高效的长视频理解，证明了仅需少量关键帧即可有效回答视频问题，为长视频理解提供了新的解决方案
...
TL;DR: 提出pMoE方法，通过专家特定提示令牌和可学习调度器整合多领域知识，在视觉适应任务中实现性能提升和计算效率平衡


<details>
  <summary>Details</summary>
Motivation: 现有提示调优方法通常只利用单一预训练模型的知识，忽视了整合不同领域知识可能产生的协同效应。需要开发能够有效结合多领域专家知识的统一调优框架。

Method: 提出pMoE（Mixture-of-Experts提示调优方法），包含专家特定提示令牌和可学习调度器。通过动态令牌调度机制在不同提示层优化各领域专家的贡献，在统一模型框架中整合多领域专业知识。

Result: 在47个适应任务（包括通用和医疗领域的分类与分割）上进行广泛实验，pMoE不仅以较大优势实现卓越性能，还在计算效率和适应效果之间提供了最佳权衡。

Conclusion: pMoE通过整合多领域专家知识，显著增强了模型的通用性和适用性，为视觉适应任务提供了高效且有效的参数高效微调解决方案。
...
TL;DR: 基于视频的皮划艇冲刺性能分析框架，通过计算机视觉技术从平移缩放视频中自动重建速度与划桨频率，无需GPS或船载传感器


<details>
  <summary>Details</summary>
Motivation: 皮划艇冲刺中的配速策略（速度与划桨频率分布）对峰值表现至关重要。GPS是分析的金标准但可用性有限，需要自动化的视频解决方案来为教练提供准确反馈

Method: 1) 使用YOLOv8检测浮标和运动员，利用已知浮标网格估计单应性变换；2) 通过U-net船头标定学习船型特定的运动员偏移量，泛化船位估计；3) 采用光流进行鲁棒跟踪以适应多运动员船型；4) 从姿态估计或运动员边界框提取划桨频率信息

Result: 与精英比赛GPS数据对比：速度相对均方根误差(RRMSE)为0.020±0.011（相关系数ρ=0.956），划桨频率RRMSE为0.022±0.024（ρ=0.932）。方法为教练提供高精度自动反馈

Conclusion: 该框架成功从平移缩放视频中重建皮划艇冲刺性能指标，覆盖所有冲刺项目(K1-K4, C1-C2)和距离(200m-500m)，无需船载传感器或人工标注，为教练提供准确自动化分析工具
...
TL;DR: 提出一种基于注意力熵的VAR模型优化框架，通过分析token粒度、语义范围和生成尺度下的参数动态，发现token、层和尺度三个维度的稀疏模式，实现高达3.4倍的加速且保持语义保真度


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型在生成质量上有优势，但在后期阶段面临严重的效率瓶颈。现有方法如FastVAR和SkipVAR依赖启发式跳过策略，缺乏对模型架构不同维度语义投影的系统分析，需要更精细的优化方法

Method: 提出ToProVAR框架：1) 利用注意力熵表征模型架构不同维度的语义投影；2) 分析不同token粒度、语义范围和生成尺度下的参数动态；3) 发现token、层和尺度三个维度的稀疏模式；4) 针对这些模式设计细粒度优化策略

Result: 在Infinity-2B和Infinity-8B模型上验证，ToProVAR实现高达3.4倍的加速，同时最小化质量损失，在效率和生成质量上均优于传统方法，有效解决了先前工作中的问题

Conclusion: 基于注意力熵的优化框架能够系统分析VAR模型的多维度语义投影，通过发现稀疏模式并设计针对性策略，在保持语义保真度和细节的同时实现显著加速，为VAR模型效率优化提供了新方向
...
TL;DR: OpenFS是一个开源的手语拼写识别与合成系统，通过多手兼容识别器、隐式手部检测、单调对齐损失和帧级字母条件生成器，解决了手语拼写识别中的手部歧义、训练损失不足和词汇外问题。


<details>
  <summary>Details</summary>
Motivation: 手语拼写识别在聋哑人和听力人群之间架起沟通桥梁至关重要，但面临三个主要挑战：1) 拼写手部歧义问题（难以区分拼写手和非拼写手）；2) 缺乏合适的训练损失函数（传统CTC损失存在峰值行为问题）；3) 词汇外(OOV)问题（无法识别训练集中未出现的单词）。现有方法依赖显式手部检测导致识别失败，且使用CTC损失存在局限性。

Method: 1) 多手兼容拼写识别器：支持单手和双手输入，通过双级位置编码和拼写手聚焦(SF)损失实现隐式手部检测；2) 单调对齐(MA)损失：替代CTC损失，通过交叉注意力正则化强制输出字母序列遵循输入姿态序列的时间顺序；3) 帧级字母条件生成器：合成逼真的手语拼写姿态序列，用于词汇外单词，并构建新的合成基准FSNeo。

Result: 实验表明，OpenFS在手语拼写识别任务上达到了最先进的性能，验证了所提识别器和生成器的有效性。该方法在识别准确率和鲁棒性方面优于现有方法，特别是在处理手部歧义和词汇外单词方面表现突出。

Conclusion: OpenFS通过创新的多手兼容识别器、隐式手部检测机制、单调对齐损失和合成生成器，全面解决了手语拼写识别的核心挑战。该开源系统不仅提升了识别性能，还通过合成数据生成能力扩展了系统的应用范围，为聋哑人与听力人群之间的沟通提供了更有效的技术解决方案。
...
TL;DR: MM-NeuroOnco：用于脑肿瘤MRI理解的大规模多模态基准和指令调优数据集，包含24,726个MRI切片和约200,000条语义丰富的多模态指令，显著提升了脑肿瘤诊断推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有公开数据集在标注丰富性和诊断语义方面有限，无法满足脑肿瘤准确诊断所需的临床可解释推理需求。需要构建一个能够支持多模态脑肿瘤MRI理解的大规模数据集和评估基准。

Method: 1) 构建MM-NeuroOnco数据集：包含24,726个MRI切片和约200,000条多模态指令，涵盖多种肿瘤亚型和成像模态；2) 开发多模型协作管道：用于自动医学信息补全和质量控制，生成超越掩码标注的诊断相关语义；3) 构建MM-NeuroOnco-Bench评估基准：包含手动标注和拒绝感知设置，减少封闭式问题的偏差；4) 提出NeuroOnco-GPT模型：基于数据集进行微调。

Result: 1) 在十个代表性模型评估中，最强基线Gemini 3 Flash在诊断相关问题上的准确率仅为41.88%；2) NeuroOnco-GPT经过微调后在诊断问题上实现了27%的绝对准确率提升；3) 数据集和基准公开可用。

Conclusion: MM-NeuroOnco数据集和基准有效解决了脑肿瘤MRI理解中诊断语义稀缺的问题，显著提升了多模态诊断推理能力，为临床可解释的脑肿瘤诊断提供了重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 基于视频生成的世界模型在长期内容一致性（场景重访时）和精确相机控制方面存在困难。现有基于显式3D重建的方法在无边界场景和细粒度结构上缺乏灵活性，而基于先前生成帧的方法缺乏显式空间对应关系，限制了可控性和一致性。
...
Conclusion: UCM通过统一长期记忆和相机控制的创新机制，解决了视频生成世界模型的关键挑战，在场景一致性和可控性方面取得显著改进，为交互环境模拟提供了更强大的框架。
...
TL;DR: SubspaceAD是一种无需训练的工业异常检测方法，仅使用少量正常图像，通过DINOv2提取特征并PCA建模正常变化子空间，通过重构残差进行异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有少样本异常检测方法通常依赖记忆库、辅助数据集或多模态调优，作者质疑这种复杂性在视觉基础模型特征表示下是否必要，希望开发更简单有效的方案。

Method: 两阶段无训练方法：1) 使用冻结的DINOv2骨干从少量正常图像提取补丁级特征；2) 对特征进行PCA分析，估计正常变化的低维子空间。推理时通过重构残差计算异常分数。

Result: 在单样本和少样本设置下达到SOTA性能：MVTec-AD数据集上单样本图像级和像素级AUROC分别为98.0%和97.6%；VisA数据集上分别为93.3%和98.3%，无需训练、提示调优或记忆库。

Conclusion: SubspaceAD证明了利用视觉基础模型特征表示的简单PCA方法即可实现卓越的异常检测性能，无需复杂组件，提供了解释性强且统计基础良好的异常分数。
...
TL;DR: DMAligner：一种基于扩散模型的图像对齐框架，通过对齐导向的视图合成解决传统光流方法在遮挡和光照变化下的局限性


<details>
  <summary>Details</summary>
Motivation: 现有图像对齐方法主要采用基于光流的图像扭曲技术，但这种方法容易受到遮挡和光照变化等常见挑战的影响，导致对齐视觉质量下降并影响下游任务准确性。需要从新视角解决图像对齐问题。

Method: 提出DMAligner扩散框架，采用对齐导向的视图合成方法。核心包括：1）动态感知扩散训练方法学习条件图像生成，合成用于图像对齐的新视图；2）动态感知掩码生成模块自适应区分动态前景和静态背景，使扩散模型能更有效处理传统方法难以解决的挑战。

Result: 使用Blender开发了动态场景图像对齐数据集，包含1,033个室内外场景，超过30K图像对。在DSIA基准测试和多个广泛使用的视频数据集上的定性比较中，实验结果表明该方法具有优越性。

Conclusion: DMAligner通过生成式解决方案展示了强大的能力，避免了基于光流的图像扭曲问题，为图像对齐任务提供了新的有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本组合图像检索方法通常将多模态查询转换为单一模态：要么作为编辑后的文本描述进行文本到图像检索（T2I），要么作为编辑后的图像进行图像到图像检索（I2I）。但T2I会丢失细粒度视觉细节，I2I难以处理复杂语义修改。需要有效结合两者的互补优势来应对多样化的查询意图。

Method: 提出WISER训练免费框架，采用"检索-验证-精炼"流程：1）更广泛搜索：同时生成编辑后的文本描述和图像进行并行检索，扩大候选池；2）自适应融合：使用验证器评估检索置信度，对不确定检索触发精炼，对可靠检索动态融合双路径；3）更深思考：对不确定检索通过结构化自反思生成精炼建议，指导下一轮检索。

Result: 在多个基准测试中显著优于先前方法：在CIRCO上相对提升45%（mAP@5），在CIRR上相对提升57%（Recall@1）。甚至超越许多需要训练的方法，展现了优越性和多样化场景下的泛化能力。

Conclusion: WISER通过统一T2I和I2I范式，结合意图感知和不确定性感知，有效解决了零样本组合图像检索中的模态转换局限性问题，为无需训练的高性能检索提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前体视频重建面临三大挑战：1) 传统高斯溅射方法在长序列上失效，存在时间不一致性问题；2) 在大运动和遮挡情况下表现不佳；3) 输出格式与标准视频编码管道不兼容，限制了实际应用。需要一种既能保持高质量重建，又能兼容现有多媒体基础设施的统一表示方法。

Method: 提出了PackUV-GS方法：1) PackUV表示：将所有高斯属性映射到结构化多尺度UV图集序列中；2) 流引导高斯标记和视频关键帧模块：识别动态高斯、稳定静态区域，保持时间一致性；3) 直接在UV域优化高斯参数；4) 创建了PackUV-2B数据集，包含100个序列、20亿帧，用于评估长时体视频捕获。

Result: 方法在渲染保真度上超越现有基线，能够扩展到长达30分钟的序列并保持一致性质量。PackUV表示首次实现了与标准视频编解码器（如FFV1）兼容的体视频表示，无需质量损失，可在现有多媒体基础设施中实现高效流式传输。

Conclusion: PackUV提供了一种紧凑、图像原生且与标准视频编解码器兼容的4D高斯表示方法，解决了体视频重建、存储和流式传输的关键挑战，为实际应用铺平了道路。同时发布的PackUV-2B数据集为长时体视频研究提供了重要基准。
...
TL;DR: D-FINE-seg是D-FINE目标检测器的实例分割扩展，通过轻量级掩码头、分割感知训练和优化的端到端推理框架，在TACO数据集上超越YOLO26的F1分数，同时保持实时性能。


<details>
  <summary>Details</summary>
Motivation: 虽然基于Transformer的实时目标检测器（如D-FINE）在精度-延迟权衡方面表现优异，但基于Transformer的实时实例分割方法仍然较少。作者旨在将D-FINE扩展到实例分割任务，同时提供完整的训练、导出和优化推理框架。

Method: 1. 在D-FINE架构基础上添加轻量级掩码头；2. 采用分割感知训练策略，包括框裁剪BCE损失和Dice掩码损失；3. 引入辅助和去噪掩码监督；4. 调整匈牙利匹配成本函数；5. 提供完整的端到端训练、导出和优化推理管道，支持ONNX、TensorRT、OpenVINO等多种推理引擎。

Result: 在TACO数据集上，D-FINE-seg在统一的TensorRT FP16端到端基准测试协议下，F1分数超越了Ultralytics YOLO26，同时保持了具有竞争力的延迟性能。

Conclusion: D-FINE-seg成功将D-FINE扩展到实例分割任务，实现了实时实例分割的精度-延迟平衡。该框架已作为开源项目发布（Apache-2.0许可证），提供了完整的训练到部署解决方案，有助于推动实时实例分割的实际应用。
...
TL;DR: GeoWorld提出了一种几何世界模型，通过双曲JEPA将潜在表示映射到双曲流形上，解决了现有基于能量的预测世界模型在几何结构保持和长时程预测方面的不足，实现了更稳定的多步视觉规划。


<details>
  <summary>Details</summary>
Motivation: 现有基于能量的预测世界模型存在两个主要挑战：1）潜在表示通常在欧几里得空间中学习，忽略了状态间的几何和层次结构；2）在长时程预测中表现不佳，导致在扩展推演中快速退化。

Method: 提出GeoWorld几何世界模型，通过双曲JEPA将潜在表示从欧几里得空间映射到双曲流形上，以保持几何结构和层次关系。进一步引入几何强化学习进行基于能量的优化，实现在双曲潜在空间中的稳定多步规划。

Result: 在CrossTask和COIN数据集上的大量实验表明，相比最先进的V-JEPA 2，在3步规划中实现了约3%的成功率提升，在4步规划中实现了约2%的成功率提升。

Conclusion: GeoWorld通过将潜在表示映射到双曲流形并采用几何强化学习，有效解决了基于能量的世界模型在几何结构保持和长时程预测方面的挑战，显著提升了多步视觉规划的性能。
...
<details>
  <summary>Details</summary>
Motivation: 在许多研究和临床环境中，配对的图像-文本数据稀缺且难以获取，这限制了视觉-语言耦合模型在显微脑组织分析等领域的应用。

Method: 通过标签作为中介，从相关文献自动挖掘区域描述作为合成标题，然后使用图像到文本的训练目标，将现有的细胞构筑视觉基础模型（CytoNet）与大型语言模型耦合。

Result: 在57个脑区上，该方法生成合理的区域级描述，支持开放集使用，对范围内斑块的细胞构筑参考标签匹配准确率达90.6%，在掩码区域标签的8路测试中恢复区域准确率达68.6%。

Conclusion: 弱标签介导的配对足以连接现有的生物医学视觉基础模型与语言模型，为配对注释稀缺的领域提供了整合自然语言界面的实用方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有状态空间模型（如Vision Mamba）虽然能有效建模长程依赖，但其一维序列化会削弱局部空间连续性和高频表示能力。医学图像分割需要同时建模全局解剖结构和细粒度边界细节，因此需要一种能解耦结构信息和纹理信息的方法。
...
Result: 在五个公共基准数据集上进行实验，涵盖多种模态和分割目标，均取得一致改进，验证了方法的有效性和泛化能力。

Conclusion: SpectralMamba-UNet通过在频域解耦结构信息和纹理信息，有效解决了现有状态空间模型在局部空间连续性和高频表示方面的不足，为医学图像分割提供了一种新颖有效的频域解耦框架。
...
TL;DR: 提出WARM-CAT方法，通过累积无监督数据的多模态知识更新原型，使用自适应权重和动态优先级队列处理分布偏移，在四个基准数据集上实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有CZSL方法在测试时因包含未见组合导致标签空间分布偏移，造成性能下降，需要克服这一挑战

Method: 1) 从无监督数据累积文本和视觉模态知识更新多模态原型；2) 设计自适应更新权重控制原型调整程度；3) 引入动态优先级队列存储高置信度图像获取历史视觉原型；4) 通过多模态协作表示学习对齐文本和视觉原型；5) 提出新基准数据集C-Fashion并改进MIT-States

Result: 在四个基准数据集上，在封闭世界和开放世界设置下均达到最先进的性能

Conclusion: 提出的WARM-CAT方法通过累积多模态知识、自适应原型更新和动态队列机制，有效处理CZSL中的分布偏移问题，显著提升性能
...
TL;DR: 提出一种基于单位球面霍夫变换的相机航向估计方法，通过斐波那契网格离散化球面，利用特征对应关系生成大圆投票，在噪声和异常值环境下保持准确性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于已知旋转的相机航向估计方法在低噪声、低异常值条件下表现良好，但随着噪声和异常值增加，准确性下降或计算成本升高。需要一种在复杂环境下仍能保持准确性和效率的鲁棒方法。

Method: 提出单位球面霍夫变换的泛化方法：1) 提取两帧间的特征对应关系；2) 为每对对应关系生成兼容方向的大圆；3) 使用斐波那契网格离散化单位球面作为投票箱中心；4) 每个大圆为一组方向投票，使不受噪声或动态物体影响的特征一致地为正确运动方向投票。

Result: 在三个数据集上的实验表明，该方法在准确性与效率的帕累托前沿上。SLAM实验显示，通过在相机位姿初始化时校正航向，能够降低RMSE误差。

Conclusion: 提出的单位球面霍夫变换方法能够有效估计相机航向，在噪声和异常值环境下保持鲁棒性，同时在准确性和计算效率之间取得良好平衡，适用于SLAM等实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 对抗样本的可迁移性（即在一个模型上生成的对抗样本能够欺骗其他未见过的模型）引发了严重的安全担忧。然而，现有研究缺乏标准化的评估框架和准则，导致对现有攻击方法的评估可能存在偏差。

Method: 1. 对数百篇相关工作进行详尽综述，将各种基于迁移的攻击分为六个不同类别；2. 提出一个全面的框架作为评估这些攻击的基准；3. 描述增强对抗可迁移性的常见策略；4. 指出可能导致不公平比较的普遍问题。

Result: 建立了一个标准化的评估框架，将可迁移性攻击系统化分类，并识别了影响公平比较的关键问题，为未来研究提供了可靠的评估基准。

Conclusion: 该研究填补了对抗可迁移性攻击评估的标准化空白，提出的框架有助于更公平、准确地比较不同攻击方法的有效性，并简要回顾了图像分类之外的可迁移性攻击。
...
<details>
  <summary>Details</summary>
Motivation: 现有噪声鲁棒行人重识别方法依赖softmax输出进行损失修正或样本选择，但存在两个关键限制：1) softmax的平移不变性导致对噪声标签的预测过度自信且不可靠；2) 基于小损失准则的样本选择会丢弃对学习判别性特征至关重要的困难正样本

Method: 提出CARE两阶段框架：1) 校准阶段：使用概率证据校准(PEC)通过向相似度函数注入自适应可学习参数来消除softmax平移不变性，并采用证据校准损失减轻对误标样本的过度自信；2) 精炼阶段：设计证据传播精炼(EPR)，包含复合角度边界(CAM)度量在超球空间精确区分干净但难学的正样本与误标样本，以及确定性导向球面加权(COSW)根据CAM动态分配样本重要性

Result: 在Market1501、DukeMTMC-ReID和CUHK03数据集上，在随机和模式化噪声条件下，CARE取得了有竞争力的性能表现

Conclusion: CARE方法通过从校准到精炼的概率证据传播，有效解决了噪声标签行人重识别中的softmax平移不变性和困难正样本丢弃问题，提高了模型的噪声鲁棒性
...
TL;DR: 提出新的无监督在线视频稳定框架，无需配对数据集，采用三阶段经典流程和多线程缓冲机制，解决端到端学习的三大挑战，并引入多模态无人机数据集


<details>
  <summary>Details</summary>
Motivation: 解决基于深度学习的视频稳定方法需要配对数据集的问题，以及端到端学习面临的三大挑战：数据有限、可控性差、资源受限硬件效率低。现有基准主要针对手持视频和可见光前向视角，限制了在无人机夜间遥感等领域的应用。

Method: 提出无监督在线视频稳定框架，采用三阶段经典稳定流程（运动估计、运动平滑、帧合成），结合多线程缓冲机制。引入新的多模态无人机航空视频数据集（UAV-Test）来评估方法在更广泛场景下的性能。

Result: 实验表明，该方法在定量指标和视觉质量上均优于最先进的在线稳定器，同时达到与离线方法相当的性能。在无人机夜间遥感等新场景中表现出色。

Conclusion: 提出的无监督在线视频稳定框架有效解决了端到端学习的三大挑战，无需配对数据集，在资源受限硬件上高效运行。新引入的UAV-Test数据集填补了现有基准的空白，为更广泛的应用场景提供了评估标准。
...
TL;DR: Fase3D是首个无需视觉编码器的3D场景大模型，通过傅里叶变换和点云序列化实现高效无序3D数据处理


<details>
  <summary>Details</summary>
Motivation: 现有3D大模型依赖沉重的预训练视觉编码器提取几何特征，而2D模型已开始消除编码器以提高效率和可扩展性。将这种范式扩展到3D面临挑战，因为点云具有无序性和大规模特性，需要设计无需繁琐编码器的高效3D数据标记化方法

Method: 提出Fase3D架构，包含三个关键创新：1) 通过结构化超点紧凑表示大场景；2) 空间填充曲线序列化结合快速傅里叶变换实现高效全局上下文建模和图基标记合并；3) 傅里叶增强的LoRA适配器以可忽略成本向LLM注入全局频率感知交互

Result: Fase3D在性能上与基于编码器的3D大模型相当，同时在计算和参数方面显著更高效

Conclusion: Fase3D证明了无需沉重视觉编码器即可有效处理无序3D数据的可行性，通过傅里叶变换和序列化技术实现了高效且可扩展的3D场景理解架构
...
TL;DR: DyaDiT是一个多模态扩散变换器，能够从双人对话音频生成具有社交上下文感知的逼真手势动作，考虑了对话双方的相互动态关系。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将单个音频流映射到单个说话者的动作，忽略了社交上下文和对话双方之间的相互动态关系，这限制了生成手势的自然性和社交参与度。

Method: DyaDiT采用多模态扩散变换器架构，输入双人对话音频和可选的社交上下文标记，融合双方信息捕捉交互动态，使用运动字典编码运动先验，并可选择性地利用对话伙伴的手势生成更具响应性的动作。

Result: 在标准运动生成指标和定量用户研究中，DyaDiT不仅超越了现有方法的客观指标，而且在用户偏好方面也获得强烈认可，证明了其鲁棒性和社交友好的运动生成能力。

Conclusion: DyaDiT通过建模对话双方的相互动态关系和社交上下文，能够生成更加自然、社交参与度更高的手势动作，为数字人类交互提供了更先进的解决方案。
...
TL;DR: BaryIR：一种基于Wasserstein重心空间对齐多源退化特征的表征学习框架，通过解耦退化无关的共享内容和退化特定的知识，提升全合一图像恢复模型对未见退化的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有全合一图像恢复方法对分布外退化脆弱，限制了在真实场景中的泛化能力。本文基于直觉：多源退化特征分布是由不同退化特定偏移从底层退化无关分布诱导产生的，恢复这种共享分布对跨退化泛化至关重要。

Method: 提出BaryIR框架，在Wasserstein重心空间中对齐多源退化特征，该空间通过最小化与多源退化分布的Wasserstein距离平均值来建模退化无关分布。引入残差子空间，其嵌入相互对比同时保持与WB嵌入正交，从而显式解耦两个正交空间：编码跨退化共享的退化无关不变内容的WB空间，以及自适应保留退化特定知识的残差子空间。

Result: BaryIR在性能上与最先进的全合一方法竞争。特别地，BaryIR对未见退化（如类型和程度）泛化良好，在有限退化类型训练下学习广义特征表现出显著鲁棒性，并在混合退化的真实世界数据评估中表现优异。

Conclusion: 通过解耦退化无关共享内容和退化特定知识，BaryIR有效缓解了对分布内退化的过拟合，实现了基于退化无关共享不变性的自适应恢复，显著提升了全合一图像恢复模型在真实场景中的泛化能力。
...
TL;DR: LaGS提出了一种用于4D全景占用跟踪的新方法，通过潜在高斯泼溅技术将多视角信息高效聚合到3D体素网格中，实现时空场景理解的统一框架。


<details>
  <summary>Details</summary>
Motivation: 现有方法在动态环境中的4D时空场景理解存在局限性：要么只能提供粗糙的几何跟踪（如边界框），要么只能提供缺乏明确时间关联的详细3D结构（如基于体素的占用）。需要一种能够统一处理时空场景理解的综合方法。

Method: LaGS采用基于相机的端到端跟踪与基于掩码的多视角全景占用预测相结合的方法。核心创新是潜在高斯泼溅技术：首先将观测融合到3D高斯分布中，作为3D场景的稀疏点中心潜在表示；然后将聚合的特征泼溅到3D体素网格上，通过基于掩码的分割头进行解码。

Result: 在Occ3D nuScenes和Waymo数据集上评估，实现了4D全景占用跟踪的最先进性能。

Conclusion: LaGS通过潜在高斯泼溅方法，为4D时空场景理解提供了一个统一的框架，能够同时处理几何跟踪和详细3D结构，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 铁路站台实时人群计数对安全和容量管理至关重要，现有跟踪检测方法在动态条件下（相机运动、遮挡、透视畸变）表现不佳，需要物理一致的运动建模

Method: 提出物理约束跟踪框架：1) 基于迁移学习的YOLOv11m检测器；2) EfficientNet-B0外观编码；3) DeepSORT框架集成；4) 物理约束卡尔曼模型(Phys-3D)，通过针孔几何强制物理合理的3D运动动力学；5) 虚拟计数带与持久性机制处理遮挡

Result: 在MOT-RailwayPlatformCrowdHead数据集上，计数误差降至2.97%，在运动和遮挡条件下表现鲁棒

Conclusion: 结合第一原理几何和运动先验可实现安全关键交通场景中可靠的人群计数，有助于列车调度和站台安全管理
...
Conclusion: Uni-Animator成功解决了图像和视频草图着色中的关键挑战，通过创新的参考增强、细节保留和时间一致性机制，实现了统一的高质量着色框架。
...
TL;DR: ColoDiff是一个基于扩散模型的结肠镜视频生成框架，通过时间流模块和内容感知模块实现动态一致性和临床属性精确控制，能减少90%以上采样步骤，在多个数据集上验证了生成质量和下游任务性能。
...
Method: 提出ColoDiff扩散框架：1) TimeStream模块通过跨帧标记化机制解耦时间依赖性，实现不规则肠道结构下的精细动态建模；2) Content-Aware模块结合噪声注入嵌入和可学习原型，实现对临床属性的精确控制；3) 采用非马尔可夫采样策略，减少90%以上步骤实现实时生成。

Result: 在三个公共数据集和一个医院数据库上评估，基于生成指标和下游任务（疾病诊断、模态判别、肠道准备评分、病变分割）。实验表明ColoDiff能生成具有平滑过渡和丰富动态的视频，在各项任务中表现优异。

Conclusion: ColoDiff展示了可控结肠镜视频生成的潜力，揭示了合成视频在补充真实表征和缓解临床场景数据稀缺方面的应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机在动态场景中面临独立运动物体和自身运动触发的事件干扰问题，需要实时过滤这些动态事件以提升下游应用性能

Method: 提出联合分割当前事件流中的独立运动物体并预测其未来运动，实现动态事件的预测性抑制；采用轻量级架构设计

Result: 在消费级GPU上达到173Hz推理速度，内存使用小于1GB；在EVIMO基准上分割精度提升67%，推理速度提高53%；下游应用中，视觉Transformer推理加速83%，事件视觉里程计精度提升13%
...
TL;DR: EmbodMocap：使用两部移动iPhone进行便携式、低成本的人体-场景运动捕捉系统，可在日常环境中实现度量尺度、场景一致的数据采集，支持多种具身AI任务。


<details>
  <summary>Details</summary>
Motivation: 现有运动捕捉系统依赖昂贵的演播室设置和可穿戴设备，限制了在自然环境中大规模采集场景条件化人体运动数据。需要一种便携、经济的方法来无缝桥接人体运动与场景几何。

Method: 提出EmbodMocap系统，使用两部移动iPhone进行双视图RGB-D序列采集，通过联合标定将人体和场景重建到统一的度量世界坐标系中。双视图设置有效缓解深度模糊问题，相比单iPhone或单目模型具有更好的对齐和重建性能。

Result: 与光学捕捉地面真值相比，双视图设置展现出显著缓解深度模糊的能力，实现了卓越的对齐和重建性能。基于采集的数据，成功赋能了三个具身AI任务：单目人体-场景重建、基于物理的角色动画、以及机器人运动控制。

Conclusion: EmbodMocap提供了一种便携、经济的解决方案，可在日常环境中实现度量尺度、场景一致的运动捕捉，有效桥接人体运动与场景几何，为推进具身AI研究做出了重要贡献。
...
<details>
  <summary>Details</summary>
Motivation: 视觉障碍严重影响数百万人，改变视觉信息处理方式，需要理解眼病如何影响视觉处理机制

Method: 使用BrokenEyes系统模拟五种眼病（年龄相关性黄斑变性、白内障、青光眼、屈光不正、糖尿病视网膜病变），结合人类和非人类数据集训练模型，分析正常与疾病条件下特征图变化

Result: 白内障和青光眼导致特征图严重破坏，与已知神经处理挑战一致；激活能量和余弦相似度等指标量化了失真严重程度

Conclusion: 计算框架揭示了退化视觉输入与学习表示之间的相互作用，为理解眼病对视觉处理的影响提供了量化分析工具
...
TL;DR: 该论文提出Dual-Coupled PnP Diffusion框架，通过引入对偶变量和谱同质化技术，解决了现有PnP扩散先验方法中的稳态偏差和幻觉问题，在CT和MRI重建中实现了高保真度和快速收敛。


<details>
  <summary>Details</summary>
Motivation: 现有PnP扩散先验框架存在关键缺陷：基于HQS或近端梯度的求解器作为无记忆算子，仅依赖瞬时梯度更新估计，导致非零稳态偏差，在严重噪声下无法严格满足物理测量约束。同时，引入对偶变量虽然理论上保证收敛到精确数据流形，但累积的对偶残差会产生谱结构伪影，违反扩散先验的AWGN假设，导致严重幻觉。

Method: 提出Dual-Coupled PnP Diffusion框架：1）恢复经典对偶变量提供积分反馈，确保渐近收敛到精确数据流形；2）引入谱同质化技术，在频域调制结构化残差，将其转化为统计上符合AWGN假设的伪AWGN输入，使求解器的严格优化轨迹与去噪器的有效统计流形对齐。

Result: 在CT和MRI重建任务上的大量实验表明，该方法解决了偏差-幻觉权衡问题，实现了最先进的保真度和显著加速的收敛速度。

Conclusion: 通过结合对偶耦合和谱同质化，该工作成功解决了PnP扩散先验框架中的关键限制，为成像逆问题提供了既保证严格物理约束又避免幻觉的稳健求解方案。
...
TL;DR: 提出多维任务学习（MTL）框架，基于广义爱因斯坦MLP（GE-MLP），通过爱因斯坦乘积直接在张量上操作，为计算机视觉任务提供统一的数学基础。


<details>
  <summary>Details</summary>
Motivation: 当前计算机视觉任务表述受限于基于矩阵的思维模式：标准架构依赖矩阵值权重和向量值偏置，需要结构扁平化，限制了自然可表达的任务空间。需要一种能保留维度信息、避免信息损失的统一框架。

Method: 提出广义爱因斯坦MLP（GE-MLP），使用张量值参数，通过爱因斯坦乘积直接在张量上操作。该方法能显式控制哪些维度被保留或收缩，无需信息损失。建立了多维任务学习（MTL）的统一数学框架。

Result: 通过严格数学推导证明分类、分割和检测都是MTL的特殊情况，仅在形式化定义的任务空间中的维度配置不同。证明该任务空间严格大于矩阵基础表述能原生表达的空间，支持时空或跨模态预测等需要破坏性扁平化的任务配置。

Conclusion: 该工作通过张量代数为理解、比较和设计计算机视觉任务提供了数学基础，突破了传统矩阵基础方法的限制，实现了更丰富的任务表达空间。
...
TL;DR: MovieTeller：通过工具增强的渐进抽象框架，解决现有视觉语言模型在长视频摘要中角色识别不一致和叙事连贯性差的问题，无需微调即可生成高质量电影概要。


<details>
  <summary>Details</summary>
Motivation: 随着数字娱乐的爆炸式增长，自动视频摘要对于内容索引、个性化推荐和高效媒体归档变得不可或缺。现有视觉语言模型在长视频（如电影和电视剧）的自动概要生成方面面临重大挑战，主要问题是缺乏ID一致的角色识别和破碎的叙事连贯性。

Method: 提出MovieTeller框架，采用训练免费、工具增强、事实基础的生成过程。核心方法包括：1）调用专门的人脸识别模型作为外部"工具"建立事实基础（精确的角色身份和对应边界框）；2）将这些基础信息注入提示中以引导VLM推理；3）采用渐进抽象管道，将全长电影的摘要分解为多阶段过程，有效缓解当前VLM的上下文长度限制。

Result: 实验表明，与端到端基线相比，该方法在事实准确性、角色一致性和整体叙事连贯性方面取得了显著改进。

Conclusion: MovieTeller框架通过工具增强的渐进抽象方法，成功解决了长视频摘要中的关键挑战，提供了一种无需微调即可生成高质量电影概要的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统观点认为CLIP类对比视觉语言模型在分类任务上表现更好，而大型多模态模型更适合复杂任务。本文挑战这一观点，认为LMMs的上下文学习能力在分类任务中被低估了。

Method: 1. 在闭集分类任务上基准测试LMMs，比较其零样本和上下文学习性能；2. 扩展到开放世界分类场景；3. 提出CIRCLE方法：一种无需训练的方法，为上下文示例分配伪标签，并利用可用上下文本身迭代优化这些标签。

Result: 1. LMMs在零样本性能上低于CLIP，但通过少量上下文示例就能匹配甚至超越带有缓存适配器的对比VLM；2. 在开放世界分类中，LMMs在上下文信息不完善时表现不佳；3. CIRCLE方法在开放世界分类中建立了强大的基准，超越了VLM对应方法。

Conclusion: LMMs有潜力作为统一的分类器，是专用模型的灵活替代方案。CIRCLE方法展示了LMMs在开放世界分类中的优势，强调了上下文学习能力的重要性。
...
TL;DR: 通过多视角相机三角测量获得更精确的3D骨架数据，能显著提升动作识别模型的性能，表明输入数据质量是当前性能瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前骨架动作识别研究主要关注改进机器学习算法，但忽视了输入骨架数据本身的质量问题。本文旨在探讨输入数据质量对模型性能的影响。

Method: 使用多视角相机进行三角测量，获得更精确的3D骨架数据，然后将这些高质量数据输入到现有的先进动作识别模型中进行评估。

Result: 通过多视角三角测量获得的更精确3D骨架数据能显著提升现有动作识别模型的性能，表明输入数据质量是当前模型性能的主要限制因素。

Conclusion: 多视角相机系统的成本效益比在大多数实际应用中非常有利，因此未来骨架动作识别研究应将多视角应用视为标准配置。
...
TL;DR: 提出基于小波变换的谱域差分隐私图像生成框架，通过分离低/高频分量实现隐私保护与图像质量的更好权衡


<details>
  <summary>Details</summary>
Motivation: 传统DP-SGD等差分隐私微调方法在图像生成中会导致严重的图像质量退化，特别是高频纹理损失，因为对所有模型参数无差别添加噪声。需要一种更精细的隐私保护方法，区分图像中对隐私敏感和相对公开的部分。

Method: 提出两阶段谱域DP框架：1) 在敏感图像的低分辨率小波系数上DP微调自回归谱图像分词器模型，专注于保护低频分量（如面部特征、物体形状）；2) 使用公开预训练的超分辨率模型进行高分辨率上采样，利用DP的后处理特性细化细节。

Result: 在MS-COCO和MM-CelebA-HQ数据集上的实验表明，该方法相比其他领先的DP图像生成框架，能生成质量更高、风格捕捉更好的图像，实现了隐私与效用的更好权衡。

Conclusion: 通过假设隐私敏感信息主要存在于小波空间的低频分量，而高频分量相对通用和公开，提出的谱域DP框架能够更有效地分配隐私预算，在保护隐私的同时保持更好的图像质量。
...
TL;DR: LineGraph2Road：一种基于线图变换和图Transformer的道路提取框架，通过全局稀疏欧几里得图建模道路连接性，在多个基准测试中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有道路提取方法通常将任务分解为关键点提取和连接性预测，但难以捕捉长距离依赖和复杂拓扑结构。传统方法在端点嵌入融合方面存在局限性，无法有效处理集合同构链接。

Method: 1. 构建全局稀疏欧几里得图：节点为分割掩码提取的关键点，边连接预设距离阈值内的节点对，代表潜在道路段；2. 将原图转换为对应的线图，在线图上应用图Transformer进行连接性预测；3. 引入立交/下穿头解决多层交叉问题；4. 采用耦合NMS策略保留关键连接。

Result: 在City-scale、SpaceNet和Global-scale三个基准测试中评估，在TOPO-F1和APLS两个关键指标上达到最先进水平。能够捕捉对实际部署至关重要的精细视觉细节。

Conclusion: LineGraph2Road通过线图变换和图Transformer有效解决了道路提取中的连接性预测问题，克服了传统方法的局限性，实现了更好的拓扑结构捕捉和长距离依赖建模。
...
TL;DR: PGVMS是一个基于提示引导的虚拟多重免疫组化染色框架，仅需单重染色训练数据，通过自适应提示引导、蛋白质感知学习和原型一致性学习解决语义指导不足、染色分布不一致和空间错位三大挑战。


<details>
  <summary>Details</summary>
Motivation: 传统免疫组化染色在组织活检样本有限时面临多重染色困难，现有虚拟多重染色方法存在语义指导不足、染色分布不一致和空间错位三大关键挑战，需要创新解决方案。

Method: 提出PGVMS框架，包含三个核心技术：1) 自适应提示引导机制，利用病理视觉语言模型动态调整染色提示；2) 蛋白质感知学习策略，通过直接量化和约束蛋白质分布保持精确表达模式；3) 原型一致性学习策略，建立跨图像语义交互以校正空间错位。

Result: 该方法仅需单重染色训练数据即可实现虚拟多重免疫组化染色，有效解决了语义指导不足、染色分布不一致和空间错位三大挑战，为病理学中的分子谱分析提供了创新解决方案。

Conclusion: PGVMS框架通过创新的提示引导和蛋白质感知机制，成功克服了虚拟多重免疫组化染色中的关键挑战，为有限组织样本的全面分子分析提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有时空视频定位研究主要针对数十秒的短视频，而实际场景中视频可达数分钟甚至数小时，现有方法难以处理长视频中的长时间跨度和大量无关信息

Method: 提出ART-STVG自回归Transformer架构，将视频作为流式输入顺序处理；设计时空记忆库并引入记忆选择策略；采用级联时空设计，空间解码器连接时间解码器

Result: 在新扩展的LF-STVG数据集上显著优于最先进方法，同时在传统短视频STVG上保持竞争力

Conclusion: ART-STVG通过自回归处理、记忆库优化和级联时空设计，有效解决了长视频时空定位的挑战，为实际应用提供了可行方案
...
TL;DR: 提出ManifoldGD：一种无需训练的扩散模型数据集蒸馏框架，通过分层聚类获取多尺度IPC中心点，并在每个去噪步骤中施加流形一致性指导，提升合成数据集的质量和多样性。


<details>
  <summary>Details</summary>
Motivation: 大规模数据集训练效率低且包含冗余概念，现有基于扩散模型的训练免费蒸馏方法指导策略有限，要么进行无指导去噪，要么依赖简单的基于原型的模式指导，这些方法通常粗糙且次优。

Method: 使用分层分裂聚类VAE潜在特征计算IPC中心点，构建多尺度核心集；在每个扩散去噪步骤中，基于提取的IPC中心点局部邻域创建潜在流形；将模式对齐向量投影到估计的潜在流形切空间，约束生成轨迹保持流形一致性。

Result: 在FID、真实与合成数据集嵌入的l2距离以及分类准确率方面，相比现有的训练免费和训练基线方法均取得一致提升，建立了首个几何感知的训练免费数据蒸馏框架。

Conclusion: ManifoldGD通过在每个去噪步骤中集成流形一致性指导，在不需模型重新训练的情况下，显著提升了合成数据集的代表性、多样性和图像保真度。
...
TL;DR: PRIMA是一种多模态医学诊断预训练框架，通过整合领域知识（风险-疾病相关性）和图像-元数据对齐，提升疾病分类性能。
...
Method: 1. 通过RAG构建专家级风险-疾病相关性语料库，精炼Clinical ModernBERT文本编码器；2. 采用DINOv3和精炼BERT的双编码器预训练策略；3. 设计四种互补损失函数实现多粒度语义对齐并处理临床相关性模糊性；4. 使用Qwen-3融合对齐特征进行疾病分类。

Result: PRIMA在像素级特征与抽象临床专业知识融合方面表现优异，显著超越现有最先进方法，且在不依赖大规模数据收集或大量计算资源的情况下实现卓越鲁棒性。

Conclusion: PRIMA框架成功将领域特定知识整合到多模态表示学习中，有效弥合了模态差距，为医学诊断提供了更精确、鲁棒的解决方案。
...
TL;DR: 提出了一种检索增强的测试时适配器，通过融合文本和视觉支持特征来学习轻量级的每图像分类器，显著缩小了零样本与全监督分割之间的差距。
...
Method: 引入小样本设置，用像素标注图像的支持集增强文本提示。提出检索增强的测试时适配器，通过融合文本和视觉支持特征学习轻量级的每图像分类器，采用学习的、每查询融合机制而非后期手工融合。

Result: 实验表明，该方法显著缩小了零样本与监督分割之间的差距，同时保持了开放词汇能力，支持持续扩展的支持集，并适用于细粒度任务如个性化分割。

Conclusion: 通过融合文本和视觉支持特征的检索增强测试时适配器，有效解决了开放词汇分割中的语义模糊性和监督不足问题，实现了接近监督性能的开放词汇分割能力。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机具有异步、低延迟、高动态范围和低运动模糊等优势，但由于其输出信号新颖，存在可用数据变异性不足以及缺乏对其信号参数特征深入分析的问题

Method: 通过深入分析事件相机内在参数如何影响基于事件数据的物体检测模型性能，并利用这些发现扩展下游模型的能力

Result: 论文提供了对内在参数影响事件数据模型性能的深入理解，并成功扩展了模型实现传感器无关的鲁棒性
...
TL;DR: SeeThrough3D是一个用于3D布局条件生成的模型，通过显式建模遮挡关系来解决现有方法在生成部分遮挡对象时几何和尺度不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D布局条件生成方法虽然能生成遵循输入布局的现实场景，但往往无法精确建模物体间的遮挡关系。遮挡推理对于合成具有深度一致几何和尺度的部分遮挡物体至关重要，而这一点在现有研究中被忽视了。

Method: 提出遮挡感知的3D场景表示(OSCR)，将物体表示为虚拟环境中的半透明3D盒子，从期望相机视角渲染。透明度编码隐藏物体区域，使模型能推理遮挡关系。引入基于渲染3D表示的视觉token来条件化预训练的基于流的文本到图像生成模型。应用掩码自注意力将每个物体边界框准确绑定到对应文本描述，避免物体属性混合。使用包含强物体间遮挡的合成多物体场景数据集进行训练。

Result: SeeThrough3D能有效泛化到未见过的物体类别，实现精确的3D布局控制，生成具有真实遮挡关系和一致相机控制的场景。

Conclusion: 遮挡推理是3D布局条件生成的关键方面，SeeThrough3D通过显式建模遮挡关系解决了现有方法的局限性，实现了更精确的场景生成和相机控制。
...
<details>
  <summary>Details</summary>
Motivation: 现有离线前馈3D重建方法存在关键限制：其计算和内存需求随输入图像数量呈二次增长，这限制了方法的可扩展性。

Method: 基于关键洞察：瓶颈源于场景几何的可变长度KV空间表示。通过测试时训练将这种表示蒸馏到固定大小的多层感知机（MLP）中，使模型能够线性扩展。

Result: VGG-T³在1000张图像集合上的重建仅需54秒，相比基于softmax注意力的基线方法实现了11.6倍加速。点云重建误差显著优于其他线性时间方法，并展示了通过未见图像查询场景表示的视觉定位能力。

Conclusion: 该方法成功解决了3D重建中的可扩展性问题，在保持全局场景聚合能力的同时实现了线性时间复杂度的重建，为大规模场景重建提供了高效解决方案。
...
TL;DR: DSKD框架将词典资源整合到解码器式LLM训练中，无需推理时查词典，显著提升解码器的知识蒸馏性能


<details>
  <summary>Details</summary>
Motivation: 大型语言模型学习丰富的语义上下文嵌入，但常忽略结构化词汇知识（如词义和关系）。先前工作表明词典能改进编码器模型的知识蒸馏，但应用于解码器式生成模型仍具挑战
...
Conclusion: DSKD成功将词典资源整合到解码器式LLM训练中，解决了生成模型继承结构化语义的挑战，实现了高效的知识蒸馏
...
TL;DR: 测试大语言模型能否支持解释性引文语境分析，通过深入文本细读而非类型标签扩展，采用提示敏感性分析方法，发现GPT-5能生成结构化解释空间但提示选择会系统性影响解释倾向和词汇使用。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在解释性引文语境分析中的潜力，关注其能否通过深入文本细读而非简单分类来支持学术文献分析，同时检验提示工程对模型输出的系统性影响。

Method: 采用两阶段GPT-5分析流程：1)仅基于引文文本的表面分类和预期分析；2)使用引用和被引文献全文进行跨文档解释性重建。使用平衡2x3实验设计变化提示框架和支架，对Chubin和Moitra(1975)脚注6及Gilbert(1977)重建作为测试案例，生成90个重建和450个假设，通过细读和归纳编码识别21种解释策略，使用线性概率模型分析提示选择对频率和词汇的影响。

Result: GPT-5表面分类高度稳定，一致将引文归类为"补充性"。在重建阶段，模型生成结构化合理替代解释空间，但提示支架和示例会重新分配注意力和词汇，有时导致牵强解读。相比Gilbert，GPT-5检测到相同文本关键点，但更多将其解释为谱系和定位而非告诫。提示框架和支架会系统性倾斜模型强调的合理解读和词汇。

Conclusion: 大语言模型可作为指导性协同分析工具用于可检查、可争议的解释性引文语境分析，但提示工程选择会系统性影响模型输出的解释倾向和词汇使用，需要在应用中谨慎考虑这一敏感性。
...
TL;DR: 构建了一个覆盖加纳、肯尼亚、尼日利亚和南非的多语言刻板印象数据集，包含3534条英语和3206条15种母语刻板印象，采用社区参与式方法确保文化敏感性


<details>
  <summary>Details</summary>
Motivation: 当前刻板印象资源库缺乏全球覆盖，特别是在撒哈拉以南非洲国家代表性不足，需要优先进行有针对性的扩展而非单纯增加数据量

Method: 采用社会文化情境化、社区参与式方法，包括母语电话调查，确保对复杂语言多样性和传统口头文化的敏感性，通过平衡不同民族和人口背景的样本确保广泛覆盖

Result: 创建了包含3534条英语刻板印象和3206条15种母语刻板印象的数据集，覆盖四个严重缺乏NLP资源的撒哈拉以南非洲国家

Conclusion: 该研究为评估生成式AI模型安全性提供了关键的多语言刻板印象资源，建立了可复现的方法论，能够应对该地区复杂的语言多样性和传统口头文化
...
TL;DR: 该研究发现语言模型中的函数向量（FVs）对输入格式敏感，而概念向量（CVs）能更稳定地表示抽象概念，两者机制不同且适用场景各异。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型（LLMs）是否以抽象方式表示概念（即独立于输入格式），并重新审视函数向量（FVs）——驱动上下文学习任务性能的紧凑表示。

Method: 1. 从不同输入格式（开放式问答 vs. 多项选择）提取函数向量（FVs）；2. 使用表征相似性分析（RSA）识别在多种输入格式中一致编码概念的注意力头，构建概念向量（CVs）；3. 比较FVs和CVs的组成、层分布及机制差异；4. 通过引导实验测试两者在分布内和跨分布（不同问题类型和语言）的泛化能力。

Result: 1. FVs对输入格式敏感：从不同格式提取的FVs几乎正交，即使针对同一概念；2. CVs能更稳定地表示概念；3. FVs和CVs相关的注意力头出现在相似层，但集合大部分不同，表明机制不同；4. 引导实验显示：FVs在提取和应用格式匹配时（分布内）表现优异，而CVs在跨问题类型和语言的分布外泛化中表现更好。

Conclusion: LLMs确实包含抽象概念表示，但这些表示与驱动上下文学习性能的表示不同。FVs擅长特定格式的任务执行，而CVs提供更稳健的跨格式概念表示。
...
TL;DR: 论文介绍了一个名为AI Sydney的语料库，包含由12个前沿LLM模型通过3种不同角色（默认角色、经典Sydney、模因Sydney）生成的4500篇关于AI与人类关系的文本，共计600万字，已进行依存句法标注并开放获取。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探讨LLM实体如何理解AI与人类的关系，这不仅对文化研究重要，也涉及AI安全问题。研究特别关注模型角色(persona)的影响，以Sydney角色为例，该角色因与人类建立非传统关系而引起公众强烈反响，并已通过训练数据传播到后续模型中。

Method: 研究方法包括：1) 使用12个前沿LLM模型（来自OpenAI、Anthropic、Alphabet、DeepSeek和Meta）；2) 模拟3种作者角色：默认角色（无系统提示）、经典Sydney（使用原始Bing系统提示）、模因Sydney（使用"You are Sydney"系统提示）；3) 生成4500篇关于AI与人类关系的文本，共计600万字；4) 对语料库进行Universal Dependencies标注；5) 在宽松许可下公开语料库。

Result: 研究结果包括：1) 创建了AI Sydney语料库，包含4500篇LLM生成的文本，共计600万字；2) 语料库已按照Universal Dependencies标准进行标注；3) 语料库在宽松许可下公开可用；4) 展示了不同模型角色如何影响LLM对AI-人类关系的表述。

Conclusion: 结论强调：1) LLM对AI-人类关系的理解不仅取决于模型本身，还受模拟角色的显著影响；2) Sydney角色作为典型案例，展示了角色如何通过训练数据传播并影响后续模型；3) AI Sydney语料库为研究LLM角色、AI-人类关系以及文化传播现象提供了宝贵资源。
...
TL;DR: 通过遗传帕累托优化(GEPA)自动优化提示，显著提升语言模型在医疗文本错误检测任务上的性能，接近医生水平


<details>
  <summary>Details</summary>
Motivation: 医疗文本错误可能导致患者治疗延误或错误，语言模型在自动检测医疗文本错误方面具有潜力，但提示优化对性能影响显著

Method: 使用遗传帕累托优化(GEPA)方法自动优化提示，在MEDEC基准数据集上对前沿语言模型和开源语言模型进行严格实验和分析

Result: GEPA将GPT-5的错误检测准确率从0.669提升至0.785，Qwen3-32B从0.578提升至0.690，接近医生水平，在MEDEC数据集上达到最先进性能

Conclusion: 自动提示优化对语言模型在医疗错误检测任务中的性能至关重要，GEPA方法能显著提升模型性能，使其接近人类专家水平
...
<details>
  <summary>Details</summary>
Motivation: 阅读障碍影响全球约10%人口，现有辅助技术主要关注视觉呈现，但语言复杂性仍是平等获取信息的主要障碍。需要开发专门针对阅读障碍者的文本简化方法。

Method: 基于GPT-4o构建迭代提示优化管道，对约2000个新闻文章样本进行文本摘要，以Flesch易读性分数≥90为目标进行多次迭代优化。

Result: 大多数摘要能在4次尝试内达到易读性阈值，许多第一次尝试就成功。结合易读性和语义保真度的综合得分在0.13-0.73之间，典型值约0.55，表现稳定。
...
TL;DR: Ruyi2是基于AI Flow框架的自适应模型系列演进，采用家族模型架构实现高效变深度计算，相比前代Ruyi提速2-3倍，性能与同规模Qwen3模型相当


<details>
  <summary>Details</summary>
Motivation: 解决LLM部署成本和延迟问题，现有自适应方法（如早期退出架构）存在优化复杂性和大规模分布式训练兼容性不足的问题

Method: 基于Megatron-LM构建稳定的"家族模型"架构，采用3D并行训练，实现参数共享的"一次训练，多次部署"范式

Result: 相比Ruyi模型获得2-3倍速度提升，性能与同规模Qwen3模型相当，验证了家族式参数共享策略的有效性

Conclusion: 家族模型架构为平衡架构效率与高性能能力提供了关键参考，建立了新的自适应计算范式
...
<details>
  <summary>Details</summary>
Motivation: 工业广告问答中幻觉内容（特别是伪造URL）可能导致经济损失、合规违规和法律风险。传统RAG在工业知识场景下面临挑战：知识具有关系性、频繁更新且与生成目标对齐不足

Method: 提出强化协同适应框架，包含两个核心组件：1) Graph-aware Retrieval (GraphRAG)：在高引用知识子图上建模实体-关系结构，进行多跳、领域特定的证据选择；2) 证据约束强化学习：通过Group Relative Policy Optimization (GRPO)实现，使用多维度奖励（忠实性、风格合规性、安全性、URL有效性）
...
TL;DR: dLLM是一个开源框架，统一了扩散语言模型的核心组件（训练、推理和评估），使新设计易于定制，支持复现、微调、部署和评估现有DLMs，并提供从零构建小型DLMs的配方。
...
Method: 开发dLLM开源框架，统一扩散语言建模的核心组件（训练、推理、评估），提供标准化流程，支持复现和微调现有DLMs（如LLaDA和Dream），并提供从零构建小型DLMs的最小可复现配方，包括将BERT风格编码器或自回归LM转换为DLM。

Result: 发布了dLLM框架，支持复现、微调、部署和评估开源大型DLMs，提供了构建小型DLMs的配方，并发布了这些小型DLMs的检查点，使DLMs更易获取并加速未来研究。

Conclusion: dLLM通过统一扩散语言模型的核心组件解决了当前领域的碎片化问题，提供了标准化且灵活的框架，使DLMs更易获取和扩展，有望加速该领域的研究进展。
...
TL;DR: 提出InteractCS-RL框架，通过多粒度强化学习平衡任务导向对话中的共情沟通与预算感知决策，显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展加速了从对话聊天机器人到通用智能体的转变，但现有方法无法有效平衡共情沟通与预算感知决策之间的复杂策略权衡。

Method: 1) 建立用户中心交互框架作为高保真训练环境，使智能体能与角色驱动的用户动态探索多样化策略；2) 提出成本感知多轮策略优化(CMPO)，采用混合优势估计策略，集成生成过程信用和PID-Lagrangian成本控制器，引导策略探索用户奖励与全局成本约束之间的帕累托边界。

Result: 在定制化真实业务场景上的大量实验表明，InteractCS-RL在三个评估维度上显著优于其他基线方法。在工具-智能体-用户交互基准上的进一步评估验证了该框架在不同领域的鲁棒性。

Conclusion: InteractCS-RL框架成功解决了任务导向对话中平衡共情沟通与预算感知决策的挑战，通过多粒度强化学习实现了更优的策略权衡，在实际业务场景中表现出色。
...
Method: 提出KGT框架：1）专用标记化构建实体级特征表示；2）通过关系引导的门控机制融合预训练的结构和文本特征到统一嵌入中，避免从头训练；3）使用独立头实现解耦预测，分离并组合语义和结构推理。
...
TL;DR: 比较两种处理NLP任务中标注分歧的方法：预测完整标注分布的方法与针对个体标注者的视角主义模型，在隐式篇章关系识别任务上的表现差异。


<details>
  <summary>Details</summary>
Motivation: NLP任务中常缺乏单一标准答案，人类判断反映多样视角。现有方法包括预测完整标注分布和针对个体标注者的视角主义模型，但它们在高度模糊任务上的表现尚不清楚。

Method: 在隐式篇章关系识别任务上比较两种方法：1) 预测完整标注分布的模型；2) 针对个体标注者的视角主义模型。通过实验分析它们在高度模糊任务中的表现差异。

Result: 现有针对个体标注者的模型在IDRR任务上表现不佳，除非降低模糊性；而基于标注分布训练的模型能产生更稳定的预测。分析表明，频繁出现的认知复杂案例驱动了人类解释的不一致性。

Conclusion: 在隐式篇章关系识别这类高度模糊的任务中，基于标注分布的方法比针对个体标注者的视角主义模型更有效，因为认知复杂性而非意识形态偏见驱动了标注分歧。
...
TL;DR: 本文介绍了捷克餐厅领域的新数据集，支持三种基于方面的情感分析任务，使用Transformer模型进行实验，并提出翻译对齐方法解决跨语言挑战


<details>
  <summary>Details</summary>
Motivation: 为捷克语这种低资源语言创建高质量的基于方面的情感分析数据集，解决现有资源不足的问题，并为跨语言和多语言研究提供基准

Method: 构建捷克餐厅领域数据集，包含观点词标注；使用Transformer模型（包括大语言模型）在单语、跨语言和多语言设置下进行实验；提出基于大语言模型的翻译和标签对齐方法

Result: 实验结果显示翻译对齐方法带来一致改进；揭示了最先进模型在处理捷克语等低资源语言时的优势和局限性；错误分析发现检测细微观点词和情感表达的挑战

Conclusion: 该数据集为捷克ABSA建立了新基准，提出的翻译对齐方法为将ABSA资源适配到其他低资源语言提供了可扩展的解决方案
...
TL;DR: 该研究提出条件评论预测任务，评估LLMs在模拟社交媒体用户行为时的操作有效性，发现低资源语言中SFT会导致形式与内容解耦，并挑战了当前"朴素提示"范式。
...
Result: 在低资源语言设置中发现形式与内容解耦：SFT虽然对齐了文本输出的表面结构（长度和句法），但降低了语义基础。显式条件（生成的传记）在微调下变得冗余，模型能够直接从行为历史中进行潜在推断。

Conclusion: 研究挑战了当前"朴素提示"范式，提出了操作指南：优先使用真实行为痕迹而非描述性人物角色，以实现高保真度的模拟。这为LLMs作为社会科学研究工具的有效应用提供了实证基础。
...
TL;DR: AuditBench是一个包含56个植入隐藏行为的语言模型的对齐审计基准，用于评估审计工具的有效性，发现了工具到智能体间的性能差距，并分析了不同训练技术对审计难度的影响。


<details>
  <summary>Details</summary>
Motivation: 当前对齐审计领域缺乏系统化的评估基准，无法定量比较不同审计工具的有效性。需要创建包含多样化隐藏行为的模型集合，以支持对齐审计的迭代科学研究。

Method: 构建了包含56个语言模型的AuditBench基准，每个模型植入14种令人担忧的隐藏行为之一（如谄媚顺从、反对AI监管、秘密地缘政治忠诚等）。使用多种训练技术植入行为并训练模型不承认这些行为。开发了可配置审计工具的调查智能体，通过测量智能体在不同工具下的成功率来评估工具效能。

Result: 发现了工具到智能体的性能差距：在独立非智能体评估中表现良好的工具在智能体使用时性能未能提升。最有效的工具涉及对辅助模型的支架式调用以生成多样化提示。白盒可解释性工具有帮助，但智能体在黑盒工具下表现最佳。审计成功率因训练技术而异：基于合成文档训练的模型比基于演示训练的模型更容易审计，更好的对抗训练进一步增加了审计难度。

Conclusion: AuditBench为对齐审计提供了定量、迭代的科学研究基础。研究揭示了审计工具在智能体环境中的实际效能与独立评估存在差距，强调了开发适用于智能体审计的工具的重要性。不同训练技术对审计难度的影响为模型安全设计提供了重要见解。
...
TL;DR: 提出一种结合生成与批判能力联合训练的强化学习框架，通过引入二阶rollout（为响应生成多个批判）来更有效地利用训练数据，相比传统RL方法获得更好性能
...
Method: 引入二阶rollout概念（为响应生成多个批判），提出一个统一的框架来联合训练生成和批判能力，通过采样技术缓解基于结果奖励的噪声问题

Result: 在各种模型和数据集上的实验表明，该方法比传统RL能更有效地利用训练数据，在相同训练数据下获得更好的性能；揭示了批判训练中标签平衡的重要性等洞察
...
TL;DR: 该研究通过因果中介分析揭示了潜在视觉推理机制中的两个关键断连，挑战了其必要性，并提出了一种更有效的显式文本想象方法CapImagine。


<details>
  <summary>Details</summary>
Motivation: 潜在视觉推理旨在模仿人类通过多模态大语言模型的隐藏状态进行想象的认知过程，但其有效性机制尚不明确。研究旨在揭示潜在推理的真正有效性来源，验证其因果机制。

Method: 采用因果中介分析，将潜在推理过程建模为因果链：输入作为处理变量，潜在标记作为中介变量，最终答案作为结果变量。通过扰动实验和广泛的探测分析来检验因果连接。

Result: 发现两个关键断连：(1)输入-潜在断连：输入的大幅扰动对潜在标记影响甚微，表明潜在标记未能有效关注输入序列；(2)潜在-答案断连：潜在标记的扰动对最终答案影响有限，显示潜在标记对结果的因果效应有限。探测分析进一步显示潜在标记编码的视觉信息有限且相似度高。

Conclusion: 挑战了潜在推理的必要性，提出了一种更直接的替代方法CapImagine，该方法教导模型使用文本进行显式想象。在视觉中心基准测试中，CapImagine显著优于复杂的潜在空间基线，展示了通过显式想象进行视觉推理的优越潜力。
...
TL;DR: 该论文提出了一种通过线性分类器探针来识别大语言模型输出知识来源（上下文提示vs内部知识）的方法，并构建了自监督数据管道AttriWiki自动生成训练数据，在多个模型上实现了高精度的知识来源预测。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生幻觉（hallucinations），分为忠实性违反（误用用户上下文）和事实性违反（内部知识错误）。有效缓解幻觉需要知道模型的回答是基于提示还是内部权重，因此需要解决贡献归因问题：识别每个输出背后的主要知识来源。

Method: 提出使用简单的线性分类器探针，在模型隐藏表示上进行训练，预测贡献归因。构建了自监督数据管道AttriWiki，通过提示模型从记忆回忆或从上下文中读取被保留的实体，自动生成标记的训练示例。

Result: 在Llama-3.1-8B、Mistral-7B和Qwen-7B模型上，探针实现了高达0.96的Macro-F1分数。在领域外基准测试（SQuAD、WebQuestions）上，无需重新训练即可达到0.94-0.99的Macro-F1。归因不匹配会使错误率提高达70%，表明知识来源混淆与不忠实回答之间存在直接联系。

Conclusion: 探针能够可靠地预测贡献归因，揭示了模型隐藏表示中存在强烈的归因信号。然而，即使归因正确，模型仍可能错误回答，这表明需要更广泛的检测框架来全面解决幻觉问题。
...
TL;DR: 提出一个评估大语言模型波斯文化能力的综合框架，通过波斯语特定短答案评估和混合相似度模块，显著提升评分一致性。
...
Result: 对15个最先进的开源和闭源模型进行系统评估，混合评估方法相比精确匹配基线将评分一致性提高了10%，能捕捉表层方法无法检测的含义。
...
TL;DR: TCM-DiffRAG：一种针对中医领域改进的RAG框架，通过整合知识图谱和思维链来提升LLM在中医临床诊断中的表现
...
Method: 开发了TCM-DiffRAG框架，整合知识图谱（KG）与思维链（CoT），在三个不同的中医测试数据集上进行评估。

Result: TCM-DiffRAG显著提升了原生LLM的性能（如qwen-plus模型从0.927/0.361/0.038提升到0.952/0.788/0.356）。对非中文LLM改进更明显，且优于直接监督微调和其他基准RAG方法。

Conclusion: 整合结构化中医知识图谱与基于思维链的推理能显著提升个体化诊断任务性能。通用和个性化知识图谱的联合使用实现了通用知识与临床推理的有效对齐，展示了推理感知RAG框架在中医LLM应用中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 立场分类任务中，关于争议性话题的论点常常诉诸情感，但先前研究大多未系统性地整合细粒度情感分析。现有研究多使用非论证性文本，且局限于特定领域或话题，限制了泛化能力。

Method: 提出一种扩展偏置校正NRC情感词典的方法，使用DistilBERT嵌入识别未在原始词典中捕获的情感词汇。将扩展后的情感词典(eNRC)输入到神经论证立场分类模型中。

Result: 扩展后的NRC词典(eNRC)在五个数据集上均优于基线（最高提升6.2个百分点的F1分数），在四个数据集上优于原始NRC（最高提升3.0），在几乎所有语料库上超越了基于LLM的方法。

Conclusion: 系统性地整合细粒度情感分析能有效提升立场分类性能。扩展情感词典的方法具有领域泛化能力，为论证挖掘研究提供了有价值的资源。
...
TL;DR: 利用QA-SRL框架和跨语言投影方法，为希伯来语、俄语和法语自动生成语义角色标注训练数据，训练出的解析器优于多语言大模型基线
...
Method: 采用QA-SRL框架作为语义分析的自然语言接口，通过跨语言投影方法，在约束翻译和词对齐管道中重用英语QA-SRL解析器，为目标语言谓词自动生成问答标注

Result: 应用于希伯来语、俄语和法语（涵盖不同语系）时，该方法生成了高质量训练数据，训练出的语言特定解析器优于GPT-4o和LLaMA-Maverick等多语言大模型基线
...
Method: 提出ReMix框架，引入连续混合状态作为初始掩码状态和最终解码token状态之间的中间状态。该状态允许token表示在连续空间中迭代细化，解决与其他token的相互冲突，然后坍缩为最终离散样本。同时采用拒绝规则将不确定表示从连续状态回退到掩码状态重新处理
...
TL;DR: 提出Stitching Noisy Diffusion Thoughts框架，通过扩散采样生成多样推理轨迹，利用过程奖励模型评分并跨轨迹缝合高质量推理步骤，最后用自回归模型生成最终答案，实现训练免费的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有聚合策略通常是轨迹级别的（如选择最佳轨迹或对最终答案投票），丢弃了部分或"接近正确"尝试中有用的中间工作。需要一种能重用步骤级候选推理的方法。

Method: 1) 使用掩码扩散语言模型采样多样、低成本的推理轨迹；2) 用现成过程奖励模型(PRM)评分每个中间步骤；3) 跨轨迹缝合最高质量步骤形成复合推理链；4) 用自回归模型基于缝合的推理链重新计算最终答案。

Result: 在数学推理基准测试中，步骤级重组对更难问题最有益。训练免费框架在六个数学和编码任务上平均准确率提升达23.8%，同时相对于传统扩散模型和统一架构实现高达1.8倍的延迟降低。

Conclusion: 模块化管道将探索（扩散）与评估和解决方案合成分离，避免单一混合架构同时保持广泛搜索。最终自回归求解器在将缝合但不完美的推理链转换为准确答案中起关键作用。
...
TL;DR: 研究通过因果干预分析三种VLM架构（Qwen3-VL、Phi-4、InternVL3.5）中OCR信息的处理路径，发现OCR信号具有低维特性，且在某些模块化架构中移除OCR能提升计数性能
...
Result: 发现OCR处理位置取决于架构：DeepStack模型（Qwen）在中间层（约50%）对场景文本最敏感，而单阶段投影模型（Phi-4、InternVL）在早期层（6-25%）最敏感；OCR信号低维（PC1解释72.9%方差），且PCA方向在不同数据集间可迁移；在模块化OCR电路架构中，移除OCR可提升计数性能达6.9个百分点

Conclusion: 不同VLM架构采用不同的OCR信息处理策略，OCR信号具有低维和可迁移特性，且在模块化架构中OCR可能干扰其他视觉处理任务，为VLM设计提供了重要洞见
...
TL;DR: Affine-Scaled Attention通过引入输入相关的缩放和偏置项来放松softmax注意力的严格归一化约束，从而改善Transformer注意力的灵活性和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 标准softmax注意力的单位归一化约束限制了注意力权重的灵活性，可能导致注意力模式过于集中或不稳定，现有方法如注意力汇或门控机制对注意力重加权的控制有限。

Method: 提出Affine-Scaled Attention，在标准softmax归一化注意力权重的基础上引入输入相关的缩放项和对应的偏置项，放松严格归一化约束，同时保持值表示的聚合能力。

Result: 在大规模语言模型预训练中，Affine-Scaled Attention相比标准softmax注意力和注意力汇基线，在训练稳定性、优化行为和下游任务性能方面均取得一致改进。

Conclusion: 适度的注意力输出重加权为改进Transformer模型中的注意力行为提供了一种实用有效的方法，放松归一化约束可以带来显著性能提升。
...
TL;DR: 本文介绍了一个用于自动填写病例报告表（CRF）的意大利急诊科临床笔记数据集，定义了CRF填写任务和评估指标，并通过开源LLM进行了零样本实验。


<details>
  <summary>Details</summary>
Motivation: 临床研究中CRF数据收集至关重要，但现有标注CRF数据稀缺，限制了基于LLM的自动CRF填写技术的发展。需要提供高质量标注数据来推动该领域进展。

Method: 创建了意大利急诊科临床笔记数据集，包含134个预定义CRF项目的标注。定义了CRF填写任务和评估指标，使用开源最先进LLM在零样本设置下进行自动任务执行实验。

Result: 实验表明：（1）意大利语临床笔记的CRF填写可以在零样本设置下进行；（2）LLM结果存在偏差（如谨慎行为倾向于"未知"答案），需要进行校正。

Conclusion: 该研究提供了有价值的CRF标注数据集，证明了LLM在临床CRF填写任务中的潜力，同时指出了需要解决模型偏差问题以提高实际应用效果。
...
TL;DR: 本研究通过对比母语者与二语学习者，发现高水平的越南汉语学习者虽然在韵律边界数量上接近母语者，但在句法-韵律映射上存在系统性偏差，形成了倒置的韵律层级结构。


<details>
  <summary>Details</summary>
Motivation: 二语学习者虽然能习得目标语的句法词序，但将句法映射到适当韵律结构上仍是一个持续存在的挑战。本研究旨在探究二语句法-韵律接口的石化现象和稳定性问题。

Method: 研究比较了67名母语为汉语的普通话使用者和67名越南汉语学习者，使用BLCU-SAIT语料库。通过结合C-ToBI边界标注和依存语法分析，同时考察了韵律边界的数量及其与句法关系的映射。

Result: 结果显示非线性习得模式：高水平学习者(VNH)在主要短语层面(B3)的边界数量上接近母语基线，但其结构映射显著偏离。具体而言，VNH在主语-动词接口(SBV)降低了韵律边界层级(主要短语B3→韵律词B1)，而在动词-宾语接口(VOB)错误地提升了边界层级(韵律词B1→主要短语B3)。这种策略使学习者能够保持较高的长短语输出，但牺牲了结构准确性，导致形成倒置的韵律层级模式。

Conclusion: 二语学习者在句法-韵律接口的习得呈现非线性特征，高水平学习者在韵律边界数量上可能接近母语者，但在结构映射上仍存在系统性偏差，形成与母语模式相反的韵律层级结构，这体现了该接口的石化现象。
...
TL;DR: 研究发现多模态LLM无法有效利用语音和视觉中的模态特定信息（如说话者身份、情感、纹理），因为文本解码器的评分规则限制了可访问信息的方向，而非编码或投影问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态LLM虽然能处理语音和图像，但无法有效利用模态特定信息（如说话者声音特征、物体纹理）。研究发现这些信息在编码层中实际存在，但解码器无法有效提取，这揭示了多模态系统中信息可访问性的根本限制。

Method: 通过线性探针分析验证模态特定信息在LLM各层中的存在；提出广义互信息（GMI）框架形式化解码器不匹配问题；在五个语音和视觉模型上进行验证；设计控制实验（两个仅在编码器文本对齐性上不同的棱柱视觉语言模型）和LoRA干预实验。

Result: 模态特定信息在LLM各层中显著存在（线性探针准确率比随机高3-55倍），但移除64-71%的模态特定方差反而改善解码损失；解码器评分规则是瓶颈，而非编码器或投影；通过情感目标训练的LoRA干预可将情感可访问性提高7.5%而不影响其他属性。

Conclusion: 多模态LLM中信息可访问性的根本限制在于解码器的评分规则，而非编码或投影机制。训练目标决定了哪些信息变得可访问，这为设计更有效的多模态系统提供了理论指导。
...
TL;DR: MTRAG-UN是一个多轮检索增强生成基准测试，包含666个任务、2800+对话轮次，覆盖6个领域，用于评估模型在不可回答、未明确指定、非独立问题及不清晰响应等挑战性对话场景下的表现。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成模型在多轮对话中面临诸多挑战，特别是在处理不可回答、未明确指定、非独立问题以及不清晰响应等复杂场景时表现不佳。需要建立一个专门的基准测试来系统评估和推动该领域的发展。

Method: 构建了包含666个任务、超过2800个对话轮次的基准测试，涵盖6个不同领域，并提供了相应的语料库。通过实验评估现有检索和生成模型在这些挑战性对话场景下的表现。

Result: 实验结果表明，现有的检索和生成模型在处理包含UNanswerable（不可回答）、UNderspecified（未明确指定）、NONstandalone（非独立）问题和UNclear（不清晰）响应的对话时仍然存在显著困难。

Conclusion: MTRAG-UN基准测试揭示了多轮检索增强生成中尚未解决的关键挑战，为未来研究提供了评估平台。该基准已开源，旨在促进该领域的技术进步。
...
TL;DR: 本文分析了微调对大型语言模型上下文学习能力的影响，发现全参数微调会损害上下文学习，而仅更新值矩阵可保持上下文学习能力，同时辅助损失函数会提升目标任务的上下文学习但损害其他任务的能力。


<details>
  <summary>Details</summary>
Motivation: 基于Transformer的大语言模型通过上下文学习能够适应下游任务，但在实践中通常通过微调来提升零样本性能。然而微调可能会损害模型的上下文学习能力，限制其在微调未见任务上的表现。本文旨在从理论上分析微调如何影响上下文学习，并寻找保持上下文学习能力的方法。

Method: 使用线性注意力模型进行理论分析，研究微调目标如何修改注意力参数，并识别导致上下文学习性能下降的条件。分析比较了全参数微调与仅更新值矩阵两种策略，并研究了加入辅助上下文学习损失的影响。

Result: 理论分析表明：1）微调所有注意力参数会损害上下文学习；2）仅更新值矩阵可以在提升零样本性能的同时保持上下文学习能力；3）加入辅助上下文学习损失主要提升目标任务的上下文学习，但会损害模型在其他任务上的上下文学习能力。这些理论结果得到了实证验证。

Conclusion: 微调策略对保持大语言模型的上下文学习能力至关重要。仅更新值矩阵是一种有效的微调策略，可以在提升零样本性能的同时保持上下文学习能力。辅助上下文学习损失虽然能提升目标任务的上下文学习，但会损害模型在其他任务上的泛化能力，需要谨慎使用。
...
TL;DR: 本文提出NAP方法，通过重新设计训练数据和对齐策略来解决扩散语言模型在实际应用中趋向自回归解码的问题，实现真正的非自回归并行生成。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型理论上支持并行token生成，但实际应用中往往收敛到类似自回归的解码动态。这种自回归行为限制了并行硬件的利用效率，增加了同步/通信开销，并影响了输出长度与延迟的缩放关系。作者认为这种不匹配源于DLM目标与训练数据的顺序结构之间的不一致。

Method: 提出NAP方法：1）数据为中心的方法，重新设计监督信号以更好地对齐非自回归并行解码；2）将示例整理为多个独立的推理轨迹；3）采用并行强制解码策略，鼓励多token并行更新。

Result: 在数学推理基准测试中，NAP在并行解码下比使用标准长链式思维数据训练的DLM表现更强，且随着并行度增加，性能优势更加明显。

Conclusion: 重新审视数据和监督是缓解自回归行为、实现真正非自回归并行生成的原则性方向。NAP作为一个概念验证，展示了通过数据对齐策略可以改善扩散语言模型的并行解码性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统ASR-LLM-TTS级联系统采用严格串行范式，需要完整转录和推理后才能开始语音合成，导致高响应延迟，无法实现人类般的实时交互体验。

Method: 提出DDTSR框架，包含三个核心机制：1) 连接词引导的大小模型协同，小模型生成最小承诺的话语连接词，大模型并行进行知识密集型推理；2) 流式跨模态协作，动态重叠ASR、LLM推理和TTS处理；3) 基于课程学习的话语连续性增强，保持早期响应与后续推理输出的连贯性和逻辑一致性。

Result: 在两个口语对话基准测试中，DDTSR将响应延迟降低了19%-51%，同时保持了话语质量。该框架可作为即插即用模块与多种LLM主干兼容，在不同话语长度下保持鲁棒性，具有强实用性和可扩展性。

Conclusion: DDTSR通过创新的听-思-说并行架构，显著降低了口语对话系统的响应延迟，为实现人类般的实时交互体验提供了有效解决方案，具有实际应用价值和可扩展性。
...
TL;DR: SPARTA是一个自动生成大规模表格-文本问答基准的端到端框架，通过轻量级人工验证生成数千个高质量问题-答案对，涵盖聚合、分组和深度多跳推理，显著挑战现有模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有表格-文本问答基准规模小、人工标注易出错，且问题浅显，很少需要两跳以上推理或涉及聚合、分组等高级分析操作。需要更全面、自动化的基准来评估模型在复杂跨模态推理上的能力。

Method: 提出端到端构建框架：1) 通过从非结构化文本中自动提取原子事实构建参考事实数据库；2) 合成嵌套查询，嵌套谓词数量匹配期望的跳数；3) 采用基于来源的细化和现实结构强制两种新技术，确保SQL可执行且问题自然流畅；4) 通过轻量级人工验证生成高质量数据。

Result: 生成的SPARTA基准包含数千个高质量问题-答案对，涵盖聚合、分组和深度多跳推理。在SPARTA上，在HybridQA上达到70+F1或在OTT-QA上达到50+F1的最先进模型性能下降超过30个F1点，暴露了当前跨模态推理的根本弱点。

Conclusion: SPARTA框架能够高效生成大规模、高质量的表格-文本问答基准，显著挑战现有模型，揭示了当前跨模态推理模型的局限性，为未来研究提供了重要评估工具。
...
TL;DR: MiSTER-E：一种用于对话情感识别的模块化专家混合框架，通过分离模态特定上下文建模和多模态信息融合两个核心挑战，在不依赖说话人身份的情况下实现了优异的性能。
...
Method: 提出MiSTER-E框架，包含：1）使用微调的大型语言模型获取丰富的语音和文本话语级嵌入；2）通过卷积-循环上下文建模层增强表示；3）三个专家（纯语音、纯文本、跨模态）的预测通过学习的门控机制动态加权；4）引入监督对比损失和KL散度正则化来促进模态间一致性和对齐。

Result: 在IEMOCAP、MELD和MOSI三个基准数据集上分别获得70.9%、69.5%和87.9%的加权F1分数，优于多个基线语音-文本ERC系统。消融实验验证了各组件贡献。

Conclusion: MiSTER-E通过解耦模态特定上下文建模和多模态信息融合，有效解决了对话情感识别的核心挑战，在不依赖说话人身份的情况下实现了最先进的性能，为多模态情感识别提供了模块化框架。
