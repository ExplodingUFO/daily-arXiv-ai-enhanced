...
TL;DR: 论文提出KPM-Bench数据集和MoPE算法，解决视频描述中细粒度运动细节缺失和幻觉问题


<details>
  <summary>Details</summary>
Motivation: 现有视频描述模型在描述细粒度运动细节方面存在局限，且存在严重幻觉问题，特别是在以运动为中心的视频中，精确描述复杂肢体动态至关重要但常被忽视

Method: 1) 提出自动化标注流程，整合基于运动学的运动计算与语言解析；2) 构建KPM-Bench数据集，包含细粒度视频-描述对、运动理解问答对和幻觉评估集；3) 提出基于语言的运动解析与提取(MoPE)算法，从文本描述中准确提取运动属性；4) 将MoPE集成到GRPO后训练框架中缓解幻觉问题

Result: 1) 创建并开源KPM-Bench数据集；2) 提出独立于大规模视觉语言或纯语言模型的精确幻觉评估指标；3) 通过MoPE+GRPO框架有效缓解幻觉问题，显著提高运动中心视频描述模型的可靠性

Conclusion: 通过KPM-Bench数据集和MoPE算法，论文为细粒度运动理解和幻觉缓解提供了系统解决方案，显著提升了运动中心视频描述的质量和可靠性
...
TL;DR: 提出3D-HIW数据集和CLUTCH系统，解决野外环境下文本-手部运动生成与描述问题，通过新型VQ-VAE架构和几何精炼实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 手部运动建模在自然场景中仍未被充分探索。现有方法依赖工作室采集的有限数据集，难以扩展到野外环境，且文本-运动对齐的动画保真度不足。

Method: (1) 构建3D-HIW数据集：结合视觉语言模型和3D手部跟踪器，从大量第一人称动作视频中标注32K个3D手部运动序列及对齐文本；(2) 提出CLUTCH系统：包含SHIFT（部件模态分解的VQ-VAE架构）用于手部运动token化，以及几何精炼阶段通过重建损失微调LLM。

Result: 在文本到手部运动生成和运动到文本描述任务上达到最先进性能，建立了首个可扩展的野外手部运动建模基准。

Conclusion: 通过3D-HIW数据集和CLUTCH系统，首次实现了对野外环境下自然手部运动的有效建模，为手部动画生成和描述提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 标准IHC染色资源密集、昂贵且耗时，许多地区无法获得。从H&E切片直接预测HER2水平是潜在替代方案，但现有像素级虚拟染色方法计算昂贵且易产生重建伪影，可能传播诊断错误。
...
Conclusion: LGD-Net通过特征级跨模态映射而非像素级图像生成，有效解决了虚拟染色方法的计算成本和伪影问题，为从H&E切片准确预测HER2表达水平提供了高效可靠的解决方案。
...
TL;DR: 提出了一种无需额外训练或仅需轻量LoRA调优的文本引导遥感图像分割方法，结合对比式和生成式视觉语言模型与SAM，在19个遥感基准测试中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型和视觉基础模型为零样本文本引导遥感图像分割提供了新机会，但现有方法仍需额外可训练组件，限制了其泛化能力和实际应用。本研究旨在探索仅依赖现有基础模型、无需额外训练即可实现文本引导遥感分割的可能性。

Method: 提出两种方法：1) 对比式方法使用CLIP作为SAM网格提案的掩码选择器，实现完全零样本开放词汇语义分割；2) 生成式方法使用GPT-5（零样本）和LoRA调优的Qwen-VL模型为SAM生成点击提示，实现推理和指代分割。两种方法均与SAM集成，形成完全无需训练或轻量LoRA调优的流程。

Result: 在19个遥感基准测试（包括开放词汇、指代和基于推理的任务）上的广泛实验表明，该方法具有强大能力。对比式方法在完全零样本设置下实现了最先进的开放词汇语义分割，生成式方法中LoRA调优的Qwen-VL模型取得了最佳结果。

Conclusion: 研究表明，仅依赖现有基础模型、无需额外训练即可实现有效的文本引导遥感图像分割。提出的简单而有效的方法为遥感分割任务提供了实用的解决方案，代码将在GitHub上开源。
...
TL;DR: 提出了首个视频查询性能预测（VQPP）基准，包含两个文本到视频检索数据集和两个CBVR系统，共56K文本查询和51K视频，为视频领域的QPP研究提供标准化评估框架。


<details>
  <summary>Details</summary>
Motivation: 查询性能预测（QPP）在文本和图像检索领域已有深入研究，但在基于内容的视频检索（CBVR）中仍未被充分探索，需要建立专门的基准来推动该领域研究。

Method: 构建了VQPP基准，包含两个文本到视频检索数据集和两个CBVR系统；探索了多种检索前和检索后性能预测器；使用最佳检索前预测器作为奖励模型，通过直接偏好优化（DPO）训练大型语言模型进行查询重写。

Result: 检索前预测器取得了有竞争力的性能，可在执行检索步骤前启用应用；成功将最佳检索前预测器应用于查询重写任务，通过DPO训练LLM；发布了包含56K文本查询和51K视频的完整基准和代码。

Conclusion: VQPP是首个视频查询性能预测基准，填补了CBVR领域QPP研究的空白，为未来视频域QPP探索提供了代表性基准，展示了检索前预测器的实用价值及其在查询重写等下游任务中的应用潜力。
...
TL;DR: 该论文对Liu和Szirányi提出的手势识别方法进行方法论分析，指出其评估协议存在严重数据泄漏问题，导致报告的高准确率不可靠。


<details>
  <summary>Details</summary>
Motivation: 分析Liu和Szirányi手势识别方法的评估协议有效性，揭示其报告的高准确率可能源于评估方法缺陷而非模型真实性能。

Method: 通过检查已发布的混淆矩阵、学习曲线和数据集构建方式，分析其帧级随机训练-测试分割方法，证明该方法混合了相同受试者的样本，导致数据泄漏。

Result: 发现报告的高准确率源于帧级随机分割导致的数据泄漏，评估未测量对未见个体的泛化能力，混淆矩阵和学习曲线显示模型过度拟合特定受试者。

Conclusion: 强调在基于视觉的手势识别研究中，特别是对于需要识别未见人员手势的应用（如无人机-人交互），必须采用受试者独立的数据划分方法。
...
TL;DR: 提出了一种用于长视频理解的新型端到端框架，包含自适应视频采样器和时空视频压缩器，结合多模态大语言模型，有效处理长视频冗余问题
...
Result: 该框架在各种基准测试中表现出色，在长视频理解任务和标准视频理解基准上均取得优异性能，证明其能够自适应有效地捕获不同时长视频序列的关键信息，实现高压缩率同时保留关键判别信息

Conclusion: 提出的方法在处理长视频序列复杂性方面展现出多功能性和高效性，为长视频理解提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在多种视觉问答基准测试中取得了显著进展，但在传统的细粒度图像分类任务上表现不佳。研究者希望探究这种性能差异的原因，并找出影响细粒度视觉理解能力的关键因素。

Method: 研究测试了大量最新的视觉语言模型在细粒度分类基准上的表现，通过一系列消融实验分析性能差异的原因。实验重点考察了不同组件（如语言模型、视觉编码器）对性能的影响，以及预训练阶段权重冻结策略对细粒度能力的影响。

Result: 研究发现：1）使用更好的语言模型能同等提升所有基准测试分数；2）更好的视觉编码器能不成比例地显著提升细粒度分类性能；3）预训练阶段对细粒度性能至关重要，特别是在预训练期间不冻结语言模型权重时效果更好。
...
TL;DR: 该论文提出了一种利用稀疏多模态测距数据（如雷达或LiDAR）改进单图像新视角合成的扩散模型方法，通过高斯过程在角度域重建密集深度图，解决了传统单目深度估计在低纹理、恶劣天气等条件下的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的单图像新视角合成方法依赖于单目深度估计提供的几何信息，但在低纹理、恶劣天气和遮挡严重的真实场景中，深度估计的可靠性和一致性有限，影响了合成视图的质量。需要更稳健的几何先验来克服这些限制。

Method: 提出多模态深度重建框架，利用极稀疏的测距传感数据（如车载雷达或LiDAR），在角度域使用局部化高斯过程建模深度，实现计算高效推理并显式量化观测有限区域的不确定性。重建的深度和不确定性可直接替换现有扩散渲染流程中的单目深度估计器。

Result: 在真实多模态驾驶场景实验中，用稀疏测距重建深度替换纯视觉深度，显著提升了单图像新视角视频生成的几何一致性和视觉质量，证明了即使在极端稀疏条件下，多模态传感也能带来实际效益。

Conclusion: 可靠的几何先验对基于扩散的视角合成至关重要，稀疏多模态测距数据能有效克服单目深度估计的局限性，为扩散模型提供更稳健的几何条件，改善新视角合成性能。
...
TL;DR: ZACH-ViT是一种紧凑型视觉Transformer，移除了位置嵌入和[CLS]标记，通过全局平均池化实现排列不变性，在医学图像分类任务中展现出与数据特性相关的性能优势。


<details>
  <summary>Details</summary>
Motivation: 传统视觉Transformer依赖位置嵌入和类别标记来编码固定的空间先验，这在自然图像中有效，但在医学成像等空间布局信息较弱或不一致的场景中可能阻碍泛化能力。需要一种更适合医学图像特性的紧凑架构。

Method: 提出ZACH-ViT（Zero-token Adaptive Compact Hierarchical Vision Transformer），移除位置嵌入和[CLS]标记，通过全局平均池化对补丁表示进行聚合实现排列不变性。采用自适应残差投影保持训练稳定性，同时严格控制参数预算。

Result: 在7个MedMNIST数据集上评估，采用严格少样本协议（每类50个样本）。ZACH-ViT（0.25M参数）在BloodMNIST上表现最佳，在PathMNIST上与TransMIL竞争，但在具有强解剖先验的数据集（OCTMNIST、OrganAMNIST）上相对优势减弱。尽管尺寸极小且无预训练，仍能达到竞争性性能并保持亚秒级推理时间。

Conclusion: 架构的归纳偏差与数据结构对齐比追求通用基准主导地位更重要。ZACH-ViT的紧凑设计和排列不变性使其适合资源受限的临床环境部署，特别是在空间布局信息较弱的医学图像任务中。
...
TL;DR: ROCKET提出了一种残差导向的多层表示对齐框架，通过共享投影器和层不变映射，将2D VLA模型与3D视觉基础模型的多层特征对齐，显著提升3D空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型通常在2D数据上预训练，缺乏3D空间理解能力。现有表示对齐方法通常只在单层应用监督，无法充分利用深度分布信息，而朴素的多层对齐会导致梯度干扰。

Method: 提出残差导向的多层表示对齐框架，使用共享投影器通过层不变映射将VLA主干的多层与强大3D视觉基础模型的多层对齐。采用Matryoshka风格稀疏激活方案平衡多个对齐损失，结合免训练层选择策略。

Result: 在LIBERO上仅需约4%计算预算即达到98.5%的SOTA成功率，在LIBERO-Plus和RoboTwin上表现优异，适用于多种VLA模型。

Conclusion: ROCKET通过残差导向的多层对齐框架有效解决了VLA模型3D空间理解不足的问题，在计算效率和性能上均显著优于现有方法。
...
TL;DR: 提出基于记忆驱动的质量感知框架(MQAF)，通过建立存储失真模式的记忆库，在有无参考图像时动态切换双模质量评估策略，减少对高质量参考图像的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有全参考图像质量评估(FR-IQA)方法依赖参考图像质量，限制了在理想参考源不可得的实际应用。受人类视觉系统积累视觉记忆能力的启发，希望开发减少对高质量参考图像依赖的评估方法。

Method: 提出记忆驱动的质量感知框架(MQAF)，建立存储失真模式的记忆库，采用双模质量评估策略：有参考图像时，自适应加权参考信息并比较失真图像与记忆库中的失真模式；无参考图像时，依赖记忆库中的失真模式推断图像质量，实现无参考质量评估(NR-IQA)。

Result: 实验结果表明，该方法在多个数据集上优于最先进的方法，同时能适应无参考和全参考两种任务。
...
TL;DR: MUOT_3M是首个伪多模态水下目标跟踪基准，包含300万帧、3030个视频，提供RGB、增强RGB、深度和语言四种模态。基于此提出的MUTrack采用SAM架构，通过视觉几何对齐、视觉语言融合和四级知识蒸馏，将多模态知识转移到单模态学生模型中，在五个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 水下目标跟踪对海洋机器人、生态监测和海洋探索至关重要，但现有基准数据集规模小且仅含RGB模态，限制了在严重颜色失真、浑浊和低能见度条件下的鲁棒性。需要大规模、多模态、多样化的数据集来推动该领域发展。

Method: 1) 构建MUOT_3M基准：包含300万帧、3030个视频(27.8小时)，标注32个跟踪属性、677个细粒度类别，提供同步的RGB、估计增强RGB、估计深度和语言四种模态；2) 提出MUTrack：基于SAM架构的多模态到单模态跟踪器，采用视觉几何对齐、视觉语言融合和四级知识蒸馏，将多模态知识转移到单模态学生模型中。

Result: 在五个水下目标跟踪基准上的广泛评估表明，MUTrack比最强的SOTA基线高出8.40%的AUC和7.80%的精度，同时以24 FPS的速度运行。MUOT_3M和MUTrack为可扩展、多模态训练但实际可部署的水下跟踪建立了新基础。

Conclusion: MUOT_3M是首个大规模伪多模态水下目标跟踪基准，解决了该领域数据集稀缺的问题。MUTrack通过创新的多模态知识蒸馏方法，实现了高性能的单模态部署，为实际水下应用提供了有效的解决方案。
...
TL;DR: 提出L-AVC任务，专注于通过多模态LLM修改图像的主观情感，并开发EPEM方法实现高效精确的情感编辑


<details>
  <summary>Details</summary>
Motivation: 现有视觉定制研究主要关注客观对齐（如语言、布局、边缘检测），忽略了主观情感内容，且缺乏通用的情感视觉定制基础模型

Method: 提出EPEM方法：1）高效情感间语义转换模块（EIC）使LLM在编辑前后高效对齐情感语义；2）精确情感外语义保留模块（PER）精确保留情感无关内容

Result: 在构建的L-AVC数据集上，EPEM方法在L-AVC任务中优于多个最先进基线，证明了情感信息对L-AVC的重要性以及EPEM在高效精确操纵情感信息方面的有效性

Conclusion: 情感信息对视觉定制至关重要，提出的EPEM方法能够高效精确地操纵图像中的主观情感，为情感视觉定制提供了有效的解决方案
...
TL;DR: 论文提出DeepSVU任务，超越传统安全视频理解仅检测威胁，新增威胁原因归因与评估，并提出UPRM方法解决物理世界信息建模与权衡挑战。


<details>
  <summary>Details</summary>
Motivation: 现有安全视频理解研究主要关注威胁检测与定位，缺乏对威胁原因生成与评估的能力。为填补这一空白，本文提出深度安全视频理解任务，旨在不仅识别和定位威胁，还能归因和评估威胁原因。

Method: 提出统一物理世界正则化混合专家方法，包含两个关键组件：统一物理世界增强混合专家块用于建模粗到细的物理世界信息，物理世界权衡正则器用于自适应权衡这些因素。

Result: 在DeepSVU指令数据集上的实验表明，UPRM方法优于多个先进的视频大语言模型和非视觉语言模型方法，验证了粗到细物理世界信息在DeepSVU任务中的重要性以及UPRM在捕获此类信息方面的有效性。

Conclusion: 论文成功定义了DeepSVU新任务，解决了现有研究的局限性，提出的UPRM方法有效建模了物理世界信息并实现了因素间的自适应权衡，为安全视频理解提供了更全面的分析框架。
...
TL;DR: 提出UAOR模块，一种无需训练、即插即用的不确定性感知观测重注入方法，通过注意力检索将关键观测信息重注入到语言模型的FFN中，提升VLA模型的置信度和忠实度


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型为提升性能通常需要额外的观测线索（深度图、点云）或辅助模块（物体检测器、编码器），这些方法需要昂贵的数据收集和额外训练。本文旨在开发一种无需训练、即插即用的模块来增强VLA模型的性能

Method: 提出不确定性感知观测重注入（UAOR）模块：当当前语言模型层表现出高不确定性（通过动作熵衡量）时，通过注意力检索将关键观测信息重注入到下一层的FFN中，帮助VLA模型在推理过程中更好地关注观测信息

Result: 综合实验表明，该方法能持续提升多种VLA模型在仿真和真实世界任务中的性能，且开销极小。UAOR无需额外观测线索或模块，成为现有VLA管道的通用实用插件

Conclusion: UAOR是一种有效、无需训练、即插即用的模块，通过不确定性感知的观测重注入机制，显著提升VLA模型的置信度和忠实度，为现有VLA系统提供了实用的性能增强方案
...
TL;DR: 提出DCAG框架，通过同时操纵DiT注意力机制中的Key和Value通道实现无需训练的图像编辑强度控制，相比仅操纵Key的方法在编辑保真度权衡上更精确。


<details>
  <summary>Details</summary>
Motivation: 现有基于DiT的图像编辑模型缺乏无需训练的编辑强度控制方法，且现有注意力操纵方法仅关注Key空间（控制注意力路由），完全忽略了Value空间（控制特征聚合），限制了编辑效果的精确控制。

Method: 首先发现DiT多模态注意力层中Key和Value投影均表现出明显的偏置-增量结构，提出双通道注意力引导(DCAG)框架，同时操纵Key通道（通过非线性softmax函数实现粗粒度控制）和Value通道（通过线性加权求和实现细粒度补充），形成二维参数空间(δ_k, δ_v)实现更精确的编辑-保真度权衡。

Result: 在PIE-Bench基准测试（700张图像，10个编辑类别）上，DCAG在所有保真度指标上均优于仅使用Key引导的方法，在局部编辑任务中改进最显著：对象删除（LPIPS降低4.9%）和对象添加（LPIPS降低3.2%）。

Conclusion: DCAG通过同时利用Key和Value通道的偏置-增量结构，提供了更精确的无需训练编辑强度控制，证明了双通道操纵在扩散变换器图像编辑中的有效性，为编辑-保真度权衡提供了更精细的控制机制。
...
Conclusion: 通过利用大语言模型提供的解耦空间和时间知识，DiST能够学习表达性多粒度原型，在少样本动作识别任务中取得了优异性能，为捕捉细粒度空间细节和多样化时间模式提供了透明度。
...
TL;DR: CityGuard：一种用于分布式摄像头隐私保护身份检索的拓扑感知Transformer框架，通过分散自适应度量学习、空间条件注意力和差分隐私嵌入实现城市尺度的人员重识别。


<details>
  <summary>Details</summary>
Motivation: 城市尺度的人员重识别面临视角变化、遮挡和域偏移等严重外观变化，同时需要遵守数据保护规则，防止共享原始图像。现有方法在隐私保护和跨视图对齐方面存在不足。

Method: 提出三个核心组件：1) 分散自适应度量学习器，根据特征分散调整实例级边界；2) 空间条件注意力，将粗略几何信息（如GPS或部署平面图）注入图自注意力；3) 差分隐私嵌入映射与紧凑近似索引，支持安全高效部署。

Result: 在Market-1501等公开基准测试和数据库规模检索研究中，该方法在检索精度和查询吞吐量方面均优于强基线，验证了框架在隐私关键城市身份匹配中的实用性。

Conclusion: CityGuard框架通过拓扑感知Transformer设计，实现了对视角变化、遮挡和域偏移的鲁棒性描述符，并在严格的差分隐私核算下提供了隐私与效用的可调平衡，适用于隐私关键的城市身份匹配应用。
...
TL;DR: TCA-T2M：一种时间一致性感知的文本到动作生成框架，通过跨序列时间对齐和运动约束块解决现有方法中时间结构不一致导致的语义错位和物理不合理问题


<details>
  <summary>Details</summary>
Motivation: 现有两阶段文本到动作生成框架通常忽略跨序列时间一致性，即相同动作在不同实例间共享的时间结构，导致语义错位和物理不合理的动作

Method: 提出TCA-T2M框架，包含：1）时间一致性感知空间VQ-VAE用于跨序列时间对齐；2）掩码运动变换器用于文本条件动作生成；3）运动学约束块减轻离散化伪影确保物理合理性

Result: 在HumanML3D和KIT-ML基准测试中达到最先进性能，证明了时间一致性对于鲁棒和连贯的文本到动作生成的重要性

Conclusion: TCA-T2M通过引入时间一致性感知机制有效解决了文本到动作生成中的跨序列时间对齐问题，显著提升了生成动作的语义准确性和物理合理性
...
<details>
  <summary>Details</summary>
Motivation: 当前多相机BEV语义地图方法依赖昂贵且标注不一致的BEV地面真值，需要减少对完全监督标注数据的依赖

Method: 两阶段训练：1) 自监督预训练阶段，将BEVFormer预测可微分重投影到图像平面，使用Mask2Former生成的多视角语义伪标签进行训练，加入时序一致性损失；2) 监督微调阶段，仅使用50%数据集进行微调

Result: 在nuScenes数据集上，微调性能超越全监督基线模型达+2.5pp mIoU，同时将标注数据使用量减半，总训练时间减少三分之二
...
TL;DR: DohaScript是一个大规模、多作者的手写印地语数据集，包含531名贡献者书写的六首传统印地语对句，旨在解决天城体手写文本基准数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 天城体手写文本在公开基准数据集中严重不足，现有资源规模有限，主要关注孤立字符或短词，缺乏受控词汇内容和作者多样性，无法捕捉天城体手写连续、融合和结构复杂的特性。

Method: 创建DohaScript数据集：收集531名独特贡献者的手写印地语文本，所有作者转录相同的六首传统印地语对句，形成平行风格语料库。包含非识别性人口统计元数据，基于客观清晰度和分辨率标准进行质量筛选，并提供页面级布局难度标注。

Result: 基线实验显示数据集具有清晰的质量分离和对未见作者的强泛化能力，证明了数据集的可靠性和实用价值。DohaScript可作为标准化、可复现的基准，推动低资源脚本环境下连续手写天城体文本的研究。

Conclusion: DohaScript填补了天城体手写文本大规模基准数据集的空白，其受控设计支持手写识别、作者识别、风格分析和生成建模等任务，为低资源脚本环境下的研究提供了标准化基准。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiT）在迭代去噪过程中计算成本高昂。现有免训练加速方法基于特征缓存和重用，但重用多个步骤可能导致潜在漂移和视觉质量下降。作者观察到模型输出在扩散轨迹的大部分区域平滑演化，支持基于预测而非简单重用的加速方法。

Method: 提出PrediT框架：1）将特征预测建模为线性多步问题，使用经典线性多步方法从历史信息预测未来模型输出；2）在高动态区域激活校正器防止误差累积；3）动态步长调制机制通过监测特征变化率自适应调整预测范围。

Result: 在多种基于DiT的图像和视频生成模型上，PrediT实现了高达5.54倍的延迟降低，同时质量下降可忽略不计。实验验证了该方法在保持生成保真度方面的有效性。

Conclusion: PrediT通过线性多步预测、校正机制和自适应步长调制，为DiT模型提供了高效且保真的免训练加速方案，显著降低了计算成本而不牺牲生成质量。
...
TL;DR: OODBench：一个自动化构建的基准测试，用于评估视觉语言模型处理分布外数据的能力，包含40K个实例-类别对，并提出了可靠的自评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型通常假设数据独立同分布，但现实应用中常遇到分布外数据，且处理不当可能带来安全风险。当前缺乏全面评估VLM处理OOD数据能力的基准测试。

Method: 提出OODBench方法，以最小人工验证的自动化方式构建基准测试，包含40K个实例级OOD实例-类别对。同时提出基于"基础到高级递进"提示问题的可靠自动评估指标，全面评估OOD数据对不同难度问题的影响。

Result: 当前VLM在OODBench上即使对于常见图像类别仍表现出显著性能下降。提出的评估指标能更全面地评估OOD数据对不同难度问题的影响。

Conclusion: OODBench为评估VLM处理OOD数据能力提供了有效基准，揭示了当前模型的局限性，提出的评估方法和总结的发现为未来OOD数据获取和评估研究提供了重要参考。
...
TL;DR: BLM-Guard是一个用于短视频广告内容审核的框架，通过结合思维链推理、基于规则的策略原则和批评者引导的奖励，实现细粒度的多模态欺骗性内容检测。
...
Method: 1) 基于规则的ICoT数据合成管道生成结构化场景描述、推理链和标签；2) 强化学习使用平衡因果一致性和策略遵从性的复合奖励优化模型；3) 多任务架构建模模态内操纵（如夸张图像）和模态间不匹配（如字幕-语音漂移）。

Result: 在真实短视频广告上的实验表明，BLM-Guard在准确性、一致性和泛化能力方面优于强基线模型。

Conclusion: BLM-Guard框架通过融合思维链推理、策略原则和强化学习，为商业广告内容审核提供了有效的细粒度多模态检测方案，显著降低了标注成本并提升了检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到动作生成模型在语义对齐方面进展迅速，但难以同时保证语义和物理真实性。生成的动画常出现脚部漂浮等物理不合理现象，需要一种能修正物理问题同时保持语义一致性的方法。

Method: 提出Distortion-aware Motion Calibrator (DMC)，一个后处理模块，采用自监督数据驱动方法。通过输入故意扭曲的动作和原始文本描述，学习生成物理合理的动作，无需复杂物理建模。

Result: DMC显著提升动作质量：在T2M上FID分数降低42.74%，T2M-GPT上降低13.20%，同时获得最高R-Precision。应用于MoMask时，穿透减少33.0%，漂浮伪影更接近真实参考。

Conclusion: DMC作为通用后处理框架，能有效提升各类文本到动作生成模型的物理合理性，同时增强语义一致性，为动作生成提供有前景的优化方案。
...
TL;DR: 该论文首次研究了离散图像分词器的对抗攻击脆弱性，提出了高效的攻击方法，并通过无监督对抗训练提升了分词器的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 离散图像分词器在多模态系统中广泛应用，但其对抗攻击脆弱性尚未被探索。与CLIP编码器不同，离散分词器的安全性研究存在空白，需要评估其脆弱性并开发防御方法。

Method: 1. 提出针对离散分词器的对抗攻击：扰动特征提取以改变分词结果，具有计算高效、应用无关的特点；2. 防御方法：受鲁棒CLIP编码器启发，采用无监督对抗训练微调分词器，保持其他组件冻结。

Result: 攻击方法在分类、多模态检索和字幕生成任务中均有效；无监督对抗训练显著提升了对抗攻击的鲁棒性，能泛化到未见任务和数据，且可利用未标注图像，比监督方法更通用。

Conclusion: 该工作首次揭示了离散图像分词器的对抗脆弱性，强调了分词器鲁棒性对下游任务的重要性，为开发安全的多模态基础模型提供了重要步骤。
...
TL;DR: DEIG是一个用于细粒度可控多实例生成的新框架，通过实例细节提取器和细节融合模块解决现有方法在复杂文本描述下的语义理解问题。


<details>
  <summary>Details</summary>
Motivation: 现有多实例生成方法在空间布局和属性绑定方面已有进展，但在处理复杂文本描述时仍面临细粒度语义理解的挑战，特别是在属性跨实例泄漏问题上。

Method: 提出DEIG框架，包含实例细节提取器（IDE）将文本编码器嵌入转换为紧凑的实例感知表示，以及细节融合模块（DFM）应用基于实例的掩码注意力防止属性跨实例泄漏。构建高质量数据集支持细粒度监督，并建立DEIG-Bench基准。

Result: DEIG在多个基准测试中在空间一致性、语义准确性和组合泛化方面持续优于现有方法，可作为即插即用模块集成到标准扩散管道中。

Conclusion: DEIG通过创新的实例感知表示和注意力机制，实现了对复杂文本描述的细粒度可控多实例生成，为解决多实例场景生成中的语义理解问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 在时尚设计早期阶段，草图能表达结构、轮廓和空间关系，而文本描述能补充材料、颜色和风格细节。现有方法在结合文本和视觉模态时，难以在利用文本局部属性指导的同时保持草图的视觉结构一致性。

Method: 提出LOTS框架：1) 多级条件阶段：在共享潜在空间中独立编码局部特征，同时保持全局结构协调；2) 扩散对引导阶段：通过注意力引导在扩散模型的多步去噪过程中整合局部和全局条件。创建Sketchy数据集，包含专业草图和非专业草图两个子集。

Result: 实验表明，该方法在增强全局结构遵循的同时，利用更丰富的局部语义指导，相比现有最先进方法取得了改进。Sketchy数据集提供了高质量、结构一致的草图，以及更具变化性和不完美的"野外"草图。

Conclusion: LOTS框架通过结合全局草图引导和多个局部草图-文本对，有效提升了时尚图像生成的质量和结构一致性，为多模态时尚设计提供了新的解决方案。
...
TL;DR: Diff2DGS：一种用于手术场景实时重建的两阶段框架，通过扩散模型修复被器械遮挡的组织，并使用可学习变形模型的2D高斯溅射来捕捉动态组织变形，在图像质量和深度精度方面均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 实时重建可变形手术场景对于推进机器人手术、改善外科医生引导和实现自动化至关重要。现有方法在遮挡区域的重建质量有限，且深度精度未得到充分评估，因为EndoNeRF和StereoMIS等基准缺乏3D地面真值。

Method: 提出Diff2DGS两阶段框架：第一阶段使用具有时间先验的扩散视频模块修复被器械遮挡的组织，确保高时空一致性；第二阶段采用带有可学习变形模型(LDM)的2D高斯溅射来捕捉动态组织变形和解剖几何结构。

Result: 在EndoNeRF上达到38.02 dB PSNR，在StereoMIS上达到34.40 dB PSNR，优于现有方法。实验表明仅优化图像质量不一定能获得最佳3D重建精度，因此进一步优化深度质量以确保更准确的几何结构。

Conclusion: Diff2DGS在手术场景重建中实现了高质量的外观和几何重建，通过两阶段方法有效处理遮挡问题，并在SCARED数据集上进行了定量深度精度分析，为手术导航和自动化提供了更可靠的3D重建解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界图像采集面临复杂光照变化和相机成像管线限制，多视角捕获中光照、传感器响应和ISP配置差异导致的光度和色度不一致性违反了光度一致性假设，降低了NeRF和3DGS等现代3D新视角合成方法的重建和渲染质量。

Method: 基于3D高斯泼溅框架，结合全局视角自适应亮度调整和局部像素级残差细化进行精确色彩校正。设计无监督目标函数，联合强制执行亮度校正以及多视角几何和光度一致性，保持显式3DGS表示不变。

Result: 在低光照、过曝光以及复杂亮度和色度变化等挑战性场景中实现了最先进的性能，在保持实时渲染效率的同时显著提升了重建保真度。

Conclusion: Luminance-GS++通过创新的光照校正策略有效解决了多视角捕获中的光度不一致问题，在不修改底层表示的情况下显著提升了3D高斯泼溅在复杂光照条件下的鲁棒性，为现实世界场景的新视角合成提供了实用解决方案。
...
TL;DR: 提出基于高斯-拉普拉斯算子(G-LoG)的双参数过滤方法，用于医学图像拓扑特征提取，在MedMNIST数据集上表现优于单参数过滤，且MLP模型基于拓扑特征能达到与复杂深度学习模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 在拓扑数据分析中，构建实用的过滤方法来检测拓扑和几何特征至关重要。本文旨在利用拉普拉斯高斯算子在增强医学图像边界方面的能力，定义更适合多参数持久性模块的特征提取方法。

Method: 提出G-LoG（高斯-拉普拉斯高斯）双参数过滤方法：1）将体积图像建模为有界函数；2）利用拉普拉斯高斯算子增强医学图像边界；3）构建双参数过滤生成拓扑特征；4）证明持久性模块的插值距离相对于有界函数的最大范数是稳定的。

Result: 1）在MedMNIST数据集上的实验表明，双参数过滤显著优于单参数过滤；2）基于双参数过滤生成的拓扑特征，简单的多层感知器(MLP)能达到与Google AutoML Vision、ResNet、AutoKeras和auto-sklearn等复杂深度学习模型相当的性能；3）理论证明了持久性模块的稳定性。

Conclusion: G-LoG双参数过滤方法为医学图像分析提供了有效的拓扑特征提取框架，不仅性能优于传统单参数方法，而且基于拓扑特征的简单MLP模型能达到与复杂深度学习模型相当的分类性能，展示了拓扑数据分析在医学图像处理中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频世界模型仅接受文本或键盘等粗粒度控制信号，无法响应XR中用户真实运动跟踪，限制了其在具身交互中的应用

Method: 1) 评估现有扩散变换器条件策略；2) 提出有效的3D头手姿态控制机制；3) 训练双向视频扩散模型教师；4) 蒸馏为因果交互系统生成第一人称虚拟环境

Result: 通过人类受试者评估显示，相比基线方法，该系统在任务性能上有提升，且用户感知到的动作控制程度显著更高

Conclusion: 该人中心视频世界模型能够响应精细的头手姿态控制，支持灵巧的手-物交互，为XR中的交互式虚拟环境生成提供了有效解决方案
...
TL;DR: CapNav是一个评估视觉语言模型在考虑智能体物理能力约束下进行室内导航的新基准，包含5类代表性智能体、45个真实室内场景、473个导航任务和2365个QA对，发现当前VLM在严格移动约束下性能显著下降


<details>
  <summary>Details</summary>
Motivation: 现实世界导航决策需要考虑智能体的物理能力约束（如扫地机器人不能爬楼梯，四足机器人可以），而现有视觉语言导航研究缺乏对智能体具体能力条件的系统评估

Method: 定义了5类代表性人类和机器人智能体，描述其物理尺寸、移动能力和环境交互能力；构建包含45个真实室内场景、473个导航任务和2365个QA对的基准数据集；评估了13个现代视觉语言模型

Result: 当前VLM的导航性能随移动约束收紧而急剧下降；即使是SOTA模型也难以处理需要空间维度推理的障碍物类型；模型在考虑智能体能力条件下的导航表现存在显著缺陷

Conclusion: CapNav基准揭示了当前VLM在能力感知导航方面的局限性，为未来VLM的具身空间推理能力发展提供了重要机遇和方向
...
TL;DR: MemStream通过扩展token预算、自适应选择策略和无训练检索专家混合，提升流式视频理解性能，在多个基准上显著超越现有方法


<details>
  <summary>Details</summary>
Motivation: 现有流式视频VQA方法使用有限的每帧token数，导致细粒度视觉细节丢失，且无法有效处理密集视频流，存在查询-帧相似度随时间增加的偏差问题

Method: 1) 扩展token预算以实现更细粒度的时空理解；2) 引入自适应选择策略减少token冗余同时保留局部时空信息；3) 提出无训练检索专家混合，利用外部模型更好识别相关帧

Result: 在CG-Bench上提升+8.0%，LVBench上+8.5%，VideoMME(Long)上+2.4%，均基于Qwen2.5-VL-7B模型，显著超越ReKV方法

Conclusion: MemStream通过解决现有方法的token限制和检索偏差问题，实现了更有效的流式视频理解和问答，在多个基准上取得了显著性能提升
...
TL;DR: 研究发现更大语言模型在预测眼动和阅读时间数据方面表现更好，但所有模型都低估了人类反应概率。大模型在完形填空数据中能提供更高质量的下一个词及其产生概率估计，因为它们对词汇共现统计不敏感，同时与人类完形反应语义更一致。


<details>
  <summary>Details</summary>
Motivation: 探索不同规模语言模型在预测人类语言处理行为（眼动、阅读时间、完形填空）方面的表现差异，理解模型规模如何影响其对不同层次语言信息的敏感性。

Method: 通过比较不同规模的语言模型在预测眼动数据、阅读时间数据和完形填空数据上的表现，分析模型对词汇共现统计的敏感性和语义对齐程度。

Result: 1. 更大语言模型在预测眼动和阅读时间数据方面表现更好；2. 所有模型都低估了人类反应概率；3. 大模型在完形填空中提供更高质量的下一个词概率估计；4. 大模型对词汇共现统计不敏感，但与人类完形反应语义更一致；5. 大模型更强的记忆能力帮助其猜测更语义合适的词，但对单词识别相关的低层次信息不敏感。

Conclusion: 模型规模增加通过增强记忆能力提高了语义对齐，但降低了对低层次语言信息的敏感性，这支持了更大模型通过记忆更多语义信息来更好地预测人类语言处理行为的观点。
...
TL;DR: 论文研究了语言模型引导向量的可靠性问题，发现引导效果在不同行为间存在差异，并探索了训练数据对引导可靠性的影响机制。


<details>
  <summary>Details</summary>
Motivation: 引导向量虽然平均有效，但其效果在不同样本间存在显著差异，对许多目标行为的控制不可靠。作者旨在探究引导可靠性在不同行为间差异的原因，以及训练数据如何影响引导效果。

Method: 通过分析训练激活差异的余弦相似度、正负激活在引导方向上的分离程度，以及不同提示变体训练的引导向量之间的方向差异和效果相关性，来研究引导可靠性的影响因素。

Result: 1) 训练激活差异的余弦相似度越高，引导越可靠；2) 正负激活在引导方向上分离越好的行为数据集，引导效果越可靠；3) 不同提示变体训练的引导向量方向不同但效果相似，且在不同数据集上的效果相关。

Conclusion: 引导向量的不可靠性源于潜在目标行为表示无法被线性引导方向有效近似。这些发现为诊断引导不可靠性提供了实用方法，并激励开发更鲁棒的引导方法，需显式考虑非线性潜在行为表示。
...
TL;DR: 提出一种基于语言模型生成语义软标签的新型神经主题模型，通过重构上下文丰富的监督信号而非传统BoW表示，显著提升主题质量和语义对齐


<details>
  <summary>Details</summary>
Motivation: 传统神经主题模型仅重构文档的词袋表示，忽略了上下文信息且受数据稀疏性困扰，需要更丰富的语义监督信号来提升主题质量

Method: 使用语言模型通过专门提示词生成下一个词的概率分布，将其投影到预定义词汇表上获得语义软标签，然后训练主题模型重构这些基于LM隐藏状态的软标签

Result: 在三个数据集上的实验表明，该方法在主题连贯性和纯度方面显著优于现有基线，同时新提出的检索指标也显示在识别语义相似文档方面大幅超越现有方法

Conclusion: 通过语言模型生成语义软标签为神经主题模型提供了更丰富的监督信号，有效解决了传统方法忽略上下文信息的问题，显著提升了主题质量和检索应用效果
...
<details>
  <summary>Details</summary>
Motivation: 现有生物医学问答系统假设医学知识普遍适用，但真实临床决策依赖于患者特定条件（如并发症、禁忌症）。现有基准缺乏条件推理评估，检索增强或图方法缺乏确保检索知识适用于特定上下文的机制。

Method: 提出CondMedQA基准（首个条件生物医学问答基准），包含答案随患者条件变化的多跳问题。提出Condition-Gated Reasoning框架，构建条件感知知识图，基于查询条件选择性激活或剪枝推理路径。

Result: CGR框架能更可靠地选择条件适当的答案，在生物医学问答基准上达到或超过最先进性能，突显显式建模条件性对稳健医学推理的重要性。

Conclusion: 显式建模条件性对稳健医学推理至关重要，CGR框架通过条件感知知识图和门控推理机制有效解决了生物医学问答中的条件推理挑战。
...
TL;DR: CUICurate：基于GraphRAG的UMLS概念集自动构建框架，通过知识图谱检索和LLM过滤，显著减少人工工作量，提高概念集完整性和精度。


<details>
  <summary>Details</summary>
Motivation: 临床命名实体识别工具通常将自由文本映射到UMLS概念唯一标识符(CUIs)，但许多下游任务需要的是包含相关同义词、子类型和超类型的概念集。目前构建此类概念集劳动密集、执行不一致，现有工具支持不足，特别是对于直接操作UMLS CUIs的NLP流水线。

Method: 提出CUICurate框架，采用基于图的检索增强生成(GraphRAG)方法：1)构建并嵌入UMLS知识图谱用于语义检索；2)针对每个目标概念，从知识图谱检索候选CUIs；3)使用大语言模型(比较GPT-5和GPT-5-mini)进行过滤和分类；4)在五个词汇异质性临床概念上评估，与人工构建基准和黄金标准概念集对比。

Result: 在所有概念上，CUICurate生成的概念集比人工基准显著更大、更完整，同时保持与人工相当的精度。GPT-5-mini在过滤阶段获得更高召回率，GPT-5的分类结果更符合临床医生判断。输出在重复运行中稳定，计算成本低廉。

Conclusion: CUICurate提供了一个可扩展、可复现的UMLS概念集构建方法，大幅减少人工工作量。通过结合基于图的检索和LLM推理，该框架生成聚焦的候选概念集，可适应不同表型和分析需求的临床NLP流水线。
...
TL;DR: 该研究针对金融问答中的文档内检索失败问题，提出了一种针对金融监管文件进行领域微调的页面级评分器，显著提升了页面召回率和块级检索性能。


<details>
  <summary>Details</summary>
Motivation: 在金融问答系统中，检索增强生成虽然广泛应用，但可靠性依赖于检索到确切的上下文来支持答案。研究发现一个常见的失败模式：虽然检索到了正确的文档，但包含答案的具体页面或块被遗漏，导致生成器基于不完整上下文进行推断。这种文档内检索失败模式在金融QA文献中缺乏系统研究。

Method: 1. 在FinanceBench的150个问题子集上评估多粒度检索（文档、页面、块级）；2. 引入基于oracle的分析来提供检索和生成性能的经验上限；3. 复现和比较多种检索策略（密集、稀疏、混合、分层检索，结合重排序和查询重构）；4. 提出领域微调的页面评分器，将页面作为文档和块之间的中间检索单元，专门针对金融监管文件微调双编码器以评估页面级相关性。

Result: 1. 不同方法的文档发现增益通常转化为更强的页面召回率；2. oracle性能表明页面和块级检索仍有改进空间；3. 提出的领域微调页面评分器显著提升了页面召回率和块级检索性能；4. 利用页面的语义连贯性，在金融文件上专门微调的页面级评分器优于传统的基于段落的分层检索方法。

Conclusion: 该研究通过引入针对金融监管文件领域微调的页面级评分器，有效解决了金融问答中的文档内检索失败问题。这种方法将页面作为中间检索单元，利用页面的语义连贯性，显著提升了页面召回率和块级检索性能，为高风险的金融问答应用提供了更可靠的检索增强生成解决方案。
...
TL;DR: 论文主张AI评估应从静态模型评估转向系统级控制功能，强调传统基准测试方法已不适应工具使用型智能体系统，需要重新定义评估在AI时代的作用。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统从静态模型发展为复合型、使用工具的系统，传统评估方法（静态基准、聚合分数、一次性成功标准）已无法有效评估系统行为。传统方法不仅无法揭示系统行为，反而会引入隐性故障模式，高基准分数经常误导团队。

Method: 本文采用批判性分析方法，而非提出新指标或更难的基准测试。通过审视评估管道如何引入隐性故障模式，分析高基准分数为何误导团队，探讨智能体系统如何根本改变性能测量的意义。

Result: 研究发现传统评估方法在智能体时代已变得模糊而非清晰，需要重新定义评估的作用。评估不应是"性能剧场"，而应成为在非确定性系统中建立信任、迭代和治理的测量学科。

Conclusion: 评估应从模型中心时代的静态检查点转变为AI时代的核心控制功能，特别是在智能体系统中。评估的核心问题是"我们能否信任系统在变化和规模化条件下按预期行为？"，这需要将评估重新定位为测量学科而非性能展示。
...
TL;DR: 研究发现，关于LLM存在党派偏见的警告会显著降低其纠正公众误解的说服效果，使说服力下降28%，表明对话式AI的说服效果受政治因素制约


<details>
  <summary>Details</summary>
Motivation: 对话式AI被提议作为纠正公众误解和传播错误信息的可扩展方式，但其效果可能取决于对其政治中立性的认知。随着LLM进入党派冲突，精英们越来越多地将其描绘为意识形态对齐的工具，需要测试这些可信度攻击是否会降低基于LLM的说服效果。

Method: 采用预注册的美国调查实验（N=2144），参与者与ChatGPT进行三轮对话，讨论个人持有的经济政策误解。实验设置中性对照组和实验组，实验组收到简短信息表明LLM对参与者所属党派存在偏见。通过转录分析评估警告如何改变互动过程。

Result: 与中性对照组相比，表明LLM对参与者党派存在偏见的简短信息使说服效果降低了28%。转录分析表明警告改变了互动：受访者更多地反驳，参与度更低且接受性更差。

Conclusion: 对话式AI的说服效果具有政治依赖性，受党派对齐认知的制约。关于LLM偏见的警告会显著削弱其纠正误解的能力，这突显了在政治极化环境中部署AI系统时需要考虑可信度问题。
...
TL;DR: 提出对抗性问答生成框架，通过对比待适应模型与专家模型的输出，生成紧凑的语义挑战性问题，以更少样本提升LLM在专业领域的适应能力


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在专业领域适应能力不足，现有合成数据方法存在两个关键缺陷：1) 对解释性推理能力支持有限；2) 生成的数据集过大且冗余，样本效率低下
...
Conclusion: 对抗性问答生成框架能有效提升LLM在专业领域的适应效率，通过生成紧凑的语义挑战性问题，解决了现有合成数据方法的冗余问题和推理能力支持不足的缺陷
...
<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法通常依赖于生成过程中的内在信号，其中注意力机制提供了基础行为的直接视图。然而，现有方法通常使用粗粒度的注意力摘要，无法捕捉注意力中的细粒度不稳定性。需要一种更精细的方法来分析注意力变化，以更好地检测幻觉。

Method: 受信号处理启发，作者引入了频率感知的注意力视角，将注意力分布建模为离散信号，并提取反映注意力快速局部变化的高频成分。基于幻觉标记与高频注意力能量相关的发现，开发了一个使用高频注意力特征的轻量级幻觉检测器。

Result: 在RAGTruth和HalluRAG基准测试上的实验表明，该方法在模型和任务上均优于基于验证、内部表示和注意力机制的方法，取得了性能提升。

Conclusion: 注意力频率分析为幻觉检测提供了新的视角，高频注意力能量反映了碎片化和不稳定的基础行为，可用于开发有效的幻觉检测器，提高大语言模型在上下文生成中的可靠性。
...
TL;DR: 论文通过无损压缩分析发现，LLM生成文本比人类文本具有更高的结构规律性和可压缩性，揭示了概率生成的结构特征，但在小尺度碎片化交互环境中这种差异会减弱。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解大语言模型通过概率采样生成文本时，如何重塑语言的结构统计组织。目前这一过程尚未得到充分表征，需要一种模型无关的方法来区分不同生成机制。

Method: 采用无损压缩作为模型无关的统计规律性度量方法，分析三个渐进复杂的信息生态系统：1)受控的人类-LLM延续文本；2)知识基础设施的生成中介（维基百科 vs Grokipedia）；3)完全合成的社交互动环境（Moltbook vs Reddit）。通过压缩行为分析揭示概率生成的结构特征。

Result: 在受控和中介环境中，LLM生成的语言比人类文本表现出更高的结构规律性和可压缩性，表明输出集中在高度循环的统计模式中。但在碎片化互动环境中，这种分离会减弱，表明小尺度表面可区分性存在基本限制。这种基于可压缩性的分离在不同模型、任务和领域中一致出现。

Conclusion: 无损压缩提供了一个简单而稳健的框架，用于量化生成系统如何重塑文本生产，为理解通信的演化复杂性提供了结构视角。该方法可以直接从表面文本观察到生成特征，无需依赖模型内部或语义评估。
...
TL;DR: FENCE是一个用于金融领域多模态越狱检测的双语数据集，包含韩语-英语的金融相关查询和基于图像的威胁，用于训练和评估越狱检测器。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型和视觉语言模型的越狱攻击对部署构成重大风险，特别是VLMs因处理文本和图像而攻击面更广。金融领域缺乏越狱检测资源，需要专门的数据集来应对这一挑战。

Method: 创建FENCE双语多模态数据集，包含金融相关查询与图像威胁配对，强调领域真实性。在商业和开源VLMs上进行实验评估漏洞，并训练基线检测器进行性能验证。

Result: 实验显示VLMs存在一致漏洞：GPT-4o有可测量的攻击成功率，开源模型暴露更大。基于FENCE训练的基线检测器在分布内准确率达99%，在外部基准测试中保持强性能。

Conclusion: FENCE为金融领域多模态越狱检测提供了专门资源，支持敏感领域更安全可靠的AI系统部署，数据集在训练可靠检测模型方面表现出鲁棒性。
...
TL;DR: 提出结合Transformer文本嵌入与语言学信息特征的混合方法检测点击诱饵标题，XGBoost模型在增强特征上达到91% F1分数，优于传统方法
...
Method: 混合方法结合Transformer文本嵌入与15个语言学信息特征，使用XGBoost分类器，对比评估了TF-IDF、Word2Vec、GloVe、LLM提示分类等基线方法

Result: 最佳模型XGBoost在增强特征上达到91% F1分数，显著优于所有基线方法，特征集增强了可解释性，突出显示第二人称代词、最高级、数字和注意力导向标点等语言学线索

Conclusion: 提出的混合方法在点击诱饵检测中表现出色，结合深度学习嵌入与显式语言学特征提高了性能与可解释性，代码和模型已发布支持可重复研究
...
TL;DR: 本文提出Info-Gain Sampler，一种用于掩码扩散模型（MDMs）的改进解码框架，通过平衡即时不确定性和未来信息增益来提升生成质量


<details>
  <summary>Details</summary>
Motivation: 现有MDM采样器采用贪心启发式方法，优先解码局部确定性最高的位置，但忽略了当前解码选择对后续步骤的下游影响，未能最小化累积不确定性。这些方法未能充分利用MDMs的非因果特性，该特性允许评估解码决策如何重塑所有剩余掩码位置的标记概率/不确定性。

Method: 提出Info-Gain Sampler解码框架，在解码每个步骤时不仅考虑当前位置的不确定性，还考虑该解码决策对未来所有掩码位置的信息增益，从而平衡即时不确定性和未来信息增益。

Result: 在推理、编码、创意写作和图像生成等多样化架构和任务上的广泛评估表明，Info-Gain Sampler始终优于现有MDM采样器。在推理任务上平均准确率提升3.6%，创意写作任务上获得63.1%的胜率，推理任务中累积不确定性从78.4降至48.6。

Conclusion: Info-Gain Sampler通过考虑解码决策的下游影响，有效利用MDMs的非因果特性，显著提升了生成质量，为MDMs提供了一种原则性的解码框架。
...
<details>
  <summary>Details</summary>
Motivation: 实时句子理解对工作记忆产生显著负荷，但现有测量方法主要基于符号语法，为句法预测分配离散、统一的成本。需要一种连续、理论中立且能从预训练模型估计的处理存储成本度量。

Method: 提出基于信息论的处理存储成本度量，定义为先前词汇在不确定性条件下对未来上下文所携带的信息量。该方法连续、理论中立，可从预训练神经语言模型估计。通过三项英语分析验证：恢复中心嵌入和关系从句的处理不对称性；与语法库中语法基存储成本相关；预测自然数据集阅读时间方差。

Result: 该度量成功恢复了中心嵌入和关系从句的已知处理不对称性；与语法基存储成本在句法标注语料库中显著相关；在两个大规模自然数据集中，超越传统信息基预测器的基线模型，预测了阅读时间方差。

Conclusion: 基于信息论的处理存储成本度量提供了一种连续、理论中立的替代方案，能够从预训练语言模型估计，有效预测实时句子理解中的处理负荷，为心理语言学理论提供了新的形式化工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法假设均匀分配推理计算能提升正确性，但研究表明推理不确定性高度局部化：少数低置信度token不成比例地导致推理错误和不必要的输出扩展

Method: 提出Confidence-Driven Contrastive Decoding方法：检测解码过程中的低置信度token，选择性干预这些位置；构建对比参考分布（将高置信度token替换为最小占位符），在低置信度位置通过减去参考分布来精炼预测

Result: CCD显著提升数学推理基准测试的准确性，同时大幅减少输出长度，KV-cache开销最小；作为无需训练的方法，通过针对性低置信度干预提升推理可靠性，避免计算冗余

Conclusion: 通过针对低置信度token的对比解码方法，能够在不增加计算冗余的情况下提升大语言模型推理的可靠性，同时减少输出长度，为测试时推理优化提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 机制可解释性分析虽然开发了强大的工具来分析大型语言模型的内部工作原理，但其复杂性造成了可访问性差距，限制了只有专家才能使用这些工具。

Method: 设计、构建和评估ELIA交互式Web应用，整合归因分析、函数向量分析和电路追踪三种关键技术，并引入新方法：使用视觉语言模型自动为这些方法产生的复杂可视化生成自然语言解释。

Result: 通过混合方法用户研究验证了该方法的有效性，发现用户明显偏好交互式、可探索的界面而非简单的静态可视化。关键发现是AI生成的解释帮助非专家弥合了知识差距；统计分析显示用户的先前LLM经验与其理解分数之间没有显著相关性，表明该系统减少了不同经验水平用户的认知障碍。
...
TL;DR: 创建了首个罗马尼亚语抑郁和焦虑语料库PsihoRo，包含205名受访者的开放式问题回答及标准化心理筛查问卷数据


<details>
  <summary>Details</summary>
Motivation: 罗马尼亚语目前缺乏开源的心理健康语料库，而现有心理健康数据收集方法存在假设偏差问题，需要更实用的数据收集策略

Method: 使用包含6个开放式问题的表格结合标准化PHQ-9和GAD-7筛查问卷收集数据，通过统计分析、罗马尼亚语LIWC文本分析、情感检测和主题建模进行特征分析

Result: 成功创建了包含205名受访者文本的PsihoRo语料库，展示了该资源在理解罗马尼亚人口心理健康文本方面的潜力

Conclusion: PsihoRo是分析和理解罗马尼亚人口心理健康文本的重要第一步，为NLP社区提供了首个罗马尼亚语心理健康资源
...
TL;DR: Vichara是一个针对印度司法系统的上诉判决预测框架，通过分解案件文档为决策点，结合IRAC框架提供可解释的预测，在多个数据集上超越现有基准。


<details>
  <summary>Details</summary>
Motivation: 印度法院面临大量案件积压，特别是上诉案件。人工智能在法律判决预测方面具有变革潜力，但需要专门针对印度司法系统的解决方案，既能准确预测又能提供可解释的推理。

Method: Vichara框架处理英文上诉案件文档，将其分解为决策点（包含法律问题、裁决机构、结果、推理和时间背景的结构化表示）。采用受IRAC框架启发并适应印度法律推理的结构化解释格式。使用GPT-4o mini、Llama-3.1-8B、Mistral-7B和Qwen2.5-7B四种大语言模型进行评估。

Result: 在PredEx和ILDC_expert两个数据集上，Vichara超越了现有判决预测基准。GPT-4o mini表现最佳（PredEx F1: 81.5，ILDC_expert F1: 80.3），其次是Llama-3.1-8B。人类评估显示GPT-4o mini在清晰度、关联性和实用性方面具有优越的可解释性。

Conclusion: Vichara框架成功解决了印度上诉案件判决预测问题，通过结构化决策点表示和IRAC启发式解释，实现了准确预测和可解释推理，为缓解司法积压提供了有效工具。
...
TL;DR: 提出双尺度验证框架用于政治立场预测，结合点式和成对人工标注，构建大规模政治立场知识库，证明序数结构可从主观连续知识中提取
...
Method: 采用双尺度验证框架，结合点式和成对人工标注；使用22个语言模型构建大规模政治立场预测知识库，包含23,228个论点，来自30场英国政治电视节目辩论

Result: 点式评估显示中等的人机一致性（Krippendorff's α=0.578），反映内在主观性；成对验证显示人机排名对齐更强（最佳模型α=0.86）

Conclusion: 贡献包括：实用的主观连续知识验证方法；已验证的结构化论证知识库；证明可从点式语言模型预测中提取序数结构，推进传统符号或分类方法不足领域的知识表示能力
...
TL;DR: VIRAASAT：针对印度文化的半自动化多跳问答数据集生成方法，包含3200+多跳问题，并提出符号链式操作（SCoM）框架提升LLMs的文化推理能力


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在需要丰富社会文化知识和多样本地背景的任务中表现不佳，特别是在印度文化相关任务上。现有文化基准存在三个主要问题：手动构建、仅包含测试事实记忆的单跳问题、扩展成本过高，导致这一缺陷未被充分衡量

Method: 提出VIRAASAT半自动化多跳方法：1) 构建包含700+专家策划文化实体的知识图谱，覆盖印度文化13个关键属性；2) 涵盖印度所有28个邦和8个中央直辖区；3) 生成3200+多跳问题。同时提出符号链式操作（SCoM）框架，训练模型内部模拟原子知识图谱操作，可靠遍历图谱拓扑结构

Result: 评估当前SOTA LLMs在VIRAASAT上的表现，发现关键推理限制：基于思维链（CoT）的微调无法有效处理低概率事实的定位和综合。实验表明，SCoM在监督微调（SFT）中比标准CoT基线性能提升高达20%

Conclusion: VIRAASAT为构建文化感知推理模型奠定了坚实基础。提出的SCoM框架通过教导模型可靠遍历知识图谱拓扑结构，显著提升了LLMs在文化推理任务上的表现，为解决LLMs在文化相关任务中的局限性提供了有效方法
