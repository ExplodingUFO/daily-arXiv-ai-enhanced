...
TL;DR: 提出SKETCHGAIT框架，将边缘检测生成的草图作为步态识别新视觉模态，与解析模态互补，实现层次解耦多模态学习


<details>
  <summary>Details</summary>
Motivation: 现有步态识别研究主要基于轮廓和解析表示，轮廓稀疏缺乏内部结构细节，解析依赖上游人体解析器导致性能不稳定。需要探索新的结构表示方法

Method: 从结构视角重新审视步态表示，提出草图作为新视觉模态，通过边缘检测器从RGB图像提取高频结构线索。提出SKETCHGAIT框架，包含两个独立模态特定学习流和一个轻量级早期融合分支

Result: 在SUSTech1K数据集上达到92.9% Rank-1准确率，在CCPG数据集上达到93.1%平均Rank-1准确率，验证了草图模态和框架的有效性

Conclusion: 草图作为无标签的密集部分级结构表示，与标签引导的解析表示在语义上解耦、结构上互补，为步态识别提供了新的有效视觉模态
...
TL;DR: DreamCAD：无需CAD标注的多模态生成框架，通过点云监督直接生成可编辑BRep，并发布最大CAD标注数据集CADCap-1M


<details>
  <summary>Details</summary>
Motivation: 现有CAD生成方法受限于小规模标注数据集（需显式设计历史或BRep标签），而数百万未标注3D网格未被充分利用，限制了可扩展CAD生成的发展

Method: 将BRep表示为参数化曲面片集合，使用可微分细分方法生成网格，实现大规模3D数据集训练同时重建连接且可编辑的表面；引入GPT-5生成的CADCap-1M数据集

Result: 在ABC和Objaverse基准测试中，DreamCAD在文本、图像和点云模态上均达到最先进性能，提升几何保真度并获得超过75%的用户偏好

Conclusion: DreamCAD通过点级监督实现可编辑BRep生成，无需CAD特定标注，解决了数据稀缺问题，推动了可扩展CAD生成的发展
...
TL;DR: ABRA通过对抗性批量表示增强解决细胞成像中的生物批次效应，将批次缓解视为领域泛化问题，在RxRx1基准上实现最先进的siRNA扰动分类性能。
...
Method: 提出对抗性批量表示增强(ABRA)，将批次缓解视为领域泛化问题。通过参数化特征统计量为结构化不确定性来显式建模批次统计波动，采用min-max优化框架在表示空间中主动合成最坏情况的生物批次扰动，并通过严格的角几何边界保持细粒度类别可区分性。为防止对抗探索中的表示崩溃，引入协同分布对齐目标。

Result: 在RxRx1和RxRx1-WILDS基准上进行广泛评估，ABRA在siRNA扰动分类任务上建立了新的最先进性能。

Conclusion: ABRA通过对抗性批量表示增强有效缓解生物批次效应，将批次统计波动建模为结构化不确定性，在保持类别区分性的同时提高模型对未见批次的泛化能力。
...
Method: PFS在现有检测器的中间BEV表示上操作，生成用于原始检测头的精炼特征图。通过稳定域偏移下的特征统计、抑制受传感器退化影响的区域、通过残差校正自适应恢复弱化线索，设计为近似恒等变换。

Result: 在nuScenes基准测试中，PFS在多种故障模式下达到最先进结果：相机丢失鲁棒性提升+1.2%，低光性能提升+4.4% mAP，同时保持仅3.3M参数的轻量级占用。

Conclusion: PFS作为一种轻量级后处理模块，能够有效提升现有BEV融合检测器的鲁棒性，无需架构修改或重新训练，具有实际部署价值。
...
TL;DR: 提出iFID（插值FID），一种改进的rFID变体，能强相关于扩散模型的生成FID（gFID），解决了rFID与gFID相关性差的问题。
...
Method: 提出iFID：对数据集每个元素，在潜在空间中找到其最近邻，插值它们的潜在表示，解码插值后的潜在向量，计算解码样本与原始数据集之间的FID。

Result: iFID与扩散gFID呈现强相关性（Pearson线性相关约0.85，Spearman秩相关约0.85），是首个展示与扩散gFID强相关的指标。同时澄清rFID与扩散细化阶段样本质量相关，而iFID与扩散导航阶段相关。

Conclusion: iFID是一种简单有效的评估指标，能准确预测潜在扩散模型的生成质量，为扩散模型的评估提供了新视角，并通过扩散泛化和幻觉理论解释了其有效性。
...
TL;DR: 提出Implicit Error Counting (IEC)方法，用于解决缺乏理想参考答案的多输出任务强化学习问题，通过隐式错误计数而非基于参考的评分来提供可靠奖励信号。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR和RaR方法依赖理想参考答案生成评分标准，但许多现实任务允许多种有效输出且缺乏单一理想答案，形成当前后训练方法的空白。

Method: 提出IEC方法：不检查回答正确部分，而是枚举错误部分，应用严重性加权评分，通过隐式分数发射和组校准转换为校准的每方面奖励。针对虚拟试衣(VTO)任务，引入Cascaded Error Counting (CEC)评估指标和Mismatch-DressCode (MDressBench)基准。

Result: CEC指标与人类偏好高度一致(60% top-1 vs. 30%其他)。在MDressBench上，IEC在CEC指标上优于RaR(5.31 vs. 5.60平坦参考；5.20 vs. 5.53非平坦)。在VITON-HD和DressCode上，IEC在8个感知指标中的6个上匹配或超越6个基线方法。

Conclusion: 当理想答案不可用时，计数错误比构建评分标准提供更强的信号，IEC填补了无参考设置下的强化学习奖励设计空白。
...
TL;DR: SemVID：一种无需训练的视频时序定位剪枝框架，通过保留查询关键证据和维持跨帧连接性，在仅使用12.5%视觉token的情况下保持95.4% mIoU精度，实现5.8倍预填充加速。


<details>
  <summary>Details</summary>
Motivation: 视频时序定位（VTG）需要在长视频中定位查询相关时刻，使用视频语言模型（VLM）计算成本极高。现有视觉token剪枝方法直接应用于VTG会导致性能严重下降，因为VTG依赖边界敏感证据和跨帧推理链。

Method: 提出SemVID训练免费剪枝框架，基于两个VTG特定剪枝原则：证据保留（ER）和连接强度（CS）。框架首先通过平衡查询相关性和帧间变化分配每帧token预算，然后选择三种token类型：对象token（查询关键证据）、运动token（捕捉过渡并作为跨帧中继）和少量上下文token（保持场景连续性）。

Result: 在VTG基准测试中，SemVID在仅使用12.5%视觉token的情况下保持95.4% mIoU精度，实现5.8倍预填充加速，在相同预算下持续优于先前方法。
...
TL;DR: OWL是一种基于视觉运动线索的感知函数，通过两种基本视觉运动线索（局部视觉逼近和刚性物体旋转）直接计算值，无需显式测量或先验知识，实现3D映射和相机航向估计。


<details>
  <summary>Details</summary>
Motivation: 解决运动过程中3D感知的复杂挑战，弥合理论概念与机器人、自主导航实际应用之间的差距，为下一代自主系统提供实时决策和交互的基础构建模块。

Method: 基于两种基本视觉运动线索：1) 固定点附近点的感知局部视觉逼近，2) 刚性物体相对于固定点的感知旋转。直接从原始视觉运动图像序列测量，无需先验知识，采用极简的基于像素的并行计算。

Result: 仿真表明OWL实现了3D对象随时间的几何恒常性，仅凭视觉运动线索就能实现缩放3D场景重建，提供了一种替代的实时3D点相对运动表示方法。

Conclusion: OWL为机器感知提供了替代视角，其影响可能扩展到自然感知，有助于更好地理解行为心理学和神经功能，可作为下一代自主系统的构建模块。
...
TL;DR: MultiHaystack是首个评估大规模跨模态检索与推理的基准，包含46,000+多模态检索候选项和747个可验证问题，揭示现有模型在检索证据后性能显著下降，凸显多模态检索是MLLMs的主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM基准主要评估单一模态理解，检索任务局限于小型单模态候选集，简化了搜索空间并高估了端到端可靠性。缺乏评估大规模异构多模态语料库中证据检索与推理能力的基准。

Method: 构建MultiHaystack基准：包含超过46,000个跨文档、图像和视频的多模态检索候选项，以及747个开放但可验证的问题。每个问题都基于检索池中唯一的验证证据项，需要跨模态证据定位和细粒度推理。

Result: 模型在提供对应证据时表现良好，但从完整语料库检索证据时性能急剧下降。最强检索器E5-V仅达到40.8%的Recall@1，GPT-5等SOTA MLLMs在提供证据时推理准确率为80.86%，但在top-5检索下降至51.4%。

Conclusion: 异构多模态检索是MLLMs的主要瓶颈。MultiHaystack作为有价值的测试平台，揭示了小规模评估所掩盖的底层限制，促进了多模态系统中检索为中心的进展。
...
TL;DR: 本文提出了Audiovisual Geolocation框架，通过视听融合解决地理定位问题，包含AVG基准数据集和三阶段框架（感知、多模态推理、精确预测），显著优于单模态基线。
...
Method: 提出三阶段框架：1) 感知阶段使用混合自回归稀疏自编码器将噪声音频分解为语义基础的"声学原子"；2) 多模态推理阶段通过Group Relative Policy Optimization微调的MLLM融合声学原子与视觉特征；3) 精确预测阶段在S^2流形上使用黎曼流匹配进行预测。

Result: 实验表明该框架显著优于单模态基线，证明可解释的声音景观感知提供了关键的正交信号，结合多模态推理可实现高精度全球定位。

Conclusion: 声音景观的可解释感知为地理定位提供了关键补充信号，与多模态推理结合能够实现高精度全球定位，AVG基准数据集为未来研究提供了高质量资源。
...
TL;DR: Any2Full是一个单阶段、领域通用、模式无关的深度补全框架，通过尺度提示适配预训练单目深度估计模型，实现高效鲁棒的深度补全。
...
Method: 将深度补全重新定义为预训练单目深度估计模型的尺度提示适配。设计尺度感知提示编码器，从稀疏输入中提取尺度线索生成统一尺度提示，引导模型产生全局尺度一致的预测同时保持其几何先验。

Result: 在广泛实验中，Any2Full展现出优越的鲁棒性和效率：比OMNI-DC在平均AbsREL指标上提升32.2%，比PriorDA在相同MDE骨干下实现1.4倍加速，建立了通用深度补全的新范式。

Conclusion: Any2Full通过尺度提示适配预训练MDE模型，实现了单阶段、领域通用、模式无关的深度补全，在精度和效率上均超越现有方法，为通用深度补全提供了新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: ImageNet基准采用单标签假设，但现实图像通常包含多个物体，导致标签噪声和学习信号受限。多标签标注能更准确反映真实视觉场景，但现有工作主要改进验证集，缺乏可扩展的高质量训练集多标签标注。

Method: 使用自监督视觉Transformer进行无监督物体发现，选择与原标签对齐的区域训练轻量级分类器，然后应用于所有区域生成整个数据集的一致多标签标注，形成全自动标注流程。

Result: 生成的多标签标注与人类判断高度一致。相比传统单标签方案，使用多标签监督训练的模型在多个架构上获得更好的域内准确率（ReaL上最高+2.0 top-1，ImageNet-V2上+1.5），在下游任务上表现出更强的迁移能力（COCO上+4.2 mAP，VOC上+2.3 mAP）。

Conclusion: 准确的多标签标注对于提升分类性能和表示学习至关重要。自动化多标签标注流程能有效增强模型性能，为视觉表示学习提供更丰富的监督信号。项目代码和生成的多标签标注已开源。
...
Method: 采用基于CLIP的多模态框架，将手术视频帧与文本手势描述对齐。使用CLIP视觉编码器提取视频帧表示，文本编码器嵌入手势句子到共享嵌入空间，通过微调改进视频手势与文本标记的对齐。训练后模型预测视频帧的手势和阶段，构建结构化手术时间线。

Result: 该方法能够自动预测手术视频帧的手势和阶段，从而构建结构化手术时间线，减少外科医生手动视频审查和标注的需求。

Conclusion: 提出的多模态框架成功利用预训练表示桥接视觉手势和文本叙述，为自动创建手术时间线提供有效解决方案，平衡了术后报告的模糊性和手动标注的耗时性。
...
TL;DR: Icarus是一个全天气天空模型，能够学习全动态范围物理捕获户外图像，通过条件生成环境贴图，实现前所未有的准确性和真实感


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的天空模型在重建高分辨率自然天空时存在不足，特别是在14EV+类别不平衡的太阳区域，导致视觉质量差、光照传输、阴影和色调失真

Method: 提出Icarus全天气天空模型，能够学习全动态范围物理捕获户外图像，支持条件生成环境贴图，允许用户直观定位太阳和云层形态，并扩展了用户控制的大气形态纹理

Result: Icarus可与全动态范围物理捕获户外图像或参数化天空模型互换，在基于图像的照明中实现了前所未有的准确性、真实感、光照方向性（阴影）和色调

Conclusion: Icarus解决了现有天空模型在高分辨率太阳区域建模的局限性，为户外场景建模提供了更准确、真实的环境贴图生成方案
...
<details>
  <summary>Details</summary>
Motivation: 现有区域布局控制方法存在两个主要问题：基于训练的方法继承数据偏差并降低图像质量；现有技术难以处理遮挡顺序，限制了实际应用。作者观察到空间布局和遮挡在去噪早期阶段就已确定，因此重新排列早期潜在结构足以修改最终输出。

Method: LayerBind采用两阶段方案：1）实例初始化阶段：利用多模态联合注意力中的上下文共享机制，创建每个实例的分支，在指定早期步骤根据层顺序融合形成具有预设布局的统一潜在表示；2）语义增强阶段：通过分层注意力增强强化区域细节并保持遮挡顺序，使用顺序分层注意力路径与标准全局路径并行工作，更新在层透明度调度器下组合。

Result: LayerBind在定性和定量评估中均表现出有效性，能够实现精确的区域控制和遮挡顺序管理，支持可编辑工作流程，如更改实例或重新排列可见顺序，展现了在创意应用中的强大潜力。

Conclusion: LayerBind是一种无需训练、即插即用的区域和遮挡控制器，适用于扩散变换器，通过分层建模方法解决了现有区域布局控制方法的局限性，为文本到图像生成提供了更实用的区域控制能力。
...
<details>
  <summary>Details</summary>
Motivation: 稠密图像检索虽然准确，但可解释性和归因性有限，且在大规模计算上成本较高。需要一种既能保持高召回率又具有可解释性和计算效率的检索方法。

Method: 1. 使用稀疏自编码器（SAE）在Vision Transformer的patch特征上提取稀疏视觉词激活；2. 应用Okapi BM25评分对视觉词进行检索，利用IDF权重抑制常见低信息词，强调稀有判别性词；3. 通过稀疏倒排索引操作实现高效检索，作为稠密重排序的第一阶段检索器。

Result: 在七个基准测试中，BM25-V的Recall@200 ≥ 0.993，仅需重排序K=200个候选即可恢复接近稠密检索的精度（平均差距0.2%）。在ImageNet-1K上训练一次的SAE可零样本迁移到七个细粒度基准，检索决策可归因于特定视觉词并量化IDF贡献。

Conclusion: BM25-V提供了一种高效、可解释的图像检索方法，结合了稀疏检索的计算效率和稠密检索的高精度，通过两阶段管道实现了接近稠密检索的性能，同时保持了可归因性和计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机因其稀疏激活和高时间分辨率在机器人视觉中日益重要，但需要更准确、实时的地点识别方法
...
Conclusion: EventGeM为事件相机提供了一种高效、准确的全局-局部特征融合视觉地点识别方法，适用于实时机器人导航和定位任务
...
Method: 提出LIPAR框架：1）检测并跳过重复的潜在补丁重新计算；2）引入注意力恢复机制，近似被剪枝token的注意力值，避免视觉伪影。无需额外训练即可与模型集成。

Result: 视频编辑吞吐量平均提升1.45倍，在NVIDIA A6000上达到12.2 FPS（基线为8.4 FPS）。生成质量未受影响，方法可无缝集成到现有模型中。

Conclusion: LIPAR框架有效利用视频潜在补丁的时间冗余性，在保持生成质量的同时显著提升计算效率，填补了传统压缩算法与现代生成管道之间的空白。
...
TL;DR: Cog2Gen3D：一个3D认知引导的扩散框架，通过语义-几何双流图融合实现可控的3D生成，解决了现有方法在绝对几何尺度不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法主要依赖几何特征作为条件，但只能建模相对关系，容易产生绝对几何尺度不一致的问题。作者认为语义信息和绝对几何共同赋能3D认知，从而能够实现物理世界中可控的3D生成。

Method: 提出Cog2Gen3D框架，包含三个关键设计：1）认知特征嵌入：将多模态信息编码为语义、几何和逻辑表示；2）3D潜在认知图：构建语义-几何双流图并通过共同基交叉注意力融合；3）认知引导的潜在扩散：使用融合后的3D认知图作为条件，引导3D高斯生成的潜在扩散过程。

Result: 在基于Marble World Labs构建的验证子集上进行广泛实验，结果表明Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法。

Conclusion: Cog2Gen3D通过统一的3D认知图框架确保了3D生成的物理合理性和结构理性，为可控的3D生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频稳定面临几何鲁棒性与全帧一致性的根本权衡：2D方法存在过度裁剪问题，而3D方法在极端运动下优化流程脆弱易失效。

Method: VS3R框架协同前馈3D重建与生成视频扩散，联合估计相机参数、深度和掩码以确保全场景可靠性；引入混合稳定渲染模块融合语义和几何线索实现动态一致性；使用双流视频扩散模型通过结构引导与语义锚点协同恢复遮挡区域并修正伪影。

Result: VS3R在不同相机模型上实现了高保真、全帧稳定，在鲁棒性和视觉质量方面显著优于最先进方法。

Conclusion: VS3R通过协同3D重建与生成视频扩散，成功解决了视频稳定中的几何鲁棒性与全帧一致性权衡问题，实现了全场景可靠的高质量稳定效果。
...
TL;DR: 提出TumorChain框架，通过多模态交错推理结合3D CT扫描和临床文本，实现从影像发现到病理预测的肿瘤分析，在1.5M数据集上验证了性能提升。


<details>
  <summary>Details</summary>
Motivation: 肿瘤分析在临床放射学和精准肿瘤学中至关重要，需要从影像发现到临床印象再到病理结论的逐步推理。现有方法缺乏可追溯性且容易产生诊断错误，因此需要开发能够模拟临床医生推理过程的多模态推理系统。

Method: 构建TumorCoT数据集（150万条带CoT标注的VQA指令配3D CT扫描），提出TumorChain多模态交错推理框架，结合3D影像编码器、临床文本理解和器官级视觉语言对齐，通过跨模态对齐和迭代交错因果推理进行证据定位、结论聚合和病理预测。

Result: 在病灶检测、印象生成和病理分类任务上相比强基线模型取得一致改进，在DeepTumorVQA基准测试上表现出强泛化能力，提高了可追溯性并减少了幻觉风险。

Conclusion: 多模态推理框架能够实现可靠且可解释的肿瘤分析，为临床实践提供了有前景的解决方案，通过模拟临床医生的思维链推理过程提高了诊断准确性和可追溯性。
...
TL;DR: PatchCue：一种基于图像块的视觉提示范式，通过将图像划分为块并在块级别表示视觉提示，显著提升视觉语言模型的视觉推理能力，优于像素级边界框和点级提示。
...
Method: 提出PatchCue范式：将图像划分为块并在块级别表示视觉提示。采用两阶段训练方法：1）冷启动监督微调，使模型输出块级提示；2）强化学习，使用过程监督的提示奖励来引导中间视觉推理步骤。
...
TL;DR: 论文系统评估了合成新视角在视频地点识别中的应用，发现小规模合成视角能提升识别性能，而大规模添加时，视角变化幅度不如添加数量和数据集图像类型重要。


<details>
  <summary>Details</summary>
Motivation: 合成新视角生成在机器人导航中有多种潜在应用价值：在基于图像的导航中，地面机器人拍摄的场景可生成新颖的俯视视角来引导空中机器人；在视频地点识别中，空中视角的地面场景新视角可帮助无人机识别地面机器人见过的地点，反之亦然。

Method: 使用五个公开的VPR图像数据库和七种典型的图像相似度方法，对合成新视角在VPR中的效果进行系统评估。

Result: 对于小规模的合成添加，新视角能改善VPR识别统计性能；对于大规模添加，视角变化的幅度不如添加的视角数量和数据集中的图像类型重要。

Conclusion: 合成新视角在视频地点识别中具有实用价值，特别是在小规模添加时能提升性能，而大规模应用时需更关注视角数量和图像类型而非视角变化幅度。
...
TL;DR: CylinderSplat：一种用于全景3D高斯溅射的前馈框架，通过圆柱形Triplane表示和双分支架构解决稀疏视图下的遮挡问题，在单视图和多视图全景新视角合成中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D高斯溅射方法在全景图像应用中面临挑战：多视图成本体积在稀疏视图场景下难以解决遮挡问题；标准笛卡尔Triplane等体积表示无法有效捕捉360°场景的固有几何结构，导致失真和混叠。
...
Conclusion: CylinderSplat通过创新的圆柱形Triplane表示和双分支架构，有效解决了全景3D高斯溅射中的几何表示和遮挡问题，为稀疏视图场景下的高质量全景新视角合成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖隐式有符号距离场和事后布局优化，无法直接从单张图像生成完整、连贯的3D场景网格。需要一种能够联合预测布局和几何形状的统一模型，生成可直接用于下游应用的高质量网格。
...
Conclusion: PixARMesh通过统一的自回归模型直接从单张RGB图像重建完整的3D室内场景网格，克服了现有方法的局限性，为3D场景重建提供了高效、高质量的解决方案。
...
TL;DR: InnoAds-Composer：单阶段电商产品海报生成框架，通过重要性分析实现主题、字形和风格的三条件高效控制，解决多阶段管道中的保真度、准确性和一致性问题


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的产品海报生成方法通常采用多阶段管道，难以同时控制主题、文本和风格，存在主题保真度差、文本渲染不准确、风格不一致等问题

Method: 提出单阶段框架InnoAds-Composer，通过层和时间步的重要性分析，将每个条件仅路由到最敏感的位置，缩短活动token序列；设计文本特征增强模块(TFEM)结合字形图像和字形裁剪特征提升中文文本渲染准确性；构建首个同时包含主题、文本和风格条件的高质量电商产品海报数据集

Result: 实验表明InnoAds-Composer在显著优于现有产品海报生成方法的同时，没有明显增加推理延迟，在主题保真度、文本准确性和风格一致性方面表现优异

Conclusion: InnoAds-Composer通过高效的三条件控制机制解决了电商产品海报生成中的关键问题，为单阶段多条件图像合成提供了有效解决方案，构建的数据集和基准为后续研究提供了重要资源
...
TL;DR: 提出CollabOD轻量级协作检测框架，解决无人机图像中小目标检测的尺度变化、结构细节退化和计算资源有限等挑战，通过结构细节保留、跨路径特征对齐和定位感知轻量化设计提升检测性能。
...
Method: 提出CollabOD框架，集成三种策略：1）结构细节保留；2）跨路径特征对齐（在跨尺度融合前对齐异构特征流）；3）定位感知轻量化设计。从图像处理、通道结构和轻量化设计三个角度优化传统无人机感知模型架构，并引入统一的细节感知检测头提升回归鲁棒性。

Result: 该设计在保持高效推理的同时增强了表示稳定性，统一的细节感知检测头在不增加额外部署开销的情况下进一步提高了回归鲁棒性。代码已开源。

Conclusion: CollabOD通过协作式设计有效解决了无人机图像小目标检测的关键挑战，在结构细节保留、特征对齐和轻量化方面进行了系统优化，为资源受限的无人机平台提供了实用的检测解决方案。
...
TL;DR: 提出Art3D框架，从几何精确的视差估计转向艺术连贯的视差合成，通过双路径架构分离全局深度参数与局部艺术效果，学习专业3D电影数据，初步实现电影级3D转换。


<details>
  <summary>Details</summary>
Motivation: 现有2D转3D方法虽几何准确但艺术性不足，无法复制专业3D电影的沉浸感和情感共鸣体验。几何重建范式将艺术意图（如零平面偏移、局部深度塑造）误认为数据噪声或模糊性。

Method: 提出艺术视差合成新范式，开发Art3D框架：1) 双路径架构解耦全局深度参数（宏观意图）与局部艺术效果（视觉笔触）；2) 通过间接监督从专业3D电影数据学习；3) 引入初步的电影对齐评估方法。

Result: 实验表明Art3D在复制关键局部出屏效果和与电影3D内容的全局深度风格对齐方面展现出潜力，为艺术驱动转换工具的新类别奠定基础。

Conclusion: 需要从物理精确的视差估计转向艺术连贯的视差合成的新范式，Art3D作为初步框架展示了这一方向的可行性，为未来艺术驱动的3D转换工具开发提供了基础。
...
TL;DR: Pano3DComposer：一种高效的端到端全景图像到3D场景生成框架，通过解耦对象生成与布局估计，在20秒内生成高保真3D场景


<details>
  <summary>Details</summary>
Motivation: 当前组合式图像到3D场景生成方法存在两个主要问题：1）依赖耗时的迭代布局优化或僵化的联合对象-布局生成；2）大多数方法基于有限视场角度的透视图像，难以创建完整的360度环境。需要一种高效、灵活且能处理全景图像的方法。

Method: 提出Pano3DComposer框架，核心创新包括：1）可插拔的对象-世界坐标转换预测器，将现成图像到3D模型生成的3D对象从局部坐标转换到世界坐标；2）基于VGGT架构改进的Alignment-VGGT，利用目标对象裁剪、多视角对象渲染和相机参数预测变换；3）使用伪几何监督训练以解决生成对象与真实对象之间的形状差异；4）针对未见域输入图像，引入从粗到精对齐机制，通过场景渲染反馈迭代优化几何一致性。

Result: 在合成和真实世界数据集上的图像/文本到3D任务中，该方法实现了优越的几何精度。在RTX 4090 GPU上，约20秒即可生成高保真3D场景。

Conclusion: Pano3DComposer通过解耦对象生成与布局估计，结合高效的坐标转换预测器和从粗到精对齐机制，实现了快速、高质量的全景图像到3D场景生成，解决了现有方法在效率和灵活性方面的限制。
...
Method: 提出CORE-Seg端到端框架，集成推理与分割，通过语义引导提示适配器实现。采用从SFT到GRPO的渐进训练策略，配备自适应双粒度奖励机制以缓解奖励稀疏问题

Result: 在ComLesion-14K基准上达到37.06%的平均Dice系数（比第二好基线高14.89%），同时将失败率降低至18.42%

Conclusion: CORE-Seg框架成功将推理能力与分割任务结合，在复杂病灶分割中实现了显著性能提升，为医学图像分析提供了新的认知推理范式
...
TL;DR: BlackMirror：一种黑盒环境下检测文本到图像模型后门攻击的新框架，通过分析生成图像中部分语义模式的稳定性来识别后门行为。


<details>
  <summary>Details</summary>
Motivation: 现有基于图像相似度的后门检测方法假设后门触发的生成图像具有强一致性，但难以应对新兴的后门攻击，这些攻击生成的图像在视觉上可能呈现多样性。研究发现，在后门攻击中，只有生成图像的部分语义模式被稳定操纵，而其余内容保持多样或良性。
...
Conclusion: BlackMirror通过分析部分语义模式的稳定性，有效解决了黑盒环境下文本到图像模型后门检测的挑战，特别是针对生成图像视觉多样性的新型后门攻击。
...
<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重构-生成差距、计算成本高、参数冗余等问题。作者旨在设计一种更高效的自编码器架构，能够同时改善重构质量和生成能力，并显著降低计算开销。

Method: RAC基于Rectified Flow设计：1）通过将解码器应用于流时间步实现多步解码，解码路径直且可校正；2）支持双向推理，解码器通过时间反转作为编码器使用，减少参数；3）采用生成式解码方法，允许沿路径校正潜在变量。

Result: 实验表明RAC在重构和生成任务上均超越SOTA VAE方法，同时计算成本降低约70%，参数数量减少近41%。

Conclusion: RAC通过Rectified Flow框架成功替代传统VAE，实现了更好的重构-生成性能平衡，显著降低了计算和参数开销，为高效自编码器设计提供了新思路。
...
TL;DR: RAID数据集：用于驾驶员风险感知研究的大规模视频数据集，包含4691个标注视频片段，涵盖多种交通场景，并提出弱监督风险对象识别框架。


<details>
  <summary>Details</summary>
Motivation: 实现零碰撞智能车辆系统需要理解驾驶员的风险感知，这是一个复杂的认知过程，涉及驾驶员对外部刺激的自愿反应以及周围道路使用者对自车的关注度。目前缺乏专门用于驾驶员风险感知研究的大规模数据集。

Method: 1. 引入RAID数据集：包含4,691个标注视频片段，涵盖驾驶员意图操作、道路拓扑、风险情境（如横穿行人）、驾驶员反应和行人注意力等标签。2. 提出弱监督风险对象识别框架：通过建模驾驶员意图操作与反应之间的关系来识别潜在风险源。3. 分析行人注意力在风险评估中的作用。

Result: 实验评估表明，该方法在RAID和HDDS数据集上分别比现有最先进方法提升了20.6%和23.1%的性能。RAID数据集为驾驶员风险感知研究提供了有价值的资源。

Conclusion: RAID数据集填补了驾驶员风险感知研究领域的数据空白，提出的弱监督风险对象识别框架有效提升了风险识别性能，行人注意力分析为风险评估提供了新的视角，有助于推动零碰撞智能车辆系统的发展。
...
Conclusion: TAR-ViTPose通过有效的时序信息聚合和恢复机制，显著提升了视频姿态估计的性能和鲁棒性，同时保持了较高的计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有NeRF和3DGS方法依赖耗时的逐场景优化，限制了实时部署；而前馈高斯泼溅方法缺乏显式流形几何，无法直接用于仿真。需要一种能高效生成仿真就绪几何模型的方法。

Method: 提出前馈三角形基元生成框架，包含像素对齐的三角形生成模块，并引入相对3D点云监督来增强几何学习的稳定性和一致性，直接从标定多视角图像预测连续三角形表面。

Result: 方法实现了高效重建，同时保持与标准图形和机器人仿真器的无缝兼容性，在单次前向传播中生成仿真就绪模型。

Conclusion: 该方法解决了现有神经渲染方法在实时部署和仿真兼容性方面的限制，为机器人和仿真应用提供了高效、几何明确的3D重建解决方案。
...
TL;DR: 提出OD-RASE框架，通过检测导致交通事故的道路结构并将其与基础设施开发连接，提升自动驾驶系统安全性


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在处理罕见情况或复杂道路结构时仍有局限，道路基础设施主要为人类驾驶员设计，安全改进通常只在事故发生后进行，这种被动方法对需要主动风险缓解的自动驾驶系统构成挑战

Method: 基于道路交通系统专业知识形式化本体论；使用大规模视觉语言模型生成基础设施改进建议，并通过本体驱动数据过滤提高可靠性；在事故前道路图像上自动标注改进建议构建新数据集；提出OD-RASE基线方法，结合LVLM和扩散模型生成基础设施改进建议和改进后道路环境的生成图像

Result: 本体驱动数据过滤能够高精度预测导致事故的道路结构和相应改进计划

Conclusion: 该工作有助于提升交通环境整体安全性，是推动自动驾驶系统更广泛采用的重要一步
...
<details>
  <summary>Details</summary>
Motivation: 现有统一图像修复框架面临特征干扰和专家专业化不足的挑战，难以有效处理多种退化类型。需要一种能够动态激活专业专家并可靠路由的机制。
...
Conclusion: SLER-IR通过球面分层专家路由和全局-局部融合机制，有效解决了多退化图像修复中的特征干扰和专家专业化问题，为统一图像修复框架提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 文档倾斜估计是文档处理系统中的关键任务，其性能直接影响后续处理步骤。随着数字化时代的兴起，需要更有效的倾斜估计方法。

Method: 在二维离散傅里叶幅度谱上应用自适应径向投影，提取文档图像的主导倾斜角度

Result: 提出的方法表现出鲁棒性和可靠性，在所有对比方法中性能最优

Conclusion: 该方法为文档倾斜估计提供了有效的解决方案，并建立了高质量评估数据集，为傅里叶基方法的改进提供了全面分析
...
TL;DR: LucidNFT：基于多奖励强化学习的流匹配真实图像超分辨率框架，通过Lucidity一致性评估器、解耦优势归一化策略和Lucidity-LR数据集解决真实图像超分辨率中的语义幻觉和优化瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 生成式真实图像超分辨率（Real-ISR）存在关键失败模式：输出可能看起来清晰但不符合低分辨率输入的证据（语义和结构幻觉），而基于低分辨率的忠实度难以在没有高分辨率真值的情况下评估。现有基于偏好的强化学习方法面临三个主要挑战：缺乏退化鲁棒的低分辨率参考忠实度信号、rollout组优化瓶颈导致优势崩溃、以及真实退化覆盖有限限制了rollout多样性和偏好信号质量。

Method: 提出LucidNFT框架，包含三个核心组件：1) LucidConsistency：退化鲁棒的语义评估器，使基于低分辨率的忠实度可测量和可优化；2) 解耦优势归一化策略：在融合前保留每个低分辨率条件rollout组内的目标间对比，防止优势崩溃；3) LucidLR：大规模真实世界退化图像集合，支持鲁棒的强化学习微调。该框架基于流匹配的真实图像超分辨率模型。

Result: 实验表明，LucidNFT能够持续改进强大的基于流的真实图像超分辨率基线，在多样化的真实世界场景中实现更好的感知-忠实度权衡，并保持稳定的优化动态。

Conclusion: LucidNFT通过解决生成式真实图像超分辨率中的关键挑战——缺乏退化鲁棒的忠实度评估、rollout组优化瓶颈和真实退化数据有限——提供了一个有效的多奖励强化学习框架，实现了更好的感知质量和忠实度平衡。
...
Method: 两阶段框架：第一阶段训练过程编码器从关键帧学习变化过程（通过自动生成中间帧使隐式过程显式化，然后采样减少冗余）；第二阶段将训练好的编码器集成到编码器-解码器模型中，使用可学习的过程查询来推断潜在过程表示，然后解码为文本描述
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM在深度信息理解方面存在不足，难以准确解释视觉数据中的深度信息，这限制了三维场景理解能力。
...
Conclusion: DeepSight作为首个专用深度MLLM，通过创新的数据集构建和模型架构改进，有效解决了现有MLLM在深度理解方面的局限性，推动了三维场景理解的发展。
...
Method: VINE引入空间-视角图在骨干特征上建模：空间图捕捉局部几何拓扑，视角图连接不同视角特征以传播视角不变的结构语义。从支持-查询特征差异中提取判别性先验来捕获类别特定对比度，重新加权SAM特征并重新校准骨干激活。通过掩码交叉注意力逐步集成前景增强的SAM特征和结构丰富的ResNet特征，生成类别一致的原型作为SAM解码器的自适应提示。
...
TL;DR: 提出两阶段框架解决字形相似度度量问题：先在人工字母表上训练教师模型，再通过师生蒸馏扩展到历史文字系统，实现监督对比学习与无监督发现的结合


<details>
  <summary>Details</summary>
Motivation: 字形和文字系统的相似度度量面临根本挑战：人工字母表中的单个字素可以可靠标注，但不同文字系统之间的历史关系存在不确定性且存在争议

Method: 两阶段框架：第一阶段在有标签的人工字母表上使用对比损失训练编码器，建立具有鲁棒判别特征的教师模型；第二阶段通过师生蒸馏扩展到历史文字系统，学生在教师知识指导下学习无监督表示，同时自由发现潜在的跨文字相似性

Result: 在多样化的文字系统上进行实验，展示了有效的少样本字形识别和有意义的文字聚类，无需真实进化关系作为基础

Conclusion: 该框架连接了监督对比学习和无监督发现，既能建立不同系统之间的硬边界，又能反映潜在历史影响的软相似性，解决了文字相似度度量的根本挑战
...
<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型使用全注意力机制具有二次计算成本，仅适用于短序列离线处理；因果注意力变体如StreamVGGT虽支持单次流式处理，但KV缓存持续增长，GPU内存很快耗尽，无法实现长序列流式推理

Method: 提出OVGGT框架：1）自选择性缓存（Self-Selective Caching）：利用FFN残差幅度压缩KV缓存，完全兼容FlashAttention；2）动态锚点保护（Dynamic Anchor Protection）：保护坐标关键令牌不被驱逐，抑制长轨迹中的几何漂移

Result: 在室内、室外和超长序列基准测试中，OVGGT能够在恒定VRAM限制下处理任意长视频，同时实现最先进的3D几何精度

Conclusion: OVGGT通过训练无关的方法解决了流式视频3D重建中的内存和计算瓶颈，实现了长序列流式推理的实际部署
...
TL;DR: 提出基于两阶段策略（目标分割+识别）的开放词汇目标识别框架，无需复杂重训练和标注，通过CLIP生成图像-文本嵌入，并引入CNN/MLP方法增强编码灵活性，在多个数据集上实现SOTA性能。
...
Method: 采用两阶段策略：1）目标分割裁剪区域；2）使用CLIP生成目标级图像嵌入和类别级文本嵌入。为减少对CLIP的依赖，引入CNN/MLP方法提取特征图并利用MLP对齐视觉特征与文本嵌入，通过SVD构建共享表示空间，最后通过嵌入相似度匹配进行识别。

Result: 在COCO、Pascal VOC和ADE20K数据集上的实验表明，无需训练的CLIP编码（无SVD）获得最高的平均AP，超越了当前SOTA方法。同时验证了CNN/MLP图像编码在OVOR中的潜力。

Conclusion: 提出的两阶段OVOR框架有效解决了现有方法的局限性，实现了高性能的开放词汇目标识别，为后续研究提供了新的方向。
...
TL;DR: S2I将3D人体骨架序列转换为图像格式，使视觉预训练模型能用于自监督骨架表示学习，解决骨架数据格式差异和数据集稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视觉预训练模型难以直接应用于3D骨架数据，因为数据格式存在根本差异。同时，大规模骨架数据集稀缺，且需要在不增加额外模型分支的情况下将骨架数据融入多模态动作识别，这些挑战构成了研究机会。

Method: 提出Skeleton-to-Image Encoding (S2I)表示方法：基于身体部位语义对关节进行分区和排列，将骨架序列转换为类图像数据，并调整到标准图像尺寸。这种编码首次使视觉预训练模型能用于自监督骨架表示学习。

Result: 在NTU-60、NTU-120和PKU-MMD数据集上的广泛实验证明了该方法在自监督骨架表示学习中的有效性和泛化能力，包括在具有挑战性的跨格式评估设置下。

Conclusion: S2I提供了一种统一的类图像格式，能自然适应异构骨架数据，成功将丰富的视觉领域知识迁移到骨架分析中，为自监督骨架表示学习提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测模型通常规模庞大，难以在资源受限设备上部署。量化虽然能压缩模型，但极端低比特量化会严重破坏细粒度视觉-语言对齐和区域间关系结构，导致性能大幅下降。

Method: 提出CR-QAT框架，包含两个核心组件：1) 课程QAT(CQAT)：将模型分区进行渐进量化，通过误差隔离确保稳定优化；2) 文本中心关系KD(TRKD)：在任务相关模块应用，通过构建文本锚定的成对相似度矩阵，全面传递教师模型的多维关系知识。

Result: 在LVIS和COCO零样本基准测试中，CR-QAT在激进低比特设置下持续优于现有QAT基线，相对AP提升分别达到38.9%和40.9%。

Conclusion: CR-QAT框架有效解决了开放词汇目标检测模型在极端量化下的性能退化问题，通过结合渐进量化和关系知识蒸馏，在保持模型压缩优势的同时显著提升了检测精度。
...
TL;DR: DynUAV是一个新的动态无人机视角多目标跟踪基准，包含42个视频序列和170万+边界框标注，专注于剧烈自运动和复杂表观轨迹带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角MOT基准缺乏真实场景中的动态复杂性，通常只包含可预测的相机运动和线性运动模式，无法充分评估跟踪器在剧烈自运动条件下的性能。

Method: 构建了包含42个视频序列的DynUAV基准，标注了超过170万个边界框，涵盖车辆、行人以及挖掘机、推土机、起重机等工业类别。基准特点是剧烈的自运动导致的复杂表观轨迹、尺度变化、视角变化和运动模糊。

Result: 对最先进跟踪器的综合评估显示，它们在DynUAV上存在明显局限性，特别是在动态条件下管理检测和关联的相互交织挑战方面表现不足。

Conclusion: DynUAV作为一个严格的基准，能够推动真实世界无人机视角MOT的进展，揭示了当前方法在动态条件下的不足，为未来研究提供了具有挑战性的测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 在医疗保健领域，手术器械的质量直接关系到临床成功和患者安全。即使是微小的制造缺陷也可能导致致命后果，因此需要精确的制造和质量控制。报告旨在通过自动化缺陷检测技术提升巴基斯坦制造的手术器械质量标准，防止经济损失并保障患者生命安全。

Method: 研究采用新开发的包含4,414张高分辨率图像的数据集，与行业领导者Daddy D Pro和Dr. Frigz International合作，整合深度学习架构包括YOLOv8、ResNet-152和EfficientNet-b4进行自动缺陷检测。通过自动光学检测（AOI）工具分析裂纹、锈蚀和结构不规则等关键缺陷。

Result: 报告提供了对巴基斯坦制造手术器械缺陷的深入分析，建立了基于深度学习的自动化缺陷检测方法。这些方法能够有效识别和分类各种制造缺陷，为制造商、医疗专业人员和监管机构提供了质量保证工具。

Conclusion: 通过整合行业专业知识和尖端技术，特别是深度学习架构，该研究显著提升了手术器械制造的质量标准。自动缺陷检测方法不仅有助于防止经济损失，更重要的是保障了患者安全，为更可靠的医疗环境做出了贡献。
...
Method: 提出两阶段编码机制：1)跨模态视觉-文本编码模块自动生成信息丰富的视觉图像和文本数据，结合多模态大语言模型；2)ISTS编码提取历史观测的补充时间特征；3)自适应查询特征提取器压缩MLLM令牌；4)带模态感知门控的多模态对齐模块缓解模态差异。
...
<details>
  <summary>Details</summary>
Motivation: 当前深度基础模型在透视图像上训练表现良好，但在360°图像上泛化能力差，主要原因是透视与全景域之间存在显著几何差异。完全微调这些模型通常需要大量全景数据，而现有方法难以在保留预训练透视先验的同时有效适应全景域。

Method: 提出RePer-360框架，包含三个核心组件：1) 轻量级几何对齐引导模块，从ERP和CP两种互补投影中提取调制信号，引导模型适应全景域而不覆盖预训练透视知识；2) 自条件AdaLN-Zero机制，生成像素级缩放因子以减少透视与全景域间的特征分布差距；3) 立方体贴图域一致性损失，提高训练稳定性和跨投影对齐。

Result: RePer-360仅使用1%训练数据即可超越标准微调方法。在相同域内训练设置下，RMSE指标提升约20%。该方法在保留预训练透视先验的同时，有效适应了全景域，显著提高了单目全景深度估计的性能。

Conclusion: 通过将重点从互补投影融合转向在保留预训练透视先验下的全景域适应，RePer-360提供了一种高效的单目全景深度估计解决方案。该方法在数据效率、性能提升和跨域适应方面均表现出色，为深度基础模型在全景域的应用提供了新思路。
...
TL;DR: RepKAN是一种新型遥感图像分类架构，结合CNN的结构效率和KAN的非线性表示能力，通过双路径设计实现物理可解释性，在EuroSAT和NWPU-RESISC45数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前遥感图像分类中，标准CNN和Transformer通常作为不可解释的黑盒模型运行，缺乏物理可解释性，限制了其在需要明确推理的地球观测任务中的应用。

Method: 提出RepKAN架构，整合CNN的结构效率和KAN（Kolmogorov-Arnold Networks）的非线性表示能力。采用双路径设计：空间线性路径和光谱非线性路径，能够自主发现类别特定的光谱指纹和物理交互流形。

Result: 在EuroSAT和NWPU-RESISC45数据集上的实验结果表明，RepKAN在提供明确的物理可解释推理的同时，性能优于当前最先进的模型。
...
<details>
  <summary>Details</summary>
Motivation: 当前高质量视觉特效(VFX)制作需要专业知识和高成本生产流程，现有AIGC系统面临特效特定数据稀缺、超自然或风格化特效建模困难等挑战，且通常需要逐特效微调，严重限制了其可扩展性和对新特效的泛化能力。

Method: EffectMaker采用统一推理-生成框架：1) 多模态大语言模型解释高级特效语义并推理如何适应目标主体；2) 扩散变换器通过上下文学习从参考视频中捕捉细粒度视觉线索；3) 语义-视觉双路径引导机制实现准确、可控、特效一致的合成；4) 构建EffectData数据集，包含130k视频和3k特效类别。

Result: 实验表明，EffectMaker在视觉质量和特效一致性方面优于现有最先进基线方法，提供了可扩展且灵活的自定义VFX生成范式。

Conclusion: EffectMaker通过语义-视觉双路径引导机制，无需逐特效微调即可实现准确、可控的特效定制化生成，为解决VFX生成中的数据稀缺和泛化问题提供了有效解决方案。
...
TL;DR: 提出MOSIV框架，用于从视频中进行多物体系统识别，通过可微分模拟器直接优化连续、每物体材料参数，解决现有方法局限于单物体场景或固定材料原型分类的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法不适用于多物体系统识别，因为它们主要关注单物体场景或使用固定材料原型进行离散材料分类。需要一种能够处理多物体交互、连续材料参数优化的新方法。

Method: 提出MOSIV框架：1) 使用可微分模拟器直接优化连续、每物体的材料参数；2) 基于视频推导的几何目标指导优化；3) 创建包含接触丰富的多物体交互的合成基准数据集进行评估。

Result: 在新建的基准数据集上，MOSIV相比适应后的基线方法显著提高了接地准确性和长时程模拟保真度，成为该新任务的强基线。分析表明物体级细粒度监督和几何对齐目标对复杂多物体环境中的稳定优化至关重要。

Conclusion: MOSIV为多物体系统识别提供了有效解决方案，通过可微分模拟和几何目标实现了连续材料参数的优化。该方法在处理复杂多物体交互方面表现出色，源代码和数据集将公开发布。
...
TL;DR: StruVis框架通过结构化视觉表示增强文本到图像生成，避免中间图像生成的计算成本，同时提供更好的视觉感知能力


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成中的推理框架存在两个问题：纯文本推理缺乏视觉上下文导致空间和视觉元素缺失；文本-图像交错推理虽然增强视觉基础但计算成本高且受限于生成器的表示能力
...
<details>
  <summary>Details</summary>
Motivation: 2D多目标跟踪面临部分遮挡导致的定位成本混淆问题，现有方法在遮挡场景下性能受限，需要专门处理遮挡的解决方案。

Method: 提出OA-SORT框架，包含三个核心模块：1) OAM分析物体遮挡状态，引入高斯地图减少背景影响；2) OAO利用遮挡状态缓解成本混淆；3) BAM抑制估计不稳定性。该框架无需训练，可直接集成到现有跟踪器中。

Result: 在DanceTrack测试集上达到63.1% HOTA和64.2% IDF1；将框架集成到四个额外跟踪器中，平均提升HOTA 2.08%和IDF1 3.05%，验证了遮挡感知的可重用性。

Conclusion: 遮挡处理对多目标跟踪至关重要，OA-SORT框架通过即插即用的遮挡感知模块有效解决遮挡问题，显著提升跟踪性能且具有良好的通用性。
...
TL;DR: 该论文提出使用分层子采样处理VGG16超列特征，并通过集成学习提高脑肿瘤分割性能，在极低样本量（N≤20）下逻辑回归表现最佳。


<details>
  <summary>Details</summary>
Motivation: 超列特征在图像分割中具有潜力，但计算复杂度随训练集大小线性增长，限制了实际应用。需要解决高维超列特征处理的计算效率问题。

Method: 对VGG16生成的超列特征应用分层子采样，研究稀疏超列上的集成学习方法（堆叠和投票集成），并与逻辑回归等简单分类器对比。

Result: 在脑肿瘤数据集上，10%分层子采样率下，N=20时最佳平均Dice分数为0.66。相比标准多尺度UNet基线，统计显著提升24.53%（p值=3.07e-11）。在极低样本量（N≤20）情况下，简单逻辑回归分类器最有效。

Conclusion: 分层子采样能有效降低超列特征计算复杂度，集成学习在稀疏超列上表现良好，但在极低样本量场景下，简单分类器（如逻辑回归）更具优势，能避免过拟合问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在字体排版控制方面存在局限，用户请求的字体样式经常被忽略或弱遵循，需要专门的数据驱动解决方案来提升排版控制能力

Method: 构建大规模字体排版数据集FontUse（约70K图像），使用分割模型和多模态大语言模型自动标注用户友好提示、文本区域位置和OCR识别字符串；通过微调现有生成器，使其能一致解释样式和使用场景条件

Result: 实验表明，使用该流程训练的模型在多样提示和布局下，比竞争基线产生更符合提示的文本渲染；引入基于Long-CLIP的度量标准来评估字体属性对齐

Conclusion: 提出的数据驱动方法和FontUse数据集有效解决了文本到图像模型中的字体排版控制问题，无需架构修改即可实现直观的字体样式和使用场景指定
...
Method: 提出GvU方法：1) 基于token级别的内在文本-图像对齐奖励机制，使模型同时作为教师和学生，利用理解分支评估自身输出并指导生成；2) 设计自监督强化学习框架，让模型通过基于理解的内在奖励信号迭代提升生成质量，无需外部监督。

Result: 实验结果表明，该方法显著提升了统一多模态模型的生成能力，同时增强了其细粒度视觉理解能力，缩小了视觉理解与生成之间的能力差距。

Conclusion: 通过利用统一多模态模型内部的理解能力来指导生成过程，可以有效提升生成质量并缩小理解与生成之间的能力差距，实现自监督的模型能力提升。
...
TL;DR: GenHOI是一种轻量级增强方法，通过时间平衡和空间选择性机制，在预训练视频生成模型中注入参考物体信息，以提升手-物体交互视频合成的物理合理性和物体一致性。


<details>
  <summary>Details</summary>
Motivation: 当前手-物体交互视频合成面临两大挑战：1）现有HOI重演方法通常在域内训练和评估，难以泛化到复杂的野外场景；2）一体化视频编辑模型虽然具有更广泛的鲁棒性，但仍存在物体外观不一致等HOI特定问题。需要一种既能保持背景真实感又能增强交互保真度的解决方案。

Method: 提出GenHOI框架，包含两个核心技术：1）时间平衡机制：采用Head-Sliding RoPE，为参考标记分配头部特定的时间偏移，将参考信息均匀分布到各帧，缓解3D RoPE的时间衰减问题，改善长距离物体一致性；2）空间选择性机制：设计两级空间注意力门，将物体条件注意力集中在HOI区域，并自适应缩放其强度，在保持背景真实感的同时增强交互保真度。

Result: 在未见过的野外场景上进行广泛定性和定量评估，结果表明GenHOI在物理合理性、物体一致性和交互保真度方面显著优于当前最先进的HOI重演方法和一体化视频编辑方法。

Conclusion: GenHOI通过轻量级增强策略有效解决了手-物体交互视频合成中的关键挑战，在保持预训练模型鲁棒性的同时，显著提升了交互场景的生成质量，为复杂野外场景下的HOI视频合成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 识别出自主VLA模型存在"窄策略限制"问题：模仿学习导致探索崩溃，限制了后续强化学习阶段的潜力，使其因反馈多样性不足而过早饱和

Method: 提出Curious-VLA两阶段框架：IL阶段引入可行轨迹扩展策略生成多个物理有效轨迹，使用逐步归一化轨迹表示适应多样数据；RL阶段提出自适应多样性感知采样优先高多样性样本，引入跨度驾驶奖励和焦点风格权重来放大奖励值跨度以提高对驾驶质量的敏感性

Result: 在Navsim基准上取得SOTA结果：PDMS 90.3，EPDMS 85.4，Best-of-N PDMS 94.8，证明能有效解锁VLA模型的探索潜力

Conclusion: Curious-VLA通过缓解利用-探索困境，成功解决了VLA模型中模仿学习导致的探索限制问题，为自主驾驶模型提供了有效的探索增强框架
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在自动驾驶应用中越来越普遍，旨在利用其推理和泛化能力处理长尾场景，但这些模型经常在自动驾驶高度相关的简单视觉问题上失败，且失败原因尚不清楚。

Method: 创建仅针对特定视觉概念不同的反事实图像集，然后在四种最先进的VLM的激活上训练线性探针来区分这些图像，评估视觉概念是否被线性编码。

Result: 结果显示：对象存在等概念被显式线性编码，而对象方向等空间概念仅通过视觉编码器保留的空间结构隐式编码；即使概念被线性编码，模型仍可能回答错误；识别出感知失败（所需视觉信息未线性编码）和认知失败（信息存在但未正确与语言语义对齐）两种失败模式；对象距离增加会快速降低相应视觉概念的线性可分性。

Conclusion: 该研究提高了对VLM在自动驾驶相关简单视觉任务上失败案例的理解，揭示了视觉信息在模型中的编码方式及其与语言处理对齐的问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在说话头生成中存在高推理延迟、时间不稳定（闪烁和身份漂移）以及音频-视觉对齐不完美的问题，需要在实际计算受限场景下实现实用的扩散基说话头生成

Method: 采用参考条件的潜在扩散框架，通过师生蒸馏方法：使用标准噪声预测目标训练的扩散教师指导轻量级学生去噪器，减少推理步骤；结合身份锚定和时间正则化减少身份漂移和闪烁；使用音素基音频条件提供粗粒度唇部运动控制

Result: 在LRS3数据集上的实验显示，蒸馏扩散模型能保留强教师的大部分重建行为，同时实现显著降低的延迟推理；提供了去噪阶段组件级指标和延迟特性分析，包括CPU和边缘计算测量

Conclusion: 蒸馏扩散模型可以在保持教师模型重建能力的同时大幅降低推理延迟，这是迈向计算受限场景下实用扩散基说话头生成的初步步骤
...
TL;DR: 本文提出情感连续说话人脸生成（EC-TFG）新任务及TIE-TFG模型，通过时序密集情感波动建模实现文本驱动下连续情感变化的逼真人脸视频生成


<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法通常只能表达固定目标情感，缺乏人类在传达信息时表现出的连续自然情感变化能力，需要解决动态情感变化建模问题

Method: 提出TIE-TFG模型，采用时序密集情感波动建模技术，生成与输入文本对应的情感变化序列，驱动合成视频中连续的面部表情变化

Result: 大量评估表明该方法在平滑情感过渡、高质量视觉效果和运动真实性方面表现优异，能够处理多种情感状态

Conclusion: EC-TFG任务和TIE-TFG模型成功解决了说话人脸生成中连续情感表达的问题，实现了更自然、更具表现力的数字人脸合成
...
TL;DR: 提出FedARKS框架解决FedDG-ReID中全局特征表示和简单平均聚合的局限性，通过鲁棒知识提取和知识选择机制提升跨域泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有联邦域泛化行人重识别方法依赖全局特征和简单平均聚合，存在两个主要问题：1) 仅用全局特征难以捕捉细粒度、域不变的局部细节；2) 统一参数平均忽略了客户端在鲁棒特征提取能力上的差异，稀释了高质量客户端的贡献

Method: 提出FedARKS框架，包含两个核心机制：RK（鲁棒知识）机制提取细粒度、域不变的局部特征；KS（知识选择）机制根据客户端特征提取能力差异进行选择性聚合

Result: 论文未提供具体实验结果，但方法旨在提升模型在未见域上的泛化能力，同时保护客户端数据隐私

Conclusion: FedARKS通过结合鲁棒知识提取和选择性知识聚合，解决了现有联邦域泛化行人重识别方法的局限性，有望在保护隐私的同时提升跨域泛化性能
...
TL;DR: Place-it-R1：基于多模态大语言模型推理的视频物体插入框架，通过Think-then-Place范式实现物理一致的视频编辑


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑技术主要关注视觉保真度而非物理因果关系，导致插入的物体与环境物理不一致。需要一种能够理解物理场景并进行环境感知推理的框架。

Method: 采用端到端框架，利用MLLM的链式思维推理协调视频扩散模型。创新包括：1) MLLM进行物理场景理解和交互推理，生成环境感知的CoT令牌和有效插入区域；2) MLLM引导的空间直接偏好优化，通过反馈评分实现视觉自然性；3) 推理时MLLM迭代触发精炼循环，形成闭环优化；4) 提供两种用户可选模式：增强物理合理性的灵活模式和保持场景完整性的标准模式。

Result: 大量实验表明，Place-it-R1相比最先进解决方案和商业模型，实现了物理一致性的视频物体插入。

Conclusion: 该框架通过结合MLLM的认知推理能力和扩散模型的生成能力，实现了物理一致且视觉自然的视频物体插入，为用户提供了在合理性和保真度之间的明确控制。
...
TL;DR: 该论文研究了视觉语言模型在结构化颜色扭曲下的系统性感知弱点，发现即使场景对人类来说仍然可识别，模型也会产生自信但荒谬的预测。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在基准测试中表现良好，但存在系统性感知弱点：像素值的结构化、大规模变化可能导致模型产生自信但荒谬的预测，即使底层场景对人类来说仍然容易识别。研究者旨在探索这一差距。

Method: 使用空间颜色混合方法，这是一种程序化的颜色扭曲家族，在自然图像上叠加结构化模式（包括RGB和Ostwald颜色系统）。研究者引入了8种空间颜色混合变体，在4个数据集上评估了9个来自3个模型家族的视觉语言模型。同时进行了包含61名参与者的人类研究。

Result: 随着扭曲程度的增加，所有模型和数据集上的准确率急剧下降，扩大语言模型规模并不能可靠地缓解这种失败。在相同扭曲条件下，人类表现显著优于视觉语言模型。简单的人类启发式预处理步骤可以恢复部分性能。

Conclusion: 视觉语言模型存在系统性感知弱点，需要感知感知的预处理和工具使用作为提高模型鲁棒性的实用策略。人类启发式预处理能有效恢复部分性能。
...
<details>
  <summary>Details</summary>
Motivation: 预测放疗期间肿瘤演化是临床关键挑战，特别是当纵向变化由解剖结构和治疗共同驱动时。需要一种能够模拟治疗诱导解剖变化的方法来支持治疗监测和适应性放疗研究。

Method: 提出虚拟治疗框架，将NSCLC进展建模为剂量感知多模态条件图像到图像翻译问题。基于CT扫描、基线临床变量和指定辐射剂量增量，合成随访CT图像。使用222名III期NSCLC患者的895个CT扫描纵向数据集，在2D和2.5D配置下对基于GAN和扩散的模型进行基准测试。

Result: 定量和定性结果表明，基于扩散的模型能更一致地从多模态、剂量感知条件中受益，产生比基于GAN的基线更稳定、解剖学上更合理的肿瘤演化轨迹。

Conclusion: 虚拟治疗框架作为NSCLC体内治疗监测和适应性放疗研究的工具有潜力，扩散模型在生成稳定、解剖学合理的肿瘤演化轨迹方面表现优于GAN模型。
...
TL;DR: VLM-RobustBench评估视觉语言模型在49种图像失真下的鲁棒性，发现模型对空间扰动（尤其是重采样和几何变形）比视觉上更严重的亮度扰动更敏感。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在标准高质量数据集上表现良好，但对其在真实世界图像失真下的性能理解不足。需要系统评估模型对各种图像扰动的鲁棒性。
...
Result: 视觉严重程度是性能下降的弱预测指标：低严重度的空间扰动（如glass_blur）比视觉上更严重的亮度扰动造成更大性能下降（平均降低8pp）。最大性能下降来自重采样和几何变形（如upsample、elastic_transform），可达34pp。当前VLM语义理解强但空间鲁棒性弱。

Conclusion: 当前视觉语言模型在语义理解方面强大，但对空间扰动脆弱。需要定义新的鲁棒性评估协议和训练机制，强调重采样和几何不变性。
...
TL;DR: 提出RF-Sampling方法，一种专门为流匹配模型设计的无需训练推理增强框架，通过理论推导证明其隐式执行文本-图像对齐分数的梯度上升，显著提升生成质量和提示对齐。


<details>
  <summary>Details</summary>
Motivation: 当前推理时增强策略主要适用于传统扩散模型，在流模型上表现不佳。随着FLUX等基于流匹配的文本到图像模型取得显著进展，需要专门针对流模型（特别是CFG蒸馏变体）的推理增强方法。

Method: 提出Reflective Flow Sampling (RF-Sampling)，一种理论基础的无需训练推理增强框架。通过形式化推导证明该方法隐式执行文本-图像对齐分数的梯度上升。利用文本表示的线性组合并结合流反演，使模型能够探索与输入提示更一致的噪声空间。

Result: 在多个基准测试上的广泛实验表明，RF-Sampling一致地提高了生成质量和提示对齐。此外，RF-Sampling是首个在FLUX上展现出一定程度测试时缩放能力的推理增强方法。

Conclusion: RF-Sampling成功填补了流模型推理增强的空白，为基于流匹配的文本到图像生成模型提供了有效的推理时优化方案，特别是在CFG蒸馏变体上表现出色。
...
TL;DR: FreeOcc：无需训练的3D语义和全景占用预测框架，利用预训练基础模型从多视角图像中恢复语义和几何信息，在未见环境中实现部署。


<details>
  <summary>Details</summary>
Motivation: 当前基于相机的3D占用预测方法通常需要昂贵的密集3D监督或针对目标域数据进行训练，限制了在未见环境中的部署。需要一种无需训练即可在未知环境中工作的解决方案。

Method: 1) 使用可提示的基础分割模型提取每视角全景先验，通过提示到分类规则获取语义；2) 使用重建基础模型恢复度量3D点云；3) 通过深度和置信度感知过滤将可靠标签提升到3D；4) 随时间融合并使用确定性细化堆栈进行体素化；5) 通过拟合和合并鲁棒的当前视角3D边界框候选来恢复实例，实现无需学习3D模型的实例感知占用。

Result: 在Occ3D-nuScenes数据集上：无需训练达到16.9 mIoU和16.5 RayIoU，与最先进的弱监督方法相当；作为伪标签生成管道训练下游模型时达到21.1 RayIoU，超越先前弱监督基线；在无需训练和弱监督全景占用预测上分别达到3.1 RayPQ和3.9 RayPQ，建立新基线。
...
Method: 1) 基于简单外观描述（如"暗色椭圆形"）实现自然图像到医学图像的结构迁移，让视觉语言模型生成结构一致的伪标签；2) 用伪标签预热静态教师模型捕获乳腺病灶的全局结构先验；3) 结合指数移动平均教师，引入不确定性熵加权融合和自适应不确定性引导的反向对比学习来改善边界判别

Result: 在四个乳腺超声数据集上的实验表明，仅用2.5%标注数据即可达到全监督模型性能，显著优于现有半监督方法

Conclusion: 该范式易于扩展：对于其他成像模态或疾病，仅需全局外观描述即可获得可靠的伪监督，能够在有限标注下实现可扩展的半监督医学图像分割
...
<details>
  <summary>Details</summary>
Motivation: 解决跨视觉模态（3D点云和全景图像）语义分割的挑战，包括标注数据稀缺和固定标签模型适应性有限的问题。

Method: 将RGB-D全景图像转换为对应的切向透视图像和3D点云，利用这些模态提取和对齐基础视觉-语言特征，通过自然语言查询生成两种输入模态的语义掩码。
...
TL;DR: 该论文提出了一种使用强化学习（RL）和多尺度反馈增强3D扩散模型的方法，通过在MRI数据上预训练扩散模型，然后使用PPO算法和结合2D切片与3D体积分析的新型奖励系统进行微调，以同时优化局部纹理细节和全局结构一致性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为3D医学图像生成的强大工具，但标准训练目标与临床相关性之间存在差距。需要开发能够生成既具有高质量局部纹理细节又保持全局结构一致性的3D医学图像的方法，以更好地服务于下游临床任务。

Method: 1. 在MRI体积数据上预训练3D扩散模型以建立生成先验；2. 使用近端策略优化（PPO）算法进行微调；3. 设计新型奖励系统，整合2D切片级评估和3D体积分析；4. 通过多尺度反馈同时优化局部纹理细节和全局结构一致性。

Result: 在BraTS 2019和OASIS-1数据集上的验证表明：1. RL反馈有效引导生成过程向更高质量分布；2. 定量分析显示Fréchet Inception Distance（FID）显著改善；3. 合成数据在下游肿瘤和疾病分类任务中相比未优化基线表现出更强的实用性。

Conclusion: 该方法成功将强化学习与多尺度反馈整合到3D扩散模型中，有效提升了生成医学图像的质量和临床相关性，为生成高质量3D医学图像提供了一种有前景的解决方案，能够更好地服务于下游临床诊断任务。
...
TL;DR: 本文提出两种技术（自动聚合和逐像素重缩放）来解决训练免费扩散分割器中存在的两个关键问题，使生成能力更好地转化为分割性能。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在生成任务上表现出色，但现有训练免费扩散分割方法未能充分利用更强大扩散模型的生成能力。研究发现存在两个关键问题：跨注意力图在多头多层之间存在差异，以及全局图无法直接转化为准确的分割语义相关性。

Method: 提出两种技术：1) 自动聚合：解决多头多层跨注意力图之间的差异，生成统一的全局表示；2) 逐像素重缩放：解决不同文本标记之间的分数不平衡问题，使全局图能准确反映语义相关性。

Result: 在标准语义分割基准上评估了该方法，并进一步将其集成到生成技术中，展示了性能提升和广泛适用性。

Conclusion: 通过自动聚合和逐像素重缩放技术，训练免费扩散分割能够更好地利用扩散模型的生成能力，解决了现有方法未能有效扩展的问题。
...
TL;DR: 提出基于骨架时序动作分割的点监督框架，仅需每个动作片段标注单帧，通过多模态骨架特征、原型相似度方法生成可靠伪标签，显著降低标注成本


<details>
  <summary>Details</summary>
Motivation: 全监督方法需要昂贵的逐帧标注且对模糊动作边界敏感，点监督方法能大幅降低标注成本同时保持性能

Method: 使用预训练统一模型编码关节、骨骼和运动多模态特征；提出原型相似度方法，结合能量函数和约束K-Medoids聚类生成伪标签；多模态伪标签集成增强可靠性

Result: 在PKU-MMD (X-Sub/X-View)、MCFS-22和MCFS-130数据集上建立新基准，性能具有竞争力，甚至超越部分全监督方法，显著减少标注工作量

Conclusion: 提出的点监督框架有效解决了骨架动作分割的标注成本问题，通过多模态特征和伪标签生成策略实现了高性能，为实际应用提供了实用解决方案
...
TL;DR: VG3S是一个将视觉基础模型的几何感知能力注入到3D高斯泼溅占用预测中的新框架，显著提升了自动驾驶场景的语义占用预测精度。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯泼溅的占用预测方法虽然计算效率高，但严重依赖准确的几何线索，而纯视觉方法往往几何信息不足。视觉基础模型具有强大的几何感知能力，但尚未被有效整合到占用预测中。

Method: 提出VG3S框架，通过一个即插即用的分层几何特征适配器，将冻结的视觉基础模型的3D几何先验知识注入到高斯泼溅占用预测中。该适配器通过特征聚合、任务特定对齐和多尺度重构来有效转换通用VFM特征。

Result: 在nuScenes占用基准测试中，VG3S相比基线在IoU上提升了12.6%，在mIoU上提升了7.5%。框架能够无缝适配不同的视觉基础模型，一致地提升占用预测精度。

Conclusion: VG3S证明了将预训练的几何感知视觉基础模型的先验知识整合到占用预测中的巨大价值，为自动驾驶场景理解提供了更准确、更高效的解决方案。
...
TL;DR: CoE是一个免训练的多模态摘要框架，通过事件链和分层事件图实现结构化推理，在八个数据集上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态摘要方法存在三个主要问题：依赖领域特定监督、隐式融合导致跨模态对齐弱、平坦的时间建模缺乏事件转换。需要一种更结构化、可解释且具有跨域泛化能力的方法。

Method: 提出CoE框架，基于分层事件图（HEG）进行结构化推理。HEG将文本语义编码为显式的事件层次结构，指导跨模态对齐和时间推理。CoE通过事件链定位关键视觉线索、建模事件演化和因果转换，并通过轻量级风格适应进行领域对齐。

Result: 在八个多样化数据集上的实验表明，CoE一致优于最先进的视频CoT基线方法，平均提升+3.04 ROUGE、+9.51 CIDEr和+1.88 BERTScore，展现了其鲁棒性、可解释性和跨域泛化能力。

Conclusion: CoE通过分层事件图驱动的结构化推理，有效解决了多模态摘要中的关键挑战，实现了免训练、可解释且具有强泛化能力的多模态摘要生成。
...
<details>
  <summary>Details</summary>
Motivation: 标准3DGS生成的高斯中心与物体几何对齐不佳，而专注于表面的重建方法常牺牲光度精度。需要一种能平衡几何准确性和渲染质量的方法

Method: 引入基于特征值熵的几何感知策略指导自适应分裂和剪枝。计算每个高斯中心k近邻协方差矩阵特征值的特征值熵，量化局部结构有序度。在交替优化框架中结合标准梯度致密化和特征值熵感知致密化

Result: 在DTU和TUM2TWIN数据集上，几何精度提升达33%，渲染质量提升达7%，高斯数量减少达50%，训练时间缩短达23%
...
TL;DR: 提出WAFL新范式，将时序伪造定位从回归和连续检测转变为离散词级二分类，通过特征重对齐和不对称损失解决特征粒度不对齐和类别不平衡问题
...
Method: 1) 识别最小有意义的伪造单元（词令牌）并与语音自然语言边界对齐；2) 引入法证特征重对齐模块将预训练语义空间映射到判别性法证流形；3) 设计基于伪影的不对称损失动态抑制真实梯度并优先处理细微法证伪影

Result: WAFL在数据集内和跨数据集设置下显著优于现有方法，同时需要更少的可学习参数并实现高计算效率

Conclusion: WAFL通过将时序伪造定位重新定义为离散词级二分类任务，有效解决了特征粒度不对齐和计算效率问题，为时序伪造检测提供了新范式
...
TL;DR: TaPD是一个用于可变历史长度轨迹预测的统一即插即用框架，通过渐进式知识蒸馏和时序回填模块，在短观测条件下显著提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测器通常假设固定长度的历史观测，但在实际场景中（如遮挡或传感器范围限制）观测长度可变且可能极短，导致性能显著下降。需要一种能够自适应不同观测长度的统一预测框架。

Method: 提出TaPD框架，包含两个协作模块：1) 观测自适应预测器(OAF)，基于渐进式知识蒸馏(PKD)，通过层次特征回归将长时观测"教师"的运动模式知识迁移到短时观测"学生"；2) 时序回填模块(TBM)，为极短历史显式重建缺失的历史片段。采用余弦退火蒸馏权重方案平衡预测监督和特征对齐，并使用解耦的预训练-重建-微调协议。

Result: 在Argoverse 1和Argoverse 2数据集上的大量实验表明，TaPD在所有观测长度下均优于强基线方法，在极短输入条件下提升尤为显著，并能以即插即用方式改进其他预测器（如HiVT）。

Conclusion: TaPD通过渐进式知识蒸馏和时序回填的统一框架，有效解决了可变历史长度轨迹预测问题，显著提升了短观测条件下的预测性能，具有良好的通用性和实用性。
...
TL;DR: HCF-RES：一种用于3D指代表达式分割的多模态框架，通过层次化视觉语义分解和渐进式多级融合，在稀疏点云基础上增强视觉语义，提升对细粒度描述的定位能力


<details>
  <summary>Details</summary>
Motivation: 现有3D指代表达式分割方法仅依赖稀疏点云，缺乏丰富的视觉语义信息，难以处理匹配多个或零目标的细粒度自然语言描述

Method: 提出HCF-RES框架：1）层次化视觉语义分解：利用SAM实例掩码指导CLIP编码，提取像素级和实例级特征，在2D到3D投影中保持对象边界；2）渐进式多级融合：通过模态内协作、2D语义与3D几何特征的跨模态自适应加权、以及语言引导的细化来整合表示

Result: 在ScanRefer和Multi3DRefer数据集上取得了最先进的结果

Conclusion: HCF-RES通过结合2D视觉语义和3D几何信息，有效解决了现有方法在细粒度描述处理上的局限性，为3D指代表达式分割提供了更强大的多模态解决方案
...
TL;DR: NOVA提出了一种创新的开放词汇自回归范式，将3D多目标跟踪从传统的基于距离的匹配转向生成式时空语义建模，利用大语言模型的自回归能力解决未知目标的泛化问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D多目标跟踪方法受限于闭集假设和"语义盲"启发式方法，无法有效泛化到未知目标类别。开放世界感知需要能够处理未知目标的跟踪系统。

Method: 将3D轨迹重新定义为结构化时空语义序列，利用大语言模型的自回归能力将跟踪任务转化为下一步序列补全过程。通过语言空间的分层结构解决语义歧义，利用常识推理保持长序列中的身份一致性。
...
Conclusion: NOVA通过将3D跟踪重新定义为生成式时空语义建模，成功解决了开放词汇场景下的目标跟踪问题，为开放世界感知提供了新范式。
...
TL;DR: StrSR是一种针对扩散变换器（DiT）的单步对抗蒸馏框架，通过谱和轨迹正则化解决Real-ISR中的轨迹失配和网格状伪影问题。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在真实图像超分辨率中表现出潜力，但其迭代采样计算成本高，需要单步蒸馏。现有单步蒸馏方法在DiT上存在轨迹失配和严重网格状周期性伪影的问题。
...
TL;DR: 该研究首次系统分析了3D语义占据预测中的标签噪声问题，建立了OccNL基准并提出DPR-Occ框架，在极端噪声下仍能保持性能稳定。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的3D体素标注存在结构伪影和动态拖尾效应等固有噪声，这引发了关键问题：自动驾驶系统能否安全依赖这种不可靠的占据监督？现有2D标签噪声学习方法在稀疏3D体素空间中会灾难性崩溃，暴露了现有范式的严重脆弱性。

Method: 提出DPR-Occ框架，通过双源部分标签推理构建可靠监督。该方法协同利用时间模型记忆和表示级结构亲和性，动态扩展和修剪候选标签集，以保留真实语义同时抑制噪声传播。

Result: 在SemanticKITTI上的大量实验表明，DPR-Occ在极端噪声下能防止几何和语义崩溃。即使在90%标签噪声下，该方法相比现有标签噪声学习基线仍能实现显著性能提升（最高2.57% mIoU和13.91% IoU）。

Conclusion: 通过连接标签噪声学习和3D感知，OccNL和DPR-Occ为动态环境中安全关键的机器人感知提供了可靠基础。该研究揭示了3D占据预测中标签噪声问题的严重性，并提出了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 生成式零样本学习通过合成未见类特征来从已见类转移知识，但面临两个内在挑战：1) 类级属性无法捕捉实例特定的视觉外观（类-实例差距）；2) 语义与视觉特征分布不匹配（语义-视觉域差距）。

Method: 提出ADiVA方法，包含两个模块：属性分布建模模块学习每个类的可转移属性分布并为未见类采样实例级属性；视觉引导对齐模块细化语义表示以更好反映视觉结构。
...
TL;DR: FlowMotion：一种基于流引导的无训练视频运动迁移框架，通过直接利用流基T2V模型的预测输出实现高效灵活的运动迁移


<details>
  <summary>Details</summary>
Motivation: 现有无训练方法依赖预训练T2V模型的中间输出构建运动引导，导致计算开销大且灵活性有限。需要更高效灵活的运动迁移方法。

Method: 提出FlowMotion框架：1）利用流基T2V模型的早期潜在预测提取运动表示（流引导）；2）引入速度正则化策略稳定优化并确保平滑运动演化；3）完全基于模型预测操作，无需额外训练。

Result: 相比现有方法，FlowMotion在时间和资源效率上表现优异，同时保持竞争性能。实现了高效灵活的视频运动迁移。

Conclusion: FlowMotion通过直接利用流基T2V模型的预测输出，提供了一种高效、灵活且无需训练的视频运动迁移框架，在效率和性能方面均优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: CBCT在牙科3D成像中广泛应用，但高密度物体（如牙科植入物）产生的伪影会严重影响图像质量和诊断准确性。现有基于扩散模型的植入物修复方法仅处理独立的2D投影，忽略了投影间的相关性，导致重建图像不一致。
...
TL;DR: DEX-AR是一种针对自回归视觉语言模型的新型可解释性方法，通过动态头过滤和序列级过滤生成每token和序列级热力图，显著提升了模型决策过程的可解释性。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型变得越来越复杂和广泛应用，理解其决策过程变得至关重要。传统的可解释性方法主要针对分类任务设计，难以处理现代自回归VLM的逐token生成过程以及视觉和文本模态间的复杂交互。

Method: DEX-AR通过计算逐token生成过程中注意力图的层间梯度来生成每token和序列级2D热力图。核心创新包括：1）动态头过滤机制，识别专注于视觉信息的注意力头；2）序列级过滤方法，聚合每token解释同时区分视觉基础和纯语言token。

Result: 在ImageNet、VQAv2和PascalVOC上的评估显示，DEX-AR在使用新型归一化困惑度度量的扰动指标以及基于分割的指标上都取得了持续改进。

Conclusion: DEX-AR成功解决了自回归视觉语言模型的可解释性挑战，通过动态过滤机制和序列级聚合方法，为理解复杂多模态模型的决策过程提供了有效工具。
...
TL;DR: 提出了一种小波增强的专家混合提示学习方法，用于零样本异常检测，通过变分自编码器建模全局语义、小波分解提取多频特征，以及语义感知的专家混合模块来提升检测性能。
...
Conclusion: 提出的小波增强专家混合提示学习方法能够有效提升零样本异常检测性能，通过全局语义建模、多频特征提取和上下文聚合来克服现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前点云语义分割严重依赖人工标注，而无监督方法研究尚处早期阶段。无监督点云学习面临缺乏标注信息和预训练的挑战，需要开发有效策略来解决这些问题。
...
Result: 在S3DIS、SemanticKITTI和Scannet数据集上广泛评估，相比无监督方法获得最佳性能。在S3DIS数据集Area-5上达到47.1%的mIoU，比经典全监督方法PointNet高出2.5%。

Conclusion: 提出的P-SLCR方法通过原型库驱动的无监督学习策略，有效解决了点云无监督语义分割的挑战，在多个数据集上超越了现有无监督方法，甚至在某些指标上超过了全监督方法。
...
<details>
  <summary>Details</summary>
Motivation: 基于扩散的世界模型在统一世界模拟方面表现出强大潜力，但迭代去噪过程计算成本过高，难以用于交互式使用和长时域推演。虽然特征缓存可以在不训练的情况下加速推理，但为单模态扩散设计的策略在迁移到世界模型时效果不佳，主要面临两个世界模型特有的障碍：多模态耦合和空间变化导致的令牌异构性，以及非均匀时间动态（少量困难令牌驱动误差增长）。

Method: 提出WorldCache缓存框架，包含两个核心技术：1）曲率引导的异构令牌预测：使用基于物理的曲率分数估计令牌可预测性，对具有方向突变的混沌令牌应用Hermite引导的阻尼预测器；2）混沌优先自适应跳过来：累积曲率归一化的无量纲漂移信号，仅在瓶颈令牌开始漂移时重新计算。

Result: 在扩散世界模型上的实验表明，WorldCache实现了高达3.7倍的端到端加速，同时保持98%的推演质量，在资源受限场景下展现出巨大优势和实用性。

Conclusion: WorldCache通过针对扩散世界模型特有的令牌异构性和非均匀时间动态问题设计专门的缓存策略，显著加速了世界模型的推理速度，同时保持了高质量的推演结果，为资源受限场景下的扩散世界模型应用提供了实用解决方案。
...
TL;DR: K-MaT是一种知识锚定流形传输框架，通过提示学习将决策结构从高端医学影像模态（如CT）迁移到低端模态（如X光），无需低端训练图像，使用融合Gromov-Wasserstein最优传输对齐提示流形。


<details>
  <summary>Details</summary>
Motivation: 现有大规模生物医学视觉语言模型在高端影像模态（如CT）上适应良好，但迁移到前线低端模态（如X光）时容易失败，陷入模态特定的捷径学习。需要一种无需低端训练图像就能实现跨模态迁移的方法。

Method: 提出K-MaT框架：1）分解提示并锚定到临床文本描述；2）使用融合Gromov-Wasserstein最优传输将低端提示流形与视觉基础的高端空间对齐；3）通过提示学习将决策结构迁移到低端模态。

Result: 在四个跨模态基准测试（皮肤镜、乳腺X光到超声、CT到胸部X光）上取得最先进结果：平均调和准确率从42.0%提升到44.1%，宏观F1分数达到36.2%。在具有挑战性的乳腺成像任务中，避免了标准方法（如CoOp）的灾难性遗忘问题（准确率降至27.0%），保持了跨模态的稳健性能。

Conclusion: 通过最优传输对齐提示流形为医学视觉语言模型的零样本跨模态部署提供了一条高效路径，能够在不使用低端训练图像的情况下实现稳健的跨模态迁移。
...
TL;DR: LATO是一种新颖的拓扑保持潜在表示，通过顶点位移场和稀疏体素VAE实现可扩展的、基于流匹配的显式3D网格生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖等值面提取或启发式网格化，难以同时保持复杂几何形状和良好拓扑结构，且推理效率较低。

Method: 1) 使用顶点位移场表示网格，通过稀疏体素VAE压缩为结构化体素潜在表示；2) VAE解码器通过渐进细分和剪裁实例化顶点位置；3) 专用连接头直接预测顶点对之间的边连接性；4) 两阶段流匹配：首先生成结构体素，然后细化体素级拓扑特征。

Result: 相比基于等值面/三角形的扩散模型和自回归生成方法，LATO能生成具有复杂几何形状、良好拓扑结构的网格，同时推理效率更高。

Conclusion: LATO提供了一种可扩展的拓扑保持潜在表示，实现了高效的显式3D网格生成，无需等值面提取或启发式网格化。
...
Method: 1. 构建DentalProbe数据集（5000张图像，含专家标注的诊断轨迹）；2. 开发Reinspection驱动的强化学习框架，结合基于评分标准的奖励和条件诊断驱动奖励；3. 提出OralGPT-Plus模型，支持迭代对称感知诊断推理

Result: OralGPT-Plus在MMOral-X基准测试和现有全景基准上均优于强基线模型，表明交互式和对称感知推理的有效性
...
<details>
  <summary>Details</summary>
Motivation: 获取密集像素级标注成本高昂，稀疏标注作为弱监督替代方案仍存在性能差距。利用3D场景重建作为辅助监督信号可以弥补这一差距

Method: 提出Rewis3d框架，利用前馈式3D重建从2D视频恢复3D几何结构，通过双师生架构在2D图像和重建的3D点云之间强制语义一致性，利用3D几何结构传播稀疏标注
...
TL;DR: CHMv2是一个基于高分辨率光学卫星影像和深度估计模型构建的全球米级分辨率冠层高度图，相比现有产品显著提升了精度，减少了高森林的偏差，并更好地保留了冠层边缘和空隙等精细结构。


<details>
  <summary>Details</summary>
Motivation: 准确的冠层高度信息对于量化森林碳储量、监测恢复与退化、评估栖息地结构至关重要，但全球范围内高保真度的机载激光扫描测量仍然分布不均。

Method: 使用基于DINOv3构建的深度估计模型，从高分辨率光学卫星影像中推导冠层高度；通过大量地理多样化的训练数据扩展、自动数据整理与配准、针对冠层高度分布定制的损失函数和数据采样策略来实现。

Result: CHMv2相比现有产品显著提高了精度，减少了高森林的偏差，并更好地保留了冠层边缘和空隙等精细结构；在独立ALS测试集以及数千万个GEDI和ICESat-2观测数据上验证了其在主要森林生物群落中的一致性能。

Conclusion: CHMv2提供了一个全球、米级分辨率的冠层高度图，通过改进的深度学习方法显著提升了冠层高度估计的准确性和精细结构保留能力，为全球森林监测提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: SAM3等基础模型在医学图像分割中虽然支持灵活的文本引导，但其预测对提示表述高度敏感，即使语义等价的描述也会产生不一致的分割结果，限制了在临床和病理工作流程中的可靠性

Method: 将提示敏感性重新定义为组间一致性问题，将语义相关的提示组织成共享相同真实掩码的提示组，提出提示组感知训练框架，包括：1）利用分割损失作为隐式排序信号的质量引导组正则化；2）采用停止梯度策略的对数级一致性约束来对齐组内预测。该方法无需修改架构且推理过程保持不变
...
TL;DR: REACT++是基于REACT架构改进的实时场景图生成模型，通过高效特征提取和原型空间中的主体-客体交叉注意力机制，在保持物体检测性能的同时提升关系预测准确率并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有场景图生成方法通常只专注于提升关系预测准确率、增强物体检测准确率或降低延迟中的单一目标，而无法同时平衡这三个关键指标。为了支持实时应用（如具身智能体推理），需要开发能够在性能与推理速度之间取得更好平衡的模型。

Method: 基于REACT架构，提出REACT++模型，采用高效特征提取技术和在原型空间中的主体-客体交叉注意力机制，优化了延迟与表征能力之间的平衡。

Result: REACT++在现有SGG模型中达到最高推理速度，在保持物体检测性能的同时提升了关系预测准确率。相比前代REACT版本，推理速度提升20%，关系预测准确率平均提升10%。

Conclusion: REACT++通过平衡延迟、关系预测准确率和物体检测性能三个目标，实现了实时场景图生成的新最优性能，为下游实时应用提供了更好的基础组件。
...
<details>
  <summary>Details</summary>
Motivation: 真实考古文物碎片重组面临侵蚀、缺失区域、不规则形状和大规模歧义等挑战，传统拼图求解器在清洁合成场景下设计，难以应对RePAIR基准中数千碎片的复杂情况

Method: 提出人机协同拼图求解框架，集成自动松弛标记求解器与交互式人工指导，用户可迭代锁定已验证位置、纠正错误并引导系统实现语义和几何一致的组装；引入两种互补交互策略：迭代锚定和连续交互细化

Result: 在多个RePAIR组上的实验表明，该混合方法在准确性和效率上显著优于全自动和手动基线，为大规模专家参与文物重组提供了实用解决方案

Conclusion: 人机协同框架有效解决了真实世界文化遗产重建的复杂性和规模问题，通过结合自动求解与人工指导，实现了大规模、高歧义场景下的文物碎片高效重组
...
### [101] [Locating and Editing Figure-Ground Organization in Vision Transformers](https://arxiv.org/abs/2603.06407)
*Stefan Arnold,René Gröbner*

Main category: cs.CV

TL;DR: BEiT视觉变换器在感知冲突中优先选择凸性完形，研究发现这种偏好由特定注意力头控制，通过调节单个注意力头可以改变完形偏好


<details>
  <summary>Details</summary>
Motivation: 研究视觉变换器在图形-背景组织中的感知模糊性，特别是当局部几何证据与全局组织先验（如格式塔凸性先验）冲突时，探索BEiT模型中这种先验的实现位置和机制

Method: 使用基于飞镖形状合成图像的受控感知冲突实验，系统性地遮蔽区域以创建凹性和凸性完形竞争；通过logit归因将内部激活投影到离散视觉码书空间，分析注意力头的直接效应

Result: BEiT在感知冲突中可靠地偏好凸性完形；图形-背景组织在早期和中间层保持模糊，在后期层突然解决；识别出头L0H9作为早期种子引入弱凸性偏置；下调该注意力头可以改变感知冲突的分布质量，使凹性证据引导完形

Conclusion: BEiT中的格式塔凸性先验由特定可识别的功能单元实现，特别是注意力头L0H9作为早期种子；通过调节单个注意力头可以连续控制完形偏好，揭示了视觉变换器中感知组织决策的神经机制
...
### [102] [Physical Simulator In-the-Loop Video Generation](https://arxiv.org/abs/2603.06408)
*Lin Geng Foo,Mark He Huang,Alexandros Lattas,Stylianos Moschoglou,Thabo Beeler,Christian Theobalt*

Main category: cs.CV

TL;DR: PSIVG框架通过将物理模拟器集成到视频扩散过程中，解决AI生成视频违反物理定律的问题，提升运动的一致性和物理合理性。
...
Method: 提出PSIVG框架：1) 从预训练扩散模型生成的模板视频重建4D场景和前景物体网格；2) 在物理模拟器中初始化并生成物理一致的运动轨迹；3) 用模拟轨迹指导视频生成器实现时空物理一致的运动；4) 提出TTCO技术，基于模拟器的像素对应关系调整文本和特征嵌入，改善纹理一致性。

Result: 综合实验表明，PSIVG生成的视频能更好地遵循真实世界物理规律，同时保持视觉质量和多样性。

Conclusion: PSIVG通过将物理模拟器集成到视频生成流程中，有效解决了现有方法违反物理定律的问题，显著提升了生成视频的物理一致性和真实感。
...
TL;DR: CLoPA是一种持续适应策略，通过在标注缓存上微调SAM的一小部分参数，将零射交互分割模型的性能快速提升到专家水平，无需修改推理流程或增加参数。


<details>
  <summary>Details</summary>
Motivation: 现有零射交互分割模型（如SAM）在多样化医学影像任务中无法稳定达到专家级性能，而标注活动产生的任务特定标注数据流为在线模型适应提供了自然补充。

Method: 提出CLoPA持续适应策略：1）仅微调SAM的一小部分参数；2）基于轻量级片段调度在标注缓存上进行训练；3）无需新增参数或修改推理流程；4）完全在现有标注工作流中运行。

Result: 在8个Medical Segmentation Decathlon任务中，CLoPA能快速将性能提升至专家水平，即使对SAM先前失败的任务也有效，多数增益在单个训练片段后实现。不同参数组的调优效果取决于任务特性和数据规模。

Conclusion: CLoPA通过持续适应有效提升交互分割性能，但对于复杂几何结构（如肝血管）的目标，实例归一化和低层特征调优会饱和，表明在最具挑战性场景中需要更深层的特征表示对齐。
...
TL;DR: WanderDream是首个用于心理探索模拟的大规模数据集，包含15.8K全景视频和158K问答对，支持在无法主动探索的受限场景下进行空间推理。


<details>
  <summary>Details</summary>
Motivation: 现实世界中许多场景（如机器人物理限制或视障用户安全考虑）无法进行主动探索，需要开发能够在有限观察下进行心理模拟推理的能力。
...
Conclusion: WanderDream为无法主动探索的场景提供了首个大规模心理探索模拟数据集，证明了心理模拟在空间推理中的重要性，并为世界模型和多模态大语言模型提供了有效的评估基准。
...
TL;DR: CaTok是一种用于图像的1D因果标记器，通过MeanFlow解码器解决现有视觉标记器与自回归语言模型因果预测模式不匹配的问题，支持快速一步生成和高保真多步采样。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型依赖因果标记化，但将其扩展到视觉领域存在挑战。现有视觉标记器要么将2D块展平为非因果序列，要么采用启发式排序与"下一标记预测"模式不匹配。扩散自编码器也存在问题：解码器条件化所有标记缺乏因果性，而应用嵌套dropout机制则引入不平衡。

Method: 提出CaTok，一种1D因果图像标记器，具有MeanFlow解码器。通过在时间间隔内选择标记并将其绑定到MeanFlow目标，学习支持快速一步生成和高保真多步采样的因果1D表示。为进一步稳定和加速训练，提出REPA-A正则化方法，将编码器特征与视觉基础模型对齐。

Result: 在ImageNet重建上达到最先进结果：0.75 FID、22.53 PSNR和0.674 SSIM，且训练轮次更少。自回归模型性能与领先方法相当。

Conclusion: CaTok成功将因果标记化范式扩展到视觉领域，解决了现有方法与非因果序列或启发式排序相关的问题，实现了高效的图像表示和生成，为视觉自回归建模提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型虽然灵活但难以通过提示或简单推理适应来控制，不适用于有严格产品要求的用例，需要构建专门支持图像编辑和增强任务的系统

Method: 首先在多样化多模态数据集上训练基础扩散模型，然后针对特定下游任务快速微调变体，产生专门化模型；包括数据集策划、训练和推理的最佳实践

Result: 在线A/B实验显示增强图像分别获得18.0%和12.5%的参与度提升；与人工评分者比较表明模型在这些任务上优于第三方模型；系统可泛化到多种下游任务

Conclusion: Pinterest Canvas通过基础模型加任务特定微调的方法有效解决了图像生成系统的控制问题，在背景增强和宽高比外绘等用例中表现出色，并能推广到多图像场景合成和图像到视频生成等多样化任务
...
TL;DR: 系统研究去噪生成模型的训练目标，重点关注损失权重和输出参数化，分析这些选择与数据流形内在维度、模型架构和数据量的相互作用


<details>
  <summary>Details</summary>
Motivation: 现有去噪生成模型训练中存在多种设计选择（损失权重、输出参数化），但缺乏对这些因素如何相互作用以及如何影响模型性能的系统理解。研究旨在为实际训练提供指导，而非提出新方法。

Method: 通过系统数值研究，分析训练目标（噪声、干净图像、速度参数化）与数据流形内在维度、模型架构、数据集大小的相互作用。实验涵盖几何可控的合成数据集和图像数据，使用PSNR（去噪精度）和FID（生成质量）进行定量评估。

Result: 研究揭示了不同训练选择之间的复杂相互作用，为流匹配模型训练提供了实用设计指导。具体发现包括不同参数化方案在不同条件下的性能表现，以及损失权重策略对模型性能的影响。

Conclusion: 去噪生成模型的训练效果受多种因素复杂影响，包括输出参数化、损失权重、数据几何特性等。研究为实际训练提供了系统指导，帮助研究人员根据具体数据特性和目标做出更明智的设计选择。
...
TL;DR: GreenRFM是一个资源高效的放射学基础模型预训练框架，通过MUST监督设计而非暴力扩展，在单24GB GPU上24小时内达到SOTA性能，或在6GB VRAM上4小时内匹配现有基准。


<details>
  <summary>Details</summary>
Motivation: 现有放射学基础模型(RFMs)开发过度依赖暴力扩展方法，直接移植自然图像处理方法，导致模型在临床实践中脆弱且昂贵。需要资源高效的方法来促进临床医生在有限硬件条件下的公平发展。

Method: 提出GreenRFM框架，采用MUST(More distilled, Ubiquitous, Semantic-enforcing, Task-aligning)监督设计原则，最大化利用监督信号而非简单堆叠训练数据量。提供两种配置：高性能模型(24GB GPU/24小时)和轻量模型(6GB VRAM/4小时)。

Result: 在超过20万张来自4个机构、2种模态的图像上进行实验。在胸部和腹部CT数据集上，无论公共还是私有基准，均超越一系列基线模型。在内部肌肉骨骼MRI图像上的结果显示相同监督原则可在不同模态间迁移。

Conclusion: GreenRFM的性能和效率挑战了"规模即一切"的教条，使临床医生即使在笔记本电脑上也能公平开发最先进的放射学基础模型，为资源高效的医学AI发展提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 专业领域（如医学影像）的视频标注成本高昂，现有标注传播方法存在局限性：视频跟踪器需要逐视频初始化且无法跨视频泛化；传统对应性方法在低纹理场景表现不佳；密集特征匹配方法缺乏时空平滑性且不支持点与掩码的统一处理。

Method: 提出Match4Annotate框架：1）基于DINOv3特征在测试时拟合SIREN隐式神经表示，生成连续高分辨率时空特征场；2）学习帧对之间的平滑隐式变形场以指导对应性匹配；3）支持点标注和掩码标注的跨视频传播。

Result: 在三个具有挑战性的临床超声数据集上评估：1）跨视频传播达到最先进水平，优于特征匹配和单次分割基线；2）在视频内传播方面与专用跟踪器保持竞争力；3）证明轻量级测试时优化特征匹配流程具有高效可扩展标注的潜力。

Conclusion: Match4Annotate为专业领域视频标注提供了一种高效、可访问的解决方案，通过隐式神经表示和变形场学习实现了点与掩码标注的统一跨视频传播，显著降低了标注成本。
...
TL;DR: Self-Flow：一种自监督流匹配范式，通过双时间步调度在生成框架中集成表征学习，无需外部模型即可实现多模态生成


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部模型存在三个问题：需要单独训练、目标不一致、缩放行为不可预测。这种依赖性源于生成模型的训练目标（去噪任务）缺乏学习语义表征的激励

Method: 提出Self-Flow自监督流匹配范式，核心机制是双时间步调度：在不同token上应用异质噪声水平，创建信息不对称，迫使模型从损坏输入中推断缺失信息，从而同时学习强表征和生成能力

Result: 方法在图像、视频和音频生成上表现优异，遵循预期的缩放定律，支持多模态训练，无需外部监督即可学习语义表征

Conclusion: Self-Flow通过将表征学习整合到生成框架中，解决了现有方法对外部模型的依赖问题，实现了更优的收敛性、生成质量和可预测的缩放行为
...
### [111] [Artificial Intelligence for Detecting Fetal Orofacial Clefts and Advancing Medical Education](https://arxiv.org/abs/2603.06522)
...
TL;DR: AI系统在超声图像上诊断胎儿口面裂，性能匹敌资深放射科医生，显著优于初级医生，并可作为医学副驾驶提升初级医生诊断能力


<details>
  <summary>Details</summary>
Motivation: 口面裂是最常见的先天性颅面异常之一，但产前准确检测具有挑战性，主要原因是经验丰富的专家稀缺且该疾病相对罕见。早期可靠诊断对于及时临床干预和降低相关发病率至关重要。

Method: 开发了一个人工智能系统，基于来自22家医院的9,215名胎儿的45,139张超声图像进行训练。该系统可作为医学副驾驶辅助放射科医生诊断，并通过试点研究评估其对专业知识发展的影响。

Result: AI系统诊断胎儿口面裂的敏感性和特异性分别超过93%和95%，与资深放射科医生性能相当，显著优于初级放射科医生。作为医学副驾驶使用时，可将初级放射科医生的敏感性提高超过6%。试点研究表明该系统能改善罕见疾病专业知识的发展。

Conclusion: 这种双重用途方法为改善诊断准确性和专家培训提供了可扩展的解决方案，特别适用于经验丰富的放射科医生稀缺的环境。AI系统不仅能直接辅助诊断，还能加速临床专业知识的发展。
...
### [112] [AV-Unified: A Unified Framework for Audio-visual Scene Understanding](https://arxiv.org/abs/2603.06530)
...
<details>
  <summary>Details</summary>
Motivation: 当前音频-视觉场景理解任务（如事件定位、解析、分割、问答等）大多独立研究，难以全面理解复杂场景和探索任务间关系，需要统一的框架进行联合学习。

Method: 1) 标准化所有任务的输入输出格式为离散token序列；2) 设计多尺度时间感知模块捕捉不同粒度的事件；3) 设计基于跨模态引导的空间感知模块建模空间关联；4) 使用任务特定文本提示增强任务适应性。

Result: 在多个基准数据集（AVE、LLP、MUSIC-AVQA、VGG-SS、AVS）上的实验表明，AV-Unified在时间、空间和时空任务上均表现出有效性。

Conclusion: AV-Unified通过统一框架实现了多种音频-视觉任务的联合学习，有效捕捉了跨模态关联，为全面理解复杂音频-视觉场景提供了系统解决方案。
...
TL;DR: 提出一种针对漫反射直接飞行时间LiDAR的空间校准方法，通过扫描反光片恢复每个像素在RGB图像平面中的足迹和相对空间灵敏度
...
Method: 使用扫描反光片结合背景减法，恢复每个像素的响应图，估计每个漫反射LiDAR像素在共置RGB图像平面中的足迹（有效支撑区域）和相对空间灵敏度

Result: 该方法在ams OSRAM TMF8828上得到验证，为跨模态对齐和融合提供了明确的LiDAR到RGB对应关系

Conclusion: 提出的空间校准程序能够有效解决漫反射LiDAR与RGB相机校准的问题，为多模态感知系统提供准确的跨模态对齐基础
...
<details>
  <summary>Details</summary>
Motivation: 否定是基本的语言运算符，但在基于扩散的生成系统中建模不足。现有方法缺乏对语言否定的正式处理，无法统一处理不同类型的否定现象。

Method: 将语言否定建模为扩散动态中语义引导的结构化可行性约束。将无分类器引导重新解释为定义语义更新方向，通过将更新投影到从语言结构导出的凸约束集上来强制执行否定。该方法无需训练，与预训练的扩散主干兼容。

Result: 实验表明该方法实现了稳健的否定合规性，同时保持了视觉保真度和结构连贯性。建立了扩散生成模型中语言否定的首个统一公式，超越了表示级评估。

Conclusion: 该工作为扩散生成模型中的语言否定提供了首个统一的形式化处理框架，能够处理多样化的否定现象，并自然地扩展到视频生成。还引入了结构化的否定中心基准套件来评估生成系统。
...
TL;DR: 该研究分析了自动驾驶数据集中冗余作为数据质量因素的影响，通过多源相机和多模态图像-LiDAR数据的冗余建模与测量，发现选择性去除冗余标签能提升目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶研究过度关注算法设计而忽视数据质量分析，特别是冗余这一基础但未被充分探索的数据质量问题。多源多模态数据中由于环境条件和传感器限制导致数据质量参差不齐，冗余可能影响模型性能。

Method: 使用nuScenes和Argoverse 2数据集，建模并测量多源相机数据（共享视野的相机）和多模态图像-LiDAR数据中的冗余。通过选择性去除冗余标签，评估对YOLOv8目标检测任务的影响。

Result: 实验表明：1）选择性去除共享视野相机的冗余图像目标标签能提升检测性能（nuScenes中mAP50从0.66提升至0.70等）；2）AV2中去除4.1-8.6%标签后mAP50保持在0.64基线附近；3）多模态分析显示图像与LiDAR数据间存在显著冗余。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLM发展主要依赖模型规模扩展，这限制了在计算资源受限的移动和边缘设备上的部署。同时，主流VLMs依赖通过大规模对比预训练（如CLIP/SigLIP）初始化的视觉编码器，但对比学习优化的判别性目标与密集描述和复杂推理所需的细粒度视觉线索存在目标不匹配问题。

Method: 提出Penguin-VL，其视觉编码器从纯文本LLM初始化，而非传统的对比预训练视觉编码器。这种方法解决了对比学习导致的粗粒度、类别级不变性对细粒度视觉线索的抑制问题，实现了更高的视觉保真度和数据效率。
...
Conclusion: Penguin-Encoder作为传统对比预训练编码器的优越替代方案，保留了密集感知和复杂推理所需的关键细粒度空间和时间线索。这使其成为计算高效VLMs的强大即插即用替代方案，能够在资源受限环境中实现高性能。
...
TL;DR: SUREON是一个大规模外科视频问答数据集，通过从外科教学视频中提取专家讲解来训练AI理解外科推理过程，包括意图、风险评估和预测等复杂问题。


<details>
  <summary>Details</summary>
Motivation: 当前外科AI只能识别手术器械等基础信息，无法理解外科医生的决策意图、风险评估和预测等推理过程。专家讲解的外科教学视频包含了这些推理信息，但缺乏大规模结构化标注数据。

Method: 从外科学术视频中系统性地提取监督信号，定义12个问题类别（安全评估、决策原理、预测等），使用多智能体管道进行大规模提取和结构化处理，构建包含134.7K视频片段、170种手术类型、206.8K问答对的数据集。

Result: 开发了SureonVLM（监督微调）和SureonVLM-R1（Group Relative Policy Optimization训练）两个模型，在SUREON基准测试中准确率超过84%，在标准外科感知任务上优于通用领域大模型，并能展示显式推理行为。

Conclusion: 通过从外科教学视频中提取专家讲解，可以大规模训练AI理解外科推理过程，显著提升外科AI的复杂问题回答能力和推理能力，为外科AI从感知向理解转变提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将多视角多帧图像独立输入LLMs导致计算冗余和空间一致性不足，而BEV表示虽有空间结构但缺乏语义丰富性，需要桥接这一差距

Method: 提出BEVLM框架，将空间一致且语义蒸馏的BEV表示与LLMs连接，使用BEV特征作为统一输入，并从LLMs向BEV表示蒸馏语义知识

Result: BEVLM使LLMs在跨视角驾驶场景中的推理准确率提升46%，通过语义蒸馏将闭环端到端驾驶性能在安全关键场景中提升29%

Conclusion: BEVLM成功桥接了空间一致的BEV表示与语义丰富的LLMs，显著提升了自动驾驶中的3D空间推理和决策能力
...
TL;DR: Omni-Diffusion是基于掩码离散扩散模型的首个任意到任意多模态语言模型，统一了文本、语音和图像的理解与生成任务。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要采用自回归架构，在架构设计上存在探索有效替代方案的空间。离散扩散模型在视觉理解和图像生成等领域展现出潜力，可作为多模态系统的有前景骨干。

Method: 采用统一的掩码离散扩散模型直接捕捉离散多模态标记的联合分布，支持双模态和多模态任务，完全基于掩码离散扩散模型构建。

Result: 在多样化基准测试中，该方法优于或与现有处理两个或更多模态的多模态系统表现相当，展现了扩散模型作为下一代多模态基础模型的显著潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前关于MLLM与监督学习模型在分类任务上的比较研究存在矛盾结论，这些矛盾源于评估协议的不完善（如输出丢弃、选择题干扰项过弱、输出映射问题）以及标注质量被忽视的影响。

Method: 1）识别并修正三种常见评估协议的关键问题；2）量化批次大小、图像顺序、文本编码器选择等被忽视的设计选择的影响；3）创建ReGT数据集（ImageNet-1k的625类多标签重标注）；4）进行控制性案例研究评估MLLM辅助人工标注的效果。

Result: 1）修正标注后MLLM性能提升高达+10.8%，显著缩小与监督模型的感知差距；2）评估协议缺陷会夸大或低估性能；3）设计选择对准确率有实质性影响；4）MLLM在约50%的困难案例中能有效辅助人工标注。

Conclusion: MLLM在分类任务中的表现不佳主要是评估协议缺陷和标注噪声的产物，而非模型固有缺陷。MLLM对标注质量敏感，且能有效辅助大规模数据集标注工作。
...
<details>
  <summary>Details</summary>
Motivation: 数字时代假新闻泛滥严重威胁公众信任和民主制度，现有浏览器扩展存在模型行为不透明、解释支持有限、用户参与度不足等问题，需要开发更有效、透明和用户中心的检测工具。

Method: 提出Aletheia浏览器扩展，采用检索增强生成(RAG)和大语言模型(LLM)检测假新闻并提供基于证据的解释；系统包含两个交互组件：讨论中心支持用户围绕标记内容进行对话，及时信息功能展示近期事实核查结果。

Result: 实验表明Aletheia在检测性能上优于最先进的基线方法；补充性用户研究（250名参与者）证实了系统的可用性和感知有效性，突显其作为透明工具对抗在线假新闻的潜力。

Conclusion: Aletheia通过结合RAG、LLM和交互功能，提供了一种有效、透明且用户中心的假新闻检测解决方案，在技术和用户体验层面均表现出色，有望成为对抗在线虚假信息的实用工具。
...
TL;DR: 论文研究语法约束解码(GCD)，证明了语法等价性定理，分析了结构歧义成本(SAC)，建立了引擎无关的下界，定义了解码成本等价类，推导了条件采样器的理论界限，并与Transformer架构集成。


<details>
  <summary>Details</summary>
Motivation: 研究语法约束解码中语法等价性对解码效率的影响，特别是不同但语言等价的上下文无关文法如何导致不同的编译状态空间和在线歧义成本，旨在建立理论框架以优化语法约束解码的性能。

Method: 将GCD建模为自回归下一个令牌分布与基于CFG编译的下推系统可达性预言机的耦合；证明预言机不变性定理；引入结构歧义成本(SAC)度量增量打包解析森林增长；建立引擎无关下界；定义解码成本等价类；推导条件采样器的Doob h-变换和KL/总变差失真界限。

Result: 证明了语言等价文法产生相同的下一个令牌掩码但可导致不同的状态空间和歧义成本；给出了典型语言a^n b^n在冗余非终结符委托下的精确控制状态膨胀计数；发现右递归下SAC为O(1)而连接下为Θ(t^2)；建立了任何在线掩码引擎在特定CFG族上必须承担Ω(t^2)工作的下界；证明了有界重写族中存在最小SAC代表。

Conclusion: 语法约束解码的性能不仅取决于语言等价性，还受文法结构影响；结构歧义成本是解码效率的关键因素；存在理论下界限制在线掩码引擎的效率；通过文法优化可最小化SAC；理论结果与Transformer和MoE架构集成，为自动化语法优化提供了理论基础。
...
TL;DR: NOTAI.AI是一个可解释的机器生成文本检测框架，通过结合曲率信号、神经特征和文体特征，在监督设置下扩展Fast-DetectGPT，使用XGBoost元分类器和SHAP解释提供透明检测。


<details>
  <summary>Details</summary>
Motivation: 开发一个可解释的机器生成文本检测系统，不仅能够准确区分人类和AI生成的文本，还能提供透明的决策依据，帮助用户理解检测结果背后的特征贡献。

Method: 1. 集成17个可解释特征：包括条件概率曲率、ModernBERT检测器分数、可读性指标和文体特征；2. 使用梯度提升树(XGBoost)作为元分类器；3. 应用SHAP进行特征归因；4. 通过LLM解释层将特征归因转化为结构化自然语言解释；5. 部署为交互式Web应用。

Result: 开发了完整的NOTAI.AI系统，包括：1. 集成多种检测特征的分类框架；2. 局部和全局特征归因能力；3. 结构化自然语言解释生成；4. 交互式Web应用支持实时分析和可视化；5. 公开源代码和演示视频确保可复现性。

Conclusion: NOTAI.AI成功构建了一个可解释的机器生成文本检测框架，通过多特征集成和透明解释机制，在保持检测性能的同时提供了用户友好的解释界面，促进了检测系统的可信度和实用性。
...
TL;DR: CoT提示增强LLM推理但增加隐私风险，通过框架量化11类PII泄漏，发现泄漏随推理预算变化且模型依赖性强，评估多种轻量级门控方法但无单一最优方案。


<details>
  <summary>Details</summary>
Motivation: 研究CoT提示如何增加LLM推理过程中的PII泄漏风险，即使模型被指示不重复PII信息。需要量化这种推理时的直接PII泄漏，并比较不同模型家族的表现。

Method: 提出模型无关框架：(1) 基于11类PII类型定义风险加权的token级泄漏事件；(2) 追踪泄漏曲线随CoT预算的变化；(3) 在结构化PII数据集上比较开源和闭源模型家族，使用分层风险分类法。评估四种轻量级门控方法：基于规则的检测器、TF-IDF+逻辑回归分类器、GLiNER-based NER模型和LLM-as-judge。

Result: CoT持续增加泄漏风险，特别是高风险类别；泄漏强烈依赖于模型家族和推理预算；增加推理预算可能放大或减弱泄漏，取决于基础模型；没有单一门控方法在所有模型或预算上表现最优，需要混合、风格自适应的门控策略。

Conclusion: CoT提示确实增加PII泄漏风险，泄漏模式具有模型和预算依赖性。需要开发混合、自适应的门控策略来平衡效用和风险，并建立可复现的通用评估协议。
...
TL;DR: 研究通过扰动框架测试LLM道德判断的稳定性与可操纵性，发现视角转换比表面编辑更能改变判断，协议选择是最大影响因素，表明LLM道德判断受叙事形式和任务框架共同影响。


<details>
  <summary>Details</summary>
Motivation: 人们越来越多地使用大型语言模型获取道德和人际指导，但这些系统无法审视缺失的上下文，只能基于给定的困境进行判断。需要测试LLM道德判断的稳定性和可操纵性，以评估其可靠性。

Method: 引入扰动框架测试LLM道德判断稳定性，使用2,939个来自r/AmItheAsshole的道德困境，生成三类内容扰动：表面编辑（词汇/结构噪声）、视角转换（声音和立场中立化）、说服线索（自我定位、社会证明、模式承认、受害者框架）。同时改变评估协议（输出顺序、指令位置、非结构化提示）。使用四个模型（GPT-4.1、Claude 3.7 Sonnet、DeepSeek V3、Qwen2.5-72B）进行评估（N=129,156个判断）。

Result: 表面扰动产生低翻转率（7.5%），基本在自一致性噪声范围内（4-13%），而视角转换导致显著更高的不稳定性（24.3%）。37.9%的困境对表面噪声稳健但在视角变化下翻转，表明模型将叙事声音作为语用线索。不稳定性集中在道德模糊案例中；无人被指责的情景最易受影响。说服扰动产生系统性方向性偏移。协议选择是所有因素中影响最大的：结构化协议间一致性仅67.6%（kappa=0.55），仅35.7%的模型-场景单元在所有三种协议中匹配。

Conclusion: LLM道德判断由叙事形式和任务框架共同产生，当结果依赖于呈现技巧而非道德实质时，会引发可重复性和公平性问题。这表明当前LLM的道德判断系统存在重大缺陷。
...
TL;DR: 该论文使用RGB基准评估RAG系统在不同场景下的鲁棒性，比较了RGB RAG基线与基于知识图谱的GraphRAG系统，并测试了三种GraphRAG定制化方案以提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然能增强LLM的能力，但检索到的不一致信息可能对LLM响应产生负面影响。需要评估RAG系统在这种条件下的鲁棒性，以设计更可靠的现实世界应用系统。

Method: 使用RGB基准语料库，在四种场景下评估LLMs：噪声鲁棒性、信息整合、负面拒绝和反事实鲁棒性。进行RGB RAG基线与基于知识图谱的GraphRAG系统的比较分析，并测试三种GraphRAG定制化方案。

Result: 结果显示GraphRAG定制化方案相比RGB基线有所改进，为设计更可靠的现实世界RAG系统提供了见解。

Conclusion: 基于知识图谱的RAG系统定制化能够提升在噪声、不一致信息等挑战性场景下的鲁棒性，这对开发可靠的现实世界RAG应用具有重要意义。
...
TL;DR: 通过全球调查评估不同文化群体对GenAI文化表征的理解与期望，提出文化敏感的开发建议


<details>
  <summary>Details</summary>
Motivation: 缺乏关于全球对GenAI应如何表征文化的态度和信念的实证证据，需要了解不同群体对文化的定义及其与GenAI表征的关系

Method: 进行大规模全球调查，覆盖欧洲、北美、南美、亚洲和非洲部分地区，收集关于文化含义以及GenAI应如何表征文化制品、概念或价值的数据，从社区直接提炼文化的工作定义

Result: 建立了对文化概念复杂性及其与生成式AI表征关系的理解，获得了不同群体对文化的定义，识别了文化表征的关键维度
...
<details>
  <summary>Details</summary>
Motivation: Transformer架构在扩展时伴随着显著的参数增长和嵌入维度冗余，需要更高效的参数利用方法。

Method: 基于三阶张量的L-积进行结构化谱分解，将token表示重塑为谱张量切片，在变换域执行注意力与前馈操作，构建张量Transformer架构。

Result: 在IMDB和AG News数据集上，编码器参数最多减少75%（p=4）的同时保持竞争力精度；在BERT-base宽度下性能恢复至同等水平。

Conclusion: 提出的L-Transformer通过谱分解实现参数高效压缩，同时引入频率相关的归纳偏置，在保持性能的同时显著减少模型参数。
...
TL;DR: 本文以巴拉圭官方语言瓜拉尼语为例，指出当前AI系统以文本为中心的设计忽视了口语优先语言和土著社区的需求，提出了一种口语优先的多智能体架构作为替代方案。


<details>
  <summary>Details</summary>
Motivation: 当前人工智能和人机交互系统通常以文本为中心，忽视了口语优先语言和土著社区的实际需求。瓜拉尼语作为巴拉圭广泛使用的官方语言，其口语实践与文本中心的设计模式存在根本性不匹配，这反映了AI语言支持在文化适应性方面的不足。

Method: 提出一种口语优先的多智能体架构，替代传统的"文本转语音"流程。该架构将瓜拉尼语自然语言理解与专门的对话状态智能体、社区主导的治理智能体解耦，创建尊重土著数据主权和双语现象的技术框架。

Result: 开发了一个技术框架，不仅关注语言识别，更注重对话中的轮转、修复和共享上下文等核心互动要素。该框架能够更好地适应瓜拉尼语的口语实践，确保数字生态系统赋能而非忽视多样化的语言实践。

Conclusion: 要使AI真正具有文化基础，必须从将口语语言适应文本中心系统，转变为将口语对话作为首要设计需求。只有通过尊重土著数据主权和双语现象的口语优先方法，才能创建真正包容和赋能的数字生态系统。
...
TL;DR: NERdME数据集包含200个手动标注的README文件，包含10,000+标注跨度和10种实体类型，用于填补学术信息抽取在代码仓库实现层面细节的空白。


<details>
  <summary>Details</summary>
Motivation: 现有学术信息抽取数据集主要关注科学论文，忽略了代码仓库中的实现层面细节。README文件描述数据集、源代码和其他实现层面工件，但其自由格式的Markdown缺乏语义结构，使得自动信息抽取变得困难。

Method: 引入NERdME数据集：包含200个手动标注的README文件，超过10,000个标注跨度和10种实体类型。使用大型语言模型和微调transformer模型建立基线结果。进行下游实体链接实验，展示README实体如何支持工件发现和元数据集成。

Result: 基线结果显示论文层面和实现层面实体之间存在明显差异，表明扩展SIE基准以包含README文件中可用的实体类型具有价值。实体链接实验证明从README派生的实体可以支持工件发现和元数据集成。

Conclusion: NERdME数据集填补了学术信息抽取在实现层面细节的空白，README文件中的实体类型对扩展SIE基准具有重要价值，能够支持代码仓库工件的发现和元数据集成。
...
TL;DR: PVminerLLM：通过监督微调的大型语言模型，从患者生成文本中结构化提取患者声音信号，显著优于基于提示的方法，实现高精度代码、子代码和证据跨度提取。


<details>
  <summary>Details</summary>
Motivation: 患者生成文本包含关键的生活经历、社会环境和护理参与信息，这些因素强烈影响依从性、护理协调和健康公平。然而，这些患者声音信号很少以结构化形式可用，限制了其在以患者为中心的结果研究和临床质量改进中的应用。因此，可靠提取此类信息对于大规模理解和解决非临床健康结果驱动因素至关重要。

Method: 引入PVminer基准用于患者声音的结构化提取，并提出PVminerLLM——专门为此任务设计的监督微调大型语言模型。该方法在多个数据集和模型规模上进行评估，包括代码预测、子代码预测和证据跨度提取任务。

Result: PVminerLLM显著优于基于提示的基线方法，在代码预测上达到83.82% F1分数，子代码预测80.74% F1，证据跨度提取87.03% F1。值得注意的是，即使使用较小的模型也能实现强大性能，表明可靠的患者声音提取无需极端模型规模即可实现。

Conclusion: PVminerLLM实现了从患者生成文本中大规模分析嵌入的社会和体验信号，为患者为中心的结果研究和临床质量改进提供了可行的技术解决方案。代码、评估脚本和训练模型将公开发布，标注数据集可根据研究请求提供。
...
<details>
  <summary>Details</summary>
Motivation: 现有图结构推理方法（如ToT、GoT、AGoT）虽然能提升某些基准测试的准确率，但引入了大量计算开销和延迟，且性能增益不稳定。研究发现推理管道中存在阶段和节点异质性：高质量规划和最终综合需要强大模型，而许多中间子任务可由轻量级模型高效解决。

Method: 提出RouteGoT框架，包含：1）图内路由机制，优先为规划和综合节点分配强大模型，为叶子子任务动态分配轻量级模型和成本效益策略；2）集成显式预算约束的全局推理调度器，控制图扩展在用户指定的令牌预算内；3）基于预测难度的节点自适应路由。

Result: 在推理、检索和多跳QA基准测试中，RouteGoT匹配或提高了准确率，同时显著减少令牌使用：相比AGoT平均提升8.1个百分点准确率，减少79.1%输出令牌。在成本-准确率权衡方面优于现有路由基线，在不同预算目标和任务下表现出更好的鲁棒性。

Conclusion: RouteGoT通过节点自适应路由和预算控制，有效解决了图结构推理中的计算效率问题，实现了可预测的性能-成本权衡，为复杂推理任务提供了更实用的解决方案。
...
Method: HART将幻觉追踪形式化为结构化建模任务，包含四个阶段：1)跨度定位：识别文本中的幻觉片段；2)机制归因：确定幻觉产生的错误机制；3)证据检索：获取支持或反对幻觉的外部证据；4)因果追踪：建立幻觉与证据之间的因果关系。基于此框架构建了首个针对幻觉追踪的结构化数据集，联合标注幻觉类型、错误机制和反事实证据集合。

Result: 在提出的数据集上的实验结果表明，HART显著优于包括BM25和DPR在内的强检索基线，验证了所提追踪范式在幻觉分析和证据对齐方面的有效性和泛化能力。

Conclusion: HART通过结构化建模方法解决了现有幻觉归因方法的局限性，提供了细粒度的幻觉追踪框架，增强了幻觉片段的可解释性和证据的可追溯性，为LLM可靠性评估提供了新的分析工具。
...
TL;DR: ReflexiCoder是一个基于强化学习的代码生成框架，通过将结构化推理轨迹（生成、反思、修正）内化到模型权重中，实现无需外部依赖的自主调试能力，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代码生成方法（"System 1"单次前向生成）在处理复杂算法任务时存在性能瓶颈。现有的迭代优化策略依赖外部oracle、执行反馈或计算昂贵的提示-响应循环，缺乏完全自主的内在反思修正能力。

Method: 提出ReflexiCoder强化学习框架，采用RL-zero训练范式，通过细粒度奖励函数优化整个反思-修正轨迹。该方法将结构化推理轨迹（初始生成、bug和优化感知的反思、自我修正）内化到模型权重中，实现无需真实反馈或执行引擎的自主调试能力。

Result: ReflexiCoder-8B在7个基准测试中建立了新的SOTA：HumanEval 94.51%（Plus 87.20%）、MBPP 81.80%（Plus 78.57%）、BigCodeBench 35.00%、LiveCodeBench 52.21%、CodeForces 37.34%。推理时计算开销降低约40%，性能可与GPT-5.1等专有模型媲美或超越。

Conclusion: ReflexiCoder将代码生成范式从外部依赖优化转向内在自主反思修正，通过强化学习内化推理轨迹，实现了高性能、高token效率的代码生成，为复杂算法任务提供了新的解决方案。
...
TL;DR: ROSE是一种改进的LLM剪枝方法，通过重新排序剪枝顺序来提升SparseGPT的性能，特别针对具有列模式的权重矩阵。
...
Method: 提出ROSE方法：1) 预剪枝识别候选权重并估计列和块剪枝损失；2) 两级重新排序：块内按列损失降序重排列，块间按块损失重排；3) 引入块损失相对范围指标识别列层，实现全模型自适应重排序。

Result: 在主流LLM（LLaMA2-7B/13B/70B、LLaMA3-8B、Mistral-7B）上的大量实验表明，ROSE超越了原始SparseGPT和其他剪枝方法。

Conclusion: 剪枝顺序对LLM剪枝性能有重要影响，ROSE通过重新排序剪枝顺序有效提升了SparseGPT框架的性能，特别适用于具有列模式的权重矩阵。
...
TL;DR: CoCA框架通过分段信用分配联合优化置信度校准和答案准确性，实现置信度优先的LLM不确定性估计


<details>
  <summary>Details</summary>
Motivation: 现有LLM不确定性估计方法多为答案优先范式，即在生成答案后才产生置信度，这限制了实际可用性。需要研究置信度优先范式，让模型在回答前输出其正确概率。

Method: 提出CoCA框架，采用GRPO强化学习方法，通过分段信用分配联合优化置信度校准和答案准确性。为置信度和答案段分配独立的奖励和组相对优势，实现稳定联合优化并避免奖励攻击。

Result: 在数学、代码和事实问答基准测试中，CoCA在保持答案质量的同时，显著改进了校准效果和不确定性区分能力，为下游应用提供了更可靠的不确定性估计。

Conclusion: 置信度优先范式通过CoCA框架的联合优化，能够提供更准确的不确定性估计，增强了LLM部署的可靠性，支持更广泛的下游应用。
...
TL;DR: 传统BPE分词方法对黏着语不适用，导致词素边界被破坏和token数量膨胀


<details>
  <summary>Details</summary>
Motivation: 当前主流的分词方法BPE及其变体本质上是脚本无关的，主要针对英语等屈折语优化。对于黏着语（如达罗毗荼语系、突厥语系、乌拉尔语系、韩语、日语等），一个单词可能编码词根、时态、体、人称、数、性别一致、格和后置词等多种语法信息，统计分词器会将这些单词分割成破坏词素边界的字节对片段，导致token数量膨胀。

Method: 论文未在摘要中明确说明具体方法，但暗示需要针对黏着语特点设计新的分词方法

Result: 摘要未提供具体实验结果，但指出了BPE分词在黏着语处理中的根本问题

Conclusion: 需要为黏着语设计专门的分词方法，以保留词素边界并减少token数量膨胀问题
...
TL;DR: 提出了ConStory-Bench基准和ConStory-Checker自动化检测管道，用于评估长文本故事生成中的叙事一致性，包含2000个提示、5大类19子类错误分类，发现事实和时间维度错误最常见且多出现在文本中部。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成长篇叙事时经常出现前后不一致的问题，包括自相矛盾的事实、人物特征和世界规则。现有的故事生成基准主要关注情节质量和流畅度，而忽略了一致性错误的系统性评估。

Method: 开发了ConStory-Bench基准，包含2000个提示，覆盖四种任务场景，定义了5大类19子类的错误分类体系。同时构建了ConStory-Checker自动化管道，能够检测矛盾并为每个判断提供明确的文本证据支持。

Result: 评估多个LLM后发现：一致性错误在事实和时间维度最常见；错误多出现在叙事中部；错误文本段具有更高的token级熵；某些错误类型倾向于同时出现。这些发现为改进长文本叙事生成的一致性提供了指导。

Conclusion: ConStory-Bench填补了长文本故事生成中一致性评估的空白，ConStory-Checker提供了自动化的检测方法。研究揭示了LLM在叙事一致性方面的系统性模式，为未来改进模型的一致性能力提供了重要见解。
...
TL;DR: 本文提出了一种基于LLM的集成系统，用于对联合国安理会决议进行语义标注，通过引入CPR和TWF评估指标来避免幻觉问题，GPT-4.1在各项任务中表现最佳，而GPT-4.1-mini以20%成本实现可比性能。


<details>
  <summary>Details</summary>
Motivation: 利用LLM性能变异性构建集成系统，完成数据清洗和语义标注任务，同时避免LLM常见的幻觉、不必要添加或遗漏问题，为具有挑战性的文本创建可靠的语义标注系统。

Method: 提出基于LLM的集成系统方法，引入两个评估指标：内容保持率（CPR）和标签格式良好性（TWF），通过多个GPT模型多次运行并选择最佳输出来构建系统。

Result: GPT-4.1在两个任务中均获得最高指标（清洗：CPR 84.9%；语义标注：CPR 99.99%和TWF 99.92%）。GPT-4.1-mini以20%成本实现可比性能。集成系统能够为所有LLM模型选择最优输出。

Conclusion: 通过集成设计和评估指标的使用，成功创建了可靠的LLM系统，能够在具有挑战性的文本上执行语义标注任务，同时平衡了性能与成本效益。
...
TL;DR: 论文提出下一代动作预测任务，通过标注360K个手机使用动作构建数据集，开发LongNAP模型结合参数化和上下文学习预测用户下一步动作，在LLM评估中显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 真正的主动AI系统需要预测用户下一步动作，这需要比稀疏提示更丰富的上下文信息。当前方法缺乏对用户多模态交互历史（屏幕截图、点击、传感器数据）的全面推理能力。

Method: 1) 使用视觉语言模型标注纵向自然计算机使用数据，发布开源标注流程；2) 提出LongNAP用户模型，结合参数化学习和上下文学习，通过策略梯度方法训练生成用户特定推理轨迹、检索相关历史轨迹并在上下文中应用以预测未来动作。

Result: 标注了20名用户1个月连续手机使用的360K个动作（1800小时屏幕时间）。LongNAP在留出数据上显著优于监督微调和提示基线（分别提升79%和39%），能泛化到未见用户。17.1%的预测轨迹与用户实际动作良好对齐（LLM-judge评分≥0.5），高置信度预测时达到26%。

Conclusion: 通过学习用户行为的完整上下文来预测用户需求已成为可行任务，具有重大机会。LongNAP展示了在无限可能动作空间中预测用户行为的潜力。
...
TL;DR: 该研究探讨了在大型语言模型（8B Llama）中解决生态谬误问题的方法，通过建模作者的语言上下文（HuLM任务）来提升模型性能，包括预训练和微调阶段的改进。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型训练和推理忽略了语言使用的基本事实：同一作者生成的多个文本序列之间存在依赖关系。先前研究表明解决这种生态谬误可以显著提升较小GPT模型的性能，本研究旨在验证这种方法是否同样适用于更大规模的8B Llama模型。

Method: 提出HuLM（Human-aware Language Modeling）方法，在作者其他时间排序文本的上下文中处理语言。研究两种变体：1）使用HuLM目标进行预训练；2）在微调阶段使用作者上下文（HuFT: Human-aware Fine-Tuning）。采用QLoRA技术进行高效参数微调。

Result: 仅通过QLoRA进行微调阶段处理生态谬误就能提升8B模型性能。基于QLoRA的持续HuLM预训练产生了人类感知模型，在八个下游任务中仅通过线性任务分类器训练就实现了性能提升。

Conclusion: 建模语言在其原始生成者（作者）的上下文中具有实用性和重要性，解决生态谬误问题能够提升大规模语言模型的性能，为语言建模提供了新的研究方向。
...
TL;DR: 提出结构化风格重写框架，通过显式解耦词汇、句法和语用三个风格维度，结合CoT蒸馏的隐式风格条件策略，使小型语言模型能够生成高保真风格化文本，在动漫角色领域超越更大基线的性能。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型在角色扮演中难以生成高度风格化的人物角色，主要面临数据稀缺和风格解耦复杂性的挑战。标准的监督微调往往只能捕捉表层语义，无法复现角色的复杂句法和语用细微差别，导致"角色偏离"的生成问题。

Method: 提出结构化风格重写框架，将风格显式解耦为三个可解释维度：1) 词汇特征（通过PMI提取），2) 句法模式（基于PCFG规则），3) 语用风格。同时引入基于思维链蒸馏的隐式风格条件策略，利用训练期间的显式推理轨迹作为强归纳偏置，使模型潜在表示与结构化风格特征对齐。

Result: 在动漫角色这一高风格化领域的广泛实验中，该方法使Qwen-1.7B模型在风格一致性和语义保真度方面显著超越更大的基线模型（如4B标准监督微调）。

Conclusion: 该方法提供了一种数据高效的范式，能够在消费级硬件上实现高质量的推理和部署，为小型语言模型的高保真风格化生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 心理健康是动态过程，受个体特质与情境交互影响。现有研究缺乏将心理理论与计算建模结合来理解动态心理状态的方法，需要开发既能预测又能解释的模型。

Method: 整合个体层面心理特质（韧性、认知扭曲、内隐动机）与基于Situational 8 DIAMONDS框架的语言推断情境特征。比较理论驱动特征与心理测量学语言模型嵌入，后者捕捉时间性和个体特异性模式。

Result: 理论驱动特征在预测幸福感方面表现出竞争性性能，同时提供更好的可解释性。定性分析显示预测性最强的特征具有心理一致性，验证了理论框架的有效性。

Conclusion: 将计算建模与心理理论整合对于在情境敏感且人类可理解的方式下评估动态心理状态具有重要价值，理论驱动方法在保持可解释性的同时能达到良好预测性能。
...
TL;DR: Track-SQL框架通过双提取模块增强生成语言模型，解决多轮Text-to-SQL中上下文和模式链接的复杂性问题，在SparC和CoSQL数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 生成语言模型在单轮Text-to-SQL中表现良好，但在多轮场景下性能不足，主要原因是难以处理上下文信息的复杂性和动态模式链接问题。
...
Conclusion: Track-SQL通过增强生成语言模型对多轮交互中模式和上下文变化的跟踪能力，显著提升了多轮Text-to-SQL的性能，代码将开源。
...
TL;DR: MASFactory：一个图中心框架，用于编排基于LLM的多智能体系统，通过Vibe Graphing将自然语言意图编译为可编辑工作流规范，提供可复用组件和可插拔上下文集成。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的多智能体系统框架在实现复杂图工作流时需要大量手动工作，复用性有限，且难以集成异构外部上下文源，限制了系统的可扩展性和易用性。

Method: 提出MASFactory框架，核心是Vibe Graphing方法——一种人在回路的方法，将自然语言意图编译为可编辑的工作流规范，再转换为可执行图。框架提供可复用组件、可插拔上下文集成以及可视化工具，支持拓扑预览、运行时跟踪和人在回路交互。

Result: 在七个公共基准测试上评估MASFactory，验证了代表性MAS方法的复现一致性以及Vibe Graphing的有效性。代码和演示视频已公开。

Conclusion: MASFactory通过图中心的方法解决了当前LLM-based MAS框架在复杂工作流实现、复用性和上下文集成方面的局限性，提供了更高效、可扩展的编排解决方案。
...
Method: 提出ViewFusion两阶段框架：第一阶段进行深思熟虑的空间预思考，推断跨视图的视角关系和空间变换，形成超越简单重描述的中间工作空间；第二阶段基于该工作空间进行问题驱动的推理以生成最终预测。使用合成推理监督训练，随后通过GRPO进行强化学习

Result: 在MMSI-Bench上，ViewFusion相比Qwen3-VL-4B-Instruct准确率提升5.3%，在需要真正跨视图对齐的示例上提升最大

Conclusion: 通过显式分离跨视图空间预对齐与问答，ViewFusion能够更有效地利用多视图信息，显著提升多视图空间推理性能，特别是在需要跨视图对齐的任务上
...
TL;DR: 研究评估了四种开源大语言模型在奥地利德语A级考试作文评分中的应用，发现模型在评分细则维度上与人类评分者的一致性最高仅40.6%，最终成绩匹配率仅32.8%，表明当前模型尚无法用于实际评分环境。


<details>
  <summary>Details</summary>
Motivation: 自动化作文评分系统旨在减轻教师评分负担并减少主观偏见。随着大语言模型的发展，研究者希望探索开源模型在德语作文评分中的应用潜力，特别是基于评分细则的评估方法。

Method: 使用101份奥地利德语A级考试匿名学生试卷（涵盖三种文本类型），评估了四种开源大语言模型：DeepSeek-R1 32b、Qwen3 30b、Mixtral 8x7b和LLama3.3 70b。采用不同上下文和提示策略，重点关注基于评分细则的评估方法。

Result: 模型在评分细则各维度上与人类评分者的最高一致性为40.6%，最终成绩与人类专家评分的匹配率仅为32.8%。结果表明，尽管较小模型能够使用标准化评分细则进行德语作文评分，但准确性不足以应用于实际评分环境。

Conclusion: 当前开源大语言模型在德语作文评分中的表现有限，无法达到实际应用所需的准确性水平。虽然模型能够应用标准化评分细则，但与人类评分的一致性较低，表明需要进一步改进才能在实际教育环境中使用。
...
<details>
  <summary>Details</summary>
Motivation: 人类问题解决受益于多样化的风格和人格特质，但当前大语言模型的发展主要关注统一的性能基准，偏向特定的行为倾向（如自信）。研究旨在探索不同经验如何塑造机器人格并影响问题解决能力。
...
Conclusion: 研究为"人格工程"提供了路线图，表明通过控制训练数据的语言学特征可以定向塑造模型人格特质，从而优化特定类型的问题解决能力。这为开发具有多样化问题解决风格的AI系统提供了理论基础。
...
TL;DR: 提出一个集成LLM和神经符号推理的流水线，用于处理文本中的省略三段论（enthymemes），包括生成隐含前提、翻译为逻辑公式和确定逻辑蕴涵关系。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的文本和对话论证通常是省略三段论（部分前提和/或主张是隐含的）。现有的NLP方法能识别省略三段论但无法解码其底层逻辑，而基于逻辑的方法假设有足够的公式知识库通过溯因推理解码。缺乏将省略三段论文本组件翻译为逻辑论证并生成解码所需逻辑公式的系统方法。

Method: 提出三阶段流水线：1) 使用大语言模型基于显式前提和主张生成中间隐含前提；2) 使用另一个LLM将自然语言翻译为逻辑公式；3) 基于SAT求解器的神经符号推理器确定蕴涵关系。

Result: 在两个省略三段论数据集上评估流水线，在精确率、召回率、F1分数和准确率等指标上显示出选择正确隐含前提的有希望性能。

Conclusion: 该研究填补了处理省略三段论时系统翻译自然语言为逻辑论证的空白，通过集成LLM和神经符号推理实现了从文本识别到逻辑解码的完整流程。
...
TL;DR: 提出基于图的方法检测对立叙事，将叙事表示为实体交互图，通过节点级因果估计构建最小因果子图，优于现有对立思维分类方法


<details>
  <summary>Details</summary>
Motivation: 现有文本分析方法依赖预定义本体标注数据，存在人类偏见且多为黑盒模型。这些方法利用非结构化线性模式识别，而非建模话语中自然出现的实体间结构化交互

Method: 提出基于图的框架：1) 将叙事表示为实体交互图；2) 在节点层面纳入因果估计，将构建的句子图提炼为最小因果子图；3) 基于此表示构建分类流程

Result: 提出的分类流程在对立思维分类任务上优于现有方法

Conclusion: 通过图表示和因果估计，能够更好地建模叙事中的结构化实体交互，减少偏见，提高对立叙事检测和分类性能
...
TL;DR: CRIMSON是一个临床基础的胸部X光报告生成评估框架，通过诊断正确性、上下文相关性和患者安全性评估报告质量，整合完整临床上下文并防止正常或临床无关发现对评分产生过度影响。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标缺乏完整的临床上下文考虑，无法区分临床显著错误与良性差异，需要开发一个能够准确反映临床重要性的评估框架。

Method: CRIMSON整合患者年龄、适应症和基于指南的决策规则等完整临床上下文，将错误分类为全面分类法（包括错误发现、遗漏发现和八种属性级错误），并根据与心胸放射科医生合作开发的指南为每个发现分配临床显著性级别（紧急、可行动非紧急、不可行动、预期/良性），实现严重性感知加权。

Result: 在ReXVal验证中与六名认证放射科医生标注的临床显著错误计数高度一致（Kendall's tau = 0.61-0.71；Pearson's r = 0.71-0.84）；在RadJudge临床挑战性场景中与专家判断一致；在RadPref放射科医生偏好基准中与放射科医生偏好对齐最强。

Conclusion: CRIMSON提供了一个临床基础的评估框架，能够准确评估胸部X光报告生成质量，优先考虑临床重要错误，并发布了相关基准和模型以实现可重复评估。
...
<details>
  <summary>Details</summary>
Motivation: 主观多轮对话任务（如情感支持）需要适应动态用户状态并优化长期交互质量的对话策略。然而，强化学习在此类任务中面临挑战：仅基于结果的训练将多轮信用分配压缩为单一轨迹级奖励，而简单的轮级分组采样在交互环境中成本过高。

Method: 提出MAPO算法：1）利用评判模型提供密集过程反馈，通过蒙特卡洛回报传播长期效应；2）引入混合优势估计器，结合轮级归一化和批次级归一化，实现细粒度且可扩展的信用分配；3）无critic设计，提高训练效率。

Result: 在EMPA、EmoBench和EQ-Bench等多个主观对话基准上，从7B到32B不同模型规模下，MAPO均优于仅基于结果的GRPO和单级归一化基线。在EMPA上，成功率提升高达9个百分点，对话分数比7B基础模型提高+43.2。尽管仅在EMPA风格环境中训练，方法在未见的情感智能基准上泛化良好，EmoBench提升高达+4分，EQ-Bench提升+3.5分。

Conclusion: 密集过程监督与混合级归一化相结合，能够实现主观、开放式多轮对话的有效且可扩展的强化学习。该方法通过细粒度信用分配和稳定优化，显著提升了训练稳定性和最终性能，并展现出良好的泛化能力。
...
TL;DR: AI-CROWD协议通过集成多个大语言模型的集体输出，在缺乏真实标注的大规模内容分析中生成共识性近似标注，而非声称获得真实标注


<details>
  <summary>Details</summary>
Motivation: 大规模内容分析面临缺乏可观测真实标注的挑战，因为通过人工编码创建基准在大数据集中变得不切实际，存在时间、成本和一致性方面的困难

Method: 引入AI-CROWD协议，利用大语言模型集合的集体输出近似真实标注，通过多数投票聚合输出，并使用诊断指标分析一致/分歧模式

Result: 协议生成共识性近似标注，识别高置信度分类，同时标记潜在歧义或模型特定偏差

Conclusion: AI-CROWD协议为大规模内容分析提供了一种实用的解决方案，通过模型集合输出近似真实标注，克服了人工标注的限制
...
TL;DR: LIT-RAGBench是一个新的RAG生成器基准测试，涵盖集成、推理、逻辑、表格和弃权五个类别，通过虚构实体和场景评估模型在实际RAG部署中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有RAG生成器基准测试覆盖有限，无法在统一条件下同时评估多种能力，而实际应用中生成器需要处理长上下文、多步推理、表格解释和证据缺失时的弃权等复杂任务。

Method: 构建包含114个人工构建的日语问题及其机器翻译+人工校对的英文版本的数据集，定义五个评估类别（集成、推理、逻辑、表格、弃权），使用LLM-as-a-Judge进行评分，报告类别和总体准确率。

Result: 在API和开源模型测试中，没有模型总体准确率超过90%，表明现有模型在综合RAG能力方面仍有不足，基准测试能有效衡量各模型在不同类别中的优势和弱点。

Conclusion: LIT-RAGBench填补了现有评估与实际应用之间的差距，为实际RAG部署中的模型选择和构建RAG专用模型提供了有价值的度量标准，数据集和评估代码已开源。
...
TL;DR: FlashPrefill：通过即时模式发现和阈值处理实现超快速预填充的框架，在256K序列上实现27.78倍加速


<details>
  <summary>Details</summary>
Motivation: 长上下文建模是大型语言模型的关键能力，但注意力机制的二次复杂度在计算密集的预填充阶段成为关键瓶颈。现有稀疏注意力机制通常存在搜索延迟大或稀疏度不足的问题。

Method: 提出FlashPrefill框架，采用快速块搜索技术同时定位动态垂直、斜线和块稀疏注意力模式，并引入动态阈值机制避免排序或累积注意力分数的开销，有效消除长尾分布以增强稀疏性。

Result: 在256K序列上实现27.78倍加速，在4K上下文长度上仍保持1.71倍加速，表明该方法在不同序列规模下都具有鲁棒性和实用性。

Conclusion: FlashPrefill通过创新的模式发现和阈值处理机制，显著提升了长上下文建模中预填充阶段的效率，解决了现有稀疏注意力方法的局限性。
...
TL;DR: SPOT框架通过将显式思维链压缩为紧凑的潜在暂停令牌，实现高效推理，减少37.5%的生成令牌，同时提升2.3个点的准确率，并提供可解释的潜在推理过程。


<details>
  <summary>Details</summary>
Motivation: 现有方法在减少思维链推理开销时存在两个关键问题：1）大多只是截断模型输出而非内化模型思考；2）潜在推理方法依赖僵化的点对点对齐，难以捕捉整个推理段的密集、变长语义，且缺乏可解释性。

Method: 提出SPOT框架，包含两个核心技术：1）Span-level Semantic Alignment（Sinkhorn最优传输目标），将每个暂停令牌软对齐到整个推理段的语义；2）Frozen-Head Decoding Constraint，保持潜在状态在冻结的预训练语言模型头下可直接解码为令牌分布。

Result: 在推理基准测试中，SPOT平均提升准确率2.3个点，同时减少37.5%的生成令牌，并能提供潜在推理过程的忠实语义解释。

Conclusion: SPOT通过灵活的潜在推理框架，在保持可解释性的同时，有效压缩思维链推理，实现了效率与性能的平衡，为高效推理提供了新方向。
...
TL;DR: 该研究对GPT-4o-Mini、Gemini-2.5-Flash、Llama 3.2、Mistral和Gemma 3等当代大语言模型在印度、东亚和东南亚地区的文化对齐性进行了多语言审计，发现这些模型在宗教观点上存在显著偏差，特别是对少数群体的负面刻板印象，且轻量级干预措施只能部分缓解问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地在多语言、多文化环境中部署，但其主要依赖英语中心训练数据，可能导致与不同社会的多元文化价值观不匹配。研究旨在评估当代LLMs在敏感宗教领域的文化对齐性，揭示其在全球部署中的潜在风险。

Method: 采用多语言审计方法，对多个LLMs在印度、东亚和东南亚地区进行文化对齐评估。使用内部表征分析（log-probs/logits），将模型观点分布与真实公众态度进行比较。同时测试了人口统计学提示和母语提示等轻量级干预措施的效果，并在多个偏见基准（CrowS-Pairs、IndiBias、ThaiCLI、KoBBQ）上进行下游评估。

Result: 研究发现：1）流行模型在广泛社会议题上能与公众意见对齐，但在宗教观点上持续失败；2）模型无法准确代表少数群体的宗教观点，经常放大负面刻板印象；3）轻量级干预措施只能部分缓解文化差距，无法完全消除；4）下游偏见基准评估显示在敏感情境中存在持续伤害和代表性不足问题。

Conclusion: 研究结果强调了系统性、基于区域的审计对于确保LLMs在全球公平部署的紧迫需求。当前模型在文化对齐方面存在显著缺陷，特别是在宗教等敏感领域，需要更深入的文化适应和评估机制。
...
TL;DR: 研究探索ChatGPT的人工视觉创造力，通过艺术家评估AI对原创艺术品的模仿能力，发现色彩纹理相似性与构图概念感知相似性存在显著差距，建议使用"风格转移仪表板"多指标评估而非单一风格指标。


<details>
  <summary>Details</summary>
Motivation: 探索人工智能在视觉艺术创作中的创造性能力，特别是ChatGPT在有意模仿原创艺术作品（绘画、雕塑、装置等）方面的表现，了解AI生成作品与原创作品之间的相似性和差异。

Method: 邀请来自罗马尼亚、保加利亚、法国、奥地利和英国的12位艺术家，每人提供3件原创作品；使用ChatGPT生成这些作品的模仿版本；结合人类评估（艺术家评分和评论）与计算方法检测视觉和风格相似性或差异。

Result: 发现基于色彩和纹理的相似性与基于构图、概念和感知的相似性之间存在显著差距；艺术家认为ChatGPT的模仿作品缺乏维度感、上下文和意图感，更像是近似引用而非能引发情感的有价值艺术品。

Conclusion: 建议使用包含互补指标的"风格转移仪表板"来评估模仿作品与原创作品的相似性，而非依赖单一风格指标；AI在艺术模仿方面仍存在局限性，特别是在传达作品的维度感、上下文和创作意图方面。
...
TL;DR: 该研究将数学文本理解建模为数学实体关系抽取任务，使用Transformer模型实现高精度关系提取，并引入可解释AI技术增强模型透明度。


<details>
  <summary>Details</summary>
Motivation: 数学文本理解面临专业实体和复杂关系的挑战，需要开发能够自动提取数学实体关系的有效方法，同时确保模型决策的可解释性。

Method: 将数学问题解释建模为数学实体关系抽取任务，其中操作数作为实体，运算符作为关系。应用基于Transformer的模型自动提取这些关系，并整合SHAP方法进行可解释性分析。

Result: BERT模型在数学实体关系抽取任务上达到99.39%的准确率。可解释性分析揭示了特定文本和数学特征如何影响关系预测，提供了特征重要性和模型行为的深入理解。

Conclusion: 通过结合Transformer学习、任务特定数据集和可解释建模，该研究为数学实体关系抽取提供了一个有效且可解释的框架，支持自动化问题解决、知识图谱构建和智能教育系统等应用。
...
TL;DR: 该研究通过引入包含道义模态的Wason选择任务数据集，系统比较了LLM在道义规则和描述性规则下的条件推理能力，发现LLM在道义规则下表现更好，且错误模式与人类匹配偏差相似。


<details>
  <summary>Details</summary>
Motivation: 随着LLM语言能力提升，其推理能力受到关注。人类推理在特定领域（特别是规范性而非纯形式化情境）表现良好，但LLM推理的领域特异性尚未充分探索。研究旨在系统比较LLM在道义规则和描述性规则下的条件推理差异。

Method: 引入新的Wason选择任务数据集，明确编码道义模态以区分道义条件句和描述性条件句。使用该数据集检验LLM在道义规则下的条件推理能力，并分析错误模式是由确认偏差（寻求支持规则的证据）还是匹配偏差（忽略否定并选择与规则词汇匹配的项目）主导。

Result: 结果显示，与人类类似，LLM在道义规则下推理表现更好，并显示出类似匹配偏差的错误模式。LLM的表现随规则类型系统变化，其错误模式与人类在该范式中的已知偏差相似。

Conclusion: LLM的条件推理能力在不同规则类型间存在系统性差异，其错误模式与人类认知偏差（特别是匹配偏差）相似，表明LLM推理机制可能与人类推理存在某些相似特征。
...
TL;DR: 该研究对基于大语言模型的英语作为第二语言写作自动评分方法进行了首次统一比较，评估了四种主要范式在雅思写作任务2上的表现，发现最佳配置（k-SFT+RAG）达到93%的F1分数，揭示了准确性-成本-鲁棒性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 先前研究通常孤立地考察各种基于LLM的自动作文评分技术，缺乏对它们在英语作为第二语言写作中相对优势的系统比较，这限制了对其实际应用价值的理解。

Method: 研究在统一的雅思写作任务2基准上评估了四种主要方法：1) 基于编码器的分类微调；2) 零样本和少样本提示；3) 指令调优和检索增强生成；4) 监督微调结合直接偏好优化和RAG。

Result: 结果显示不同方法在准确性、成本和鲁棒性之间存在明显权衡。最佳配置（整合k-SFT和RAG）取得了最强的整体结果，F1分数达到93%。

Conclusion: 该研究提供了现代基于LLM的英语L2写作自动评分策略的首次统一实证比较，展示了这些方法在自动评分写作任务中的潜力，并公开了代码供进一步研究使用。
...
TL;DR: 该研究将三段论数据集转换为溯因推理格式，评估大语言模型在溯因推理中的准确性和偏见，探讨其与人类认知的差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明大语言模型与人类在形式推理中存在相似偏见，但批评这些偏见可能不公平，因为人类推理不仅包含形式演绎，还涉及基于有限信息的溯因推理。需要探究LLMs在溯因推理中的表现，以更全面评估其推理能力。

Method: 将三段论数据集转换为适合溯因推理的格式，用于评估最先进的大语言模型在溯因推理中的准确性，分析其是否存在偏见。

Result: 研究发现大语言模型在溯因推理中表现出特定偏见，识别了需要改进的潜在领域，强调了超越形式演绎的语境化推理的重要性。

Conclusion: 该研究对于推进LLMs在复杂推理任务中的理解和应用至关重要，为弥合机器与人类认知之间的差距提供了见解，强调了需要更全面的推理评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前XAI方法存在两个主要问题：一是采用"一刀切"范式，忽视了用户在专业知识、目标和认知需求方面的差异；二是虽然大语言模型可以将技术解释转化为自然语言，但存在忠实性不足和幻觉问题。需要开发既能个性化又能保证解释可靠性的XAI系统。
...
Result: 在医疗和金融领域的自动和人工评估表明：验证-精炼循环显著提高了完整性和风格对齐性，优于无验证生成。人工研究进一步证实：预期偏好向量与感知风格高度一致，对生成随机性具有鲁棒性，质量评估持续为正。

Conclusion: PONTE通过将个性化建模为闭环验证过程，解决了XAI中的个性化与可靠性平衡问题。该框架能够生成既适应用户需求又保持技术忠实性的解释，为构建可信赖的AI系统提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 当前ASR系统大多局限于单语环境和短孤立话语，现有上下文感知ASR存在多语言支持有限以及语音与上下文表示对齐不足的问题。

Method: 采用冻结的语音编码器和仅解码器语言模型，通过轻量级投影模块连接两者，支持结构化上下文提示（对话历史和偏置词），并使用对比学习目标在共享嵌入空间中对齐语音和上下文表示。

Result: 在超过1,500小时的真实世界对话语音数据（涵盖11种语言和5种英语方言）上评估，上下文输入持续改善识别质量，对比对齐在不同上下文类型上提供额外增益，总体性能提升超过5%。

Conclusion: 上下文建模和跨模态对齐在多语言ASR中具有重要性，所提框架在保持预训练模型模块化的同时，有效支持多语言和口音，并通过对比学习改善语音与上下文交互。
...
TL;DR: KCLarity团队参与SemEval 2026的CLARITY任务，研究政治话语中歧义和回避技术的分类，比较了两种建模方法：直接预测清晰度标签 vs 先预测回避标签再推导清晰度，并探索了辅助训练变体和零射设置下的解码器模型。


<details>
  <summary>Details</summary>
Motivation: 研究政治话语中歧义和回避技术的自动分类方法，为CLARITY共享任务开发有效的模型，探索不同建模策略对分类性能的影响。

Method: 采用两种建模策略：(1) 直接预测清晰度标签；(2) 先预测回避标签，然后通过任务分类层次结构推导清晰度。同时探索了多种辅助训练变体，并在回避优先的框架下评估了零射设置中的解码器模型。

Result: 两种建模策略在公开测试集上表现相当。在编码器模型中，RoBERTa-large在公开测试集上取得最佳结果，而零射GPT-5.2在隐藏评估集上表现出更好的泛化能力。

Conclusion: 两种建模方法都能有效处理政治话语中的歧义和回避技术分类任务，不同模型在不同评估集上各有优势，零射解码器模型在泛化能力方面表现出潜力。
